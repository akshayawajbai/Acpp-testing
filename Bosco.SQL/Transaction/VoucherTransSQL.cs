        ...
        private string GetVoucherTransactionSQL()
        {
            ...
            switch (sqlCommandId)
            {
                case SQLCommand.VoucherTransDetails.Add:
                    {
                        query = "INSERT INTO VOUCHER_TRANS ( " +
                               ...
                               "CHEQUE_NO," +
                               "MATERIALIZED_ON," +
                               "STATUS,NARRATION,PAN_NUMBER,GST_NUMBER,GST,CGST,SGST,IGST,REFERENCE_NUMBER," + //bharath 04/06/2026 Ledger PAN and GST set
                               "CHEQUE_REF_DATE, CHEQUE_REF_BANKNAME, CHEQUE_REF_BRANCH, LEDGER_GST_CLASS_ID, FUND_TRANSFER_TYPE_NAME,BENEFICIARY_NAME,BENEFICIARY_ACC_NO,BENEFICIARY_BANK_BRANCH,BENEFICIARY_CHEQUE_IN_FAVOUR,BENEFICIARY_CROSS_USING, " +
                               ...
                               "?CHEQUE_NO," +
                               "?MATERIALIZED_ON," +
                               "?STATUS,?NARRATION,?LEDGER_PAN,?LEDGER_GST, ?GST, ?CGST, ?SGST, ?IGST, ?REFERENCE_NUMBER," + //bharath 04/06/2026 Ledger PAN and GST set
                               "?CHEQUE_REF_DATE, ?CHEQUE_REF_BANKNAME, ?CHEQUE_REF_BRANCH, ?LEDGER_GST_CLASS_ID,?FUND_TRANSFER_TYPE_NAME,?BENEFICIARY_NAME,?BENEFICIARY_ACC_NO,?BENEFICIARY_BANK_BRANCH, ?BENEFICIARY_CHEQUE_IN_FAVOUR,?BENEFICIARY_CROSS_USING," +
                               "?EXCHANGE_RATE, ?LIVE_EXCHANGE_RATE, ?ACTUAL_AMOUNT)";
                        break;
                    }
                ...
                case SQLCommand.VoucherTransDetails.FetchJournalDetailById: // 07/02/2025,chinna
                    {
                        query = "SELECT VMT.VOUCHER_ID, TB.BOOKING_ID,MLG.GROUP_ID, TB.EXPENSE_LEDGER_ID, TB.PARTY_LEDGER_ID, TB.DEDUCTEE_TYPE_ID, TB.AMOUNT AS 'TDS_AMOUNT',\n" +
                                "       CASE\n" +
                                ...
                                "       VMT.CURRENCY_COUNTRY_ID, CONTRIBUTION_AMOUNT, VMT.EXCHANGE_RATE, CALCULATED_AMOUNT, VMT.ACTUAL_AMOUNT, EXCHANGE_COUNTRY_ID,\n" +
                                "       VT.EXCHANGE_RATE AS LEDGER_EXCHANGE_RATE, VT.LIVE_EXCHANGE_RATE, VMT.IS_CASH_BANK_STATUS, IFNULL(VMT.CONTRA_MULTI_FLAG,0) AS CONTRA_MULTI_FLAG\n" +
                                ...
                                " WHERE VMT.VOUCHER_ID = ?VOUCHER_ID AND (VMT.VOUCHER_TYPE = 'JN' || VMT.VOUCHER_TYPE = 'RC') { AND ((?SUB_LEDGER_ID = 1 AND ML.GROUP_ID NOT IN (14)) or (?SUB_LEDGER_ID = 0 AND ML.GROUP_ID NOT IN (14))) } AND VMT.STATUS = 1\n"; //Dinesh Remove for 12 and 13 GID 
                ...
                case SQLCommand.VoucherTransDetails.FetchInterProjectJournalBankLinesForEdit:
                    {
                        query = "SELECT VMT.VOUCHER_ID, TB.BOOKING_ID,MLG.GROUP_ID, TB.EXPENSE_LEDGER_ID, TB.PARTY_LEDGER_ID, TB.DEDUCTEE_TYPE_ID, TB.AMOUNT AS 'TDS_AMOUNT',\n" +
                                "       CASE\n" +
                                "         WHEN TB.BOOKING_ID > 0 AND VT.TRANS_MODE = 'DR' AND\n" +
                                ...
                                "       IFNULL(LEDGER_GST_CLASS_ID, ?LEDGER_GST_CLASS_ID) AS LEDGER_GST_CLASS_ID, IFNULL(GIM.GST_INVOICE_ID, 0) AS GST_INVOICE_ID, ML.IS_GST_LEDGERS, \n" +
                                "       VMT.CURRENCY_COUNTRY_ID, CONTRIBUTION_AMOUNT, VMT.EXCHANGE_RATE, CALCULATED_AMOUNT, VMT.ACTUAL_AMOUNT, EXCHANGE_COUNTRY_ID,\n" +
                                "       VT.EXCHANGE_RATE AS LEDGER_EXCHANGE_RATE, VT.LIVE_EXCHANGE_RATE, VMT.IS_CASH_BANK_STATUS, IFNULL(VMT.CONTRA_MULTI_FLAG,0) AS CONTRA_MULTI_FLAG, VMT.PROJECT_ID AS VOUCHER_MASTER_PROJECT_ID\n" +
                                " FROM VOUCHER_MASTER_TRANS VMT\n" +
                                " INNER JOIN VOUCHER_TRANS VT ON VMT.VOUCHER_ID = VT.VOUCHER_ID\n" +
                                " INNER JOIN MASTER_LEDGER ML ON VT.LEDGER_ID = ML.LEDGER_ID\n" +
                                " INNER JOIN MASTER_LEDGER_GROUP MLG ON ML.GROUP_ID = MLG.GROUP_ID\n" +
                                " LEFT JOIN TDS_BOOKING AS TB ON VMT.VOUCHER_ID = TB.VOUCHER_ID\n" +
                                " LEFT JOIN GST_INVOICE_MASTER GIM ON GIM.BOOKING_VOUCHER_ID = VMT.VOUCHER_ID\n" +
                                " WHERE IFNULL(VMT.CONTRA_MULTI_FLAG,0) = ?CONTRA_MULTI_FLAG AND VMT.VOUCHER_TYPE = 'JN' AND VMT.STATUS = 1 AND MLG.GROUP_ID = 12 AND IFNULL(ML.ACCESS_FLAG,0) <> 2\n" +
                                " { AND ((?SUB_LEDGER_ID = 1 AND ML.GROUP_ID NOT IN (14)) or (?SUB_LEDGER_ID = 0 AND ML.GROUP_ID NOT IN (14))) }\n" +
                                // 31/07/2026, Chinna - show pair bank lines in create order (first selected / first saved voucher), not forced CR-first
                                " ORDER BY VMT.VOUCHER_ID, VT.SEQUENCE_NO";
                        break;
                    }
                case SQLCommand.VoucherTransDetails.FetchTransDetails:
                    {
                        query = "SELECT VT.VOUCHER_ID,\n" +
                               ...
                               "       VT.CHEQUE_REF_DATE, VT.CHEQUE_REF_BANKNAME, VT.CHEQUE_REF_BRANCH, VT.FUND_TRANSFER_TYPE_NAME, VT.BENEFICIARY_NAME,VT.BENEFICIARY_ACC_NO,VT.BENEFICIARY_BANK_BRANCH,VT.BENEFICIARY_CHEQUE_IN_FAVOUR,VT.BENEFICIARY_CROSS_USING," +
                               "       T.LEDGER_ID AS REF_LEDGER_ID,T.AMOUNT AS REF_AMOUNT, VT.NARRATION, VT.PAN_NUMBER, VT.GST_NUMBER, VM.VOUCHER_DEFINITION_ID,\n" +
                ...
                case SQLCommand.VoucherTransDetails.FetchCashBankDetails:
                    {
                        ...
                        query = "SELECT VT.VOUCHER_ID,\n" +
                                ...
                                "       VT.CHEQUE_REF_DATE, VT.CHEQUE_REF_BANKNAME, VT.CHEQUE_REF_BRANCH, VT.FUND_TRANSFER_TYPE_NAME,VT.BENEFICIARY_NAME, VT.BENEFICIARY_ACC_NO,VT.BENEFICIARY_BANK_BRANCH,VT.BENEFICIARY_CHEQUE_IN_FAVOUR,VT.BENEFICIARY_CROSS_USING, VM.VOUCHER_DEFINITION_ID,\n" +
                                "       VT.EXCHANGE_RATE, VT.LIVE_EXCHANGE_RATE, VT.ACTUAL_AMOUNT\n" +
                                "  FROM VOUCHER_TRANS VT\n" +
                                " INNER JOIN VOUCHER_MASTER_TRANS VM\n" +
                                "    ON VT.VOUCHER_ID = VM.VOUCHER_ID\n" +
