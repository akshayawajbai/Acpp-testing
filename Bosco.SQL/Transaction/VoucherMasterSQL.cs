        ...
        private string GetVoucherMasterSQL()
        {
            ...
            switch (sqlCommandId)
            {
                case SQLCommand.VoucherMaster.Add:
                    {
                        query = "INSERT INTO VOUCHER_MASTER_TRANS ( " +
                            ...
                            //"MODIFIED_ON," +
                               ...
                               "CREATED_BY_NAME, " +
                               "VOUCHER_DEFINITION_ID, AUTHORIZATION_STATUS, AUTHORIZATION_UPDATED_ON, AUTHORIZATION_UPDATED_BY_NAME, IS_MULTI_CURRENCY,IS_CASH_BANK_STATUS, CONTRA_MULTI_FLAG) " +
                               "VALUES( " +
                               "?VOUCHER_DATE, " +
                               "?PROJECT_ID, " +
                               "?VOUCHER_NO, " +
                            ...
                            //" NOW()," +
                               ...
                               "IF(?AUTHORIZATION_STATUS=1, ?AUTHORIZATION_UPDATED_BY_NAME, ''), ?IS_MULTI_CURRENCY,?IS_CASH_BANK_STATUS, ?CONTRA_MULTI_FLAG)";
                        break;
                    }
                ...
                case SQLCommand.VoucherMaster.Update:
                    {
                        query = "UPDATE VOUCHER_MASTER_TRANS SET " +
                            ...
                            //"IS_AUDITOR_MODIFIED =?IS_AUDITOR_MODIFIED,"+
                                    "VOUCHER_DEFINITION_ID=?VOUCHER_DEFINITION_ID, IS_MULTI_CURRENCY=?IS_MULTI_CURRENCY,IS_CASH_BANK_STATUS=?IS_CASH_BANK_STATUS, CONTRA_MULTI_FLAG=?CONTRA_MULTI_FLAG " +
                ...
                case SQLCommand.VoucherMaster.SaveGSTInvoiceMasterDetails:
                    {
                        // On 20/01/2026, Chinna - Added CURRENCY_COUNTRY_ID column in GST_INVOICE_MASTER INSERT query after BILLING_COUNTRY_ID
                        query = @"INSERT INTO GST_INVOICE_MASTER (GST_VENDOR_INVOICE_NO, BOOKING_VOUCHER_ID, BOOKING_VOUCHER_TYPE, GST_VENDOR_INVOICE_DATE, 
                                    GST_VENDOR_INVOICE_TYPE, GST_VENDOR_ID, PERIOD_FROM, PERIOD_TO, DUE_DATE,
                                    TRANSPORT_MODE, VEHICLE_NUMBER, SUPPLY_DATE, SUPPLY_PLACE, BILLING_NAME, BILLING_GST_NO, BILLING_ADDRESS, BILLING_STATE_ID, BILLING_COUNTRY_ID, CURRENCY_COUNTRY_ID,
                                ...
                                VALUES (?GST_VENDOR_INVOICE_NO, ?BOOKING_VOUCHER_ID, ?BOOKING_VOUCHER_TYPE, ?GST_VENDOR_INVOICE_DATE, 
                                    ...
                                    IF(?BILLING_STATE_ID=0, null,?BILLING_STATE_ID), IF(?BILLING_COUNTRY_ID=0, null, ?BILLING_COUNTRY_ID), ?CURRENCY_COUNTRY_ID,
                ...
                case SQLCommand.VoucherMaster.UpdateGSTInvoiceMasterDetails:
                    {
                        // On 20/01/2026, Chinna - Added CURRENCY_COUNTRY_ID column in GST_INVOICE_MASTER UPDATE query after BILLING_COUNTRY_ID
                        query = @"UPDATE GST_INVOICE_MASTER SET GST_VENDOR_INVOICE_NO = ?GST_VENDOR_INVOICE_NO, BOOKING_VOUCHER_ID=?BOOKING_VOUCHER_ID,
                                    ...
                                    GST_VENDOR_INVOICE_TYPE = ?GST_VENDOR_INVOICE_TYPE, GST_VENDOR_ID = ?GST_VENDOR_ID, PERIOD_FROM = ?PERIOD_FROM,PERIOD_TO =?PERIOD_TO,
                                    DUE_DATE = ?DUE_DATE,
                                    ...
                                    BILLING_COUNTRY_ID = IF(?BILLING_COUNTRY_ID = 0, null, ?BILLING_COUNTRY_ID), CURRENCY_COUNTRY_ID = ?CURRENCY_COUNTRY_ID,
                ...
                case SQLCommand.VoucherMaster.FetchGSTInvoiceMasterDetailsById:
                    {
                        query = @"SELECT GIM.GST_INVOICE_ID, GST_VENDOR_INVOICE_NO, GST_VENDOR_INVOICE_DATE, GST_VENDOR_INVOICE_TYPE, GST_VENDOR_ID, PERIOD_FROM, PERIOD_TO, DUE_DATE,
                ...
                case SQLCommand.VoucherMaster.FetchGSTPendingInvoices:
                    {
                        // On 20/01/2026, Chinna - Removed CURRENCY_COUNTRY_ID filter from SQL, filtering will be done in C# code
                        // Added CURRENCY_COUNTRY_ID to SELECT for C# filtering
                        //GROUP_CONCAT(DISTINCT ML.LEDGER_NAME SEPARATOR ', ') AS LEDGER_NAME
                        // Group by VT.VOUCHER_ID
                        // GROUP_CONCAT(IF(GST > 0, ML.LEDGER_NAME, VTAG.LEDGER_NAME), '') AS LEDGER_NAME

                        query = @"SELECT  GIM.GST_INVOICE_ID, VM.VOUCHER_SUB_TYPE, GIM.GST_VENDOR_INVOICE_NO, GIM.GST_VENDOR_INVOICE_DATE, ASV.VENDOR, VM.PARTY_LEDGER_NAME, VM.LEDGER_NAME,
                                  ...
                                  IFNULL(GIM.CURRENCY_COUNTRY_ID, 0) AS CURRENCY_COUNTRY_ID
                                  FROM GST_INVOICE_MASTER GIM
                                  ...
                                  INNER JOIN (SELECT VM.VOUCHER_ID, VM.VOUCHER_SUB_TYPE, GROUP_CONCAT(IF(GST=0, ML.LEDGER_NAME, null), '') AS PARTY_LEDGER_NAME,
                                      -- GROUP_CONCAT(IF(GST > 0, ML.LEDGER_NAME, VTAG.LEDGER_NAME), '') AS LEDGER_NAME,
                                      GROUP_CONCAT(IF(GST > 0, ML.LEDGER_NAME, VTAG.LEDGER_NAME), '') AS LEDGER_NAME,
                                      ...
                                      LEFT JOIN (SELECT VT.VOUCHER_ID, GROUP_CONCAT(DISTINCT ML.LEDGER_NAME SEPARATOR ', ') AS LEDGER_NAME
                                                 FROM VOUCHER_TRANS VT INNER JOIN MASTER_LEDGER ML ON ML.LEDGER_ID = VT.LEDGER_ID
                                                 WHERE VT.TRANS_MODE <> ?TRANS_MODE GROUP BY VT.VOUCHER_ID) AS VTAG ON VTAG.VOUCHER_ID = VM.VOUCHER_ID
                                  ...
                                  UNION ALL
                                        SELECT
                                           AM.IN_OUT_ID AS GST_INVOICE_ID, VM.VOUCHER_SUB_TYPE,
                                          AM.BILL_INVOICE_NO AS GST_VENDOR_INVOICE_NO,
                                          AM.IN_OUT_DATE AS GST_VENDOR_INVOICE_DATE,
                                          ASV.VENDOR,
                                          GROUP_CONCAT(DISTINCT IF(VT.TRANS_MODE = ?TRANS_MODE, ML.LEDGER_NAME, NULL)) AS PARTY_LEDGER_NAME,
                                          GROUP_CONCAT(DISTINCT IF(VT.TRANS_MODE <> ?TRANS_MODE, ML.LEDGER_NAME, NULL)) AS LEDGER_NAME,
                                          (SELECT SUM(AOD1.AMOUNT)
                                            FROM ASSET_IN_OUT_DETAIL AOD1
                                            WHERE AOD1.IN_OUT_ID = AM.IN_OUT_ID) AS AMOUNT,
                                          0 AS TAXABLE_AMOUNT,
                                           (
                                            (SELECT SUM(AOD1.AMOUNT)
                                             FROM ASSET_IN_OUT_DETAIL AOD1
                                             WHERE AOD1.IN_OUT_ID = AM.IN_OUT_ID)
                                            - IFNULL(VGI.BALANCE, 0)
                                          ) AS BALANCE,
                                          0 AS GST,
                                          CONCAT( AM.BILL_INVOICE_NO,' - ',DATE_FORMAT(AM.IN_OUT_DATE, '%d/%m/%Y'),' - ',ASV.VENDOR) AS VENDOR_GST_INVOICE,
                                          IFNULL(VM.CURRENCY_COUNTRY_ID, 0) AS CURRENCY_COUNTRY_ID
                                        FROM ASSET_IN_OUT_MASTER AM
                                        LEFT JOIN ASSET_STOCK_VENDOR ASV
                                          ON ASV.VENDOR_ID = AM.VENDOR_ID
                                        LEFT JOIN VOUCHER_MASTER_TRANS VM
                                          ON VM.VOUCHER_ID = AM.VOUCHER_ID
                                        LEFT JOIN VOUCHER_TRANS VT 
                                          ON VT.VOUCHER_ID = AM.VOUCHER_ID
                                        LEFT JOIN MASTER_LEDGER ML
                                          ON ML.LEDGER_ID = VT.LEDGER_ID
                                      LEFT JOIN (
                                            SELECT
                                                VGI.GST_INVOICE_ID,
                                                SUM(VGI.AMOUNT) AS BALANCE
                                            FROM VOUCHER_GST_INVOICE VGI
                                            INNER JOIN ASSET_IN_OUT_MASTER AM1
                                                ON AM1.IN_OUT_ID = VGI.GST_INVOICE_ID
                                            WHERE
                                             VGI.VOUCHER_ID <> ?VOUCHER_ID   
                                             AND AM1.IN_OUT_DATE <= ?VOUCHER_DATE
                                            GROUP BY VGI.GST_INVOICE_ID
                                        ) AS VGI
                                        ON VGI.GST_INVOICE_ID = AM.IN_OUT_ID
                                        WHERE 
                                          AM.IN_OUT_DATE <= ?VOUCHER_DATE
                                          AND AM.VOUCHER_ID > 0
                                           AND (VM.STATUS = 1 OR VM.VOUCHER_ID = ?VOUCHER_ID)   
                                           AND (VM.PROJECT_ID IN (?PROJECT_ID) OR VM.VOUCHER_ID = ?VOUCHER_ID) 
                                        GROUP BY AM.IN_OUT_ID
                                        HAVING
                                          SUM(IF(ML.GROUP_ID IN (12,13), 1, 0)) = 0  ;";
                        break;
                    }
                case SQLCommand.VoucherMaster.FetchGSTPendingInvoicesBookingDetailsByInvoiceId:
                    {
                        ...
                        query = @"SELECT GIM.GST_INVOICE_ID, VM.PARTY_LEDGER_ID, VM.LEDGER_ID, GIM.GST_VENDOR_INVOICE_NO, GIM.GST_VENDOR_INVOICE_DATE, ASV.VENDOR, VM.PARTY_LEDGER_NAME, VM.LEDGER_NAME,
                                    ...
                                    CONCAT(CONCAT(GIM.GST_VENDOR_INVOICE_NO, CONCAT( ' - ' , DATE_FORMAT(GIM.GST_VENDOR_INVOICE_DATE, '%d/%m/%Y'))), CONCAT(' - ', ASV.VENDOR)) AS VENDOR_GST_INVOICE
                                        FROM GST_INVOICE_MASTER GIM
                                    INNER JOIN ASSET_STOCK_VENDOR ASV ON ASV.VENDOR_ID = GIM.GST_VENDOR_ID
                                    INNER JOIN (SELECT VM.VOUCHER_ID, IF(GST = 0, ML.LEDGER_ID, NULL) AS PARTY_LEDGER_ID, 
                                          IF(GST > 0, ML.LEDGER_ID, NULL) AS LEDGER_ID, 
                                          IF(GST=0, ML.LEDGER_NAME, '') AS PARTY_LEDGER_NAME, SUM(IF(GST = 0, VT.AMOUNT, 0)) AS AMOUNT,
                                         IF(GST>0, ML.LEDGER_NAME, '') AS LEDGER_NAME, SUM(IF(GST > 0, VT.AMOUNT, 0)) AS TAXABLE_AMOUNT, SUM(VT.GST) AS GST
                                          FROM VOUCHER_MASTER_TRANS VM
                                          INNER JOIN VOUCHER_TRANS VT ON VT.VOUCHER_ID = VM.VOUCHER_ID
                                    ...
                                    UNION ALL
                                   SELECT
                                        AM.IN_OUT_ID AS GST_INVOICE_ID,
                                        MAX(IF(VT.TRANS_MODE = 'Cr', ML.LEDGER_ID, NULL)) AS PARTY_LEDGER_ID,
                                        NULL AS LEDGER_ID,
                                        AM.BILL_INVOICE_NO AS GST_VENDOR_INVOICE_NO,
                                        AM.IN_OUT_DATE AS GST_VENDOR_INVOICE_DATE,
                                        ASV.VENDOR,
                                        GROUP_CONCAT(DISTINCT IF(VT.TRANS_MODE = 'Cr', ML.LEDGER_NAME, NULL)) AS PARTY_LEDGER_NAME,
                                        GROUP_CONCAT(DISTINCT IF(VT.TRANS_MODE <> 'Cr', ML.LEDGER_NAME, NULL)) AS LEDGER_NAME,
                                        (SELECT SUM(AOD1.AMOUNT)
                                            FROM ASSET_IN_OUT_DETAIL AOD1
                                            WHERE AOD1.IN_OUT_ID = AM.IN_OUT_ID) AS AMOUNT,
                                        0 AS TAXABLE_AMOUNT,
                                    ...
                                    WHERE
                                        AM.VOUCHER_ID > 0
                                        AND VM.STATUS = 1
                                        AND VM.PROJECT_ID IN (1)
                                        AND AM.IN_OUT_ID = ?GST_INVOICE_ID 
                                    GROUP BY AM.IN_OUT_ID
                                    HAVING
                                        SUM(IF(ML.GROUP_ID IN (12,13), 1, 0)) = 0";
                        break;
                ...
                case SQLCommand.VoucherMaster.IsCashBankEnbled:
                    {
                        query = @"select COUNT(IS_CASH_BANK_STATUS) AS STATUS from voucher_master_trans where voucher_type ='JN' and IS_CASH_BANK_STATUS=1 and status =1";
                        break;
                    }
                ...
                case SQLCommand.VoucherMaster.FetchVoucherIdsByContraMultiFlag:
                    {
                        query = "SELECT VOUCHER_ID, PROJECT_ID, VOUCHER_NO FROM VOUCHER_MASTER_TRANS\n" +
                                " WHERE IFNULL(CONTRA_MULTI_FLAG,0) = ?CONTRA_MULTI_FLAG AND VOUCHER_TYPE = 'JN' AND STATUS = 1\n" +
                                " ORDER BY VOUCHER_ID";
                        break;
                    }
                //chinna
                ...
                case SQLCommand.VoucherMaster.FetchJournalDetails:
                    {
                        ...
                        "       FD.FD_ACCOUNT_ID,\n" +
                        "       VMT.VOUCHER_NO,\n" +
                        "       VMT.VOUCHER_DATE,VMT.VOUCHER_TYPE AS VOUCHERTYPE,CASHBANK.LEDGER_NAME AS CASHBANK,\n" +
                        ...
                        "         WHEN 'CN' THEN 'Contra'\n" +
                        "         ELSE 'Journal' END, \n" +
                        "       MV.VOUCHER_NAME) AS VOUCHER_TYPE, IF(VMT.IS_CASH_BANK_STATUS=1,'Yes', 'No') AS IS_CASH_BANK_STATUS, IFNULL(VMT.CONTRA_MULTI_FLAG, 0) AS CONTRA_MULTI_FLAG, IFNULL(VMT.IS_MULTI_CURRENCY, 0) AS IS_MULTI_CURRENCY, VMT.VOUCHER_DEFINITION_ID, IF(VMT.AUTHORIZATION_STATUS=1, 'Authorized', 'Unauthorized') AS AUTHORIZATION_STATUS,\n" +
                ...
                case SQLCommand.VoucherMaster.FetchMasterDetails:
                    {
                        ...
                        query = "SELECT VM.VOUCHER_ID,\n" +
                            ...
                            //"       END, IF(VM.VOUCHER_DEFINITION_ID<=4, '', CONCAT('-', MV.VOUCHER_NAME))) AS VOUCHER_TYPE,\n" +
                              ...
                              "         WHEN 'CN' THEN 'Contra'\n" +
                              "         ELSE 'Journal' END, \n" +
                              "       MV.VOUCHER_NAME) AS VOUCHER_TYPE, VM.VOUCHER_DEFINITION_ID, IFNULL(VM.CONTRA_MULTI_FLAG, 0) AS CONTRA_MULTI_FLAG,\n" +
                              ...
                              "      VM.NAME_ADDRESS,VM.PAN_NUMBER,VM.GST_NUMBER,\n" + //bharath 04/06/2026 Ledger PAN and GST set
                              ...
                              "                            CASE\n" +
                              "                              WHEN ML.LEDGER_SUB_TYPE = 'BK' THEN\n" +
                              "                               CONCAT(CONCAT(IF(?DISPLAY_INPUT_LAN=1,IFNULL(ML.DISPLAY_LEDGER_NAME,ML.LEDGER_NAME),ML.LEDGER_NAME), ' - '),\n" +
                              "                                      CONCAT(MB.BANK, ' - '),\n" +
                              "                                      MB.BRANCH)\n" +
                              "                              ELSE\n" +
                              "                               CONCAT( IF(ML.LEDGER_CODE ='' OR ML.LEDGER_CODE IS NULL, '', CONCAT( ML.LEDGER_CODE,  '  ')), IF(?DISPLAY_INPUT_LAN=1,IFNULL(ML.DISPLAY_LEDGER_NAME,ML.LEDGER_NAME), ML.LEDGER_NAME))\n" +
                              "                            END AS LEDGER_NAME,\n" +
                              "                            VT.TRANS_MODE,\n" +
                ...
                case SQLCommand.VoucherMaster.FetchMasterAuditLogHistory:
                    {
                        query = "SELECT VM.VOUCHER_ID, MP.PROJECT_ID, VOUCHER_DATE, VOUCHER_NO, VOUCHER_SUB_TYPE,\n" +
                              ...
                              "         WHEN 'CN' THEN 'Contra'\n" +
                              "         ELSE 'Journal' END, \n" +
                              "     MV.VOUCHER_NAME) AS VOUCHER_TYPE, VM.VOUCHER_DEFINITION_ID, IFNULL(VM.CONTRA_MULTI_FLAG, 0) AS CONTRA_MULTI_FLAG, NAME_ADDRESS, \n" +
                ...
                case SQLCommand.VoucherMaster.FetchVouchersForAuthorization:
                    {
                        query = "SELECT VM.VOUCHER_ID, MP.PROJECT_ID, VM.VOUCHER_DATE, " +
                                 ...
                                 "        VM.VOUCHER_DEFINITION_ID, IFNULL(VM.CONTRA_MULTI_FLAG, 0) AS CONTRA_MULTI_FLAG, NAME_ADDRESS, VM.CREATED_ON,\n" +
                ...
                case SQLCommand.VoucherMaster.FetchMasterByID:
                    {
                        query = "SELECT " +
                               ...
                               "CONTRIBUTION_TYPE," +
                               "CONTRIBUTION_AMOUNT," +
                               "MT.CURRENCY_COUNTRY_ID," +
                               "EXCHANGE_RATE," +
                               "CALCULATED_AMOUNT," +
                               "ACTUAL_AMOUNT," +
                               "EXCHANGE_COUNTRY_ID, (SELECT VT.LIVE_EXCHANGE_RATE FROM VOUCHER_TRANS VT WHERE VT.VOUCHER_ID = MT.VOUCHER_ID ORDER BY VT.SEQUENCE_NO ASC LIMIT 1) AS LIVE_EXCHANGE_RATE," +
                               "MT.STATUS," +
                               "CREATED_BY," +
                               "MODIFIED_BY," +
                               ...
                               "AUTHORIZATION_UPDATED_ON, GIM.GST_INVOICE_ID AS GST_INVOICE_ID, IFNULL(GIM2.GST_INVOICE_ID, AIM.IN_OUT_ID) AS BOOKING_INVOICE_ID, IFNULL(MT.CONTRA_MULTI_FLAG, 0) AS CONTRA_MULTI_FLAG " + // IFNULl(INV.GST_INVOICE_ID, 0) AS BOOKING_INVOICE_I
                               ...
                               " LEFT JOIN GST_INVOICE_MASTER GIM2 " +
                               " ON GIM2.GST_INVOICE_ID = VGI.GST_INVOICE_ID " +
                               " AND GIM2.BOOKING_VOUCHER_ID > 0 " +
                               " AND GIM2.BOOKING_VOUCHER_TYPE = 'JN' " +
                               "LEFT JOIN ASSET_IN_OUT_MASTER AIM " +
                               " ON AIM.IN_OUT_ID = VGI.GST_INVOICE_ID " +
                               " AND AIM.VOUCHER_ID > 0 " +
                               " AND GIM2.GST_INVOICE_ID IS NULL " +
                        ...
                        //        "NARRATION, NAME_ADDRESS, PAN_NUMBER,GST_NUMBER, MT.GST_VENDOR_INVOICE_NO, MT.GST_VENDOR_INVOICE_DATE, MT.GST_VENDOR_INVOICE_TYPE, MT.GST_VENDOR_ID," +
                        //        "VOUCHER_DEFINITION_ID, IFNULL(MT.IS_AUDITOR_MODIFIED , 0) AS IS_AUDITOR_MODIFIED, AUTHORIZATION_STATUS, AUTHORIZATION_UPDATED_BY_NAME,  " +
                        //        "AUTHORIZATION_UPDATED_ON, GIM.GST_INVOICE_ID AS GST_INVOICE_ID, IFNULl(INV.GST_INVOICE_ID, 0) AS BOOKING_INVOICE_ID " + // IFNULl(INV.GST_INVOICE_ID, 0) AS BOOKING_INVOICE_I
                        //        "FROM VOUCHER_MASTER_TRANS MT " +
                        //        "INNER JOIN MASTER_PROJECT MP ON MT.PROJECT_ID=MP.PROJECT_ID " +
                        //        "LEFT JOIN GST_INVOICE_MASTER GIM ON ((GIM.BOOKING_VOUCHER_ID = MT.VOUCHER_ID AND GIM.BOOKING_VOUCHER_ID = ?VOUCHER_ID) " +
                        //        "      OR (GIM.BOOKING_VOUCHER_ID IS NULL AND GIM.GST_VENDOR_INVOICE_NO = MT.GST_VENDOR_INVOICE_NO) )" +
                ...
                case SQLCommand.VoucherMaster.FetchBRS:
                    {
                        ...
                        //  "       CHEQUE_REF_DATE,\n" +
                        query = "SELECT MT.VOUCHER_ID, VMT.VOUCHER_NO, MT.SEQUENCE_NO, VOUCHER_DATE, CHEQUE_NO,\n" +
                        "   IF(CHEQUE_REF_DATE='0001-01-01 00:00:00','',DATE_FORMAT(CHEQUE_REF_DATE,'%d/%m/%Y')) AS CHEQUE_REF_DATE,\n" +
                        "   MATERIALIZED_ON,\n" +
                        "   CASE VMT.VOUCHER_TYPE\n" +
                         "       WHEN 'RC' THEN 'Receipt'\n" +
                         "       WHEN 'PY' THEN 'Payment'\n" +
                         "       WHEN 'CN' THEN 'Contra'\n" +
                         "       ELSE 'Journal'\n" +
                         "   END AS VOUCHER_TYPE,\n" +
                        " T.LEDGER_ID, T.LEDGER_NAME,\n" +
                        "   CASE\n" +
                        ...
                        "                    MT.LEDGER_ID,\n" +
                        "                    ML.LEDGER_CODE,\n" +
                        "                    IF(?DISPLAY_INPUT_LAN=1,IFNULL(ML.DISPLAY_LEDGER_NAME,ML.LEDGER_NAME), ML.LEDGER_NAME) AS LEDGER_NAME\n" +
                ...
                case SQLCommand.VoucherMaster.FetchVoucherIdByClientRefId:
                    {
                        ...
                        //old query 24/10/2025
                        // bharath - bank vouchers entry dublicate 
                        query = " SELECT VMT.VOUCHER_ID,VMT.VOUCHER_NO FROM VOUCHER_MASTER_TRANS VMT \n" +
                             " INNER JOIN VOUCHER_TRANS VT ON VT.VOUCHER_ID = VMT.VOUCHER_ID \n" +
                             "INNER JOIN MASTER_LEDGER ML ON ML.LEDGER_ID = VT.LEDGER_ID \n" +
                             " WHERE CLIENT_CODE = ?CLIENT_CODE AND ML.GROUP_ID NOT IN (13) AND (VMT.CLIENT_REFERENCE_ID =?CLIENT_REFERENCE_ID)\n" +
