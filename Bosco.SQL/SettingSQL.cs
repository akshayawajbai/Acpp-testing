        ...
        private string GetSettingSQL()
        {
            ...
            switch (sqlCommandId)
            {
                ...
                case SQLCommand.Setting.FetchChequePrintingSetting:
                    {
                        //query = "SELECT BANK_ID, SETTING_NAME, SETTING_VALUE FROM MASTER_SETTING_CHEQUE_PRINTING WHERE BANK_ID=?BANK_ID;"; // bharath 09/07/2026 for fetching bank name
                        query = "SELECT BANK_ID, BANK_FORMAT, SETTING_NAME, SETTING_VALUE FROM MASTER_SETTING_CHEQUE_PRINTING WHERE BANK_ID = ?BANK_ID AND INSTR(SETTING_NAME, '_') = 0;";
                        break;
                    }
                    //bharath 09/07/2026 default bank fetch
                case SQLCommand.Setting.FetchChequeSettingforBankwise:
                    {
                        query = @"SELECT 
                    BANK_ID,
                    REPLACE(SETTING_NAME, CONCAT(?DEFAULT_BANK, '_'), '') AS SETTING_NAME,
                    SETTING_VALUE
              FROM MASTER_SETTING_CHEQUE_PRINTING
              WHERE BANK_ID = 0
                AND SETTING_NAME LIKE CONCAT(?DEFAULT_BANK, '_%');"; ;
                        break;
                    }
                case SQLCommand.Setting.InsertUpdateChequePrintingSetting:
                    {
                        query = "INSERT INTO MASTER_SETTING_CHEQUE_PRINTING (BANK_ID, BANK_FORMAT,SETTING_NAME, SETTING_VALUE ) " +
                                        "VALUES (?BANK_ID, ?BANK_FORMAT ,?SETTING_NAME, ?SETTING_VALUE)" +
                                        "ON DUPLICATE KEY UPDATE BANK_ID = ?BANK_ID, BANK_FORMAT = ?BANK_FORMAT, SETTING_NAME = ?SETTING_NAME, SETTING_VALUE=?SETTING_VALUE";
