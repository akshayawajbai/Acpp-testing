        ...
        private string GetMappingSQL()
        {
            ...
            switch (sqlCommandId)
            {
                ...
                case SQLCommand.Mapping.FetchProjectforLookup:
                    {
                        ...
                        Query = "SELECT " +
                                    ...
                                    "IFNULL(CASE WHEN ?DISPLAY_INPUT_LAN = 1 THEN MP.DISPLAY_PROJECT ELSE MP.PROJECT END, MP.PROJECT) AS PROJECT_NAME," +
                                    "CONCAT(IFNULL(CASE WHEN ?DISPLAY_INPUT_LAN = 1 THEN MP.DISPLAY_PROJECT ELSE MP.PROJECT END, MP.PROJECT), CONCAT(' - ', MD.DIVISION)) AS 'PROJECT',(SELECT ' ') AS  TRANS_MODE ,MP.CUSTOMERID " +            
                                "FROM " +
                                    " MASTER_PROJECT MP " +
                                    " INNER JOIN MASTER_DIVISION MD ON " +
                ...
                case SQLCommand.Mapping.FetchProjects:
                    {
                        Query = "SELECT " +
                                  ...
                                  "CONCAT(IF(?DISPLAY_INPUT_LAN = 1, IFNULL(MP.DISPLAY_PROJECT, MP.PROJECT), MP.PROJECT), CONCAT(' - ',MD.DIVISION)) AS PROJECT, "+
                                  "(SELECT ' ') AS  TRANS_MODE,MP.CUSTOMERID " +
                              "FROM " +
                                  " MASTER_PROJECT MP " +
                                  " INNER JOIN MASTER_DIVISION MD ON " +
                                  " MP.DIVISION_ID=MD.DIVISION_ID " +
                ...
                case SQLCommand.Mapping.LoadProjectMappingGrid:
                    {
                        ...
                        Query = @"SELECT IF(IFNULL(LB.TRANS_MODE, '') = '',
                                          ...
                                          LB.TRANS_MODE) AS TRANS_MODE,
                                       P.PROJECT_ID,
                                       IF(?DISPLAY_INPUT_LAN = 1,IFNULL(P.DISPLAY_PROJECT, P.PROJECT),P.PROJECT) AS PROJECT,
                                       PL.LEDGER_ID,
                                       IF(PL.LEDGER_ID IS NULL, 0, 1) AS 'SELECT_TEMP',
                                       IF(PL.LEDGER_ID IS NULL, 0, 1) AS 'SELECT_TEMP_EDIT',
                                       IF(AMOUNT IS NULL, 0.00,
                                          IF(ML.CUR_COUNTRY_ID > 0
                                             AND (ML.GROUP_ID IN (12, 13, 14)
                                                  OR (?IS_BSGRMG = 1 AND ?LEDGER_GROUP_IS_SUNDRY_FC = 1)),
                                             AMOUNT_FC,
                                             AMOUNT)) AS AMOUNT,
                                       LB.TRANS_FLAG,
                ...
                case SQLCommand.Mapping.LoadProjectCostCentreGrid:
                    {
                        Query = @"SELECT 
                                    P.PROJECT_ID, 
                                    IF(?DISPLAY_INPUT_LAN = 1,IFNULL(P.DISPLAY_PROJECT, P.PROJECT), P.PROJECT) AS PROJECT,
                                    PCC.COST_CENTRE_ID,IF(PCC.COST_CENTRE_ID IS NULL, 0,1) AS 'SELECT_TEMP',
                ...
                case SQLCommand.Mapping.IsMadeBudgetForLedger:
                    {
                        Query = @"SELECT IF(BL.PROPOSED_AMOUNT=0.00, 0,1) AS LEDGER_ID FROM BUDGET_MASTER BA
                ...
                case SQLCommand.Mapping.LoadAllLedgers:
                    {
                        ...
                        Query = @"SELECT ML.LEDGER_ID,ML.GROUP_ID,IF(MIP.LEDGER_ID IS NULL,0,MIP.LEDGER_ID) AS LEGAL_ENTITY_LEDGER_ID,IF(MIP.CUSTOMERID IS NULL,0,MIP.CUSTOMERID) AS CUSTOMER_ID, ML.CUR_COUNTRY_ID AS CUR_COUNTRY_ID,
 CONCAT('( ',
              MC.CURRENCY_SYMBOL,
              ' : ',
              IFNULL(MC.COUNTRY, ''),
                ...
                case SQLCommand.Mapping.FetchPurposeMapped:
                    {
                        Query = "SELECT  PP.CONTRIBUTION_ID,CODE," +
                                     "MCH.FC_PURPOSE, AMOUNT,TRANS_MODE FROM PROJECT_PURPOSE AS PP " +
                                     //"LEFT JOIN MASTER_CONTRIBUTION_HEAD AS MCH " +//bharath 24/07/2026 
                                     "INNER JOIN MASTER_CONTRIBUTION_HEAD AS MCH " +
                ...
                case SQLCommand.Mapping.FetchMappedActiveDonors:
                    {
                        ...
                        "       STATE_NAME AS STATE,\n" +
                        "       ADDRESS,\n" +
                        "       MD.PAN,\n" +   // bharath to get PAN number from donor in Transactionmultiadd
                        "       CASE\n" +
                        "         WHEN TYPE = 1 THEN\n" +
                        "          'Institutional'\n" +
                        "         ELSE\n" +
