    ...
    public class CostCentreSQL : IDatabaseQuery
    {
        ...
        private string GetCostCentreSQL()
        {
            ...
            switch (sqlCommandId)
            {
                ...
                case SQLCommand.CostCentre.FetchCostCentreIdByApiId:
                    {
                        query = "SELECT COST_CENTRE_ID FROM MASTER_COST_CENTRE WHERE THIRD_PARTY_COST_CENTRE_ID = ?THIRD_PARTY_COST_CENTRE_ID";
                        break;
                    }
                case SQLCommand.CostCentre.AddApi:
                    {
                        query = "INSERT INTO MASTER_COST_CENTRE (" +
                                "ABBREVATION, COST_CENTRE_NAME, NOTES, THIRD_PARTY_COST_CENTRE_ID) VALUES(" +
                                "?ABBREVATION, ?COST_CENTRE_NAME, ?NOTES, ?THIRD_PARTY_COST_CENTRE_ID)";
                        break;
                    }
                case SQLCommand.CostCentre.UpdateAllByApiId:
                    {
                        query = "UPDATE MASTER_COST_CENTRE SET " +
                                "ABBREVATION = ?ABBREVATION, " +
                                "COST_CENTRE_NAME = ?COST_CENTRE_NAME, " +
                                "NOTES = ?NOTES " +
                                "WHERE THIRD_PARTY_COST_CENTRE_ID = ?THIRD_PARTY_COST_CENTRE_ID";
                        break;
                    }
                case SQLCommand.CostCentre.FetchIdMap:
                    {
                        query =
                            "SELECT DISTINCT " +
                            "  MC.COST_CENTRE_ID        AS LocalId, " +
                            "  MC.THIRD_PARTY_COST_CENTRE_ID AS ApiId " +
                            "FROM MASTER_COST_CENTRE MC " +
                            "WHERE MC.THIRD_PARTY_COST_CENTRE_ID IS NOT NULL " +
                            "  AND MC.THIRD_PARTY_COST_CENTRE_ID > 0 " +
                            "ORDER BY MC.COST_CENTRE_ID;";
                        break;
                    }
                case SQLCommand.CostCentre.FetchVoucherUsage:
                    {
                        query =
                            "SELECT DISTINCT COST_CENTRE_ID AS LocalId " +
                            "FROM VOUCHER_CC_TRANS " +
                            "WHERE COST_CENTRE_ID IS NOT NULL AND COST_CENTRE_ID > 0 " +

                            "UNION " +
                            "SELECT DISTINCT COST_CENTRE_ID AS LocalId " +
                            "FROM project_purpose_costcentre " +
                            "WHERE COST_CENTRE_ID IS NOT NULL AND COST_CENTRE_ID > 0 " +

                            "UNION " +
                            "SELECT DISTINCT COST_CENTRE_ID AS LocalId " +
                            "FROM BUDGET_COSTCENTER " +
                            "WHERE COST_CENTRE_ID IS NOT NULL AND COST_CENTRE_ID > 0 " +

                            "UNION " +
                            "SELECT DISTINCT COST_CENTRE_ID AS LocalId " +
                            "FROM BUDGET_STRENGTH_DETAIL " +
                            "WHERE COST_CENTRE_ID IS NOT NULL AND COST_CENTRE_ID > 0 " +

                            "UNION " +
                            "SELECT DISTINCT COST_CENTRE_ID AS LocalId " +
                            "FROM MASTER_REPORT_BUDGET_NEW_PROJECTS_DETAILS " +
                            "WHERE COST_CENTRE_ID IS NOT NULL AND COST_CENTRE_ID > 0 " +

                            "ORDER BY LocalId;";
                        break;
                    }
                case SQLCommand.CostCentre.DeleteUnusedCostCentre:
                    {
                        query =
                            "DELETE FROM project_costcentre WHERE COST_CENTRE_ID = ?COST_CENTRE_ID; " +
                            "DELETE FROM costcategory_costcentre WHERE COST_CENTRE_ID = ?COST_CENTRE_ID; " +
                            "DELETE FROM master_cost_centre WHERE COST_CENTRE_ID = ?COST_CENTRE_ID;";
                        break;
                    }
            }
            return query;
