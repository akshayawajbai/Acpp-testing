    ...
    public class BudgetSQL : IDatabaseQuery
    {
        ...
        private string GetBudgetSQL()
        {
            ...
            switch (sqlCommandId)
            {
                ...
                case SQLCommand.Budget.FetchByIdtoUdeg:
                    {
                        query = @"SELECT BM.BUDGET_ID,
                        MAX(BM.BUDGET_NAME) AS BUDGET_NAME,
                        MAX(BT.BUDGET_TYPE) AS BUDGET_TYPE,
                        MAX(BM.DATE_FROM) AS DATE_FROM,
                        MAX(BM.DATE_TO) AS DATE_TO,
                        GROUP_CONCAT(DISTINCT MP.PROJECT) AS PROJECT,
                        ML.LEDGER_NAME,
                        BL.PROPOSED_AMOUNT,
                        BL.APPROVED_AMOUNT,
                        MCC.COST_CENTRE_NAME,
                        MCCC.COST_CENTRE_CATEGORY_NAME,
                        BC.PROPOSED_AMOUNT AS CC_PROPOSED_AMOUNT,
                        BC.APPROVED_AMOUNT AS CC_APPROVED_AMOUNT,
                        BL.TRANS_MODE,
                        MN.NATURE_ID
                    FROM BUDGET_MASTER BM
                    LEFT JOIN BUDGET_TYPE BT
                        ON BM.BUDGET_TYPE_ID = BT.BUDGET_TYPE_ID
                    ...
                    INNER JOIN MASTER_NATURE MN
                          ON MN.NATURE_ID = MLG.NATURE_ID
                    LEFT JOIN BUDGET_COSTCENTER BC
                        ON BM.BUDGET_ID = BC.BUDGET_ID AND ML.LEDGER_ID = BC.LEDGER_ID
                    LEFT JOIN MASTER_COST_CENTRE MCC
                        ON BC.COST_CENTRE_ID = MCC.COST_CENTRE_ID 
                    LEFT JOIN COSTCATEGORY_COSTCENTRE CC
                    ON CC.COST_CENTRE_ID = MCC.COST_CENTRE_ID
                    LEFT JOIN MASTER_COST_CENTRE_CATEGORY MCCC
                     ON MCCC.COST_CENTRECATEGORY_ID = CC.COST_CATEGORY_ID WHERE BM.BUDGET_ID =?BUDGET_ID
                    GROUP BY BC.COST_CENTRE_ID, ML.LEDGER_ID";
                        break;
                    }
                ...
                case SQLCommand.Budget.FetchBudgetProjectforLookup:
                    {
                        query = "SELECT " +
                                    ...
                                    //bharath 19/02/2026 language update
