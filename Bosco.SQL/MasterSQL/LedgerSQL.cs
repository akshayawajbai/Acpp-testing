        ...
        private string GetLedgerSQL()
        {
            ...
            switch (sqlCommandId)
            {
                case SQLCommand.LedgerBank.Add:
                    {
                        // 17/02/2026, added DISPLAY_LEDGER_NAME for second-level language (e.g. French)
                ...
                case SQLCommand.LedgerBank.Update:
                    {
                        query = "UPDATE MASTER_LEDGER SET " +
                                    "LEDGER_CODE =?LEDGER_CODE, " +
                                    "LEDGER_NAME =?LEDGER_NAME, " +
                                    "DISPLAY_LEDGER_NAME = ?DISPLAY_LEDGER_NAME, " + //bharath 23/06/2026 To update Null in display ledger in DB
                                    "GROUP_ID=?GROUP_ID, " +
                                    "LEDGER_TYPE=?LEDGER_TYPE," +
                                    "LEDGER_SUB_TYPE=?LEDGER_SUB_TYPE," +
                                    "BANK_ACCOUNT_ID=?BANK_ACCOUNT_ID, " +
                ...
                case SQLCommand.LedgerBank.UpdateByHeadOffice: //On 04/07/2019, to skip updation of BO ledger option
                    {
                        ...
                        query = "UPDATE MASTER_LEDGER SET " +
                                    "LEDGER_CODE =?LEDGER_CODE, " +
                                    "LEDGER_NAME =?LEDGER_NAME, " +
                                    "DISPLAY_LEDGER_NAME =?DISPLAY_LEDGER_NAME, " +
                                    "GROUP_ID=?GROUP_ID, " +
                                    "BUDGET_GROUP_ID=?BUDGET_GROUP_ID, " +
                                    "BUDGET_SUB_GROUP_ID=?BUDGET_SUB_GROUP_ID, " +
                                    "LEDGER_TYPE=?LEDGER_TYPE," +
                ...
                case SQLCommand.LedgerBank.FetchAll:
                    {
                        ...
                        query = @"SELECT ML.LEDGER_ID,
                                            ...
                                            -- bharath 18/02/2026 language update                   
                                             CASE
                                            WHEN ML.LEDGER_SUB_TYPE = 'BK' THEN
                                                CONCAT(IFNULL(CASE WHEN ?DISPLAY_INPUT_LAN = 1 THEN NULLIF(ML.DISPLAY_LEDGER_NAME,'') ELSE ML.LEDGER_NAME END, ML.LEDGER_NAME), ' - ', MB.BANK, ' - ', MB.BRANCH)
                                            ELSE
                                                IFNULL(CASE WHEN ?DISPLAY_INPUT_LAN = 1 THEN NULLIF(ML.DISPLAY_LEDGER_NAME,'') ELSE ML.LEDGER_NAME END,ML.LEDGER_NAME)
                                            END AS LEDGER_NAME,
                                            IFNULL(MC.CURRENCY_NAME, '') AS CURRENCY_NAME, ML.OP_EXCHANGE_RATE,
                                            ...
                                            CASE
                                                ...
                                                WHEN ML.LEDGER_SUB_TYPE = 'FD' THEN
                                                    'Fixed Deposit'
                                                ELSE
                                                    IFNULL(CASE WHEN ?DISPLAY_INPUT_LAN = 1 THEN MG.DISPLAY_LEDGER_GROUP ELSE MG.LEDGER_GROUP END, MG.LEDGER_GROUP)
                                            END AS 'GROUP',
                                            ML.LEDGER_TYPE,
                ...
                case SQLCommand.LedgerBank.FetchAllIntegration: // FILTERED closed date on 13.04.2022
                    {
                        //bharath ledger id and project id added to udeg
                        query = @"SELECT  ML.LEDGER_ID, MG.GROUP_ID,
                                            CASE
                                                WHEN ML.LEDGER_SUB_TYPE = 'BK' THEN
                                                -- CONCAT(CONCAT(ML.LEDGER_NAME, ' - '), CONCAT(BANK, ' - '), BRANCH)
                                                ML.LEDGER_NAME
                                        ...
                                        WHERE ML.STATUS = 0 {AND UP.ROLE_ID=?USERROLE_ID} {AND PROJECT_CATEGORY_ID =?PROJECT_CATOGORY_ID} AND (ML.GROUP_ID IN (12,13) OR ML.ACCESS_FLAG <>2) AND ML.LEDGER_NAME <> 'cash12' ORDER BY SORT_ID ASC;";
                        break;
                    }
                case SQLCommand.LedgerBank.FetchCCIntegraion: // FILTERED closed date on 13.04.2022
                    {
                        query = @"SELECT MCC.COST_CENTRE_ID,COST_CENTRE_NAME,COST_CENTRE_CATEGORY_NAME FROM MASTER_COST_CENTRE MCC LEFT JOIN 
                                COSTCATEGORY_COSTCENTRE CCC ON CCC.COST_CENTRE_ID = MCC.COST_CENTRE_ID
                                LEFT JOIN MASTER_COST_CENTRE_CATEGORY MCCC 
                                ON MCCC.COST_CENTRECATEGORY_ID = CCC.COST_CATEGORY_ID;";
                        break;
                    }
                ...
                case SQLCommand.LedgerBank.BankAccountFetchAll:
                    {
                        query = "SELECT MA.BANK_ACCOUNT_ID, MA.ACCOUNT_CODE, MA.ACCOUNT_NUMBER, MB.BANK, MB.BRANCH, MA.DATE_OPENED AS DATE_OPENED, " +
                                    ...
                                    "PL.PROJECT," +
                                    "PL.NO_PROJECTS, IFNULL(MC.CURRENCY_NAME, '') AS CURRENCY_NAME, ML.OP_EXCHANGE_RATE " +
                                    ...
                                    "LEFT JOIN MASTER_COUNTRY MC ON MC.COUNTRY_ID = ML.CUR_COUNTRY_ID " +
                                    "LEFT JOIN (SELECT GROUP_CONCAT(IF(?DISPLAY_INPUT_LAN = 1,IFNULL(MP.DISPLAY_PROJECT, MP.PROJECT),MP.PROJECT)SEPARATOR ', ') AS PROJECT,  " +
                ...
                case SQLCommand.LedgerBank.FetchLedgerByGroup: //To combine Bank and Branch include with Bank Accounts
                    {
                        // Chinna 10/04/2026: ?BANK_ALL=0 — same project filter as before. ?BANK_ALL=1 — non-bank still PL.PROJECT_ID=?PROJECT_ID.
                        // Chinna 23/07/2026: ?BANK_ALL=1 AND GROUP_ID=12 — one row per PROJECT_LEDGER map (single project suffix + ROW_PROJECT_ID); no GROUP_CONCAT / MIN(PROJECT_ID). Unmapped bank still one row with 'Unmapped'.
                        query = " SELECT " +
                                     " ML.BANK_ACCOUNT_ID, " +
                                     " ML.GROUP_ID, " +
                                     " MP.NATURE_ID, " +
                                     " ML.LEDGER_ID,ML.LEDGER_CODE, " +
                                     " ML.IS_COST_CENTER, " +
                                   " CONCAT(IF(?DISPLAY_INPUT_LAN=1,IFNULL(ML.DISPLAY_LEDGER_NAME,ML.LEDGER_NAME),ML.LEDGER_NAME),CONCAT(' - ', IF(?DISPLAY_INPUT_LAN=1,IFNULL(MP.DISPLAY_LEDGER_GROUP,MP.LEDGER_GROUP),MP.LEDGER_GROUP)), IF(ML.LEDGER_CODE = '', '', CONCAT(' (', ML.LEDGER_CODE, ')'))," +
                                     " CASE WHEN ML.GROUP_ID = 12 THEN CONCAT(' (', MB.BANK, ' - ', MB.BRANCH, ')') ELSE '' END, " +
                                     " IF(?BANK_ALL = 1 AND ML.GROUP_ID = 12, CONCAT(' - ', IFNULL(IF(?DISPLAY_INPUT_LAN = 1, IFNULL(MPRJ.DISPLAY_PROJECT, MPRJ.PROJECT), MPRJ.PROJECT), 'Unmapped')), '')) AS LEDGER_NAME," +
                                     "ML.IS_BANK_INTEREST_LEDGER, IS_BANK_FD_PENALTY_LEDGER, IS_TDS_LEDGER, IS_GST_LEDGERS, " +
                                     "IFNULL(GST_HSN_SAC_CODE,'') AS GST_HSN_SAC_CODE, GST_SERVICE_TYPE, IFNULL(TCP.GST_ID,0) AS GST_ID, ML.DATE_CLOSED, IFNULL(PL.PROJECT_ID, 0) AS ROW_PROJECT_ID" +
                                     " FROM MASTER_LEDGER  ML " +
                                     "  LEFT JOIN MASTER_BANK_ACCOUNT MBA ON ML.LEDGER_ID = MBA.LEDGER_ID" +
                                     "  LEFT JOIN MASTER_BANK MB ON MB.BANK_ID = MBA.BANK_ID" +
                                     " LEFT JOIN TDS_CREDTIORS_PROFILE TCP ON TCP.LEDGER_ID = ML.LEDGER_ID " +
                                     " LEFT JOIN PROJECT_LEDGER PL ON PL.LEDGER_ID=ML.LEDGER_ID " +
                                     " LEFT JOIN MASTER_PROJECT MPRJ ON MPRJ.PROJECT_ID = PL.PROJECT_ID, " +
                                     ...
                                     " AND ((?BANK_ALL = 0 AND PL.PROJECT_ID = ?PROJECT_ID) OR (?BANK_ALL = 1 AND ML.GROUP_ID <> 12 AND PL.PROJECT_ID = ?PROJECT_ID) OR (?BANK_ALL = 1 AND ML.GROUP_ID = 12)) " +
                ...
                case SQLCommand.LedgerBank.FetchCasBankSundry:   //Added By Dinesh 09-04-2026
                    {
                        query = "SELECT ML.BANK_ACCOUNT_ID,MB.BANK_ID,\n" +
                                "       MP.NATURE_ID,\n" +
                                "       ML.GROUP_ID,\n" +
                ...
                case SQLCommand.LedgerBank.IsBudgetedLedger:
                    {
                        ...
                        // bharath 14/01/2026
                        // For SRBRMG users – this logic auto-cleans BUDGET_LEDGER when no PROJECT_BUDGET_LEDGER exists.
                        query = "DELETE FROM BUDGET_LEDGER " +
                                "WHERE LEDGER_ID = ?LEDGER_ID " +
                                "  AND LEDGER_ID NOT IN (SELECT LEDGER_ID FROM PROJECT_BUDGET_LEDGER);" +
                                "SELECT LEDGER_ID FROM PROJECT_BUDGET_LEDGER WHERE LEDGER_ID = ?LEDGER_ID;";
                        break;
                    }
                ...
                case SQLCommand.LedgerBank.FetchBankLedgerProjectForInterJournal:
                    {
                        query = "SELECT MIN(PL.PROJECT_ID) AS PROJECT_ID, ML.GROUP_ID AS GROUP_ID\n" +
                                "  FROM MASTER_LEDGER ML\n" +
                                " INNER JOIN PROJECT_LEDGER PL ON PL.LEDGER_ID = ML.LEDGER_ID\n" +
                                " WHERE ML.LEDGER_ID = ?LEDGER_ID AND ML.STATUS = 0\n" +
                                " GROUP BY ML.GROUP_ID";
                        break;
                    }
                case SQLCommand.LedgerBank.FetchInternalAdjustmentLedgerByProject:
                    {
                        // ACCESS_FLAG = 2 and LEDGER_NAME contains the configured text (e.g. INTERNAL ADJUSTMENT).
                        query = "SELECT MIN(ML.LEDGER_ID) AS LEDGER_ID\n" +
                                "  FROM MASTER_LEDGER ML\n" +
                                " INNER JOIN PROJECT_LEDGER PL ON PL.LEDGER_ID = ML.LEDGER_ID\n" +
                                " WHERE PL.PROJECT_ID = ?PROJECT_ID\n" +
                                "   AND ML.ACCESS_FLAG = 2\n" +
                                "   AND ML.STATUS = 0\n" +
                                "   AND ML.LEDGER_NAME LIKE CONCAT('%', ?LEDGER_NAME, '%')";
                        break;
                    }
                ...
                case SQLCommand.LedgerBank.FetchLedgerByIncludeCostCentre:
                    {
                        query = "SELECT LEDGER_ID,\n" +
                            //bharath 18/02/2026 language update
                            //"       MLG.LEDGER_GROUP,\n" +
                             "       IFNULL(CASE WHEN ?DISPLAY_INPUT_LAN = 1 THEN MLG.DISPLAY_LEDGER_GROUP ELSE MLG.LEDGER_GROUP END, MLG.LEDGER_GROUP) AS LEDGER_GROUP,\n" +
                                "       LEDGER_CODE,\n" +
                            //"       LEDGER_NAME,\n" +
                                 "       IFNULL(CASE WHEN ?DISPLAY_INPUT_LAN = 1 THEN ML.DISPLAY_LEDGER_NAME ELSE ML.LEDGER_NAME END, ML.LEDGER_NAME) AS LEDGER_NAME,\n" +
                ...
                case SQLCommand.LedgerBank.FetchLedgerByIncludeBankFDPenaltyLedger:
                    {
                        query = "SELECT LEDGER_ID,  " +
                            //"MLG.LEDGER_GROUP, LEDGER_CODE, LEDGER_NAME,\n" +
                           " IFNULL(CASE WHEN ?DISPLAY_INPUT_LAN = 1 THEN MLG.DISPLAY_LEDGER_GROUP ELSE MLG.LEDGER_GROUP END, MLG.LEDGER_GROUP) AS LEDGER_GROUP, ML.LEDGER_CODE, IFNULL(CASE WHEN ?DISPLAY_INPUT_LAN = 1 THEN ML.DISPLAY_LEDGER_NAME ELSE ML.LEDGER_NAME END, ML.LEDGER_NAME) AS LEDGER_NAME,\n" +
                ...
                case SQLCommand.LedgerBank.FetchProvinceLedgers: //added By Dinesh 18-05-2026
                    {
                        query = "SELECT ML.LEDGER_ID,\n" +
                                  "       MLG.LEDGER_GROUP,\n" +
                                  "       ML.LEDGER_CODE,\n" +
                                  "       IFNULL(CASE WHEN ?DISPLAY_INPUT_LAN = 1 THEN ML.DISPLAY_LEDGER_NAME ELSE ML.LEDGER_NAME END, ML.LEDGER_NAME) AS LEDGER_NAME,\n" +
                                  "       ML.IS_PROVINCE_LEDGER AS SELECT_TMP\n" +
                                  "  FROM MASTER_LEDGER ML\n" +
                                  " INNER JOIN MASTER_LEDGER_GROUP MLG\n" +
                                  "    ON MLG.GROUP_ID = ML.GROUP_ID\n" +
                                  " WHERE MLG.NATURE_ID = 1;";
                        break;
                    }
                ...
                case SQLCommand.LedgerBank.UpdateLedgerProvinceOne: //Added by dinesh 18-05-2026
                    {
                        query = "UPDATE MASTER_LEDGER SET IS_PROVINCE_LEDGER = 1 WHERE LEDGER_ID IN (?LEDGER_ID)";
                        break;
                    }
                case SQLCommand.LedgerBank.UpdateLedgerOptionProvincezero: //Added by dinesh 18-05-2026
                    {
                        query = "UPDATE MASTER_LEDGER SET IS_PROVINCE_LEDGER=0;";
                        break;
                    }
                ...
                case SQLCommand.LedgerBank.FetchLedgerByFixedGroup:
                    {
                        query = "SELECT ML.BANK_ACCOUNT_ID,\n" +
                            ...
                            //"       CONCAT(ML.LEDGER_NAME,\n" +
                                "       CONCAT(IFNULL(IF(?DISPLAY_INPUT_LAN = 1, ML.DISPLAY_LEDGER_NAME, ML.LEDGER_NAME), ML.LEDGER_NAME),\n" +
                ...
                case SQLCommand.LedgerBank.FetchLedgerById:
                    {
                        query = "SELECT LEDGER_NAME AS LEDGER, IFNULL(GST_HSN_SAC_CODE,'') AS GST_HSN_SAC_CODE, GROUP_ID FROM MASTER_LEDGER WHERE LEDGER_ID IN (?LEDGER_ID);";
                        break;
                    }
                ...
                case SQLCommand.LedgerBank.ChangeLedgerGroupNameInMaster:
                    {
                        query = "UPDATE MASTER_LEDGER_GROUP SET LEDGER_GROUP = ?LEDGER_GROUP WHERE LEDGER_GROUP = ?OLD_LEDGER_GROUP;";
                        break;
                    }
                case SQLCommand.LedgerBank.ChangeLedgerNameInMasterAOS:
                    {
                        query = "UPDATE MASTER_LEDGER SET LEDGER_NAME = ?LEDGER_NAME WHERE LEDGER_NAME = ?OLD_LEDGER_NAME;";
                        break;
                    }
