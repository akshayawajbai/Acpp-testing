        ...
        private string GetCostCentreCatogorySQL()
        {
            ...
            switch (sqlCommandId)
            {
                ...
                case SQLCommand.CostCentreCategory.CheckCostcentreCostCategoryExists:
                    ...
                    //bharath
                case SQLCommand.CostCentreCategory.DeleteMappings:
                    {
                        query = @"
                                  DELETE ccc
                                  FROM costcategory_costcentre ccc
                                  LEFT JOIN master_cost_centre mc
                                  ON mc.COST_CENTRE_ID = ccc.COST_CENTRE_ID
                                  WHERE ccc.COST_CATEGORY_ID = ?COST_CATEGORY_ID
                                  AND mc.COST_CENTRE_ID IS NULL";
                        break;
                    }

                //bharath
                case SQLCommand.CostCentreCategory.FetchCostCentreCategoryIdByApiId:
                    {
                        query = "SELECT COST_CENTRECATEGORY_ID FROM MASTER_COST_CENTRE_CATEGORY WHERE THIRD_PARTY_COST_CENTRE_CATEGORY_ID = ?THIRD_PARTY_COST_CENTRE_CATEGORY_ID";
                        break;
                    }

                case SQLCommand.CostCentreCategory.AddWithApi:
                    {
                        query = "INSERT INTO MASTER_COST_CENTRE_CATEGORY (" +
                                "COST_CENTRE_CATEGORY_NAME, THIRD_PARTY_COST_CENTRE_CATEGORY_ID) VALUES (" +
                                "?COST_CENTRE_CATEGORY_NAME, ?THIRD_PARTY_COST_CENTRE_CATEGORY_ID)";
                        break;
                    }

                case SQLCommand.CostCentreCategory.UpdateWithApi:
                    {
                        query = "UPDATE MASTER_COST_CENTRE_CATEGORY SET " +
                                "COST_CENTRE_CATEGORY_NAME = ?COST_CENTRE_CATEGORY_NAME, " +
                                "THIRD_PARTY_COST_CENTRE_CATEGORY_ID = ?THIRD_PARTY_COST_CENTRE_CATEGORY_ID " +
                                "WHERE COST_CENTRECATEGORY_ID = ?COST_CENTRECATEGORY_ID";
                        break;
                    }
            }
