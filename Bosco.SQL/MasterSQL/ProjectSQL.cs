        ...
        private string GetProjectSQL()
        {
            ...
            switch (sqlCommandId)
            {
                case SQLCommand.Project.Add:
                    {
                        // 17/02/2026, added DISPLAY_PROJECT for second-level language (e.g. French)
                ...
                case SQLCommand.Project.FetchProjects: // 15/11/2024, Chinna, Included the ITR
                    {
                        query = "SELECT MP.PROJECT_ID, PROJECT_CODE,"+
                                  //"PROJECT,"+
                                  "IFNULL(IF(?DISPLAY_INPUT_LAN = 1, MP.DISPLAY_PROJECT, MP.PROJECT), MP.PROJECT) AS PROJECT," +
                                  "DIVISION," +
                ...
                case SQLCommand.Project.FetchProjectsIntegration:
                    {
                        query =// "SELECT MP.PROJECT_ID,PROJECT " +
                                "SELECT MP.PROJECT_ID,PROJECT,MPC.PROJECT_CATOGORY_ID,MPC.PROJECT_CATOGORY_NAME " +
                                " FROM MASTER_PROJECT MP" +
                                    " INNER JOIN MASTER_DIVISION MD ON" +
                                    " MP.DIVISION_ID=MD.DIVISION_ID" +
                                    " INNER JOIN MASTER_PROJECT_CATOGORY AS MPC" +
                                    " ON MP.PROJECT_CATEGORY_ID=MPC.PROJECT_CATOGORY_ID " +
                                    " LEFT JOIN USER_PROJECT UP ON MP.PROJECT_ID=UP.PROJECT_ID " +
                                    " WHERE DELETE_FLAG<>1 {AND UP.ROLE_ID=?USERROLE_ID} {AND MPC.PROJECT_CATOGORY_ID=?PROJECT_CATOGORY_ID} " +
                                    " {AND (MP.DATE_CLOSED IS NULL OR MP.DATE_CLOSED >= ?DATE_CLOSED) } " + //On 12/07/2018, This property is used to skip Projects which is closed on or equal to this date
                                    " GROUP BY PROJECT ORDER BY MP.PROJECT ASC";
                        break;
                    }
                ...
                case SQLCommand.Project.FetchVoucherDetailsByProjectId:
                    {
                        query = "SELECT MV.VOUCHER_ID,VOUCHER_NAME,MV.IS_NARRATION_ENABLED,MV.IS_PAN_ENABLED, MV.IS_GST_ENABLED ,VOUCHER_TYPE,VOUCHER_METHOD,PREFIX_CHAR,SUFFIX_CHAR,STARTING_NUMBER,NUMBERICAL_WITH,PREFIX_WITH_ZERO, " + //bharath 04/06/2026 Ledger PAN and GST set
                ...
                case SQLCommand.Project.FetchProjectDetails:
                    {
                        query = "SELECT PROJECT_ID, CONCAT(CONCAT(IFNULL(IF(?DISPLAY_INPUT_LAN = 1, MP.DISPLAY_PROJECT, MP.PROJECT), MP.PROJECT), '-'), DIVISION) AS PROJECT\n" +
                ...
                case SQLCommand.Project.FetchProjectBySociety:
                    {
                        ...
                        query = "SELECT " +
                                  ...
                                  "CONCAT( " +
                            //"MP.PROJECT, " + 
                                   "IFNULL(CASE WHEN ?DISPLAY_INPUT_LAN = 1 THEN MP.DISPLAY_PROJECT ELSE MP.PROJECT END, MP.PROJECT), " +
                                  "CONCAT(' - ',MD.DIVISION)) AS 'PROJECT', " +
                                  "MP.DATE_CLOSED, " +
                            ...
                            //"MP.PROJECT AS PROJECT_NAME " +
                              ...
                              "FROM " +
                                  ...
                                  " { AND MP.PROJECT_CATEGORY_ID=?PROJECT_CATOGORY_ID} " +
                ...
                case SQLCommand.Project.ChangeProjectNameInMaster:
                    {
                        query = "UPDATE MASTER_PROJECT SET PROJECT = ?PROJECT WHERE PROJECT = ?OLD_PROJECT;";
                        break;
                    }
