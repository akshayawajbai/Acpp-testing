    ...
    public class PurposeSQL : IDatabaseQuery
    {
        ...
        public string GetPurposeSQL()
        {
            ...
            switch (sqlCommandId)
            {
                ...
                case SQLCommand.Purposes.DeleteSDBRMGDefaults:
                    {
                        query = "DELETE FROM MASTER_CONTRIBUTION_HEAD WHERE CONTRIBUTION_ID BETWEEN 1 AND 56 ";
                        break;
                    }
                ...
                case SQLCommand.Purposes.FetchAllWithProject:
                    {
                        query = "SELECT MCH.CONTRIBUTION_ID, MCH.CODE, MCH.FC_PURPOSE, " +
                                "IFNULL(GROUP_CONCAT(DISTINCT PP.PROJECT_ID ORDER BY PP.PROJECT_ID SEPARATOR ','), '') AS PROJECT_ID " +
                                "FROM MASTER_CONTRIBUTION_HEAD MCH " +
                                "LEFT JOIN PROJECT_PURPOSE PP ON MCH.CONTRIBUTION_ID = PP.CONTRIBUTION_ID " +
                                "GROUP BY MCH.CONTRIBUTION_ID, MCH.CODE, MCH.FC_PURPOSE " +
                                "ORDER BY MCH.CONTRIBUTION_ID ASC";
                        break;
                    }
                case SQLCommand.Purposes.FetchProjectsByPurpose:
                    {
                        query = "SELECT DISTINCT PROJECT_ID FROM PROJECT_PURPOSE WHERE FIND_IN_SET(CONTRIBUTION_ID, ?CONTRIBUTION_ID)";
                        break;
                    }
            }
            return query;
