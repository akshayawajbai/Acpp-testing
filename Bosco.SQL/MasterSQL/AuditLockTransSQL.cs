    ...
    public class AuditLockTransSQL : IDatabaseQuery
    {
        ...
        private string GetStateSQL()
        {
            ...
            switch (sqlCommandId)
            {
                ...
                case SQLCommand.AuditLockTrans.AddUsers: //Added By Dinesh 13-05-2026
                    {
                        query = @"INSERT INTO master_lock_trans_user(LOCK_TRANS_ID,USER_ID) VALUES(?LOCK_TRANS_ID,?USER_ID)";
                        break;
                    }
                case SQLCommand.AuditLockTrans.UpdateUsers: //Added By Dinesh 13-05-2026
                    {
                        query = @"UPDATE master_lock_trans_user SET USER_ID = ?USER_ID WHERE LOCK_TRANS_ID = ?LOCK_TRANS_ID";
                        break;
                    }

                case SQLCommand.AuditLockTrans.UpdateUsersMain:
                    {
                        query = query = @" INSERT INTO MASTER_LOCK_TRANS_USER (LOCK_TRANS_ID, USER_ID)
                                            SELECT ?LOCK_TRANS_ID, ?USER_ID
                                            FROM DUAL
                                            WHERE NOT EXISTS
                                            (
                                                SELECT 1
                                                FROM MASTER_LOCK_TRANS_USER
                                                WHERE LOCK_TRANS_ID = ?LOCK_TRANS_ID
                                                  AND USER_ID = ?USER_ID
                                            )";
                                    break;
                    }
                case SQLCommand.AuditLockTrans.GetLastLockTransId://Added By Dinesh 13-05-2026
                    {
                        query = @"SELECT LOCK_TRANS_ID
                                  FROM MASTER_LOCK_TRANS
                                  ORDER BY LOCK_TRANS_ID DESC
                                  LIMIT 1";
                        break;
                    }
                case SQLCommand.AuditLockTrans.DeleteUsers: //Added By Dinesh 13-05-2026
                    {
                        query = @"DELETE FROM master_lock_trans_user
                                    WHERE LOCK_TRANS_ID = ?LOCK_TRANS_ID";
                        break;
                    }
                case SQLCommand.AuditLockTrans.FetchUsers://Added By Dinesh 13-05-2026
                    {
                        query = @"SELECT USER_ID FROM master_lock_trans_user
                                WHERE LOCK_TRANS_ID = ?LOCK_TRANS_ID";
                        break;
                    }
                ...
                case SQLCommand.AuditLockTrans.FetchAuditLockDetailsForProjectAndDate:
                    {
                        ...
                        //break;//Modifyed By Dinesh 14-05-2026 : Purpose  of Valid On User Id 
                        //query = " SELECT MLT.DATE_FROM, MLT.DATE_TO, MP.PROJECT \n" +
                        //               " FROM MASTER_LOCK_TRANS MLT \n" +
                        //               " INNER JOIN MASTER_PROJECT AS MP \n" +
                        //               "     ON MP.PROJECT_ID = MLT.PROJECT_ID \n" +
                        //               " WHERE MLT.PROJECT_ID = ?PROJECT_ID \n" +
                        //               " AND ?VOUCHER_DATE BETWEEN MLT.DATE_FROM AND MLT.DATE_TO \n" +
                        //               " AND NOT EXISTS ( \n" +
                        //               "     SELECT 1 \n" +
                        //               "     FROM MASTER_LOCK_TRANS_USER MLTU \n" +
                        //               "     WHERE MLTU.LOCK_TRANS_ID = MLT.LOCK_TRANS_ID \n" +
                        //               "     AND MLTU.USER_ID = ?USER_ID \n" +
                        //               " )";
                        query = " SELECT MLT.DATE_FROM, MLT.DATE_TO, MP.PROJECT \n" +
                                    " FROM MASTER_LOCK_TRANS MLT \n" +
                                    " INNER JOIN MASTER_PROJECT AS MP \n" +
                ...
                case SQLCommand.AuditLockTrans.FetchAuditLockDetailByProjectDateRange:
                    {
                        ...
                        //break; //Modifyed By Dinesh 14-05-2026 : Purpose  of Valid On User Id 
                        query = " SELECT MLT.LOCK_TRANS_ID, MLT.DATE_FROM, MLT.DATE_TO, MP.PROJECT \n" +
                                   " FROM MASTER_LOCK_TRANS MLT \n" +
                                   " INNER JOIN MASTER_PROJECT AS MP \n" +
                                   "     ON MP.PROJECT_ID = MLT.PROJECT_ID \n" +
                                   " WHERE MLT.PROJECT_ID = ?PROJECT_ID \n" +
                                   " AND ((?DATE_FROM BETWEEN MLT.DATE_FROM AND MLT.DATE_TO) \n" +
                                   "      OR (?DATE_TO BETWEEN MLT.DATE_FROM AND MLT.DATE_TO)) \n" +
                                   " AND EXISTS ( \n" +
                                   "     SELECT 1 \n" +
                                   "     FROM MASTER_LOCK_TRANS_USER MLTU \n" +
                                   "     WHERE MLTU.LOCK_TRANS_ID = MLT.LOCK_TRANS_ID \n" +
