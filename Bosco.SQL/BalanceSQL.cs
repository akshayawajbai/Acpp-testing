                ...
                case SQLCommand.TransBalance.FetchOpBalanceList:
                    {
                        ...
                        query = "SELECT PL.PROJECT_ID, PL.LEDGER_ID,IF(MIP.LEDGER_ID IS NULL,0,MIP.LEDGER_ID) AS LEGAL_ENTITY_LEDGER_ID,IF(MIP.CUSTOMERID IS NULL,0,MIP.CUSTOMERID) AS CUSTOMER_ID, " +
                                "ML.BANK_ACCOUNT_ID, ML.GROUP_ID,ML.ACCESS_FLAG, " +
                                          "ML.CUR_COUNTRY_ID AS CUR_COUNTRY_ID, " +
                                ...
                                "CASE " +
                                    ...
                                    "CONCAT(IFNULL(CASE WHEN ?DISPLAY_INPUT_LAN = 1 THEN ML.DISPLAY_LEDGER_NAME ELSE ML.LEDGER_NAME END, ML.LEDGER_NAME), ' - ', MB.BANK, ' - ', MB.BRANCH) " +
                                    //"WHEN LEDGER_GROUP='Bank Accounts' THEN " +
                                    // "CONCAT(CONCAT(ML.LEDGER_NAME,' - '),CONCAT(MB.BANK,' - '),MB.BRANCH) " +
                                    "ELSE " +
                                     //"ML.LEDGER_NAME " +
                                     "IFNULL(CASE WHEN ?DISPLAY_INPUT_LAN = 1 THEN ML.DISPLAY_LEDGER_NAME ELSE ML.LEDGER_NAME END, ML.LEDGER_NAME) " +
                                "END AS 'LEDGER_NAME', " +
                                //"LG.LEDGER_GROUP, " +
                                 "IFNULL(CASE WHEN ?DISPLAY_INPUT_LAN = 1 THEN LG.DISPLAY_LEDGER_GROUP ELSE LG.LEDGER_GROUP END, LG.LEDGER_GROUP) AS LEDGER_GROUP, " +
                                ...
                                "CASE WHEN LEDGER_TYPE='GN' " +
                                  ...
                                  "CASE " +
                                        ...
                                        "ELSE " +
                                            //"LEDGER_GROUP " +
                                            "IFNULL(CASE WHEN ?DISPLAY_INPUT_LAN = 1 THEN LG.DISPLAY_LEDGER_GROUP ELSE LG.LEDGER_GROUP END, LG.LEDGER_GROUP) " +
                                        "END " +
                                    "END AS 'TYPE', " +
                                ...
                                " IFNULL(LB.AMOUNT_FC, 0) AS AMOUNT_FC, " +
                                " IF(ML.OP_EXCHANGE_RATE = 0 OR ML.OP_EXCHANGE_RATE IS NULL, IF(MCE.EXCHANGE_RATE = 0 OR MCE.EXCHANGE_RATE IS NULL, 1, MCE.EXCHANGE_RATE), ML.OP_EXCHANGE_RATE) AS OP_EXCHANGE_RATE " +
                ...
                case SQLCommand.TransBalance.FetchBudgetMappedLedgerList:
                    {
                        query = @" SELECT
                                    PL.PROJECT_ID,
                                    PL.LEDGER_ID,
                                   IF(MIP.LEDGER_ID IS NULL,0,MIP.LEDGER_ID) AS LEGAL_ENTITY_LEDGER_ID,IF(MIP.CUSTOMERID IS NULL,0,MIP.CUSTOMERID) AS CUSTOMER_ID,
                                    ML.BANK_ACCOUNT_ID,
                                    ML.GROUP_ID,
                                    ML.ACCESS_FLAG,
                                   ...
                                   -- bharath 18/02/2026 language update
                                    ...
                                    IFNULL(CASE WHEN ?DISPLAY_INPUT_LAN = 1 THEN LG.DISPLAY_LEDGER_GROUP ELSE LG.LEDGER_GROUP END, LG.LEDGER_GROUP) AS LEDGER_GROUP,
                                    IFNULL(BL_PROJ.PROPOSED_AMOUNT, 0) AS AMOUNT,
                                    ML.IS_TDS_LEDGER,
                                    LG.NATURE_ID,
                                    ML.LEDGER_CODE,
                                    COALESCE(NULLIF(LB.TRANS_MODE, ''),
                                             CASE WHEN LG.NATURE_ID IN (1,4) THEN 'CR' ELSE 'DR' END) AS TRANS_MODE,
                                    CASE
                                        WHEN ML.LEDGER_TYPE = 'GN' THEN 'General'
                                        WHEN ML.LEDGER_TYPE = 'IK' THEN 'In kind'
                                        ELSE ML.LEDGER_TYPE
                                    END AS `GROUP`,
                                    CASE
                                        WHEN ML.LEDGER_SUB_TYPE = 'BK' THEN 'Bank Accounts'
                                        WHEN ML.LEDGER_SUB_TYPE = 'FD' THEN 'Fixed Deposit'
                                      --  ELSE LG.LEDGER_GROUP
                                        ELSE IFNULL(CASE WHEN ?DISPLAY_INPUT_LAN = 1 THEN LG.DISPLAY_LEDGER_GROUP ELSE LG.LEDGER_GROUP END, LG.LEDGER_GROUP)
                                    END AS `TYPE`,
                                    IFNULL(LB.LEDGER_ID, 0) AS UPDATE_MODE
                                ...
                                LEFT JOIN (
                                    ...
                                    GROUP BY BP.PROJECT_ID, BL.LEDGER_ID
                                ) AS BL_PROJ
                                       ON BL_PROJ.PROJECT_ID = PL.PROJECT_ID
                                      AND BL_PROJ.LEDGER_ID  = PL.LEDGER_ID
                                WHERE PL.PROJECT_ID = ?PROJECT_ID  {AND PL.LEDGER_ID = ?LEDGER_ID} {AND ML.GROUP_ID = ?GROUP_ID}
                                ORDER BY ML.SORT_ID";
                ...
                case SQLCommand.TransBalance.FetchBalance:
                    {
                        ...
                        query = "SELECT LG.GROUP_ID, LG.GROUP_CODE, " +
                                 " CASE WHEN LG.GROUP_ID = " + (int)Utility.FixedLedgerGroup.Cash + " THEN CONCAT('  ',IF(?DISPLAY_INPUT_LAN=1,LG.DISPLAY_LEDGER_GROUP,LG.LEDGER_GROUP)) " + // Done on 18/05/2017, refer top history #2
                                 "WHEN LG.GROUP_ID = " + (int)Utility.FixedLedgerGroup.BankAccounts + " THEN CONCAT(' ',IF(?DISPLAY_INPUT_LAN=1,LG.DISPLAY_LEDGER_GROUP,LG.LEDGER_GROUP)) " +
                                 "WHEN LG.GROUP_ID = " + (int)Utility.FixedLedgerGroup.FixedDeposit + " THEN CONCAT('',IF(?DISPLAY_INPUT_LAN=1,LG.DISPLAY_LEDGER_GROUP,LG.LEDGER_GROUP)) " +
                                 "END AS LEDGER_GROUP, " +
