    ...
    public class AssetInwardOutwardSQL : IDatabaseQuery
    {
        ...
        private string GetAssetInOutSQL()
        {
            ...
            switch (sqlCommandId)
            {
                ...
                case SQLCommand.AssetInOut.FetchAssetInOutMasterByFlag:
                    {
                        ...
                        query = "SELECT AM.IN_OUT_ID,\n" +
                                  ...
                                  "       ASV.VENDOR_ID,\n" +
                                  "       AM.SOLD_TO,\n" +
                                  "       CASE WHEN AM.FLAG='DN' THEN 'Donate'\n" +
                                  "            WHEN AM.FLAG='PU' THEN 'Purchase'\n" +
                                  "            WHEN AM.FLAG='DS' THEN 'Dispose'\n" +
                                  "            WHEN AM.FLAG='SL' THEN 'Sales'\n" +
                                  "            WHEN AM.FLAG='IK' THEN 'In-Kind' END AS VOUCHER_TYPE,\n" +
                                  "       T.QUANTITY,\n" +
                                  "       ASV.VENDOR,\n" +
                                  "       AM.PROJECT_ID,\n" +
                                  "       AM.VOUCHER_ID,\n" +
                                  ...
                                  "       AM.FLAG,\n" +
                                  "       ASV.BRANCH_ID,\n" +
                                  "       VMT.CURRENCY_COUNTRY_ID,\n" +
                                  "       SUBSTRING_INDEX(GROUP_CONCAT(AI.ASSET_ITEM ORDER BY AID.IN_OUT_DETAIL_ID), ',', 1) AS FIRST_ASSET_NAME,\n" +
                                  "       CASE\n" +
                                  "           WHEN GRP.GROUP_ID IN (12,13)\n" +
                                  "                THEN ROUND(AM.TOT_AMOUNT,2)\n" +
                                  "           WHEN IFNULL(VGI.GST_AMOUNT,0) = 0\n" +
                                  "                THEN 0.00\n" +
                                  "           ELSE ROUND(IFNULL(VGI.GST_AMOUNT,0),2)\n" +
                                  "       END AS PAID_AMOUNT,\n" +
                                  "       CASE\n" +
                                  "           WHEN GRP.GROUP_ID IN (12,13)\n" +
                                  "                THEN 0.00\n" +
                                  "           WHEN IFNULL(VGI.GST_AMOUNT,0) = 0\n" +
                                  "                THEN ROUND(AM.TOT_AMOUNT,2)\n" +
                                  "           ELSE ROUND(AM.TOT_AMOUNT - IFNULL(VGI.GST_AMOUNT,0),2)\n" +
                                  "       END AS BALANCE\n" +
                                  "  FROM ASSET_IN_OUT_MASTER AM\n" +
                                  "  LEFT JOIN ASSET_STOCK_VENDOR ASV\n" +
                                  "    ON AM.VENDOR_ID = ASV.VENDOR_ID\n" +
                                  "  JOIN (SELECT IN_OUT_ID, SUM(QUANTITY) AS QUANTITY\n" +
                                  "          FROM ASSET_IN_OUT_DETAIL\n" +
                                  "         GROUP BY IN_OUT_ID) T\n" +
                                  "    ON AM.IN_OUT_ID = T.IN_OUT_ID\n" +
                                  "  LEFT JOIN ASSET_IN_OUT_DETAIL AID\n" +
                                  "    ON AM.IN_OUT_ID = AID.IN_OUT_ID\n" +
                            ...
                            /*  PAYMENT JOIN */
                                 "  LEFT JOIN (\n" +
                                 "      SELECT VGI.GST_INVOICE_ID, SUM(VGI.AMOUNT) AS GST_AMOUNT\n" +
                                 "      FROM VOUCHER_GST_INVOICE VGI\n" +
                                 "      INNER JOIN ASSET_IN_OUT_MASTER AM1\n" +
                                 "          ON AM1.IN_OUT_ID = VGI.GST_INVOICE_ID\n" +
                                 "          AND AM1.FLAG = 'PU'\n" +
                                 "      WHERE NOT EXISTS (\n" +
                                 "          SELECT 1 FROM GST_INVOICE_MASTER GIM\n" +
                                 "          WHERE GIM.GST_INVOICE_ID = VGI.GST_INVOICE_ID\n" +
                                 "          AND GIM.BOOKING_VOUCHER_TYPE = 'JN'\n" +
                                 "          AND GIM.BOOKING_VOUCHER_ID = VGI.VOUCHER_ID\n" +
                                 "      )\n" +
                                 "      GROUP BY VGI.GST_INVOICE_ID\n" +
                                 "  ) VGI ON VGI.GST_INVOICE_ID = AM.IN_OUT_ID\n" +
                                  "  LEFT JOIN (\n" +
                                  "      SELECT AIM.IN_OUT_ID,\n" +
                                  "             CASE \n" +
                                  "                 WHEN SUM(CASE WHEN ML.GROUP_ID IN (12,13) THEN 1 ELSE 0 END) > 0\n" +
                                  "                      THEN 12\n" +
                                  "                 ELSE 26\n" +
                                  "             END AS GROUP_ID\n" +
                                  "      FROM ASSET_IN_OUT_MASTER AIM\n" +
                                  "      INNER JOIN VOUCHER_TRANS VT\n" +
                                  "          ON VT.VOUCHER_ID = AIM.VOUCHER_ID\n" +
                                  "      INNER JOIN MASTER_LEDGER ML\n" +
                                  "          ON ML.LEDGER_ID = VT.LEDGER_ID\n" +
                                  "      GROUP BY AIM.IN_OUT_ID\n" +
                                  "  ) GRP ON GRP.IN_OUT_ID = AM.IN_OUT_ID\n" +
                                  " WHERE AM.PROJECT_ID = ?PROJECT_ID\n" +
                                  "   AND AM.IN_OUT_DATE BETWEEN ?DATE_FROM AND ?DATE_TO\n" +
                                  "   AND FIND_IN_SET(AM.FLAG, ?FLAG)\n" +
                ...
                case SQLCommand.AssetInOut.FetchAssetPaymentDetail:
                    {
                        query = "SELECT \n" +
                                "    @rownum := @rownum + 1 AS `No Payment`, \n" +
                                "    T.`Date`, \n" +
                                "    T.`Cash/Bank`, \n" +
                                "    T.`Paid Amount`, \n" +
                                "    T.IN_OUT_ID \n" +
                                ...
                                "    SELECT \n" +
                                "        AIM1.IN_OUT_ID, \n" +
                                "        AIM2.VOUCHER_DATE AS `Date`, \n" +
                                "        ML.LEDGER_NAME AS `Cash/Bank`, \n" +
                                "        ROUND(AIM2.CONTRIBUTION_AMOUNT,2) AS `Paid Amount` \n" +
                                "    FROM Voucher_gst_Invoice VGI \n" +
                                "    INNER JOIN Asset_in_out_master AIM1 \n" +
                                "        ON VGI.GST_INVOICE_ID = AIM1.IN_OUT_ID \n" +
                                "        AND AIM1.FLAG = 'PU' \n" +
                                "        AND NOT EXISTS ( \n" +
                                "            SELECT 1 FROM GST_INVOICE_MASTER GIM \n" +
                                "            WHERE GIM.GST_INVOICE_ID = VGI.GST_INVOICE_ID \n" +
                                ...
                                "            AND GIM.BOOKING_VOUCHER_ID = VGI.VOUCHER_ID \n" +
                                "        ) \n" +
                                "    LEFT JOIN Voucher_master_trans AIM2 \n" +
                                "        ON VGI.VOUCHER_ID = AIM2.VOUCHER_ID \n" +
                                "    LEFT JOIN Voucher_trans VT \n" +
                                "        ON VT.VOUCHER_ID = VGI.VOUCHER_ID \n" +
                                "    LEFT JOIN Master_Ledger ML \n" +
                                "        ON ML.LEDGER_ID = VT.LEDGER_ID \n" +
                                "    WHERE ML.GROUP_ID IN (12,13) \n" +
                                "      AND AIM1.IN_OUT_ID IN (?IN_OUT_ID) \n" +
                                "    ORDER BY AIM2.VOUCHER_DATE \n" +
                                ") T, \n" +
                                "(SELECT @rownum := 0) r;";
                        break;
                    }
                ...
                case SQLCommand.AssetInOut.FetchAssetInOutDetailById:
                    {
                        ...
                        query = "SELECT AID.IN_OUT_DETAIL_ID,\n" +
                                  ...
                                  " WHERE AIM.IN_OUT_ID = ?IN_OUT_ID\n" +
                                  " GROUP BY IN_OUT_DETAIL_ID;";
                        //query = "SELECT AID.SEQUENCE_NO, " +   //Modifiyed by Dinesh 
                        //                "       AID.IN_OUT_DETAIL_ID, " +
                        //                "       NULL AS CR_DETAIL_ID,  " +
                        //                "       LOCATION_ID, " +
                        //                "       AIM.IN_OUT_ID, " +
                        //                "       AID.ITEM_ID, " +
                        //                "       AID.SOURCE, " +
                        ...
                        //                "INNER JOIN ASSET_IN_OUT_DETAIL AID " +
                        //                "   ON AIM.IN_OUT_ID = AID.IN_OUT_ID " +
                        //                "LEFT JOIN ASSET_ITEM_DETAIL AI " +
                        //                "   ON AID.ITEM_ID = AI.ITEM_ID " +
                        //                "INNER JOIN ASSET_ITEM A " +
                        //                "   ON AID.ITEM_ID = A.ITEM_ID " +
                        //                "LEFT JOIN ( " +
                        //                "   SELECT SUM(AT.AMOUNT) AS ACT_AMT, AT.IN_OUT_DETAIL_ID " +
                        //                "   FROM ASSET_IN_OUT_MASTER AIM " +
                        //                "   INNER JOIN ASSET_IN_OUT_DETAIL AID " +
                        //                "       ON AIM.IN_OUT_ID = AID.IN_OUT_ID " +
                        //                "   INNER JOIN ASSET_TRANS AT " +
                        ...
                        //                "   WHERE FLAG IN ('SL','DN','DS') " +
                        //                "     AND AIM.IN_OUT_ID = ?IN_OUT_ID " +
                        //                "   GROUP BY AT.IN_OUT_DETAIL_ID, ITEM_ID " +
                        //                ") TF ON AID.IN_OUT_DETAIL_ID = TF.IN_OUT_DETAIL_ID " +
                        //                "LEFT JOIN ( " +
                        //                "   SELECT SUM(IN_QTY) - SUM(OUT_QTY) AS AVAIL_QTY, ITEM_ID, PROJECT_ID " +
                        //                "   FROM ( " +
                        //                "       SELECT IOD.ITEM_ID, AIM.PROJECT_ID, " +
                        //                "           CASE WHEN FLAG IN ('PU','IK','OP') THEN COUNT(FLAG) ELSE 0 END AS IN_QTY, " +
                        //                "           CASE WHEN FLAG IN ('SL','DN','DS') THEN COUNT(FLAG) ELSE 0 END AS OUT_QTY " +
                        //                "       FROM ASSET_IN_OUT_MASTER AIM " +
                        //                "       INNER JOIN ASSET_IN_OUT_DETAIL IOD " +
                        //                "           ON AIM.IN_OUT_ID = IOD.IN_OUT_ID " +
                        //                "       LEFT JOIN ASSET_TRANS AT " +
                        //                "           ON IOD.IN_OUT_DETAIL_ID = AT.IN_OUT_DETAIL_ID " +
                        //                "       GROUP BY IOD.IN_OUT_DETAIL_ID " +
                        //                "   ) TT GROUP BY ITEM_ID, PROJECT_ID " +
                        //                ") FNL ON AID.ITEM_ID = FNL.ITEM_ID " +
                        //                "       AND AIM.PROJECT_ID = FNL.PROJECT_ID " +
                        //                "WHERE AIM.IN_OUT_ID = ?IN_OUT_ID " +
                        //                "GROUP BY AID.IN_OUT_DETAIL_ID " +

                        //                /* ===== ONLY NEW ADDITION START ===== */
                        //                "UNION ALL " +
                        //                "SELECT CR.SEQUENCE_NO, " +
                        //                "       NULL AS IN_OUT_DETAIL_ID, " +
                        //                "       CR.CR_DETAIL_ID, " +
                        //                "       NULL AS LOCATION_ID, " +
                        //                "       CR.IN_OUT_ID, " +
                        //                "       CR.LEDGER_ID AS ITEM_ID, " +
                        //                "       CR.SOURCE,  " +
                        //                "       NULL AS QUANTITY, " +
                        //                "       CR.AMOUNT AS AMOUNT, " +
                        ...
                        //                "FROM ASSET_CR_DETAIL CR " +
                        //                "WHERE CR.IN_OUT_ID = ?IN_OUT_ID " +
                ...
                case SQLCommand.AssetInOut.SaveAssetInOutDetail:
                    {
                        query = "INSERT INTO ASSET_IN_OUT_DETAIL\n" +
                               "  (IN_OUT_ID,ITEM_ID,QUANTITY,SOURCE, SEQUENCE_NO,DEPRECIATION_AMOUNT, AMOUNT,BALANCE_OP_DATE)\n" +  // SALVAGE_VALUE,
                                "VALUES\n" +
                                 "  (?IN_OUT_ID,?ITEM_ID,?QUANTITY,?SOURCE,?SEQUENCE_NO, ?DEPRECIATION_AMOUNT, ?AMOUNT, ?BALANCE_OP_DATE)";  // ?SALVAGE_VALUE,
                        break;
                    }
                ...
                case SQLCommand.AssetInOut.SaveAssetCrDetail: //---Dinesh
                    {
                        query = "INSERT INTO asset_cr_detail\n" +
                                "  (IN_OUT_ID, SEQUENCE_NO,SOURCE,LEDGER_ID, AMOUNT)\n" +
                                "VALUES\n" +
                                "  (?IN_OUT_ID, ?SEQUENCE_NO,?SOURCE, ?LEDGER_ID, ?AMOUNT)";
                        break;
                    }
                case SQLCommand.AssetInOut.UpdateAssetCrDetail: //---Dinesh
                    {
                        query = "UPDATE asset_cr_detail\n" +
                                "   SET \n" +
                                "       IN_OUT_ID   = ?IN_OUT_ID,\n" +
                                "       SEQUENCE_NO = ?SEQUENCE_NO,\n" +
                                "       LEDGER_ID   =  ?LEDGER_ID,\n" +
                                "       SOURCE      =  ?SOURCE,\n" +
                                "       AMOUNT      = ?AMOUNT\n" +
                                " WHERE CR_DETAIL_ID = ?CR_DETAIL_ID";
                        break;
                    }
                case SQLCommand.AssetInOut.DeleteAssetCrDetail:  //Added By Dinesh 05-01-2026 : purpose of Delete Cr Rows in Asset purchase grid 
                    {
                        query =
                            "DELETE FROM ASSET_CR_DETAIL " +
                            "WHERE IN_OUT_ID = ?IN_OUT_ID AND LEDGER_ID = ?LEDGER_ID";
                        break;
                    }
                ...
                case SQLCommand.AssetInOut.UpdateAssetTrans:
                    {
                        query = "UPDATE ASSET_TRANS SET DEPRECIATION_AMOUNT=?DEPRECIATION_AMOUNT, AMOUNT=?AMOUNT, GAIN_AMOUNT=?GAIN_AMOUNT, LOSS_AMOUNT=?LOSS_AMOUNT \n" +
                                "WHERE IN_OUT_DETAIL_ID=?IN_OUT_DETAIL_ID AND ITEM_DETAIL_ID=?ITEM_DETAIL_ID";
                        break;
                    }
                case SQLCommand.AssetInOut.CheckAssetTransRowExists:
                    {
                        query = "SELECT COUNT(*) AS CNT FROM ASSET_TRANS WHERE IN_OUT_DETAIL_ID=?IN_OUT_DETAIL_ID AND ITEM_DETAIL_ID=?ITEM_DETAIL_ID";
                        break;
                    }
