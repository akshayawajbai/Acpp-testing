        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (cboComponentData.Text == "")
            {
               ...
               //XtraMessageBox.Show("Select one of the Component to Edit", "Payroll ", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                XtraMessageBox.Show(MessageRender.GetMessage(MessageCatalog.Payroll.CommonPayroll.PAYROLL_COMMON_SELECT_ONE_COMPONENT_EDIT), "Payroll", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                cboComponent.Focus();
                return;
            }
            if (txtReplaceCom.Text == "")
            {
                ...
                XtraMessageBox.Show(MessageRender.GetMessage(MessageCatalog.Payroll.CommonPayroll.PAYROLL_COMMON_ENTER_THE_VALUE), "Payroll", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                txtReplaceCom.Focus();
                return;
            }
            //on 23-06-2007 @ 11 AM
            ...
            if (failedUpdation != 0)
            {
               ...
               // XtraMessageBox.Show("Component is updated only for the eligible staff(s).", "Payroll", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                XtraMessageBox.Show(MessageRender.GetMessage(MessageCatalog.Payroll.CommonPayroll.PAYROLL_COMMON_COMPONENT_UPDATE_ELIGIBLE),"Payroll", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
            else
                //Dinesh 30-09-2025
                // XtraMessageBox.Show("Component Values are updated Successfully.", "Payroll", MessageBoxButtons.OK, MessageBoxIcon.Information);
                XtraMessageBox.Show(MessageRender.GetMessage(MessageCatalog.Payroll.CommonPayroll.PAYROLL_COMMON_COMPONENT_VALUE_UPDATE_SUCCESS), "Payroll", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            if (UpdateHeld != null)
            {
                UpdateHeld(this, e);
            }
