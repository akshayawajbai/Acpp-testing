...
namespace Bosco.Utility
{
    public class AcMEERPFTP : IDisposable
    {
        ...
        /// <summary>
        /// Gets portal base URL. If config has an endpoint with "acmeerp.org", returns its authority (e.g. http://acmeerp.org).
        /// Else uses the DataSynchronizer endpoint from config and returns the base before /DataSyncService (e.g. http://3.110.87.166/acptest/DataSyncService/DataSynchronizer.svc -> http://3.110.87.166/acptest) for testing/move-to-live. Default: http://acmeerp.org/
        /// 02/03/2026
        /// </summary>
        public string GetAcmeerpPortalURL()
        {
            ...
            try
            {
                ...
                {
                    if (endpoint.Address != null && endpoint.Address.AbsoluteUri.Contains("acmeerp.org"))
                    {
                ...
                // No acmeerp.org in config: use DataSynchronizer endpoint base for testing (e.g. endpoint address="http://3.110.87.166/acptest/DataSyncService/DataSynchronizer.svc" -> base http://3.110.87.166/acptest)
                foreach (ChannelEndpointElement endpoint in serviceModelClientConfigSection.Endpoints)
                {
                    if (endpoint.Contract == null || endpoint.Address == null)
                        continue;
                    if (endpoint.Contract.IndexOf("IDataSynchronizer", StringComparison.OrdinalIgnoreCase) < 0)
                        continue;

                    string absoluteUri = endpoint.Address.AbsoluteUri;
                    int idx = absoluteUri.IndexOf("/DataSyncService", StringComparison.OrdinalIgnoreCase);
                    if (idx > 0)
                    {
                        rtn = absoluteUri.Substring(0, idx).TrimEnd('/');
                        if (!rtn.EndsWith("/"))
                            rtn += "/";
                        return rtn;
                    }
                    rtn = absoluteUri.TrimEnd('/');
                    if (!rtn.EndsWith("/"))
                        rtn += "/";
                    return rtn;
                }
