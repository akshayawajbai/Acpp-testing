...
namespace Bosco.Utility
{
    public class CommonMethod : IDisposable
    {
        ...
        public static bool IsAuditorViewOnlyMode()
        {
            return SettingProperty.Current.AuditorRights == (int)YesNo.Yes;
        }

        private static bool IsAuditorAllowedActivity(int activityId)
        {
            try
            {
                if (!Enum.IsDefined(typeof(Forms), activityId))
                {
                    return false;
                }

                Forms activityType = (Forms)activityId;
                EnumSetMember enumSetMember = new EnumSetMember();
                string operation = enumSetMember.GetDescriptionFromEnumValue(activityType);
                return operation == OperationsList.VIEW.ToString()
                    || operation == OperationsList.PRINT.ToString();
            }
            catch
            {
                return false;
        ...
        private static DataTable FilterAuditorAllowedFormRights(DataTable dtForm)
        {
            if (dtForm == null || dtForm.Rows.Count == 0 || !IsAuditorViewOnlyMode())
            {
                return dtForm;
            }

            EnumSetMember enumSetMember = new EnumSetMember();
            DataTable dtFiltered = dtForm.Clone();
            foreach (DataRow dr in dtForm.Rows)
            {
                if (dr["ENUMTYPE"] == null || dr["ENUMTYPE"] == DBNull.Value)
                {
                    continue;
                }

                string enumTypeName = dr["ENUMTYPE"].ToString();
                if (!Enum.IsDefined(typeof(Forms), enumTypeName))
                {
                    continue;
                }

                Forms activityType = (Forms)enumSetMember.GetEnumItemType(typeof(Forms), enumTypeName);
                string operation = enumSetMember.GetDescriptionFromEnumValue(activityType);
                if (operation == OperationsList.VIEW.ToString()
                    || operation == OperationsList.PRINT.ToString())
                {
            ...
            return dtFiltered.Rows.Count == 0 ? null : dtFiltered;
        }

        public static DataTable ApplyUserRightsForForms(int ActivityId)
        {
            DataTable dtForm = null;
            try
            {
                dtForm = ApplyUserRightsForFormsInternal(ActivityId);
                if (IsAuditorViewOnlyMode())
                {
                    dtForm = FilterAuditorAllowedFormRights(dtForm);
                }
            }
            catch (Exception ex)
            {
        ...
        private static DataTable ApplyUserRightsForFormsInternal(int ActivityId)
        {
            DataTable dtForm = null;
            if (dtApplyUserRights != null && dtApplyUserRights.Rows.Count != 0)
            {
        ...
        public static int ApplyUserRights(int ActivityId)
        {
            ...
            try
            {
                if (IsAuditorViewOnlyMode() && !IsAuditorAllowedActivity(ActivityId))
                {
                    return 0;
                }
