...
namespace Payroll.SQL
{
    public class PayrollSQL : IDatabaseQuery
    {
        ...
        /// <summary>
        /// MySQL: first segment of SCALEOFPAY (e.g. 1,00-0,00-0,00 or 10.000,00-500-50000).
        /// Do not use REPLACE(..., ',', ''): that turns 1,00 into the integer digits 100.
        /// </summary>
        private static string SqlMysqlFirstScaleSegmentAsDecimal(string scaleColumnQualifiedName)
        {
            return "CAST(REPLACE(REPLACE(TRIM(SUBSTRING_INDEX(IFNULL(" + scaleColumnQualifiedName + ",''), '-', 1)), '.', ''), ',', '.') AS DECIMAL(13,2))";
        }

        /// <summary>
        /// Same locale gate as <see cref="SqlMysqlStaffGridBasicPayExpression"/> (EU/SDBANG: comma decimal, dot grouping).
        /// When true, PAY/varchar paths should use comma-decimal normalization instead of REPLACE(PAY,',','').
        /// </summary>
        private static bool SqlMysqlStaffGridBasicPayLocaleNormalizationEnabled()
        {
            try
            {
                var sp = SettingProperty.Current;
                if (sp != null && sp.IS_SDBANG
                    && string.Equals((sp.DecimalSeparator ?? string.Empty).Trim(), ",", StringComparison.Ordinal)
                    && string.Equals((sp.GroupingSeparator ?? string.Empty).Trim(), ".", StringComparison.Ordinal))
                {
                    return true;
                }
            }
        ...
        /// <summary>
        /// Staff grids: numeric basic column aliased as SCALEOFPAY.
        /// EU/SDBANG scale (dot thousands, comma decimal) uses <see cref="SqlMysqlFirstScaleSegmentAsDecimal"/>.
        /// India/default (comma group, dot decimal): stripping dots from scale turns 10.00 into 1000 — use SFS.PAY.
        /// </summary>
        private static string SqlMysqlStaffGridBasicPayExpression()
        {
            if (SqlMysqlStaffGridBasicPayLocaleNormalizationEnabled())
            {
                return SqlMysqlFirstScaleSegmentAsDecimal("SFS.SCALEOFPAY") + " AS 'SCALEOFPAY'";
            }
            return "CAST(TRIM(REPLACE(Replace(SFS.SCALEOFPAY, '-', ' '),',', '')) AS DECIMAL(13,2)) as 'SCALEOFPAY'";
        }

        /// <summary>
        /// BASICPAY for <see cref="SQLCommand.Payroll.ResetPayExtraInfo"/> — <c>PRSTAFFTEMP.COMPVALUE</c> varchar (two decimals, dot in text):
        /// <list type="bullet">
        /// <item><b>Grouping <c>.</c>, decimal <c>,</c></b> (SDBANG EU): if <c>PAY</c> contains a comma, strip thousands dots then comma→dot; if <b>no</b> comma, <c>TRIM</c> only (so <c>10000.00</c> is not turned into <c>1000000</c>). Then <c>DECIMAL(15,2) AS CHAR</c>.</item>
        /// <item><b>Grouping <c>,</c>, decimal <c>.</c></b>: remove thousands commas only, then <c>DECIMAL(15,2) AS CHAR</c>. Do not use <c>LIKE '%,%'</c> “EU” heuristics — that turns <c>3,333</c> into <c>3.333</c> / <c>3.33</c>.</item>
        /// <item>Otherwise legacy <c>REPLACE(STFS.PAY,',','')</c>.</item>
        /// </list>
        /// </summary>
        private static string SqlMysqlResetPayExtraInfoBasicPayCompValueSql()
        {
            const string payDotGroupParsed =
                "CASE WHEN IFNULL(STFS.PAY,'') LIKE '%,%' THEN REPLACE(REPLACE(TRIM(IFNULL(STFS.PAY,'')), '.', ''), ',', '.') ELSE TRIM(IFNULL(STFS.PAY,'')) END";
            const string payCommaGroupParsed = "REPLACE(TRIM(IFNULL(STFS.PAY,'')), ',', '')";
            string payDotGroupTwoDecimals = "CAST(CAST((" + payDotGroupParsed + ") AS DECIMAL(15,2)) AS CHAR)";
            string payCommaGroupTwoDecimals = "CAST(CAST((" + payCommaGroupParsed + ") AS DECIMAL(15,2)) AS CHAR)";
            try
            {
                var sp = SettingProperty.Current;
                if (sp != null)
                {
                    string grp = (sp.GroupingSeparator ?? string.Empty).Trim();
                    string dec = (sp.DecimalSeparator ?? string.Empty).Trim();
                    if (sp.IS_SDBANG
                        && string.Equals(grp, ".", StringComparison.Ordinal)
                        && string.Equals(dec, ",", StringComparison.Ordinal))
                    {
                        return payDotGroupTwoDecimals;
                    }
                    if (string.Equals(grp, ",", StringComparison.Ordinal)
                        && string.Equals(dec, ".", StringComparison.Ordinal))
                    {
                        return payCommaGroupTwoDecimals;
                    }
                }
            }
            catch
        ...
        private string GetPayrollSQL()
        {
            ...
            switch (sqlCommandId)
            {
                ...
                case SQLCommand.Payroll.ResetPayExtraInfo://On 30/07/2019
                    {
                        query = "INSERT INTO PRSTAFFTEMP (PAYROLLID, STAFFID, COMPONENTID, COMPVALUE) \n" +
                                    ...
                                    "(CASE WHEN LNKVALUE = '" + PayRollExtraPayInfo.EARNING1 + "' THEN EARNING1\n" +
                                        ...
                                        "WHEN LNKVALUE = '" + PayRollExtraPayInfo.BASICPAY + "' THEN " + SqlMysqlResetPayExtraInfoBasicPayCompValueSql() + "\n" +
                ...
                case SQLCommand.Payroll.PaymentAdviceBank:
                    {
                        ...
                        query = "SELECT SF.STAFFID, SF.ACCOUNT_NUMBER, CAST(PS.COMPVALUE AS DECIMAL(12,2)) AS \"NET PAY\"," +
                                ...
                                "LEFT JOIN PR_NAME_TITLE PNT ON PNT.NAME_TITLE_ID = SF.NAME_TITLE_ID\n" +
                                "WHERE PS.PAYROLLID = ?PAYROLLID AND ST.GROUPID IN (?GROUPID) AND PR.PROCESS_COMPONENT_TYPE = " + (int)PayRollProcessComponent.NetPay + "\n" +
                            //"AND (?BANK_ACCOUNT_ID IS NULL OR ?BANK_ACCOUNT_ID = 0 OR ST.BANK_ACCOUNT_ID IS NULL OR IFNULL(ST.BANK_ACCOUNT_ID, 0) = ?BANK_ACCOUNT_ID)\n" +
                            // Bank filter logic:
                            // 1. If BANK_ACCOUNT_ID is 0/null -> show all staff
                            // 2. If selected group has mapped bank staff -> show only selected bank staff
                            // 3. If selected group has no mapped bank staff -> show all staff in that group
                                "AND (?BANK_ACCOUNT_ID IS NULL OR ?BANK_ACCOUNT_ID = 0 OR " +
                                "IFNULL(ST.BANK_ACCOUNT_ID, 0) = ?BANK_ACCOUNT_ID OR " +
                                "NOT EXISTS (SELECT 1 FROM PRStaffGroup ST2 " +
                                "WHERE ST2.PAYROLLID = ST.PAYROLLID " +
                                "AND ST2.GROUPID = ST.GROUPID " +
                                "AND IFNULL(ST2.BANK_ACCOUNT_ID, 0) > 0))\n" +
                                   "ORDER BY P.GROUPNAME, ST.STAFFORDER";
                        break;
                    }
                ...
                case SQLCommand.Payroll.AddPrStaff:
                    {
                        ...
                        //break;
                        query = "INSERT INTO PRStaffGroup(StaffId, GroupId, StaffOrder, ACCOUNT_NUMBER, ACCOUNT_IFSC_CODE, ACCOUNT_BANK_BRANCH, PAYMENT_MODE_ID,\n" +
                                   "DEPARTMENT_ID, WORK_LOCATION_ID, BANK_ACCOUNT_ID, BANK_NAME, PayrollId)\n" +
                                   "SELECT StaffId, GroupId, StaffOrder, ACCOUNT_NUMBER, ACCOUNT_IFSC_CODE, ACCOUNT_BANK_BRANCH, PAYMENT_MODE_ID,\n" +
                                   "DEPARTMENT_ID, WORK_LOCATION_ID, BANK_ACCOUNT_ID, BANK_NAME, ?NEWPAYROLLID FROM PRStaffGroup\n" +
                                   "WHERE Payrollid = ?PAYROLLID;\n" +
                                   "INSERT INTO PRSTAFF_STATUTORY_COMPLIANCE (STAFF_ID, PAYROLL_ID, STATUTORY_COMPLIANCE)\n" +
                                   "SELECT STAFF_ID, ?NEWPAYROLLID, STATUTORY_COMPLIANCE FROM PRSTAFF_STATUTORY_COMPLIANCE WHERE PAYROLL_ID = ?PAYROLLID";
                        break;
                ...
                // #Dinesh03-08-2026 - Auto fix old months: copy staff bank mapping to empty months (no client SQL)
                case SQLCommand.Payroll.SyncPRStaffGroupBankForOldMonths:
                    {
                        query = @"UPDATE PRSTAFFGROUP AS tgt
                                    INNER JOIN (
                                      SELECT p1.STAFFID, p1.BANK_ACCOUNT_ID, p1.BANK_NAME
                                      FROM PRSTAFFGROUP p1
                                      INNER JOIN (
                                        SELECT STAFFID, MAX(PAYROLLID) AS MAX_PID
                                        FROM PRSTAFFGROUP
                                        WHERE IFNULL(BANK_ACCOUNT_ID, 0) > 0
                                        GROUP BY STAFFID
                                      ) latest ON latest.STAFFID = p1.STAFFID AND latest.MAX_PID = p1.PAYROLLID
                                    ) src ON src.STAFFID = tgt.STAFFID
                                    SET tgt.BANK_ACCOUNT_ID = src.BANK_ACCOUNT_ID,
                                        tgt.BANK_NAME = src.BANK_NAME
                                    WHERE IFNULL(tgt.BANK_ACCOUNT_ID, 0) = 0";
                        break;
                    }
                ...
                case SQLCommand.Payroll.PayrollStaffList:
                    {
                        ...
                        "       " + SqlMysqlStaffGridBasicPayExpression() + ",\n" +
                        "       dateofjoin AS 'Date Of Join',\n" +
                        "       CASE\n" +
                ...
                case SQLCommand.Payroll.PaymonthStaffProfile:
                    {
                        query = @"SELECT PR.PAYROLLID, PR.PRNAME, SF.staffid, PR.PRDATE, SF.empno, IFNULL(PG.GROUPID, 0) AS GROUPID, SY.GROUPNAME,
                                ...
                                dateofbirth AS 'Date Of Birth', " + SqlMysqlStaffGridBasicPayExpression() + @",
                                dateofjoin AS 'Date Of Join',
                ...
                case SQLCommand.Payroll.PayrollStaffDetails:
                    {
                        query = "SELECT st.staffid AS 'STAFF ID'," + "\r\n" +
                           ...
                           "       IFNULL(PG.BANK_ACCOUNT_ID, 0) AS BANK_ACCOUNT_ID, IFNULL(PG.BANK_NAME, '') AS BANK_NAME," + "\r\n" + // #Dinesh 03-07-2026
                           ...
                           "             IFNULL(BANK_ACCOUNT_ID,0) AS BANK_ACCOUNT_ID, IFNULL(BANK_NAME,'') AS BANK_NAME,\n" + // #Dinesh 03-07-2026
                ...
                case SQLCommand.Payroll.PayrollStaffOutOfService:
                    {
                        ...
                        query = "SELECT tb.staffid AS 'STAFF ID',\n" +
                                ...
                                "       DATE_FORMAT(tb.dateofbirth, '%d/%m/%y') AS 'Date Of Birth'," + SqlMysqlStaffGridBasicPayExpression() + ",\n" +
                ...
                case SQLCommand.Payroll.PayrollStaffInservice:
                    {
                        ...
                        query = "SELECT t.staffid AS 'STAFF ID',\n" +
                                ...
                                "       ROUND(DATEDIFF(current_date, t.dateofbirth) / 365) AS Age,\n" +
                                "       DATE_FORMAT(t.dateofbirth, '%d/%m/%y') AS 'Date Of Birth',\n" +
                                "       t.dateofjoin AS 'Date Of Join'," + SqlMysqlStaffGridBasicPayExpression() + ",\n" +
                                "       CASE\n" +
...
//                        break;
                        ...
                        query = @"INSERT INTO PRSTAFFGROUP(STAFFID, GROUPID, STAFFORDER, ACCOUNT_NUMBER, ACCOUNT_IFSC_CODE,
                                    ACCOUNT_BANK_BRANCH, PAYMENT_MODE_ID, DEPARTMENT_ID, WORK_LOCATION_ID, BANK_ACCOUNT_ID, BANK_NAME, PAYROLLID) 
                                    VALUES (?STAFFID, ?GROUPID, ?STAFFORDER, ?ACCOUNT_NUMBER, ?ACCOUNT_IFSC_CODE, 
                                    ?ACCOUNT_BANK_BRANCH, ?PAYMENT_MODE_ID, IF(?DEPARTMENT_ID=0, NULL, ?DEPARTMENT_ID) , IF(?WORK_LOCATION_ID=0, NULL, ?WORK_LOCATION_ID), 
                                    IF(?BANK_ACCOUNT_ID=0, NULL, ?BANK_ACCOUNT_ID), IF(?BANK_ACCOUNT_ID=0, NULL, ?BANK_NAME), ?PAYROLLID ) ";
                        break;
                    }
                case SQLCommand.Payroll.InsertPRStaffGroupForAllMonths:
                    {
                        ...
                        query = "INSERT INTO PRSTAFFGROUP (GROUPID, STAFFORDER, ACCOUNT_NUMBER, ACCOUNT_IFSC_CODE,\n" +
                               "ACCOUNT_BANK_BRANCH, PAYMENT_MODE_ID, DEPARTMENT_ID, WORK_LOCATION_ID, BANK_ACCOUNT_ID, BANK_NAME, PAYROLLID, STAFFID)\n" +
                               "SELECT ?GROUPID AS GROUPID, ?STAFFORDER AS STAFFORDER, ?ACCOUNT_NUMBER AS ACCOUNT_NUMBER, ?ACCOUNT_IFSC_CODE AS ACCOUNT_IFSC_CODE, \n" +
                               "?ACCOUNT_BANK_BRANCH AS ACCOUNT_BANK_BRANCH , ?PAYMENT_MODE_ID AS PAYMENT_MODE_ID,\n" +
                               "IF(?DEPARTMENT_ID=0, NULL, ?DEPARTMENT_ID), IF(?WORK_LOCATION_ID=0, NULL, ?WORK_LOCATION_ID), IF(?BANK_ACCOUNT_ID=0, NULL, ?BANK_ACCOUNT_ID), IF(?BANK_ACCOUNT_ID=0, NULL, ?BANK_NAME),\n" +
                               "PAYROLLID, ?STAFFID AS STAFFID \n" +
                               "FROM PRCREATE PR WHERE PAYROLLID <> ?PAYROLLID;";
                        break;
                    }
...
//                        break;
                        query = @"UPDATE PRSTAFFGROUP SET GROUPID = ?GROUPID, ACCOUNT_NUMBER=?ACCOUNT_NUMBER, ACCOUNT_IFSC_CODE=?ACCOUNT_IFSC_CODE,
                                    ACCOUNT_BANK_BRANCH=?ACCOUNT_BANK_BRANCH, PAYMENT_MODE_ID=?PAYMENT_MODE_ID, DEPARTMENT_ID= IF(?DEPARTMENT_ID=0, NULL,?DEPARTMENT_ID), 
                                    WORK_LOCATION_ID= IF(?WORK_LOCATION_ID=0, NULL, ?WORK_LOCATION_ID), BANK_ACCOUNT_ID= IF(?BANK_ACCOUNT_ID=0, NULL, ?BANK_ACCOUNT_ID), 
                                    BANK_NAME= IF(?BANK_ACCOUNT_ID=0, NULL, ?BANK_NAME) 
                                    WHERE STAFFID =?STAFFID AND PAYROLLID = ?PAYROLLID";
                        break;
                    }
                // #Dinesh03-08-2026 - Bank mapping is one-time for staff; apply to all payroll months until manually cleared
                case SQLCommand.Payroll.UpdatePRStaffGroupBankForAllMonths:
                    {
                        query = @"UPDATE PRSTAFFGROUP SET BANK_ACCOUNT_ID = IF(?BANK_ACCOUNT_ID=0, NULL, ?BANK_ACCOUNT_ID),
                                    BANK_NAME = IF(?BANK_ACCOUNT_ID=0, NULL, ?BANK_NAME)
                                    WHERE STAFFID = ?STAFFID";
                        break;
                    }
                ...
                case SQLCommand.Payroll.FetchPayrollComponetIdandName:
                    {
                        query = "SELECT DISTINCT " + "\n" +
                                 ...
                                 "FROM prcomponent A " + "\n" +
                                 "WHERE " + "\n" +
                                 "      A.RELATEDCOMPONENTS LIKE CONCAT( " + "\n" +
                                 "            '%', " + "\n" +
                                 "            CONVERT('ê' USING latin1), " + "\n" +
                                 "            ( " + "\n" +
                                 "                SELECT COMPONENTID " + "\n" +
                                 "                FROM prcomponent " + "\n" +
                                 "                WHERE LINKVALUE = 'PayingSalaryDays' " + "\n" +
                                 "                LIMIT 1 " + "\n" +
                                 "            ), " + "\n" +
                                 "            CONVERT('ê' USING latin1), " + "\n" +
                                 "            '%' " + "\n" +
                                 "      ) " + "\n" +
                                 "AND A.RELATEDCOMPONENTS LIKE CONCAT( " + "\n" +
                                 "            '%', " + "\n" +
                                 "            CONVERT('ê' USING latin1), " + "\n" +
                                 "            ( " + "\n" +
                                 "                SELECT COMPONENTID " + "\n" +
                                 "                FROM prcomponent " + "\n" +
                                 "                WHERE LINKVALUE = 'TotalDaysInPayMonth' " + "\n" +
                                 "                LIMIT 1 " + "\n" +
                                 "            ), " + "\n" +
                                 "            CONVERT('ê' USING latin1), " + "\n" +
                                 "            '%' " + "\n" +
                ...
                case SQLCommand.Payroll.InsertRangeCondtions:
                    {
                        query = "INSERT INTO PAYROLL_RANGE_FORMULA(COMPONENTID,LINK_COMPONENT_ID,MIN_VALUE,MAX_VALUE,MAX_SLAB,TYPE_ID) VALUES(?COMPONENTID,?LINK_COMPONENT_ID,?MIN_VALUE,?MAX_VALUE,?MAX_SLAB, ?TYPE_ID);";//added By Dinesh 25-10-2025
                        break;
                    }
