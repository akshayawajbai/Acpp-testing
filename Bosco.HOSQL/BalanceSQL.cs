        ...
        private string GetBalanceSQL()
        {
            ...
            switch (sqlCommandId)
            {
                ...
                case SQLCommand.TransBalance.FetchTransaction:
                    {
                        query = "SELECT VM.VOUCHER_ID, VM.PROJECT_ID, VM.VOUCHER_TYPE, VT.LEDGER_ID, ML.GROUP_ID, VM.VOUCHER_DATE, VT.AMOUNT, " +
                            ...
                            // Chinna 22.11.2025, to change Contra Entries related
                           "CASE " +
                            "WHEN VM.IS_MULTI_CURRENCY=1 AND VM.VOUCHER_TYPE='CN' AND VT.TRANS_MODE='DR' " +
                            "THEN IF(VM.IS_MULTI_CURRENCY=1, VT.EXCHANGE_RATE, 1) * VT.AMOUNT " +
                            "ELSE IF(VM.IS_MULTI_CURRENCY=1, VM.ACTUAL_AMOUNT, 1) " +
                        ...
                        //    "CASE " +
                        //        "WHEN VM.IS_MULTI_CURRENCY = 1 AND VM.VOUCHER_TYPE = 'CN' AND VT.TRANS_MODE = 'DR' " +
                        //        "THEN CASE WHEN VT.EXCHANGE_RATE = (SELECT VT2.EXCHANGE_RATE FROM VOUCHER_TRANS AS VT2 " +
                        //        "WHERE VT2.VOUCHER_ID = VM.VOUCHER_ID AND VT2.BRANCH_ID = VM.BRANCH_ID AND VT2.LOCATION_ID = VM.LOCATION_ID AND VT2.TRANS_MODE = 'CR' LIMIT 1) " +
                        //        "THEN IF(VM.IS_MULTI_CURRENCY = 1, VM.ACTUAL_AMOUNT, 1) " +
                        //        "ELSE IF(VM.IS_MULTI_CURRENCY = 1, VT.EXCHANGE_RATE, 1) * VM.ACTUAL_AMOUNT " +
                        //        "END " +
                        //        "ELSE IF(VM.IS_MULTI_CURRENCY = 1, VM.ACTUAL_AMOUNT, 1) " +
                        //        "END AS ACTUAL_AMOUNT " +
                        //        "FROM VOUCHER_MASTER_TRANS AS VM " +
                ...
                case SQLCommand.TransBalance.FetchCashBankBaseCurrencyExchangeRate:
                    {
                        // 30/03/2026, Allow guarded SDBAFE general-ledger opening balance updates to reuse the same balance flow.
                        query = "SELECT ML.LEDGER_ID, ML.GROUP_ID, ML.LEDGER_TYPE, IF(ML.OP_EXCHANGE_RATE=0 OR ML.OP_EXCHANGE_RATE IS NULL, 1, OP_EXCHANGE_RATE) AS OP_EXCHANGE_RATE," +
                                "IF(CUR.EXCHANGE_RATE=0 OR CUR.EXCHANGE_RATE IS NULL, 1, EXCHANGE_RATE) AS EXCHANGE_RATE, ML.CUR_COUNTRY_ID\n" +
