...
namespace AcMEDSync.Model
{
    ...
    public class BalanceSystem : DsyncSystemBase
    {
        ...
        // Chinna added AmountCurrentFC
        public ResultArgs UpdateOpBalance(string opBalDate, int projectId, int ledgerId, double amountCurrent,
            string transModeCurrent, TransactionAction transAction, double amountCurrentFC = 0)
        {
            return UpdateOpBalance(opBalDate, 0, projectId, ledgerId, amountCurrent, transModeCurrent, transAction, amountCurrentFC);
        }

        public ResultArgs UpdateOpBalance(string opBalDate, int branchId, int projectId, int ledgerId, double amountCurrent, string transModeCurrent, TransactionAction transAction, double amountCurrentFC = 0)
        {
            ResultArgs result = null;
            DateTime dateOpBal = DateTime.Parse(opBalDate).AddDays(-1);
            Int32 groupid = 0;
            bool isCashBank = false;
            bool isSDBAFEGeneralLedgerCurrency = false;
            bool isCurrencyEnabledLedger = false;
            ...
            if (this.AllowMultiCurrency == 1)
            {
                ...
                if (resulexchagnerate.Success && resulexchagnerate.DataSource.Table != null && resulexchagnerate.DataSource.Table.Rows.Count > 0)
                {
                    ...
                    if (dtBaseExchangeRate.Rows.Count > 0)
                    {
                        ...
                        isCurrencyEnabledLedger = NumberSet.ToInteger(dtBaseExchangeRate.Rows[0][this.AppSchema.Ledger.CUR_COUNTRY_IDColumn.ColumnName].ToString()) > 0;
                        ...
                        // 30/03/2026, Enable general-ledger opening balance FC reset/update only for SDBAFE/BSGRMG.
                        // else if ((this.IS_SDBAFE || this.IS_BSGRMG) &&
                        else if ((this.IS_BSGRMG) &&
                            dtBaseExchangeRate.Columns.Contains(this.AppSchema.Ledger.LEDGER_TYPEColumn.ColumnName) &&
                            dtBaseExchangeRate.Rows[0][this.AppSchema.Ledger.LEDGER_TYPEColumn.ColumnName].ToString() == "GN" &&
                            isCurrencyEnabledLedger)
                        {
                            CashBankBaseExchangerate = NumberSet.ToDouble(dtBaseExchangeRate.Rows[0][this.AppSchema.Ledger.OP_EXCHANGE_RATEColumn.ColumnName].ToString());
                            if (CashBankBaseExchangerate == 0)
                                CashBankBaseExchangerate = NumberSet.ToDouble(dtBaseExchangeRate.Rows[0][this.AppSchema.Country.EXCHANGE_RATEColumn.ColumnName].ToString());
                            if (CashBankBaseExchangerate == 0)
                                CashBankBaseExchangerate = 1;
                            isSDBAFEGeneralLedgerCurrency = true;
                        }
                    }
                }
            }
            ...
            if (result.Success)
            {
                ...
                if (dtLedgerOpBal != null && dtLedgerOpBal.Rows.Count > 0)
                {
                    ...
                    if (this.AllowMultiCurrency == 1 && (isCashBank || isSDBAFEGeneralLedgerCurrency))
                    {
            ...
            if (isCashBank)
            {
                if (!IsClientBranch)
                {
                    amountCurrentfc = amountCurrentFC;
                    amountCurrent = amountCurrent;
                    // amountCurrent is already in base currency from source
                }
                else
                {
                    // Original logic for regular operations
                    amountCurrentfc = amountCurrent;
                    amountCurrent = (amountCurrent * CashBankBaseExchangerate);
                }

                // Chinna 31/10/2025, Chinna
                //amountCurrentfc = amountCurrent;
                //amountCurrent = (amountCurrent * CashBankBaseExchangerate);
            }
            else
            {
                if (isSDBAFEGeneralLedgerCurrency)
                {
                    // BSGRMG GN + currency: MappingSystem passes FC only; convert to local once here (avoid FC × rate × rate).
                    double fcAmount = amountCurrentFC > 0 ? amountCurrentFC : amountCurrent;
                    double rate = CashBankBaseExchangerate;
                    if (rate == 0)
                        rate = 1;
                    amountCurrentfc = fcAmount;
                    amountCurrent = fcAmount * rate;
                }
                else if (this.AllowMultiCurrency == 1 && amountCurrentFC > 0)
                {
                    amountCurrentfc = amountCurrentFC;
                }
                else
            ...
            try
            {
                ...
                if (result.Success && result.DataSource.Table != null)
                {
                    ...
                    foreach (DataRow dr in dtLedgerBalance.Rows)
                    {
                        ...
                        if (Pid > 0 && Lid > 0)
                        {
                            result = UpdateOpBalance(opdate, Pid, Lid, amountfc, transmode, TransactionAction.EditAfterSave, amountfc);
            ...
            double amount, double amountfc, string transMode, string transFlag)
        {
            ...
            using (DataManager dataManager = new DataManager(SQLCommand.TransBalance.UpdateBalance, DataBaseTypeName, SQLAdapterTypeName)) //DataBaseType.HeadOffice, SQLAdapterType.HOSQL
            {
                ...
                dataManager.Parameters.Add(this.AppSchema.LedgerBalance.TRANS_FC_MODEColumn, transMode.ToUpper());
            ...
            BalanceType balanceType, Int32 CurrencyCurrencyId = 0)
        {
            ...
            using (DataManager dataManager = new DataManager(DataBaseTypeName)) //DataBaseType.HeadOffice
            {
                ...
                //if (this.DisplayInputLanguage == "1")
                //{
                //    dataManager.Parameters.Add(this.AppSchema.LedgerGroup.DISPLAY_LEDGER_GROUPColumn, 1);
                //}
                //else
                //{
                //    dataManager.Parameters.Add(this.AppSchema.LedgerGroup.DISPLAY_LEDGER_GROUPColumn, 0);
                //}
