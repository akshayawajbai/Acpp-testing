...
namespace AcMEDSync.Model
{
    public class ImportVoucherSystem : DsyncSystemBase
    {
        ...
        private const string COUNTRY_CURRENCY_TABLE_NAME = "CountryCurrency";
        ...
        private DataTable dtCountryCurrency { get; set; }
        ...
        public double OpExchangeRate { get; set; }
        ...
        public string CountryCurrencyApplicableFrom { get; set; }
        public string CountryCurrencyApplicableTo { get; set; }
        public string CountryCurrencyExchangeRate { get; set; }
        ...
        private Double BalanceAmountFC { get; set; }
        private string BalanceTransMode { get; set; }
        private string BalanceTransFCMode { get; set; }
        ...
        // 19/12/2025, Chinna.
        // Reset class-level variables at start to prevent state issues between runs
        public ResultArgs ImportVouchers(string VoucherXml)
        {
            bool isBeginTransaction = false;
            ProblemDataDetails = string.Empty;

            // 19/12/2025, Chinna: Reset resultArgs at start of each import to prevent state issues
            resultArgs = new ResultArgs();
            // 19/12/2025, Chinna: Reset connection string to Portal (admin_portal) at start of each import
            // This ensures Portal connections work even if previous run used HeadOffice connection
            Bosco.DAO.Configuration.ConfigurationHandler.Instance.ConnectionString = "";
                ...
                finally
                {
                    ...
                    if (VoucherImportType == ImportType.HeadOffice)
                    {
                        ...
                        else
                        {
                            ...
                            string contextPart = (ProblemDataDetails ?? string.Empty)
                                .Replace("<br>", ". ")
                                .Replace("<br/>", ". ")
                                .Trim();
                            string errorDetail = ExtractSyncErrorDetail(resultArgs != null ? resultArgs.Message : null);
                            string failedMessage;
                            if (!string.IsNullOrEmpty(contextPart) && !string.IsNullOrEmpty(errorDetail))
                                failedMessage = contextPart + ". " + errorDetail;
                            else if (!string.IsNullOrEmpty(contextPart))
                                failedMessage = contextPart;
                            else if (!string.IsNullOrEmpty(errorDetail))
                                failedMessage = errorDetail;
                            else
                                failedMessage = "Import failed; no details available.";
                            string statusRemark = "Problem in Importing Vouchers. " + failedMessage;
                            UpdateDataSynStatus(DataSyncMailType.Failed, statusRemark);
                            // Mail Error Message: only specific error (e.g. The Record is Available '23-64759-0')
                            string mailErrorMessage = !string.IsNullOrEmpty(errorDetail)
                                ? errorDetail
                                : failedMessage;
                            SendMail(DataSyncMailType.Failed, mailErrorMessage);

                            //AcMEDataSynLog.WriteLog(resultArgs.Message);
                            //// Prefer ProblemDataDetails for mail/status to avoid raw technical errors in email
                            //string failedMessage = !string.IsNullOrEmpty(ProblemDataDetails)
                            //    ? (ProblemDataDetails ?? string.Empty).Replace("<br>", ". ").Replace("<br/>", ". ")
                            //    : (resultArgs != null && !string.IsNullOrEmpty(resultArgs.Message)
                            //        ? "Problem in Importing Vouchers. Please check the sync log for details."
                            //        : "Import failed; no details available.");
                            //string statusRemark = "Problem in Importing Vouchers. " + failedMessage;
                            //UpdateDataSynStatus(DataSyncMailType.Failed, statusRemark);
                            //SendMail(DataSyncMailType.Failed, failedMessage);

                            // chinna 06/03/2026

                            //AcMEDataSynLog.WriteLog(resultArgs.Message);
                            //string failedMessage = (resultArgs != null && !string.IsNullOrEmpty(resultArgs.Message)) ? resultArgs.Message : "Import failed; no details available.";
                            ////UpdateDataSynStatus(DataSyncMailType.Failed, "Problem in Importing Vouchers. " + resultArgs.Message);
                            //UpdateDataSynStatus(DataSyncMailType.Failed, "Problem in Importing Vouchers. " + resultArgs.Message);
                            //SendMail(DataSyncMailType.Failed, failedMessage);
                        }
                        // 19/12/2025, Chinna: Reset connection string back to Portal after HeadOffice operations complete
                        // This ensures next run can connect to Portal (admin_portal) database
                        Bosco.DAO.Configuration.ConfigurationHandler.Instance.ConnectionString = "";
                    }
        ...
        private static string ExtractSyncErrorDetail(string rawMessage)
        {
            if (string.IsNullOrWhiteSpace(rawMessage))
                return string.Empty;
            const string prefixWithComma = "Problem in updating Voucher Master details,";
            if (rawMessage.StartsWith(prefixWithComma, StringComparison.OrdinalIgnoreCase))
                return rawMessage.Substring(prefixWithComma.Length).Trim();
            const string prefix = "Problem in updating Voucher Master details";
            if (rawMessage.StartsWith(prefix, StringComparison.OrdinalIgnoreCase))
            {
                int comma = rawMessage.IndexOf(',');
                if (comma >= 0)
                    return rawMessage.Substring(comma + 1).Trim();
            }
                                ...
                                if (resultArgs.Success)
                                {
                                    ...
                                    if (resultArgs.Success)
                                    {
                                        ...
                                        if (resultArgs.Success)
                                        {
                                            ...
                                            {
                                                if (VoucherImportType != ImportType.SplitProject)
                                                    resultArgs = ImportCountryCurrency();
                                                {
                        ...
                        if (resultArgs.Success)
                        {
                            ...
                            if (!isLegalEnityExists)
                            {
                                using (DataManager dataManager = new DataManager(SQLCommand.LegalEntity.Add))
                                {
                                    ...
                                    dataManager.Parameters.Add(AppSchema.LegalEntity.COUNTRY_IDColumn, GetId(Id.LocalCountry));
                                    State = drItem[AppSchema.LegalEntity.STATEColumn.ColumnName].ToString();
                                    dataManager.Parameters.Add(AppSchema.LegalEntity.STATE_IDColumn, GetId(Id.LocalState));
                                ...
                                if (resultArgs.Success)
                                {
                                    ...
                                    if (!isLedgerExists)
                                    {
                                        ...
                                        if (GroupId != (int)FixedLedgerGroup.BankAccounts)
                                        {
                                            ...
                                            //On 7/10/2025, To check and validate 
                                            if (dtLedger.Columns.Contains(this.AppSchema.Country.CURRENCY_COUNTRYColumn.ColumnName))
                                            {
                                                CountryName = drLedger[this.AppSchema.Country.CURRENCY_COUNTRYColumn.ColumnName].ToString();
                                            }

                                            if (dtLedger.Columns.Contains(this.AppSchema.Ledger.OP_EXCHANGE_RATEColumn.ColumnName))
                                            {
                                                OpExchangeRate = this.NumberSet.ToDouble(drLedger[this.AppSchema.Ledger.OP_EXCHANGE_RATEColumn.ColumnName].ToString());
                                            }
                            ...
                            if (!string.IsNullOrEmpty(Vendor))
                            {
                                ...
                                if (resultArgs.Success)
                                {
                                    ...
                                    if (!isVendorExists)
                                    {
                                        ...
                                        CountryId = VoucherImportType.Equals(ImportType.SplitProject) ? GetId(Id.LocalCountry) : GetId(Id.Country); // GetId(Id.Country);


                                        // 07/05/2026, chinna, State to State_name choosen
                                        StateName = drAssetStockVendors[this.AppSchema.State.STATE_NAMEColumn.ColumnName].ToString();
                                        StateId = VoucherImportType.Equals(ImportType.SplitProject) ? GetId(Id.LocalState) : GetId(Id.State); // GetId(Id.State);
                            ...
                            if (!string.IsNullOrEmpty(LedgerName))
                            {
                                ...
                                if (LedgerId > 0)
                                {
                                    ...
                                    if (resultArgs.Success)
                                    {
                                        ...
                                        StateId = VoucherImportType.Equals(ImportType.SplitProject) ? GetId(Id.LocalState) : GetId(Id.State);
                                        CountryId = VoucherImportType.Equals(ImportType.SplitProject) ? GetId(Id.LocalCountry) : GetId(Id.Country);
                ...
                using (DataManager dataManager = new DataManager())
                {
                    if (dtCountry != null && dtCountry.Rows.Count > 0)
                    {
                        foreach (DataRow drCountry in dtCountry.Rows)
                        {
                            ...
                            {
                                resultArgs = VoucherImportType.Equals(ImportType.SplitProject) ? IsExists(SQLCommand.ImportVoucher.IsLocalCountryExists) : IsExists(SQLCommand.ImportVoucher.IsCountryExists);
                                if (resultArgs.Success)
                                {
        ...
        /// </summary>
        /// <returns></returns>
        private ResultArgs ImportCountryCurrency()
        {
            AcMEDataSynLog.WriteLog("ImportCountryCurrency Started.");
            try
            {
                bool isCountryCurrencyExists = true;
                using (DataManager dataManager = new DataManager())
                {
                    if (dtCountryCurrency != null && dtCountryCurrency.Rows.Count > 0)
                    {
                        foreach (DataRow drCountryCurrency in dtCountryCurrency.Rows)
                        {
                            CountryCurrencyName = drCountryCurrency[this.AppSchema.Country.COUNTRYColumn.ColumnName].ToString();

                            if (!string.IsNullOrEmpty(CountryCurrencyName))
                            {
                                CountryId = GetId(Id.CountryCurrency);
                                CountryCurrencyApplicableFrom = drCountryCurrency[this.AppSchema.Country.APPLICABLE_FROMColumn.ColumnName].ToString();
                                CountryCurrencyApplicableTo = drCountryCurrency[this.AppSchema.Country.APPLICABLE_TOColumn.ColumnName].ToString();
                                resultArgs = IsExists(SQLCommand.ImportVoucher.IsCountryCurrencyExists);
                                if (resultArgs.Success)
                                {
                                    isCountryCurrencyExists = resultArgs.DataSource.Sclar.ToInteger > 0;

                                    //Insert Currency Country Details
                                    CurrencyCountry = drCountryCurrency[this.AppSchema.Country.COUNTRYColumn.ColumnName].ToString();
                                    CountryCurrencyApplicableFrom = drCountryCurrency[this.AppSchema.Country.APPLICABLE_FROMColumn.ColumnName].ToString();
                                    CountryCurrencyApplicableTo = drCountryCurrency[this.AppSchema.Country.APPLICABLE_TOColumn.ColumnName].ToString();
                                    CountryCurrencyExchangeRate = drCountryCurrency[this.AppSchema.Country.EXCHANGE_RATEColumn.ColumnName].ToString();

                                    resultArgs = SaveCountryCurrencyDetails(isCountryCurrencyExists);
                                }
                            }
                            else
                            {
            ...
            try
            {
                ...
                using (DataManager dataManager = new DataManager())
                {
                    if (dtState != null && dtState.Rows.Count > 0)
                    {
                        ...
                        {
                            // 12/12/2025
                            StateName = drState[this.AppSchema.State.STATE_NAMEColumn.ColumnName].ToString(); //drState[this.AppSchema.State.STATE_NAMEColumn.ColumnName].ToString();
                            ...
                            {
                                resultArgs = VoucherImportType.Equals(ImportType.SplitProject) ? IsExists(SQLCommand.ImportVoucher.IsLocalStateExists) : IsExists(SQLCommand.ImportVoucher.IsStateExists);
                                if (resultArgs.Success)
                                {
                                    ...
                                    if (!isStateExists)
                                    {
                                        ...
                                        StateCountryId = VoucherImportType.Equals(ImportType.SplitProject) ? GetId(Id.LocalCountry) : GetId(Id.Country);
                                        if (StateCountryId > 0)
                                        {
                            ...
                            if (!string.IsNullOrEmpty(AccountNumber))
                            {
                                ...
                                if (resultArgs != null && resultArgs.Success)
                                {
                                    ...
                                    if (!isLedgerExists)
                                    {
                                        ...
                                        if (dtBankAccount.Columns.Contains(this.AppSchema.Country.CURRENCY_COUNTRYColumn.ColumnName))
                                        {
                                            CountryName = drBank[this.AppSchema.Country.CURRENCY_COUNTRYColumn.ColumnName].ToString();
                                        }
                                        if (dtBankAccount.Columns.Contains(this.AppSchema.Country.CURRENCY_COUNTRYColumn.ColumnName))
                                        {
                                            OpExchangeRate = NumberSet.ToDouble(drBank[this.AppSchema.Ledger.OP_EXCHANGE_RATEColumn.ColumnName].ToString());
                                        }
        ...
        private ResultArgs SaveLedgerDetails()
        {
            try
            {
                using (DataManager dataManager = new DataManager(SQLCommand.ImportVoucher.InsertLedger, DataBaseTypeName, SQLAdapterType.HOSQL))
                {
                    ...
                    //dataManager.Parameters.Add(this.AppSchema.Ledger.CUR_COUNTRY_IDColumn, VoucherImportType.Equals(ImportType.SplitProject) ? GetId(Id.LocalCountry) : GetId(Id.Country));
                    dataManager.Parameters.Add(this.AppSchema.Ledger.CUR_COUNTRY_IDColumn, VoucherImportType.Equals(ImportType.SplitProject) ? (object)DBNull.Value : GetId(Id.Country));
                    ...
                    //On 07/10/2025
                    dataManager.Parameters.Add(this.AppSchema.Ledger.OP_EXCHANGE_RATEColumn, OpExchangeRate);
        ...
        private ResultArgs SaveCountryDetails()
        {
            try
            {
                using (DataManager dataManager = new DataManager(VoucherImportType.Equals(ImportType.SplitProject) ? SQLCommand.ImportVoucher.InsertLocalCountry : SQLCommand.ImportVoucher.InsertCountry, DataBaseTypeName, SQLAdapterType.HOSQL))
                {
        ...
        /// </summary>
        /// <returns></returns>
        private ResultArgs SaveCountryCurrencyDetails(bool iscurrency)
        {
            try
            {
                using (DataManager dataManager = new DataManager(iscurrency ? SQLCommand.ImportVoucher.UpdateCountryCurrency : SQLCommand.ImportVoucher.InsertCountryCurrency, DataBaseTypeName, SQLAdapterType.HOSQL))
                {
                    dataManager.Parameters.Add(AppSchema.Country.COUNTRY_IDColumn, GetId(Id.CountryCurrency));
                    dataManager.Parameters.Add(AppSchema.Country.APPLICABLE_FROMColumn, CountryCurrencyApplicableFrom);
                    dataManager.Parameters.Add(AppSchema.Country.APPLICABLE_TOColumn, CountryCurrencyApplicableTo);
                    dataManager.Parameters.Add(AppSchema.Country.EXCHANGE_RATEColumn, CountryCurrencyExchangeRate);
                    dataManager.Parameters.Add(AppSchema.VoucherMaster.BRANCH_IDColumn, "0");
                    dataManager.Parameters.Add(AppSchema.VoucherMaster.LOCATION_IDColumn, "0");
                    dataManager.DataCommandArgs.IsDirectReplaceParameter = true;
                    resultArgs = dataManager.UpdateData();
                }
            }
            ...
            {
                resultArgs.Message = "Error in Saving Country Currency Details: " + ex.ToString();
            }
            finally { }
        ...
        private ResultArgs SaveStateDetails()
        {
            try
            {
                using (DataManager dataManager = new DataManager(VoucherImportType.Equals(ImportType.SplitProject) ? SQLCommand.ImportVoucher.InsertLocalState : SQLCommand.ImportVoucher.InsertState, DataBaseTypeName, SQLAdapterType.HOSQL))
                {
                    dataManager.Parameters.Add(AppSchema.State.STATE_CODEColumn, StateCode);
                    dataManager.Parameters.Add(AppSchema.State.STATEColumn, StateName);
            ...
            try
            {
                if (dtVoucherMaster != null && dtVoucherMaster.Rows.Count > 0)
                {
                    foreach (DataRow drMaster in dtVoucherMaster.Rows)
                    {
                        ...
                        if (dtVoucherMaster.Columns.Contains(this.AppSchema.Country.CURRENCY_COUNTRYColumn.ColumnName))
                        {
                            CountryName = drMaster[this.AppSchema.Country.CURRENCY_COUNTRYColumn.ColumnName].ToString();
                            CurrencyCountryId = VoucherImportType.Equals(ImportType.SplitProject) ? GetId(Id.LocalCountry) : GetId(Id.Country);
                        }
                        else
                        {
                            CurrencyCountryId = 0;
                        }
                        if (dtVoucherMaster.Columns.Contains(this.AppSchema.Country.EXCHANGE_COUNTRYColumn.ColumnName))
                        {
                            CountryName = drMaster[this.AppSchema.Country.EXCHANGE_COUNTRYColumn.ColumnName].ToString();
                            ExchageCountryId = VoucherImportType.Equals(ImportType.SplitProject) ? GetId(Id.LocalCountry) : GetId(Id.Country);
                        }
                        else
                        {
                            ExchageCountryId = 0;
                        ...
                        IsMultiCurrency = this.NumberSet.ToInteger(drMaster[this.AppSchema.VoucherMaster.IS_MULTI_CURRENCYColumn.ColumnName].ToString());
                        ...
                        if (!resultArgs.Success)
                        {
                            AcMEDataSynLog.WriteLog("Voucher Id: " + VoucherId + ", BranchId: " + BranchId + ", Voucher Date :" + VoucherDate.ToShortDateString() + ", Project Id :" + ProjectId + ",Message :" + resultArgs.Message);
                            break;
                        }
                    }
                }
                    ...
                    if (dtVoucherTrans != null && dtVoucherTrans.Rows.Count > 0)
                    {
                        ...
                        if (dvTrans.Count > 1)
                        {
                            foreach (DataRowView dr in dvTrans)
                            {
                                ...
                                if (dtVoucherTrans.Columns.Contains(this.AppSchema.VoucherTransaction.LIVE_EXCHANGE_RATEColumn.ColumnName))
                                {
                                    LiveExchangeRate = this.NumberSet.ToDecimal(dr[this.AppSchema.VoucherTransaction.LIVE_EXCHANGE_RATEColumn.ColumnName].ToString());
                                }

                                if (dtVoucherTrans.Columns.Contains(this.AppSchema.VoucherTransaction.EXCHANGE_RATEColumn.ColumnName))
                                {
                                    ExchangeRate = this.NumberSet.ToDecimal(dr[this.AppSchema.VoucherTransaction.EXCHANGE_RATEColumn.ColumnName].ToString());
                                }

                                if (dtVoucherTrans.Columns.Contains(this.AppSchema.VoucherTransaction.ACTUAL_AMOUNTColumn.ColumnName))
                                {
                                    ActualAmount = this.NumberSet.ToDecimal(dr[this.AppSchema.VoucherTransaction.ACTUAL_AMOUNTColumn.ColumnName].ToString());
                                }
        ...
        private ResultArgs SaveVoucherMasters()
        {
            try
            {
                ...
                using (DataManager dataManager = new DataManager((VoucherImportType == ImportType.HeadOffice) ? SQLCommand.ImportVoucher.InsertVoucherMaster : SQLCommand.ImportVoucher.InsertVoucherMasterBranch, DataBaseTypeName, SQLAdapterType.HOSQL))
                {
                    ...
                    dataManager.Parameters.Add(this.AppSchema.VoucherMaster.CURRENCY_COUNTRY_IDColumn, VoucherImportType.Equals(ImportType.SplitProject) ? GetId(Id.LocalCountry) : GetId(Id.Country));
                    ...
                    dataManager.Parameters.Add(this.AppSchema.VoucherMaster.EXCHANGE_COUNTRY_IDColumn, VoucherImportType.Equals(ImportType.SplitProject) ? GetId(Id.LocalCountry) : GetId(Id.Country));
                    ...
                    dataManager.Parameters.Add(this.AppSchema.VoucherMaster.IS_MULTI_CURRENCYColumn, IsMultiCurrency);
        ...
        private ResultArgs SaveVoucherTrans()
        {
            try
            {
                using (DataManager dataManager = new DataManager(SQLCommand.ImportVoucher.InsertVoucherTrans, DataBaseTypeName, SQLAdapterType.HOSQL))
                {
                    ...
                    dataManager.Parameters.Add(this.AppSchema.VoucherTransaction.LIVE_EXCHANGE_RATEColumn, LiveExchangeRate);
                    dataManager.Parameters.Add(this.AppSchema.VoucherTransaction.EXCHANGE_RATEColumn, ExchangeRate);
                    dataManager.Parameters.Add(this.AppSchema.VoucherTransaction.ACTUAL_AMOUNTColumn, ActualAmount);
            ...
            try
            {
                if (dtLedgerBalance != null && dtLedgerBalance.Rows.Count > 0)
                {
                    ...
                    foreach (DataRow drBalance in dtLedgerBalance.Rows)
                    {
                        ...
                        if (dtLedgerBalance.Columns.Contains(this.AppSchema.LedgerBalance.AMOUNT_FCColumn.ColumnName))
                        {
                            BalanceAmountFC = this.NumberSet.ToDouble(drBalance[this.AppSchema.LedgerBalance.AMOUNT_FCColumn.ColumnName].ToString());
                        }
                        BalanceTransMode = drBalance[this.AppSchema.LedgerBalance.TRANS_MODEColumn.ColumnName].ToString();
                        if (dtLedgerBalance.Columns.Contains(this.AppSchema.LedgerBalance.TRANS_FC_MODEColumn.ColumnName))
                        {
                            BalanceTransFCMode = drBalance[this.AppSchema.LedgerBalance.TRANS_FC_MODEColumn.ColumnName].ToString();
                        }
        ...
        private ResultArgs UpdateOpeningBalance()
        {
            try
            {
                using (BalanceSystem balance = new BalanceSystem())
                {
                    ...
                    {
                        resultArgs = balance.UpdateOpBalance(BalanceDate.ToString(), (IsClientBranch) ? 0 : BranchId, ProjectId, LedgerId, BalanceAmount, BalanceTransMode, TransactionAction.Cancel, BalanceAmountFC);
                        resultArgs = balance.UpdateOpBalance(BalanceDate.ToString(), (IsClientBranch) ? 0 : BranchId, ProjectId, LedgerId, BalanceAmount, BalanceTransMode, TransactionAction.New, BalanceAmountFC);
                    }
                    else
                    {
                        resultArgs = balance.UpdateOpBalance(BalanceDate.ToString(), (IsClientBranch) ? 0 : BranchId, ProjectId, LedgerId, BalanceAmount, BalanceTransMode, TransactionAction.New, BalanceAmountFC);
                ...
                if (dtFDAccount != null && dtFDAccount.Rows.Count > 0)
                {
                    ...
                    if (dvFDAccount != null && dvFDAccount.Count > 0)
                    {
                        foreach (DataRowView drFD in dvFDAccount)
                        {
                            ...
                            if (dtFDAccount.Columns.Contains(this.AppSchema.FDAccount.CONTRIBUTION_AMOUNTColumn.ColumnName))
                            {
                                ContributionAmount = NumberSet.ToDecimal(drFD[this.AppSchema.FDAccount.CONTRIBUTION_AMOUNTColumn.ColumnName].ToString());
                            }
                            if (dtFDAccount.Columns.Contains(this.AppSchema.FDAccount.EXCHANGE_RATEColumn.ColumnName))
                            {
                                ExchangeRate = NumberSet.ToDecimal(drFD[this.AppSchema.FDAccount.EXCHANGE_RATEColumn.ColumnName].ToString());
                            }
                            if (dtFDAccount.Columns.Contains(this.AppSchema.FDAccount.IS_MULTI_CURRENCYColumn.ColumnName))
                            {
                                IsMultiCurrency = this.NumberSet.ToInteger(drFD[this.AppSchema.FDAccount.IS_MULTI_CURRENCYColumn.ColumnName].ToString());
                            }
                            if (dtFDAccount.Columns.Contains(this.AppSchema.FDAccount.ACTUAL_AMOUNTColumn.ColumnName))
                            {
                                ActualAmount = NumberSet.ToDecimal(drFD[this.AppSchema.FDAccount.ACTUAL_AMOUNTColumn.ColumnName].ToString());
                            }
                            if (dtFDAccount.Columns.Contains(this.AppSchema.Country.CURRENCY_COUNTRYColumn.ColumnName))
                            {
                                CountryName = drFD[this.AppSchema.Country.CURRENCY_COUNTRYColumn.ColumnName].ToString();
                            }
                    ...
                    if (dtFDVoucherTrans != null && dtFDVoucherTrans.Rows.Count > 0)
                    {
                        ...
                        if (dvTrans.Count > 1)
                        {
                            foreach (DataRowView dr in dvTrans)
                            {
                                ...
                                if (dtFDVoucherTrans.Columns.Contains(this.AppSchema.VoucherTransaction.LIVE_EXCHANGE_RATEColumn.ColumnName))
                                {
                                    LiveExchangeRate = this.NumberSet.ToDecimal(dr[this.AppSchema.VoucherTransaction.LIVE_EXCHANGE_RATEColumn.ColumnName].ToString());
                                }
                                if (dtFDVoucherTrans.Columns.Contains(this.AppSchema.VoucherTransaction.EXCHANGE_RATEColumn.ColumnName))
                                {
                                    ExchangeRate = this.NumberSet.ToDecimal(dr[this.AppSchema.VoucherTransaction.EXCHANGE_RATEColumn.ColumnName].ToString());
                                }
                                if (dtFDVoucherTrans.Columns.Contains(this.AppSchema.VoucherTransaction.ACTUAL_AMOUNTColumn.ColumnName))
                                {
                                    ActualAmount = this.NumberSet.ToDecimal(dr[this.AppSchema.VoucherTransaction.ACTUAL_AMOUNTColumn.ColumnName].ToString());
                                }
        ...
        private ResultArgs SaveFDAccountDetails()
        {
            ...
            try
            {
                using (DataManager dataManager = new DataManager(VoucherImportType.Equals(ImportType.SplitProject) ? SQLCommand.ImportVoucher.InsertFDAcountForSplitProject : SQLCommand.ImportVoucher.InsertFDAccount, DataBaseTypeName, SQLAdapterType.HOSQL))
                {
                    ...
                    dataManager.Parameters.Add(this.AppSchema.FDAccount.CONTRIBUTION_AMOUNTColumn, ContributionAmount);
                    dataManager.Parameters.Add(this.AppSchema.FDAccount.EXCHANGE_RATEColumn, ExchangeRate);
                    dataManager.Parameters.Add(this.AppSchema.FDAccount.IS_MULTI_CURRENCYColumn, IsMultiCurrency);
                    dataManager.Parameters.Add(this.AppSchema.FDAccount.ACTUAL_AMOUNTColumn, ActualAmount);
                    dataManager.Parameters.Add(this.AppSchema.FDAccount.CURRENCY_COUNTRY_IDColumn, VoucherImportType.Equals(ImportType.SplitProject) ? GetId(Id.LocalCountry) : GetId(Id.Country));  // GetId(Id.Country)
                    ...
                    //IsMultiCurrency = this.NumberSet.ToInteger(drFD[this.AppSchema.FDAccount.IS_MULTI_CURRENCYColumn.ColumnName].ToString());
                    //ActualAmount = NumberSet.ToDecimal(drFD[this.AppSchema.FDAccount.ACTUAL_AMOUNTColumn.ColumnName].ToString());
                    //CountryName = drFD[this.AppSchema.FDAccount.CURRENCY_COUNTRY_IDColumn.ColumnName].ToString();
                    ...
                    if (dtGSTInvoiceMaster != null && dtGSTInvoiceMaster.Rows.Count > 0)
                    {
                        ...
                        if (dtgstinvoicemaster.DefaultView.Count > 0)
                        {
                            ...
                            if (resultArgs.Success)
                            {
                                ...
                                GSTMasterBillingCountryId = VoucherImportType.Equals(ImportType.SplitProject) ? GetId(Id.LocalCountry) : GetId(Id.Country); // GetId(Id.Country);
                                if (GSTMasterBillingCountryId > 0)
                                {
                                    ...
                                    GSTMasterBillingStateId = VoucherImportType.Equals(ImportType.SplitProject) ? GetId(Id.LocalState) : GetId(Id.State); // GetId(Id.State);
                                }

                                CountryName = StateName = string.Empty;
                                CountryName = drGSTInvoiceMaster[AppSchema.GSTInvoiceMaster.SHIPPING_COUNTRYColumn.ColumnName].ToString();
                                GSTMasterShippingCountryId = VoucherImportType.Equals(ImportType.SplitProject) ? GetId(Id.LocalCountry) : GetId(Id.Country); // GetId(Id.Country);
                                if (GSTMasterShippingCountryId > 0)
                                {
                                    ...
                                    GSTMasterShippingStateId = VoucherImportType.Equals(ImportType.SplitProject) ? GetId(Id.LocalState) : GetId(Id.State); // GetId(Id.State);
                                }
            ...
            try
            {
                ...
                foreach (var item in query)
                {
                    switch (item)
                    {
                        ...
                        case COUNTRY_CURRENCY_TABLE_NAME:
                            {
                                dtCountryCurrency = dsReadXML.Tables[COUNTRY_CURRENCY_TABLE_NAME];
                                break;
            ...
            try
            {
                using (DataManager dataManager = new DataManager(DataBaseTypeName))
                {
                    ...
                    switch (queryId)
                    {
                        ...
                        case SQLCommand.ImportVoucher.IsLocalCountryExists:
                            {
                                dataManager.DataCommandArgs.SQLCommandId = SQLCommand.ImportVoucher.IsLocalCountryExists;
                                dataManager.Parameters.Add(this.AppSchema.Country.COUNTRYColumn, CountryName);
                                TypeofExists = Id.Country.ToString() + " (" + CountryName + ")";
                                break;
                            }
                        case SQLCommand.ImportVoucher.IsCountryCurrencyExists:
                            {
                                dataManager.DataCommandArgs.SQLCommandId = SQLCommand.ImportVoucher.IsCountryCurrencyExists;
                                dataManager.Parameters.Add(this.AppSchema.Country.COUNTRY_IDColumn, CountryId);
                                dataManager.Parameters.Add(AppSchema.Country.APPLICABLE_FROMColumn, CountryCurrencyApplicableFrom);
                                dataManager.Parameters.Add(AppSchema.Country.APPLICABLE_TOColumn, CountryCurrencyApplicableTo);
                                TypeofExists = Id.Country.ToString() + " (" + CountryName + ")";
                        ...
                        case SQLCommand.ImportVoucher.IsStateExists:
                            {
                                ...
                                dataManager.Parameters.Add(this.AppSchema.State.STATEColumn, StateName);
                                TypeofExists = Id.State.ToString() + " (" + StateName + ")";
                                break;
                            }
                        case SQLCommand.ImportVoucher.IsLocalStateExists:
                            {
                                dataManager.DataCommandArgs.SQLCommandId = SQLCommand.ImportVoucher.IsLocalStateExists;
                                dataManager.Parameters.Add(this.AppSchema.State.STATEColumn, StateName);
                                TypeofExists = Id.State.ToString() + " (" + StateName + ")";
                                break;
                            }
            ...
            try
            {
                using (DataManager dataManager = new DataManager(DataBaseTypeName))
                {
                    ...
                    switch (queryId)
                    {
                        ...
                        case Id.LocalCountry:
                            {
                                dataManager.DataCommandArgs.SQLCommandId = SQLCommand.ImportVoucher.GetLocalCountryId;
                                dataManager.Parameters.Add(this.AppSchema.Country.COUNTRYColumn, CountryName);
                                break;
                            }
                        case Id.CountryCurrency:
                            {
                                dataManager.DataCommandArgs.SQLCommandId = SQLCommand.ImportVoucher.GetCountryCurrencyId;
                                dataManager.Parameters.Add(this.AppSchema.Country.COUNTRYColumn, CountryCurrencyName);
                                break;
                            }
                        case Id.LegalEntity:
                            {
                        ...
                        case Id.LocalState:
                            {
                                dataManager.DataCommandArgs.SQLCommandId = SQLCommand.ImportVoucher.GetLocalStateId;
                                dataManager.Parameters.Add(this.AppSchema.State.STATEColumn, StateName);
                                break;
                            }
                        case Id.SubLedger:
        ...
        // 19/12/2025, Chinna.
        // Fixed NullReferenceException: Initialize resultArgs if null before use
        private ResultArgs ValidateLicenseKey()
        {
            AcMEDataSynLog.WriteLog("ValidateLicenseKey Started.");
            // 19/12/2025, Chinna: Initialize resultArgs if null to prevent NullReferenceException
            if (resultArgs == null)
            {
                resultArgs = new ResultArgs();
            }
                ...
                if (dsXML != null && dsXML.Tables.Count > 0)
                {
                    ...
                    if (dtHeader != null && dtHeader.Rows.Count > 0)
                    {
                        ...
                        if ((!string.IsNullOrEmpty(drHeader["DATE_FROM"].ToString())) && (!string.IsNullOrEmpty(drHeader["DATE_TO"].ToString())))
                        {
                            ...
                            if (dtHeader.Columns.Contains(AppSchema.VoucherMaster.IS_MULTI_CURRENCYColumn.ColumnName))
                            {
                                IsMultiCurrency = this.NumberSet.ToInteger(dtHeader.Rows[0][AppSchema.VoucherMaster.IS_MULTI_CURRENCYColumn.ColumnName].ToString());
                                SettingProperty.Current.AllowMultiCurrency = IsMultiCurrency;
                            }
                        }
                        else
                        {
        ...
        /// <returns>ResultArgs with its properties</returns>
        // 19/12/2025, Chinna.
        // Fixed NullReferenceException: Initialize resultArgs and use local result variable properly
        private ResultArgs UpdateDataSynStatus(DataSyncMailType status, string Remark)
        {
            ...
            AcMEDataSynLog.WriteLog("UpdateDataSynStatus Started: " + status.ToString());
            try
            {
            ...
            //try
            //{
            //    //VoucherImportType = ImportType.Portal;
            //    //if (VoucherImportType == ImportType.Portal)
            //    //{
            //    //using (DataManager dataManager = new DataManager(SQLCommand.ImportVoucher.UpdateDataSynStatus, DataBaseType.Portal, SQLAdapterType.HOSQL))
            //    using (DataManager dataManager = new DataManager(IsClientBranch ? SQLCommand.ImportVoucher.UpdateSubBranchDsyncStatus : SQLCommand.ImportVoucher.UpdateDataSynStatus,
            //       DataBaseType.Portal, SQLAdapterType.HOSQL))
            //    {
            //        dataManager.Parameters.Add(this.AppSchema.DataSyncStatus.HEAD_OFFICE_IDColumn, HeadOfficeId);
            ...
            //        //dataManager.DataCommandArgs.IsDirectReplaceParameter = true;
            //        result = dataManager.UpdateData();
            //        // 19/12/2025, Chinna: Update resultArgs with result to maintain consistency
            //        resultArgs = result;
            //    }
            //    //  }
