...
namespace Bosco.Report.Base
{
    public class ReportBase : DevExpress.XtraReports.UI.XtraReport
    {
        ...
        // 25/05/2026, chinna - Approved amount columns only for Monthly Abstract RPT-001/002/003
        protected int ShowBudgetApprovedAmountInMonthlyReportForReport
        {
            get
            {
                ...
                if (ReportProperties != null && !string.IsNullOrEmpty(ReportProperties.ReportId))
                    reportId = ReportProperties.ReportId;

                if (reportId == "RPT-001" || reportId == "RPT-002" || reportId == "RPT-003")
                    return 1;
        ...
        public virtual void ShowReport()
        {
            ...
            bool isStandardRpt = (this.reportId == "RPT-STD" || this.reportId == "RPT-152" || this.reportId == "RPT-239" || FromViewScreen); //this.reportId == "RPT-163" || this.reportId == "RPT-180"
            if (isStandardRpt)
            {
        ...
        public ResultArgs GetBalanceDetail(bool isOpening, string balDate, string projectIds, string groupIds, string fixedBankcloseddate = "")
        {
            ...
            using (DataManager dataManager = new DataManager(SQLCommand.TransBalance.FetchBalance))
            {
                ...
                //----------------------------------------------------------------------------------------------------------------------------------
                //bharath 26/02/2026 Language update
        ...
        public ResultArgs GetProjects()
        {
            ...
            using (DataManager dataManager = new DataManager(SQLCommand.Project.FetchProjects))
            {
                ...
                dataManager.Parameters.Add(this.ReportParameters.DISPLAY_INPUT_LANColumn, this.settingProperty.DisplayInputLanguage == "1" ? 1 : 0);
