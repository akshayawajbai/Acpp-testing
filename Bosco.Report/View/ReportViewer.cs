...
namespace Bosco.Report.View
{
    public partial class ReportViewer : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        public bool IsResizingReports
        {
            get
            {
                return (this.ReportId == "RPT-001" || this.ReportId == "RPT-002" || this.ReportId == "RPT-003" || // Monthly Abstract Receipts, Payments and Both
                    ...
                    this.ReportId == "RPT-101" || this.ReportId == "RPT-062" || this.ReportId == "RPT-240" ||  //Multi Column Cash Band Bank, this.ReportId == "RPT-030
        ...
        private void UpdatePageSetupVisibility()
        {

            if (this.ReportId == "RPT-240")
            {
                pageSetup.Visibility = DevExpress.XtraBars.BarItemVisibility.Never;
                pageSetup.Enabled = false;
            }
            else
            {
                pageSetup.Visibility = DevExpress.XtraBars.BarItemVisibility.Always;
            }
        }
            ...
            if (activeReport != null)
            {
                            ...
                            activeReport.ReportId != "RPT-212")
                {
                    ...
                    if (ReportProperty.Current.ReportId == "RPT-101" || // Only For Multi Column Cashbank Report Control to be re initialised so This method is called to re-initializes by Praveen
                        ...
                        (ReportProperty.Current.ReportId == "RPT-030" && activeReport.dialogResult == DialogResult.OK) || (ReportProperty.Current.ReportId == "RPT-240" && activeReport.dialogResult == DialogResult.OK) ||
        ...
        {
            if (CommonMethod.IsAuditorViewOnlyMode())
            {
                MessageRender.ShowMessage(MessageRender.GetMessage(MessageCatalog.Common.COMMON_USER_RIGHTS));
                return;
            }
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(folderpath))
                {
                                         ...
                                         " in the selected path \"" + folderpath + "\" ?", MessageBoxButtons.YesNo, MessageBoxIcon.Question)) == DialogResult.Yes)
                    {
                        ...
                        // ------------------------------------------ chinna 07-04-2026---------
                        // Old method was copied and put in the files 
                        for (int pageIndex = 0; pageIndex < activeReport.Pages.Count; pageIndex++)
                        {
                            Page p = activeReport.Pages[pageIndex];
                            staffcode = string.Empty;
                            pwd = string.Empty;

                            PayPayslip payslipReport = activeReport as PayPayslip;
                            if (payslipReport != null)
                            {
                                staffcode = payslipReport.GetPayslipExportBaseNameForPage(pageIndex);
                            }
                            if (string.IsNullOrEmpty(staffcode) && p.Watermark != null && p.Watermark.Text != null)
                            {
                                staffcode = p.Watermark.Text;
                            }
                            if (!string.IsNullOrEmpty(staffcode) && staffcode.IndexOf("_") > 0)
                            {
        ...
        private void HandleReportViewerButtons()
        {
            ...
            if (this.ReportId == "RPT-027" ||
                ...
                this.ReportId == "RPT-028" || this.ReportId == "RPT-047" || this.ReportId == "RPT-030" || this.ReportId == "RPT-240" ||
