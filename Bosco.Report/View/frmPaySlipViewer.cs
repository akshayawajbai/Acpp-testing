    ...
    public partial class frmPaySlipViewer : Bosco.Utility.Base.frmBase
    {
        ...
        private void btnShow_Click(object sender, EventArgs e)
        {
            if (ApplyReportCriteria())
            {
                ...
                if (ReportProperty.Current.ReportId == "RPT-170")
                {
                    ...
                    if (glkBankAc.EditValue != null && ReportProperty.Current.PayrollPaymentModeId != 0 )
                    {
                        ...
                        if (drvBankAc != null)
                        {
                            ...
                            // #Dinesh 03-07-2026 - Pass selected bank account id to filter staff in payment advice report
                            ReportProperty.Current.PayrollPaymentBankAccountId = string.IsNullOrEmpty(drvBankAc["BANK_ACCOUNT_ID"].ToString()) ?
                                0 : UtilityMember.NumberSet.ToInteger(drvBankAc["BANK_ACCOUNT_ID"].ToString());
                            //ReportProperty.Current.SaveReportSetting();
                        }
                    }
                    else
                    {
                        ReportProperty.Current.PayrollPaymentBankAccountNo = string.Empty;
                        ReportProperty.Current.PayrollPaymentBankAddress = string.Empty;
                        ReportProperty.Current.PayrollPaymentBankAccountLedgerId = string.Empty;
                        ReportProperty.Current.PayrollPaymentBankAccountId = 0;
                    }
                }
        ...
        private void EnableOptions()
        {
            ...
            if (ReportProperty.Current.ReportId == "RPT-170") //Advice Payment
            {
                ...
                lcAuthSign1.Visibility = lcAuthSign2.Visibility = DevExpress.XtraLayout.Utils.LayoutVisibility.Never;
                lcBankAc.Visibility = lcPaymentMode.Visibility = lcChkShowSignDetails.Visibility = lcGrpSign1.Visibility = lcGrpSign2.Visibility = lcGrpSign3.Visibility = DevExpress.XtraLayout.Utils.LayoutVisibility.Always;
        ...
        private void LoadAllBanksAccounts()
        {
            using (DataManager dataManager = new DataManager(SQLCommand.LedgerBank.BankAccountFetchAll))
            {
                ...
                {
                    DataTable dtBankAccounts = resultArgs.DataSource.Table;
                    if (!dtBankAccounts.Columns.Contains("BANK_DISPLAY"))
                    {
                        dtBankAccounts.Columns.Add("BANK_DISPLAY", typeof(string));
                    }
                    foreach (DataRow dr in dtBankAccounts.Rows)
                    {
                        ...
                        dr["BANK_DISPLAY"] = string.Format("{0} ({1})", accountNo, bankName);
                    }
                    UtilityMember.ComboSet.BindGridLookUpCombo(glkBankAc, dtBankAccounts, "BANK_DISPLAY", this.appSchema.BankAccount.LEDGER_IDColumn.ColumnName);
                    colBankLedger.FieldName = "BANK_DISPLAY";
                    colBankLedgerId.Visible = false;
                    colBankLedger.VisibleIndex = 0;
                    if (dtBankAccounts.Rows.Count > 0)
                    {
                        glkBankAc.EditValue = ReportProperty.Current.PayrollPaymentBankAccountLedgerId;
                    }
