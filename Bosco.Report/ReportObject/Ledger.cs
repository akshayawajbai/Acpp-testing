...
namespace Bosco.Report.ReportObject
{
    public partial class Ledger : Bosco.Report.Base.ReportHeaderBase
    {
        ...
        public Ledger()
        {
            ...
            // Layout tweaks to avoid large blank spaces when narration is long
            // Allow narration to flow across pages instead of moving the whole row/table
            if (this.Detail != null)
            {
                this.Detail.KeepTogether = false;
            }
            if (xrtblBindSource != null)
            {
                xrtblBindSource.KeepTogether = false;
            }

            if (xrRowNarration != null)
            {
                xrRowNarration.KeepTogether = false;
            }

            if (xrRowData != null)
            {
                // Keep the numeric/data row intact; move it to next page if it doesn't fit
                xrRowData.KeepTogether = true;
            }

            if (xrcellNarration != null)
            {
                xrcellNarration.KeepTogether = false;
                xrcellNarration.CanGrow = true;   // ensure long text can extend and split
                xrcellNarration.CanShrink = true; // keep compact when empty/short
                xrcellNarration.WordWrap = true;  // wrap lines properly
                xrcellNarration.Multiline = true; // ensure multi-line rendering
            }
        }
        #endregion
        ...
        public static string OPBALTITLE = MessageRender.GetMessage(MessageCatalog.Criteria.OPENING_BALANCE);
        ...
        public override void ShowReport()
        {
            ...
            //bharath 29/07/2026 for ledger index issues
            TOCPageOffset = 0;
            ...
            if (IsDrillDownMode)
            {
                ...
                if (this.ReportProperties.DrillDownProperties != null && this.ReportProperties.DrillDownProperties.Count > 0)
                {
                        ...
                        && (GroupId != (int)FixedLedgerGroup.Cash && GroupId != (int)FixedLedgerGroup.BankAccounts && GroupId != (int)FixedLedgerGroup.FixedDeposit))
                    {
                        ...
                        else if (RecentDrillingReportId == "RPT-030" || BaseReportId == "RPT-030" || RecentDrillingReportId == "RPT-240" || BaseReportId == "RPT-240") //TB report - Current and Closing
                        {
        ...
        public void BindReport()
        {
            ...
            // old logic: only drill-down with currency showed symbol; closing balance had extra null check
            ...
            // new logic: like FinalReceiptsPayments - set currency/caption from selection (report criteria or drill-down)
            if (this.settingProperty.AllowMultiCurrency == 1)
            {
                string currencySymbol = (DrillCurrencyCountryId > 0 && !string.IsNullOrEmpty(DrillCurrencySymbol))
                    ? DrillCurrencySymbol
                    : (!string.IsNullOrEmpty(ReportProperties.CurrencyCountrySymbol)
                        ? ReportProperties.CurrencyCountrySymbol
                        : (ReportProperties.CurrencyCountryId > 0 ? this.GetCurrencySymbol(ReportProperties.CurrencyCountryId) : string.Empty));
                if (!string.IsNullOrEmpty(currencySymbol))
                {
                    xrCapDebit.Text = CapDebit + " (" + currencySymbol + ")";
                    xrCapCredit.Text = CapCredit + " (" + currencySymbol + ")";
                    xrCapclosingBalance.Text = CapClosingBalance + " (" + currencySymbol + ")";
                }
                else
                {
                    this.SetCurrencyFormat(xrCapDebit.Text, xrCapDebit);
        ...
        public ResultArgs BindLedgerSource()
        {
            ...
            using (DataManager dataManager = new DataManager())
            {
                ...
                // Priority: Drill-down currency > Normal currency > Default "0"
                if (this.settingProperty.AllowMultiCurrency == 1)
                {
                    if (this.IsDrillDownMode && this.DrillCurrencyCountryId > 0)
                    {
                        // Use currency from drill-down properties
                        dataManager.Parameters.Add(this.ReportParameters.CURRENCY_COUNTRY_IDColumn, this.DrillCurrencyCountryId);
                    }
                    else if (this.ReportProperties.CurrencyCountryId > 0)
                    {
                        // Use currency from report properties (normal mode)
                        dataManager.Parameters.Add(this.ReportParameters.CURRENCY_COUNTRY_IDColumn, this.ReportProperties.CurrencyCountryId);
                    }
                    else
                    {
                        // Default to "0" when no currency specified
                        dataManager.Parameters.Add(this.ReportParameters.CURRENCY_COUNTRY_IDColumn, "0");
                    }
                }
                else
                {
                    // Multi-currency disabled, use default
                    dataManager.Parameters.Add(this.ReportParameters.CURRENCY_COUNTRY_IDColumn, "0");
                }
                // 31/03/2026, SDBAFE opening rules: GN ledgers use AMOUNT_FC only when CUR_COUNTRY_ID matches ?CURRENCY_COUNTRY_ID (see Report.Ledger LB2 CASE).
                // - OP hidden (normal ledger): keep IS_SDBAFE=1 so FC column matches transaction filter (legacy).
                // - OP shown + a specific currency (normal or drill, e.g. RPT-028 Income & Expenditure): IS_SDBAFE=1 so opening matches Euro-only / dollar-only view.
                // - OP shown + base currency (0): IS_SDBAFE=0 so opening uses base amounts.
                int isSdbafe = 0;
                //if (this.settingProperty.AllowMultiCurrency == 1 &&
                //    (this.settingProperty.IS_SDBAFE || this.settingProperty.IS_BSGRMG))
                if (this.settingProperty.AllowMultiCurrency == 1 &&
              (this.settingProperty.IS_BSGRMG))
                {
                    int effectiveCurrencyCountryId = 0;
                    if (this.IsDrillDownMode && this.DrillCurrencyCountryId > 0)
                        effectiveCurrencyCountryId = this.DrillCurrencyCountryId;
                    else if (this.ReportProperties.CurrencyCountryId > 0)
                        effectiveCurrencyCountryId = this.ReportProperties.CurrencyCountryId;

                    if (!this.IsDrillDownMode)
                    {
                        if (this.ReportProperties.ShowLedgerOpBalance == 0)
                            isSdbafe = 1;
                        else if (effectiveCurrencyCountryId > 0)
                            isSdbafe = 1;
                    }
                    else if (this.ReportProperties.ShowLedgerOpBalance == 1 && effectiveCurrencyCountryId > 0)
                        isSdbafe = 1;
                }
                dataManager.Parameters.Add("IS_SDBAFE", isSdbafe);
        ...
        private void xrLedgerDebitBalance_SummaryRowChanged(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(OPBalanceDate))
            {
                ...
                xrOPBalanceCaption.Text = MessageRender.GetMessage(MessageCatalog.Criteria.OPENING_BALANCE_AS_ON) + this.UtilityMember.DateSet.ToDate(dtTemp, "dd-MM-yyyy");
            }
            else
            {
                xrOPBalanceCaption.Text = MessageRender.GetMessage(MessageCatalog.Criteria.OPENING_BALANCE);
            }
        ...
        private void Ledger_AfterPrint(object sender, EventArgs e)
        {
            if (!this.IsDrillDownMode &&
                ...
                dtTOCList.Rows.Count > 0)
            {
                UcTOC uctoc = CreateTOCDocument(0);
                TOCPageOffset = uctoc.Pages.Count;

                UpdateTOCPages();

                uctoc = CreateTOCDocument(0);

                if (uctoc.Pages.Count != TOCPageOffset)
                {
                    TOCPageOffset = uctoc.Pages.Count;
                    UpdateTOCPages();
                    uctoc = CreateTOCDocument(0);
        ...
        /// <summary>
        /// Builds the TOC sub-report. Set noOfTocPages before CreateDocument when
        /// PAGE_NO values are content-only; use 0 when PAGE_NO already includes TOC offset.
        /// </summary>
        private UcTOC CreateTOCDocument(int noOfTocPages)
        {
            UcTOC uctoc = new UcTOC();
            uctoc.CCTableWidth = xrtblHeaderCaption.WidthF;
            uctoc.NoOfTOCPages = noOfTocPages;
            uctoc.BindCCDetails(dtTOCList);
            uctoc.CreateDocument();
            return uctoc;
        }
        ...
        private void UpdateTOCPages()
        {
            ...
            List<string> updatedLedgers = new List<string>();
            ...
            for (int i = 0; i < this.Pages.Count; i++)
            {
                ...
                while (iterator.MoveNext())
                {
                    ...
                    if (brick != null && brick.Value != null && brick.Value.ToString().StartsWith("Target_"))
                    {
                        ...
                        if (updatedLedgers.Contains(ledgerName))
                            continue;
