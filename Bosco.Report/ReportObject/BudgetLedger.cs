...
namespace Bosco.Report.ReportObject
{
    public partial class BudgetLedger : Bosco.Report.Base.ReportHeaderBase
    {
            ...
            DateTime dtPDateFrom, DateTime dtPDateTo, Int32 budgetAction, Int32 PreviousBudgetId)
        {
            ...
            if (resultArgs.Success && resultArgs.DataSource.Table != null)
            {
                ...
                if (this.ReportProperties.IncludeAllBudgetLedgers == 0)
                {
                    ...
                    if (this.AppSetting.IS_BSG_ESP)
                    {
                        // For BSG_ESP, show all ledgers without any filter
                        dtIncomeSource.DefaultView.RowFilter = string.Empty;
                    }
                    else if (this.AppSetting.IS_SDB_INM)
                    {
                        // For SDB_INM, show ledgers with amounts > 0 OR cash/bank/FD groups
                        dtIncomeSource.DefaultView.RowFilter = "PROPOSED_CURRENT_YR > 0 OR APPROVED_CURRENT_YR > 0 OR APPROVED_PREVIOUS_YR > 0 OR ACTUAL > 0 OR GROUP_ID IN (12, 13, 14)";
                    }
                    else
                    {
                        // For others, show ledgers with amounts > 0 OR cash/bank/FD groups
                        dtIncomeSource.DefaultView.RowFilter = "PROPOSED_CURRENT_YR > 0 OR APPROVED_CURRENT_YR > 0 OR APPROVED_PREVIOUS_YR > 0 OR GROUP_ID IN (12, 13, 14)";
                    }
                ...
                if (transmode == TransMode.CR)
                {
                    bool usedDetailCashBankAppend = (this.AppSetting.ShowCashBankFDDetailLedgerInBudgetProposed == 1 ||
                        (AppSetting.IS_ABEBEN_DIOCESE || AppSetting.IS_DIOMYS_DIOCESE || AppSetting.IS_CMF_CONGREGATION));
                    if (usedDetailCashBankAppend)
                    {
                    ...
                    //For detail append (CMF etc.) opening Cash/Bank/FD come from the appended GROUP_ID (13/12/14) rows.
                    //For others, AppendCashBankBalances already assigned these fields directly (same values shown as opening balance).
                    if (usedDetailCashBankAppend)
                    {
                ...
                else if (transmode == TransMode.DR && !AppSetting.IS_SDB_INM)
                {
                    // Closing Cash/Bank/FD for all customers except SDB (same attach used by CMF)
                ...
                //07/07/2020 - Opening/CMF original sort (do not change income behavior)
                if (AppSetting.IS_CMF_CONGREGATION)
                {
                    this.ReportProperties.SortByLedger = 1;// true by default for cmf
                ...
                // Expense Closing only: force Cash/Bank/FD (NATURE_ID=5) before Grand Total
                else if (!AppSetting.IS_SDB_INM && BudgetTransMode == TransMode.DR)
                {
                    Detail.SortFields.Add(new GroupField("NATURE_ID", XRColumnSortOrder.Ascending));
                }

                ConfigureOpeningClosingHeaders();

                if (grpBudgetSubGroupHeader.GroupFields.Count == 0)
                {
                    grpBudgetSubGroupHeader.GroupFields.Add(new GroupField(reportSetting1.BUDGETVARIANCE.BUDGET_SUB_GROUP_IDColumn.ColumnName));
                }
        ...
        /// <summary>
        /// Income: Opening Balance caption uses xrRowOPCL in ledger group header.
        /// Expense: Closing Balance uses the same xrRowOPCL table row (duplicate opening pattern).
        /// </summary>
        private void ConfigureOpeningClosingHeaders()
        {
            grpClosingBalanceHeader.Visible = false;
            grpClosingBalanceHeader.GroupFields.Clear();
        }

        private void MarkClosingBalanceRow(DataRow budgetRow)
        {
            budgetRow[reportSetting1.BUDGETVARIANCE.NATURE_IDColumn.ColumnName] = 5;
            budgetRow[reportSetting1.BUDGETVARIANCE.BUDGET_GROUPColumn.ColumnName] = MessageRender.GetMessage(MessageCatalog.Criteria.CLOSING_BALANCE);
            budgetRow[reportSetting1.BUDGETVARIANCE.BUDGET_GROUP_IDColumn.ColumnName] = 0;
            budgetRow[reportSetting1.BUDGETVARIANCE.BUDGET_SUB_GROUPColumn.ColumnName] = string.Empty;
            budgetRow[reportSetting1.BUDGETVARIANCE.BUDGET_SUB_GROUP_IDColumn.ColumnName] = 0;
            budgetRow["BUDGET_GROUP_SORT_ID"] = 99;
        }
        ...
        private void AppendCashBankBalances(DataTable dtIncomeBudget)
        {
            ...
            //Assign Cash/Bank/FD balances for the Budget Note (treated the same as CMF, but summed directly
            //here since these clients add a combined GROUP_ID=0 row). Values match the displayed opening balance.
            cashOpeningBalance = CashCLBalance;
            bankOpeningBalance = BankCLBalance;
            fdOpeningBalance = FDCLBalance;
            totalOpeningBalance = CashCLBalance + BankCLBalance + FDCLBalance;
        ...
        private void AppendCashBankFDRow(DataTable dtIncomeBudget, DataTable dtCashBankDetail, bool isOpeningbalance, bool isAttachClosingBalanes = false)
        {
            foreach (DataRow dr in dtCashBankDetail.Rows)
            {
                ...
                if (dtIncomeBudget.DefaultView.Count > 0)
                {
                    ...
                    if (isAttachClosingBalanes)
                    {
                        // Closing only: do not treat balances as proposed/approved expense lines
                        MarkClosingBalanceRow(dtIncomeBudget.DefaultView[0].Row);
                        dtIncomeBudget.DefaultView[0]["ACTUAL"] = balanceamt;
                    }
                    else
                    {
                        // Opening (income) - original behavior, unchanged
                        dtIncomeBudget.DefaultView[0]["PROPOSED_CURRENT_YR"] = balanceamt;
                        dtIncomeBudget.DefaultView[0]["APPROVED_CURRENT_YR"] = balanceamt;
                    }
                ...
                else
                {
                    ...
                    // Opening keeps empty budget group (original). Closing marks Closing Balance.
                    if (isAttachClosingBalanes)
                    {
                        MarkClosingBalanceRow(drCashBank);
                    }
                    else
                    {
                        //On 27/01/2022 for CMF - Opening Balance path (do not change)
                    ...
                    if (isAttachClosingBalanes)
                    {
                        // Closing: keep previous approved empty; ACTUAL holds balance
                    }
                    else
                    {
                        // Opening - original behavior
        ...
        private void ShowCMFNoteToProvince()
        {
            ...
            // Note prints once, on the payment (expense) side only, for all customers except SDB
            grpFooterCMFBudgetNote.Visible = false;

            if (!AppSetting.IS_SDB_INM && BudgetTransMode == TransMode.DR)
            {
                if (this.DataSource != null)
                {
                    ...
                    if (AppSetting.IS_CMF_CONGREGATION)
                    {
                        // CMF: sum the 'Ordinary Expenses' and 'Extraordinary Expenses' budget groups separately
                    ...
                    {
                        // Others (except SDB): no Ordinary/Extraordinary groups, so sum ALL budgeted expenses
                        // the same way the income budget amount is summed (exclude Cash/Bank/FD groups 13/12/14)
                        ordinaryExprense = UtilityMember.NumberSet.ToDouble(dtRpt.Compute("SUM(PROPOSED_CURRENT_YR)", "(GROUP_ID>0 AND LEDGER_ID>0) AND GROUP_ID NOT IN (12, 13, 14)").ToString());
                        extraordinaryExprense = 0;
                    }
                    ...
                    // Formula note under Budget Note table (bold key parts). Shows only when this band is visible.
                    string formulaNoteHtml = "<div style='font-family:Tahoma;font-size:9pt;line-height:150%;'>"
                        + "<b>Note: Surplus/Deficit</b> is calculated as "
                        + "<b>Cash + Bank Balance + Budgeted Proposed Income − (Budgeted Ordinary Expenses + Budgeted Extraordinary Expenses)</b>. "
                        + "<b>Fixed Deposits</b> are displayed for reference only and are not included in the Surplus/Deficit calculation."
                        + "</div>";
                    using (System.IO.MemoryStream noteStream = new System.IO.MemoryStream(System.Text.Encoding.UTF8.GetBytes(formulaNoteHtml)))
                    {
                        xrLblCMFBudgetNoteFormula.LoadFile(noteStream, DevExpress.XtraReports.UI.XRRichTextStreamType.HtmlText);
                    }
                }
            }
        }
        ...
        private void grpBudgetGroupHeader_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            // Closing caption is shown via opening-balance table row (xrRowOPCL), not budget group header
            if (BudgetTransMode != TransMode.DR || AppSetting.IS_SDB_INM)
            {
                return;
            }
            if (GetCurrentColumnValue(reportSetting1.BUDGETVARIANCE.BUDGET_GROUPColumn.ColumnName) == null)
            {
                return;
            }
            string budgetGroup = GetCurrentColumnValue(reportSetting1.BUDGETVARIANCE.BUDGET_GROUPColumn.ColumnName).ToString();
            string closingCaption = MessageRender.GetMessage(MessageCatalog.Criteria.CLOSING_BALANCE);
            if (string.Equals(budgetGroup, closingCaption, StringComparison.OrdinalIgnoreCase))
            {
                e.Cancel = true;
            }
        }
        ...
        private void grpLedgerGroupHeader_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            ...
            // Duplicate opening-balance table row for opening (income) and closing (expense) captions
            xrRowOPCL.Visible = false;
            if (GetCurrentColumnValue("GROUP_ID") != null)
            {
                Int32 groupid = this.UtilityMember.NumberSet.ToInteger(GetCurrentColumnValue("GROUP_ID").ToString());
                Int32 natureId = 0;
                if (GetCurrentColumnValue(reportSetting1.BUDGETVARIANCE.NATURE_IDColumn.ColumnName) != null)
                {
                    natureId = this.UtilityMember.NumberSet.ToInteger(GetCurrentColumnValue(reportSetting1.BUDGETVARIANCE.NATURE_IDColumn.ColumnName).ToString());
                }

                if (BudgetTransMode == TransMode.CR && groupid == (int)FixedLedgerGroup.Cash)
                {
                    xrRowOPCL.Visible = true;
                    xrCellLedgerGrpOpClBalances.Text = MessageRender.GetMessage(MessageCatalog.Criteria.OPENING_BALANCE);
                }
                else if (BudgetTransMode == TransMode.DR && !AppSetting.IS_SDB_INM &&
                    groupid == (int)FixedLedgerGroup.Cash && natureId == 5)
                {
                    xrRowOPCL.Visible = true;
                    xrCellLedgerGrpOpClBalances.Text = MessageRender.GetMessage(MessageCatalog.Criteria.CLOSING_BALANCE);
                }
        ...
        private void grpClosingBalanceHeader_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            e.Cancel = true;
        }
        ...
        {
            // Combined Closing Cash/Bank/FD Sub Total (one row before Grand Total)
            if (BudgetTransMode == TransMode.DR && !this.AppSetting.IS_SDB_INM)
            {
                ...
                {
                    e.Result = GetClosingCashBankFdSum("APPROVED_PREVIOUS_YR");
                }
                e.Handled = true;
            }
        }
        ...
        {
            // Combined Closing Cash/Bank/FD Sub Total
            if (BudgetTransMode == TransMode.DR && !this.AppSetting.IS_SDB_INM)
            {
                e.Result = GetClosingCashBankFdSum("ACTUAL");
                e.Handled = true;
            }
        }
        ...
        private void xrSSumProposed_SummaryGetResult(object sender, SummaryGetResultEventArgs e)
        {
            ...
            if (BudgetTransMode == TransMode.DR && !this.AppSetting.IS_SDB_INM)
            {
                ...
                {
                    e.Result = GetClosingCashBankFdSum("PROPOSED_CURRENT_YR");
                }
                e.Handled = true;
            }
        }

        private void xrSSumApproved_SummaryGetResult(object sender, SummaryGetResultEventArgs e)
        {
            // Combined Closing Cash/Bank/FD Sub Total
            if (BudgetTransMode == TransMode.DR && !this.AppSetting.IS_SDB_INM)
            {
                if (this.AppSetting.IS_CMF_CONGREGATION)
                {
                    e.Result = TotalApprovedBudgetedIncome - TotalApprovedBudgetedExpense;
                }
                else
                {
                    e.Result = GetClosingCashBankFdSum("APPROVED_CURRENT_YR");
                }
                e.Handled = true;
            }
        }

        private double GetClosingCashBankFdSum(string columnName)
        {
            double sum = 0;
            if (this.DataSource != null)
            {
                DataTable dtRpt = this.DataSource as DataTable;
                if (dtRpt != null && dtRpt.Columns.Contains(columnName))
                {
                    object result = dtRpt.Compute("SUM(" + columnName + ")", "GROUP_ID IN (12, 13, 14)");
                    sum = UtilityMember.NumberSet.ToDouble(result == null || result == DBNull.Value ? "0" : result.ToString());
                }
            }
            return sum;
        }
        ...
        {
            // Expense Closing only: hide separate Cash/Bank/FD Sub Totals
            if (BudgetTransMode == TransMode.DR && !AppSetting.IS_SDB_INM &&
                GetCurrentColumnValue(reportSetting1.BUDGET_LEDGER.GROUP_IDColumn.ColumnName) != null)
            {
                int closingGroupId = UtilityMember.NumberSet.ToInteger(GetCurrentColumnValue(reportSetting1.BUDGET_LEDGER.GROUP_IDColumn.ColumnName).ToString());
                if (closingGroupId == ((int)FixedLedgerGroup.Cash)
                    || closingGroupId == ((int)FixedLedgerGroup.BankAccounts)
                    || closingGroupId == ((int)FixedLedgerGroup.FixedDeposit))
                {
                    e.Cancel = true;
                    return;
                }
            ...
            // Opening / CMF original footer behavior (Income unchanged)
            if (this.AppSetting.IS_CMF_CONGREGATION)
            {
                if (GetCurrentColumnValue(reportSetting1.BUDGET_LEDGER.GROUP_IDColumn.ColumnName) != null)
                {
                    ...
                    if (GetNextColumnValue(reportSetting1.BUDGET_LEDGER.GROUP_IDColumn.ColumnName) != null)
                    {
        ...
        private void grpBudgetFooter_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            ...
            // One combined Sub Total for Closing Cash/Bank/FD (all non-SDB expense)
            if (BudgetTransMode == TransMode.DR && !this.AppSetting.IS_SDB_INM)
            {
                xrRowGrandSTotalCLBalance.Visible = true;
                xrTableCell15.Text = "Sub Total";
            }
        }
