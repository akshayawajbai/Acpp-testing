...
namespace Bosco.Report.ReportObject
{
    public partial class BudgetLedgerINM : Bosco.Report.Base.ReportHeaderBase
    {
        ...
        public double TotalActualIncome
        {
            get
            {
            ...
            set
            {
                totalActualincome = value;
            }
        }

        double totalActualexpense = 0;
        public double TotalActualExpense
        {
            get
            {
            ...
            DateTime dtPDateFrom, DateTime dtPDateTo, Int32 budgetAction, Int32 PreviousBudgetId)
        {
            ...
            if (transmode == TransMode.CR)
            {
                ...
                xtDifferenceTop.Text = "Difference";
            ...
            else
            {
                ...
                xrtblDiffDown.Text = "Actual - Proposed";
            ...
            if (resultArgs.Success && resultArgs.DataSource.Table != null)
            {
                ...
                if (transmode == TransMode.CR)
                {
                    ...
                    totalActualincome = UtilityMember.NumberSet.ToDouble(dtIncomeSource.Compute("SUM(ACTUAL)", string.Empty).ToString());
                ...
                else
                {
                    ...
                    totalActualexpense = UtilityMember.NumberSet.ToDouble(dtIncomeSource.Compute("SUM(ACTUAL)", string.Empty).ToString());
                ...
                // - both values for difference
                foreach (DataRow row in dtIncomeSource.Rows)
                {
                    decimal actual = row["ACTUAL"] != DBNull.Value ? Convert.ToDecimal(row["ACTUAL"]) : 0;
                    decimal proposed = row["PROPOSED_CURRENT_YR"] != DBNull.Value ? Convert.ToDecimal(row["PROPOSED_CURRENT_YR"]) : 0;

                    row["BUDGET_ACT_PROP_DIFF"] = proposed - actual;
                }
        ...
        {
            if (BudgetTransMode == TransMode.DR)
                e.Result = totalProposedBudgetedExpense - TotalActualExpense;
            else
                e.Result = totalProposedBudgetedIncome - TotalActualIncome;
            e.Handled = true;
        }
