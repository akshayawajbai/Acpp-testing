...
namespace Bosco.Report.ReportObject
{
    public partial class PayPayslip : Bosco.Report.Base.ReportHeaderBase
    {
        ...
        /// <summary>
        /// Per-page export base name (staffcode_staffname) for PDF export when separate pages is on.
        /// Not shown on the report — replaces former watermark-only channel used by ReportViewer.
        /// </summary>
        private readonly Dictionary<int, string> _payslipExportBaseNameByPageIndex = new Dictionary<int, string>();
        #endregion
        ...
        {
            InitializeComponent();
            this.PrintingSystem.StartPrint += (s, ev) => { _payslipExportBaseNameByPageIndex.Clear(); };
        }
        #endregion

        /// <summary>
        /// Export file base name for a printed page (0-based), e.g. EMP001_John Doe — used by ReportViewer when watermark is not assigned.
        /// </summary>
        public string GetPayslipExportBaseNameForPage(int pageIndex)
        {
            string value;
            return _payslipExportBaseNameByPageIndex.TryGetValue(pageIndex, out value) ? value : null;
        }
        ...
        private void BindPaySlipReport()
        {
            ...
            _payslipExportBaseNameByPageIndex.Clear();
        ...
        {
            // On 20/09/2022, per-page name for exporting payslips to separate PDFs (SDBINB).
            // ------------------------------------------ chinna 07-04-2026---------
            ...
            if (sender != null && ReportProperty.Current.PayrollPayslipInSeparatePages == 1)
            {
                ...
                if (lbl.Tag != null && !string.IsNullOrEmpty(lbl.Text))
                {
                    ...
                    string exportfilename = staffcode + "_" + SanitizeForFileName(staffname);
                    _payslipExportBaseNameByPageIndex[e.PageIndex] = exportfilename;
                }
            }
        }
