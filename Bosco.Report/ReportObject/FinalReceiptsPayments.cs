...
namespace Bosco.Report.ReportObject
{
    public partial class FinalReceiptsPayments : Bosco.Report.Base.ReportHeaderBase
    {
        ...
        public void BindReceiptPaymentsSource()
        {
            ...
            /*DataTable dtReceipt = receiptLedgers.ReportData;
            DataTable dtPayment = paymentLedgers.ReportData;
            DataTable dtNullifiedLedgers = new DataTable();
            dtNullifiedLedgers.Columns.Add("LEDGER_ID", typeof(System.UInt32));

            var resultOpening = from drLedgerName in dtReceipt.AsEnumerable()
                                join drOPBalance in dtPayment.AsEnumerable() on drLedgerName.Field<UInt32>("LEDGER_ID") equals drOPBalance.Field<UInt32>("LEDGER_ID")
                                where (drLedgerName.Field<decimal>("RECEIPTAMT") - drOPBalance.Field<decimal>("PAYMENTAMT")) == 0
                                select dtNullifiedLedgers.LoadDataRow(new object[]
                                        {
                                            drLedgerName.Field<UInt32>("LEDGER_ID"),
                                        }, false);

            dtNullifiedLedgers = resultOpening.CopyToDataTable();
            receiptLedgers.ReportData.DefaultView.RowFilter = "LEDGER_ID <> 1337";
            paymentLedgers.ReportData.DefaultView.RowFilter = "LEDGER_ID <> 1337";*/

            //GenerateFinalData(receiptLedgers.ReportData, paymentLedgers.ReportData);
