...
namespace Bosco.Report.ReportObject
{
    public partial class JournaInvoice : Bosco.Report.Base.ReportHeaderBase
    {
        ...
        public JournaInvoice()
        {
            ...
            xrTableCell3.BeforePrint += new System.Drawing.Printing.PrintEventHandler(this.xrcellPeriodFrom_BeforePrint); // #DineshCNK26-06-2026 - Show Academic Year as date range.
            // BindReport();
        }
        ...
        private void BindReport()
        {
            if (!string.IsNullOrEmpty(ReportProperties.PrintCashBankVoucherId))
            {
                ...
                if (resultArgs.DataSource != null && resultArgs.DataSource.Table.Rows.Count > 0)
                {
                    ...
                    if (dtDataSource.Columns.Contains("CURRENCY_SYMBOL"))
                    {
                        xrcellSumTotalCurrencySymbol.Text = dtDataSource.Rows[0]["CURRENCY_SYMBOL"].ToString();
                    }
        ...
        private void xrcellPeriodFrom_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            // #DineshCNK26-06-2026 - Show Academic Year period as "02/06/2026 - 10/06/2026".
            XRTableCell xrcell = sender as XRTableCell;
            if (xrcell == null)
            {
                return;
            ...
            string periodRange = GetPrintablePeriodDateRange();
            xrcell.Text = periodRange;
            xrTableRow1.Visible = !string.IsNullOrEmpty(periodRange);
            xrTableRow3.Visible = !string.IsNullOrEmpty(periodRange);
        }

        private string GetPrintablePeriodDateRange()
        {
            ...
            string periodFrom = GetPrintablePeriodDate(GetCurrentColumnValue(this.reportSetting1.GST_MASTER_INVOICE.PERIOD_FROMColumn.ColumnName));
            string periodTo = GetPrintablePeriodDate(GetCurrentColumnValue(this.reportSetting1.GST_MASTER_INVOICE.PERIOD_TOColumn.ColumnName));
        ...
        private string GetPrintablePeriodDate(object periodDate)
        {
            if (periodDate == null || periodDate == DBNull.Value)
            {
                return string.Empty;
            }

            string dateText = periodDate.ToString().Trim();
            if (string.IsNullOrEmpty(dateText) || dateText == "0000-00-00" || dateText.StartsWith("0000-00-00 ") ||
                dateText == "0001-01-01" || dateText.StartsWith("0001-01-01 ") || dateText == "01/01/0001")
            {
                return string.Empty;
            }

            DateTime periodDateValue;
            string[] dateFormats = { "dd/MM/yyyy", "d/M/yyyy", "yyyy-MM-dd", "yyyy-MM-dd HH:mm:ss", "MM/dd/yyyy", "M/d/yyyy" };
            if (DateTime.TryParseExact(dateText, dateFormats, CultureInfo.InvariantCulture, DateTimeStyles.None, out periodDateValue) ||
                DateTime.TryParse(dateText, out periodDateValue))
            {
                return (periodDateValue <= DateTime.MinValue) ? string.Empty : periodDateValue.ToString("dd/MM/yyyy");
            }
