...
namespace Bosco.Report.ReportObject
{
    public partial class MonthlyAbstractPayments : Bosco.Report.Base.ReportHeaderBase
    {
        ...
        // BSGESP Custom Ordering - Sort by Ledger Code instead of Ledger Group Order
        private const string BsgEspCodeRankColumn = "BSGESP_CODE_RANK";
        ...
        public void BindPaymentSource()
        {
            ...
            if (resultArgs.Success)
            {
                ...
                if (dvPayment != null)
                {
                    ...
                    {
                        ApplyBsgEspLedgerCodeRank(dvPayment);
                    }
                }
        ...
        private void SetReportSetting(DataView dvPayment, AccountBalance accountBalance)
        {
            ...
            tcCapAmountBudget.Text = MessageRender.GetMessage(MessageCatalog.Criteria.BUDEGT_TEXT) + UtilityMember.DateSet.ToDate(settingProperty.YearFrom, false).Year + "-" + UtilityMember.DateSet.ToDate(settingProperty.YearTo, false).ToString("yy");
        ...
        // BSGESP Custom Ordering - Sort by Ledger Code instead of Ledger Group Order
        // Builds a numeric rank (0,1,2,...) for every row based on the ascending Ledger Code order.
        // The Parent Group / Ledger Group bands are then ordered by the MIN rank they contain, so the whole
        // report follows Ledger Code order while keeping the Group -> Sub Group -> Ledger hierarchy intact.
        private void ApplyBsgEspLedgerCodeRank(DataView dvPayment)
        {
            if (dvPayment == null || dvPayment.Table == null)
            {
                return;
            }

            DataTable table = dvPayment.Table;
            if (!table.Columns.Contains(BsgEspCodeRankColumn))
            {
                table.Columns.Add(BsgEspCodeRankColumn, typeof(int));
            }

            string ledgerCodeColumn = reportSetting1.MonthlyAbstract.LEDGER_CODEColumn.ColumnName;
            DataRow[] sortedRows = table.Select(string.Empty, ledgerCodeColumn + " ASC");
            for (int index = 0; index < sortedRows.Length; index++)
            {
                sortedRows[index][BsgEspCodeRankColumn] = index;
            }
        }
        ...
        private void SortByLedgerorGroup()
        {
            ...
            // BSGESP Custom Ordering - Sort by Ledger Code instead of Ledger Group Order
            // Only when order is set to Ledger Code and both "Show by Ledger Group" and "Show Ledger Code" are enabled.
            // The group sorting summary is arithmetic, so we order the Parent Group / Ledger Group bands by the
            // MINIMUM numeric Ledger-Code rank (BSGESP_CODE_RANK) they contain. That rank is built in BindPaymentSource
            // from the ascending Ledger Code order, so a group/sub-group whose earliest ledger code is "A01" appears
            // before one whose earliest code is "C01". The ledger (detail) order keeps the existing Ledger Code
            // workflow below. Other provinces / other options are unaffected.
            if (this.AppSetting.IS_BSG_ESP
                && this.ReportProperties.SortByLedger == 0
                && this.ReportProperties.ShowByLedgerGroup == 1
                && this.ReportProperties.ShowLedgerCode == 1)
            {
                if (grpParentGroup.Visible)
                {
                    grpParentGroup.SortingSummary.Enabled = true;
                    grpParentGroup.SortingSummary.FieldName = BsgEspCodeRankColumn;
                    grpParentGroup.SortingSummary.Function = SortingSummaryFunction.Min;
                    grpParentGroup.SortingSummary.SortOrder = XRColumnSortOrder.Ascending;
                    grpParentGroup.SortingSummary.IgnoreNullValues = true;
                }

                if (grpLedgerGroup.Visible)
                {
                    grpLedgerGroup.SortingSummary.Enabled = true;
                    grpLedgerGroup.SortingSummary.FieldName = BsgEspCodeRankColumn;
                    grpLedgerGroup.SortingSummary.Function = SortingSummaryFunction.Min;
                    grpLedgerGroup.SortingSummary.SortOrder = XRColumnSortOrder.Ascending;
                    grpLedgerGroup.SortingSummary.IgnoreNullValues = true;
                }
            }
        ...
        {
            //if (xrSubBalance.HeightF > 0)
            //{
            //    xrtblGrandTotal.TopF = xrSubBalance.TopF + xrSubBalance.HeightF + 10;
            //}

            //xrSubSignFooter.LeftF = xrtblGrandTotal.LeftF;
            //xrSubSignFooter.TopF = xrtblGrandTotal.TopF + xrtblGrandTotal.HeightF + 10;
        }

        private void xrSubSignFooter_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            //xrSubSignFooter.LeftF = xrtblGrandTotal.LeftF;
        }
