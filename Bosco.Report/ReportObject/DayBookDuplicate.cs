...
{
    public partial class DayBookDuplicate : Bosco.Report.Base.ReportHeaderBase
    {
        #region Constructor
        public DayBookDuplicate()
        {
            InitializeComponent();
        ...
        {
            //On 22/02/2021, whether to show all against ledger names or show only top of the against ledger
            // This property is controlled by report criteria selection (0 or 1)
                    ...
                    if (ReportProperty.Current.ReportFlag == 0)
                    {
                        ...
                        if (resultArgs.Success)
                        {
                            ...
                            if (dvDayBook != null && dvDayBook.Count != 0)
                            {
                                ...
                                //On 22/02/2021, Apply voucher grouping for ShowAllAgainstLedgers = 1
                                // Note: Voucher grouping is handled in BeforePrint events, not in data modification
        ...
        private void SetReportBorder()
        {
            ...
            {
                xrCapReceipt.Text = "Receipt (" + ReportProperties.CurrencyCountrySymbol + ")";
                xrCapPerce.Text = "Province % (" + ReportProperties.CurrencyCountrySymbol + ")";
            }
            else
            {
                this.SetCurrencyFormat(xrCapPerce.Text, xrCapPerce);
                this.SetCurrencyFormat(xrCapReceipt.Text, xrCapReceipt);
            }
        }

        /// <summary>
        /// Override AlignContentTable to handle voucher grouping when ShowAllAgainstLedgers = 1
        /// </summary>
        public override XRTable AlignContentTable(XRTable table)
        {
            // Always use base class for default borders, then override in BeforePrint events
            return base.AlignContentTable(table);
        }
        ...
        private ResultArgs GetReportSource()
        {
            try
            {
                string DayBookDup = this.GetBankReportSQL(SQL.ReportSQLCommand.BankReport.DayBookDuplicate);
                using (DataManager dataManager = new DataManager())
                {
                    ...
                    dataManager.Parameters.Add(this.ReportParameters.SHOW_PROVINCE_PERCENTAGEColumn, this.ReportProperties.ShowProvincePercentage);//added By Dinesh 
                    ...
                    resultArgs = dataManager.FetchData(DAO.Data.DataSource.DataView,DayBookDup);
                }
            }
            catch (Exception ex)
            {
