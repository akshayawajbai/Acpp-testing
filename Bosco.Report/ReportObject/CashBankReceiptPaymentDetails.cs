...
namespace Bosco.Report.ReportObject
{
    public partial class CashBankReceiptPaymentDetails : Bosco.Report.Base.ReportBase
    {
        ...
        public float ColumnThroughWidth
        {
            ...
            set
            {
                xrlblThrough.WidthF = xrlblThroughSpace1.WidthF = xrlblThroughSpace2.WidthF = xrthroughspace3.WidthF = value;
            }
        }
        ...
        public ResultArgs BindCashBankReceiptsDetails()
        {
            ...
            if (!string.IsNullOrEmpty(CashBankReceipts))
            {
                using (DataManager dataManager = new DataManager())
                {
                    ...
                    xrcellChequeBene.Font = (this.ReportProperties.VoucherPrintValueBold == "1") ? new Font(xrcellChequeBene.Font.FontFamily, xrcellChequeBene.Font.Size, FontStyle.Bold)
                   : new Font(xrcellChequeBene.Font.FontFamily, xrAmount.Font.Size, FontStyle.Regular);
        ...
        private void xrcellChequeBene_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            string chequeBeneName = GetCurrentColumnValue("BENEFICIARY_NAME") != null ? GetCurrentColumnValue("BENEFICIARY_NAME").ToString() : string.Empty;
            string chequeBeneAcc = GetCurrentColumnValue("BENEFICIARY_ACC_NO") != null ? GetCurrentColumnValue("BENEFICIARY_ACC_NO").ToString() : string.Empty;
            string chequeBeneBankBranch = GetCurrentColumnValue("BENEFICIARY_BANK_BRANCH") != null ? GetCurrentColumnValue("BENEFICIARY_BANK_BRANCH").ToString() : string.Empty;

            if (!string.IsNullOrEmpty(chequeBeneName) || !string.IsNullOrEmpty(chequeBeneAcc) || !string.IsNullOrEmpty(chequeBeneBankBranch))
            {
                xrcellChequeBene.Text = "Beneficiary Details.: " + chequeBeneName + " , " + chequeBeneAcc + ", " + chequeBeneBankBranch;
            }
            else
            {
                e.Cancel = true;
                xrRowBeneficiaryDetails.Visible = false;
            }

        }

        private void xrRowBeneficiaryDetails_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            string chequeBeneName = GetCurrentColumnValue("BENEFICIARY_NAME") != null ? GetCurrentColumnValue("BENEFICIARY_NAME").ToString() : string.Empty;
            string chequeBeneAcc = GetCurrentColumnValue("BENEFICIARY_ACC_NO") != null ? GetCurrentColumnValue("BENEFICIARY_ACC_NO").ToString() : string.Empty;
            string chequeBeneBankBranch = GetCurrentColumnValue("BENEFICIARY_BANK_BRANCH") != null ? GetCurrentColumnValue("BENEFICIARY_BANK_BRANCH").ToString() : string.Empty;
