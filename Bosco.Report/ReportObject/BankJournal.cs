    ...
    public partial class BankJournal : ReportHeaderBase
    {
        ...
        private DataTable GetReportSource()
        {
            ...
            if (this.ReportProperties.ShowCCDetails == 1)
            {
                ...
                AssignCCDetailBankJournal();
            }
            else
            {
                // user unselected -> nuke any previous CC data
                if (dtCCDetails != null) dtCCDetails.Clear();
                xrSubreportCCDetails.ReportSource = null;
                xrSubreportCCDetails.HeightF = 0f;
            }
        ...
        private void xrtblSource_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            ...
            {
                xrDate.Borders = BorderSide.Top | BorderSide.Left | BorderSide.Right;
                xrVoucherNo.Borders = BorderSide.Top | BorderSide.Left | BorderSide.Right;
                xrLedgerCode.Borders = BorderSide.Top | BorderSide.Left | BorderSide.Right;
                xrLedger.Borders = BorderSide.Top | BorderSide.Left | BorderSide.Right;
                xrChequeNo.Borders = BorderSide.Top | BorderSide.Left | BorderSide.Right;
                xrReceipt.Borders = BorderSide.Top | BorderSide.Left | BorderSide.Right;
                xrPayments.Borders = BorderSide.Top | BorderSide.Left | BorderSide.Right;
            }
            else
            {
                ...
                xrLedgerCode.Borders = BorderSide.Top | BorderSide.Right | BorderSide.Bottom;
                xrLedger.Borders = BorderSide.Top | BorderSide.Right | BorderSide.Bottom;
                xrChequeNo.Borders = BorderSide.Top | BorderSide.Right | BorderSide.Bottom;
            ...
            //    {
            //        XRTable table = row.Table;

            //        if (table.Rows.IndexOf(row) == 0)   
            //        {
            //            cell.Borders = BorderSide.Top | BorderSide.Left | BorderSide.Right;
            //        }
            //        else
            //        {
        ...
        private void ShowCCDetails()
        {
            ...
            if (this.ReportProperties.ShowCCDetails == 1)
            {
                if (this.ReportParameters.VOUCHER_IDColumn != null &&
                    this.ReportParameters.LEDGER_IDColumn != null &&   // ✅ Ledger check
                    dtCCDetails != null &&
                    dtCCDetails.Rows.Count > 0 &&
                    GetCurrentColumnValue(this.ReportParameters.VOUCHER_IDColumn.ColumnName) != null &&
                    GetCurrentColumnValue(this.ReportParameters.LEDGER_IDColumn.ColumnName) != null)
                {
                    // VoucherId
                    object valVoucher = GetCurrentColumnValue(this.ReportParameters.VOUCHER_IDColumn.ColumnName);
                    Int32 VoucherId = 0;
                    if (valVoucher != null && valVoucher != DBNull.Value)
                    {
                        VoucherId = UtilityMember.NumberSet.ToInteger(valVoucher.ToString());
                    }

                    // LedgerId
                    object valLedger = GetCurrentColumnValue(this.ReportParameters.LEDGER_IDColumn.ColumnName);
                    Int32 LedgerId = 0;
                    if (valLedger != null && valLedger != DBNull.Value)
                    {
                        LedgerId = UtilityMember.NumberSet.ToInteger(valLedger.ToString());
                    }

                    if (xrSubreportCCDetails.ReportSource == null)
                    {
                        xrSubreportCCDetails.ReportSource = new UcCCDetailCBank();
                    }
                    ...
                    // ✅ Filter both VoucherId + LedgerId
                    string colVoucher = this.ReportParameters.VOUCHER_IDColumn.ColumnName;
                    string colLedger = this.ReportParameters.LEDGER_IDColumn.ColumnName;

                    dtCCDetails.DefaultView.RowFilter =
                        colVoucher + "=" + VoucherId + " AND " + colLedger + "=" + LedgerId;

                    DataTable dtCc = dtCCDetails.DefaultView.ToTable();

                    ccDetails.BindCCDetails(dtCc, true, true, false, true);
                    ccDetails.DateWidth = 0;
                    ccDetails.CCTableWidth = xrCapLedgerName.WidthF + xrCapChequeNo.WidthF + xrCapReceipts.WidthF + xrCapPayments.WidthF;
                    ccDetails.CCNameWidth = xrCapLedgerName.WidthF + xrCapChequeNo.WidthF;
                    ccDetails.CCDebitWidth = xrCapReceipts.WidthF;
                    ...
                    // Hide if only Receipts/Payments mode selected
                    if ((this.ReportProperties.ShowOnlyReceipts == 1 && this.ReportProperties.ShowOnlyPayments == 0) ||
                        (this.ReportProperties.ShowOnlyPayments == 1 && this.ReportProperties.ShowOnlyReceipts == 0))
                    {
                        xrSubreportCCDetails.Visible = false;
        ...
        private void AssignCCDetailBankJournal()
        {
            ResultArgs resultArges = null;
            try
            {
                string sqlDetails = this.GetReportCostCentre(SQL.ReportSQLCommand.CostCentre.CCDeatilCashBankJournal);
                string dataProgress = this.GetProgressiveDate(this.ReportProperties.DateFrom);
                ...
                using (DataManager dataManger = new DataManager())
                {
                    ...
                    dataManger.Parameters.Add(this.ReportParameters.DATE_PROGRESS_FROMColumn, dataProgress);
                ...
                {
                    dtCCDetails = resultArges.DataSource.Table;
            ...
            catch (Exception ee)
            {
                ...
                MessageRender.ShowMessage("Error Fetching Bank Journal  Details:" + ee.Message, true);
            }
        }
        ...
        private void ProperBorderForLedgerRow(bool ccFound)
        {
            if (ccFound)
            {
                xrDate.Borders = BorderSide.Top | BorderSide.Left | BorderSide.Right | BorderSide.Bottom;  // remove Top
                xrVoucherNo.Borders = BorderSide.Top | BorderSide.Right | BorderSide.Bottom;
                xrLedgerCode.Borders = BorderSide.Top | BorderSide.Right | BorderSide.Bottom;
                xrLedger.Borders = BorderSide.Top | BorderSide.Right | BorderSide.Bottom;
                xrChequeNo.Borders = BorderSide.Top | BorderSide.Right | BorderSide.Bottom;
            ...
            else
            {
                ...
                xrLedgerCode.Borders = BorderSide.Right | BorderSide.Bottom;
                xrLedger.Borders = BorderSide.Right | BorderSide.Bottom;
                xrChequeNo.Borders = BorderSide.Right | BorderSide.Bottom;
        ...
        private void xrSubreportCCDetails_AfterPrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
