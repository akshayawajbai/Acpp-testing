...
namespace Bosco.Report.ReportObject
{
    public partial class CashBankBookSinglecol : ReportHeaderBase
    {
        ...
        private DataTable dtCCReceiptDetails = new DataTable();//added By Dinesh  06-01-2025
        private DataTable dtCCPaymentDetails = new DataTable();//added By Dinesh  06-01-2025
        ...
        private ResultArgs GetReportSource()
        {
            try
            {
                ...
                //Dinesh C 06-01-2025
                xrSubReceipt.Visible = false;
                if (this.ReportProperties.ShowCCDetails == 1)
                {
                    //xrSubPayment.Visible = true;
                    xrSubReceipt.Visible = true;
                    AssignCCDetailCashBankBook();

                }
                xrSubPayment.Visible = false;
                if (this.ReportProperties.ShowCCDetails == 1)
                {
                    xrSubPayment.Visible = true;
                    AsseignCCPayDetails();
                }
                if (this.ReportProperties.ShowCCDetails == 0)
                {
                    xrSubPayment.HeightF = 0;
                    xrSubReceipt.HeightF = 0;
                }
            }
            catch (Exception ee)
            {
        ...
        private void xrtblBindSource_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            ...
            if (this.ReportProperties.ShowCCDetails == 1)
            {
                ...
                xrReceiptNo.Borders = BorderSide.Top | BorderSide.Left | BorderSide.Right | BorderSide.Bottom;
                xrLedgerCode.Borders = BorderSide.Top | BorderSide.Left | BorderSide.Right | BorderSide.Bottom;
                xrReceipt.Borders = BorderSide.Top | BorderSide.Left | BorderSide.Right | BorderSide.Bottom;
                xrRec_Cash.Borders = BorderSide.Top | BorderSide.Left | BorderSide.Right | BorderSide.Bottom;
                xrRec_Bank.Borders = BorderSide.Top | BorderSide.Left | BorderSide.Right | BorderSide.Bottom;
                xrPay_VoucherNo.Borders = BorderSide.Top | BorderSide.Left | BorderSide.Right | BorderSide.Bottom;
                xrPay_LedgerCode.Borders = BorderSide.Top | BorderSide.Left | BorderSide.Right | BorderSide.Bottom;
                xrPayments.Borders = BorderSide.Top | BorderSide.Left | BorderSide.Right | BorderSide.Bottom;
                xrPaymentCash.Borders = BorderSide.Top | BorderSide.Left | BorderSide.Right | BorderSide.Bottom;
                xrPaymentBank.Borders = BorderSide.Top | BorderSide.Left | BorderSide.Right | BorderSide.Bottom;
            }
        }
        ...
        /// <summary>
        /// Receipt ,Cost Center details added to the Cash and Bank Book report by Dinesh C on 20-01-2026
        /// </summary>
        private void ShowReceCCDetails()
        {
            xrSubReceipt.Visible = false;
            if (this.ReportProperties.ShowCCDetails == 1)
            {
                if (dtCCReceiptDetails.Rows.Count > 0)
                {
                    ...
                    UcCCDetail ccDetails = xrSubReceipt.ReportSource as UcCCDetail;
                    if (ccDetails == null)
                    {
                        ccDetails = new UcCCDetail();
                        xrSubReceipt.ReportSource = ccDetails;
                    }
                    dtCCReceiptDetails.DefaultView.RowFilter = string.Empty;
                    dtCCReceiptDetails.DefaultView.RowFilter = reportSetting1.CashBankBook.VOUCHER_IDColumn.ColumnName + " = " + VoucherId;
                    DataTable dtCC = dtCCReceiptDetails.DefaultView.ToTable();
                    if (dtCC.Rows.Count == 0)
                    {
                        xrSubReceipt.Visible = false;
                        dtCCPaymentDetails.DefaultView.RowFilter = string.Empty;
                        return;
                    }
                    ccDetails.CCNameAlignment = DevExpress.XtraPrinting.TextAlignment.MiddleRight;
                    ccDetails.BindCCDetails(dtCC, true, true, true, true);
                    ccDetails.DateWidth = 0;
                    xrSubReceipt.LeftF = xrCapDate.LeftF;
                    ccDetails.CCCreditCaption = "Credit";
                    ccDetails.CCDebitCaption = "Debit";
                    ccDetails.CCTableWidth = xrCapDate.WidthF + xrCapRNo.WidthF + xrCapReceiptcode.WidthF + xrCapReceipt.WidthF + xrCapRecBank.WidthF + xrCapRecCash.WidthF + 2;
                    ccDetails.CCNameWidth = xrCapDate.WidthF + xrCapRNo.WidthF + xrCapReceiptcode.WidthF + xrCapReceipt.WidthF;
                    ccDetails.CCCreditWidth = xrCapRecCash.WidthF;
                    ccDetails.HideReportHeaderFooter();
                    ccDetails.CCDebitWidth = xrCapRecBank.WidthF;
                    dtCCReceiptDetails.DefaultView.RowFilter = string.Empty;
                    xrSubReceipt.Visible = (dtCC.Rows.Count > 0);
                }
                else
                {
                    xrSubReceipt.Visible = false;
                }
                if (!xrSubReceipt.Visible)
                {
                    if (Detail.Controls.Contains(xrSubReceipt))
                    {
                        Detail.Controls.Remove(xrSubReceipt);
                    }
                }
                else
                {
                    Detail.Controls.Add(xrSubReceipt);
                }
            }
        }

        private void AssignCCDetailCashBankBook()
        {
            ResultArgs resultArges = null;

            try
            {
                string sqlDetails = this.GetReportCostCentre(SQL.ReportSQLCommand.CostCentre.CCDetailCashBankBook);
                string dateProgerss = this.GetProgressiveDate(this.ReportProperties.DateFrom);
                using (DataManager dataManger = new DataManager())
                {
                    dataManger.Parameters.Add(this.ReportParameters.DATE_FROMColumn, this.ReportProperties.DateFrom);
                    dataManger.Parameters.Add(this.ReportParameters.DATE_PROGRESS_FROMColumn, dateProgerss);
                    dataManger.Parameters.Add(this.ReportParameters.DATE_TOColumn, this.ReportProperties.DateTo);
                    dataManger.Parameters.Add(this.ReportParameters.VOUCHER_TYPEColumn, TransType.RC.ToString());
                    dataManger.Parameters.Add(this.ReportParameters.PROJECT_IDColumn, this.ReportProperties.Project);
                    dataManger.Parameters.Add(this.ReportParameters.TRANS_MODEColumn, "CR,DR");
                    dataManger.Parameters.Add(this.ReportParameters.YEAR_FROMColumn, settingProperty.YearFrom);
                    dataManger.Parameters.Add(this.ReportParameters.YEAR_TOColumn, settingProperty.YearTo);

                    if (this.AppSetting.AllowMultiCurrency == 1 && this.ReportProperties.CurrencyCountryId > 0)
                    {
                        dataManger.Parameters.Add(this.ReportParameters.CURRENCY_COUNTRY_IDColumn, this.ReportProperties.CurrencyCountryId);
                    }
                    else
                    {
                        dataManger.Parameters.Add(this.ReportParameters.CURRENCY_COUNTRY_IDColumn, "0");
                    }
                    dataManger.DataCommandArgs.IsDirectReplaceParameter = true;

                    resultArges = dataManger.FetchData(DAO.Data.DataSource.DataTable, sqlDetails);
                }
                if (resultArges.Success && resultArges.DataSource.Table != null)
                {
                    dtCCReceiptDetails = resultArges.DataSource.Table;
                }
        ...
        /// Payment
        /// </summary>
        private void ShowPayCCDetails()
        {
            xrSubPayment.Visible = false;
            if (this.ReportProperties.ShowCCDetails == 1)
            {
                if (dtCCPaymentDetails.Rows.Count > 0)
                {
                    object receObj = GetCurrentColumnValue("PAY_VOUCHER_ID");
                    if (receObj == null)
                    {
                        xrSubPayment.Visible = false;
                        return;
                    }
                    int receiptVoucherId = UtilityMember.NumberSet.ToInteger(receObj.ToString());
                    UcCCDetail ccDetails = xrSubPayment.ReportSource as UcCCDetail;
                    if (ccDetails == null)
                    {
                        ccDetails = new UcCCDetail();
                        xrSubPayment.ReportSource = ccDetails;
                    }
                    dtCCPaymentDetails.DefaultView.RowFilter = string.Empty;
                    dtCCPaymentDetails.DefaultView.RowFilter = reportSetting1.CashBankBook.VOUCHER_IDColumn.ColumnName + " = " + receiptVoucherId;

                    DataTable dtCCPay = dtCCPaymentDetails.DefaultView.ToTable();
                    if (dtCCPay.Rows.Count == 0)
                    {
                        xrSubPayment.Visible = false;
                        dtCCPaymentDetails.DefaultView.RowFilter = string.Empty;
                        return;
                    }
                    ccDetails.BindCCDetails(dtCCPay, true, true, true, true);
                    ccDetails.DateWidth = 0;
                    xrSubPayment.LeftF = xrCapPaymentVNo.LeftF;
                    ccDetails.CCCreditCaption = "Credit";
                    ccDetails.CCDebitCaption = "Debit";
                    ccDetails.CCTableWidth = xrCapPaymentVNo.WidthF + xrCapPaymentCode.WidthF + xrCapPayment.WidthF + xrCapPayCash.WidthF + xrCapPayBank.WidthF + 80;
                    ccDetails.CCNameWidth = xrCapPaymentVNo.WidthF + xrCapPaymentCode.WidthF + xrCapPayment.WidthF;
                    ccDetails.CCDebitWidth = xrCapPayCash.WidthF;
                    ccDetails.CCCreditWidth = xrCapPayBank.WidthF;
                    ccDetails.HideReportHeaderFooter();
                    dtCCPaymentDetails.DefaultView.RowFilter = string.Empty;
                    xrSubPayment.Visible = (dtCCPay.Rows.Count > 0);
                }
                else
                {
                    xrSubPayment.Visible = false;
                }
                if (!xrSubPayment.Visible)
                {
                    if (Detail.Controls.Contains(xrSubPayment))
                    {
                        Detail.Controls.Remove(xrSubPayment);
                    }
                }
                else
                {
                    Detail.Controls.Add(xrSubPayment);
                }
            }
        }

        private void AsseignCCPayDetails()
        {
            ResultArgs resultAgres = null;
            try
            {
                string sqlDetails = this.GetReportCostCentre(SQL.ReportSQLCommand.CostCentre.CCDetailCashandBankBookPay);
                string dateProgerss = this.GetProgressiveDate(this.ReportProperties.DateFrom);
                using (DataManager dataManger = new DataManager())
                {
                    dataManger.Parameters.Add(this.ReportParameters.DATE_FROMColumn, this.ReportProperties.DateFrom);
                    dataManger.Parameters.Add(this.ReportParameters.DATE_PROGRESS_FROMColumn, dateProgerss);
                    dataManger.Parameters.Add(this.ReportParameters.DATE_TOColumn, this.ReportProperties.DateTo);
                    dataManger.Parameters.Add(this.ReportParameters.VOUCHER_TYPEColumn, TransType.PY.ToString());
                    dataManger.Parameters.Add(this.ReportParameters.PROJECT_IDColumn, this.ReportProperties.Project);
                    dataManger.Parameters.Add(this.ReportParameters.TRANS_MODEColumn, "CR,DR");
                    dataManger.Parameters.Add(this.ReportParameters.YEAR_FROMColumn, settingProperty.YearFrom);
                    dataManger.Parameters.Add(this.ReportParameters.YEAR_TOColumn, settingProperty.YearTo);
                    ...
                    resultAgres = dataManger.FetchData(DAO.Data.DataSource.DataTable, sqlDetails);
                }
                if (resultAgres.Success && resultAgres.DataSource.Table != null)
                {
                    dtCCPaymentDetails = resultAgres.DataSource.Table;
                }
