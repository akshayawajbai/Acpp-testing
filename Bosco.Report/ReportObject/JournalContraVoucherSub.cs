    ...
    public partial class JournalContraVoucherSub : Bosco.Report.Base.ReportHeaderBase
    {
        ...
        public ResultArgs GetJournal()
        {
            ...
            using (DataManager dataManager = new DataManager())
            {
                ...
                dataManager.Parameters.Add(this.appSchema.Project.DISPLAY_INPUT_LANColumn, SettingProperty.Current.DisplayInputLanguage == "1" ? 1 : 0);//Dinesh Contra
                dataManager.DataCommandArgs.IsDirectReplaceParameter = true;
        ...
        private void xrDebitAmount_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            ...
            if (DebitAmount != 0)
            {
                ...
                xrDebitAmount.Text = FormatAmountWithCurrencyPrefix(DebitAmount, currencySymbol);
                e.Cancel = false;
            }
            else
            {
        ...
        private void xrCreditAmount_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            ...
            if (CreditAmount != 0)
            {
                ...
                xrCreditAmount.Text = FormatAmountWithCurrencyPrefix(CreditAmount, currencySymbol);
                e.Cancel = false;
            }
            else
            {
        ...
        private string FormatAmountWithCurrencyPrefix(double amount, string currencySymbol)
        {
            if (settingProperty.AllowMultiCurrency == 1 &&
                !string.IsNullOrEmpty(currencySymbol))
            {
                return currencySymbol + " " + ReportProperty.Current.NumberSet.ToNumber(amount);
            }
            else
            {
                // Show amount without currency symbol (default behavior)
                return ReportProperty.Current.NumberSet.ToNumber(amount);
            }
        }
