    ...
    public partial class BudgetAnnualRealizationLedgers : Bosco.Report.Base.ReportHeaderBase
    {
            ...
            try
            {
                ...
                if (dtRptBudgetSource != null)
                {
                    ...
                    if (this.ReportProperties.IncludeAllLedger == 0)
                    {
                        ...
                        {
                            filter = "(" +
                                 this.reportSetting1.BUDGETVARIANCE.APPROVED_AMOUNTColumn.ColumnName + " > 0 OR " +
                                 this.reportSetting1.BUDGETVARIANCE.ACTUAL_AMOUNTColumn.ColumnName + " <> 0 OR " +
                                 //this.reportSetting1.Receipts.GROUP_IDColumn.ColumnName + " = " + (int)FixedLedgerGroup.Cash + " OR " +  Commended By Diensh 13-03-2026
                                 this.reportSetting1.Receipts.GROUP_IDColumn.ColumnName + " = " + (int)FixedLedgerGroup.BankAccounts + " OR " +
                                 this.reportSetting1.Receipts.GROUP_IDColumn.ColumnName + " = " + (int)FixedLedgerGroup.FixedDeposit + ")";
                            //Remove Cash Ledger  //Added By Diensh 13-03-2026 
                            filter += " AND " + this.reportSetting1.Receipts.GROUP_IDColumn.ColumnName + " NOT IN (12,13)";
                        }
                        else
                        {
                            filter = "(" +
        ...
        private void SetReportBorder()
        {
            ...
            if (this.DataSource != null)
            {
                ...
                grpFooterBudgetNature.Visible = records; //grpFooterLedgerGroup.Visible = records;
                if (this.IsSdbRmg)
                {
                    grpFooterLedgerGroup.Visible = false; //Dinesh 
                }
                else
                {
                    grpFooterLedgerGroup.Visible = records;
                }
