...
{
    public partial class CostCentreLedger : Bosco.Report.Base.ReportHeaderBase
    {
        #region Constructor
        ...
        string TempCostCentre = string.Empty;
        double CCDebit = 0.0;
        double CCCredit = 0.0;
        int count = 0;
        ...
        {
            grpHeadrLedgerName.Visible = false;
            grpHeaderCC.Visible = grpFooterCC.Visible = grpHeaderCostcentreCategoryName.Visible = Detail.Visible = ReportFooter.Visible = grpFooterLedger.Visible = false;

            if ((string.IsNullOrEmpty(this.ReportProperties.DateFrom) ||
                string.IsNullOrEmpty(this.ReportProperties.DateTo) ||
                this.ReportProperties.Project == "0" || this.ReportProperties.Ledger == "0") || this.ReportProperties.CostCentre == "0")
            {
                SetReportTitle();
            ...
            {
                grpHeaderCostcentreCategoryName.GroupFields[0].FieldName = string.Empty;
            }
            else
            {
                grpHeaderCostcentreCategoryName.GroupFields[0].FieldName = "COST_CENTRE_CATEGORY_NAME";
            }

            grpHeadrLedgerName.GroupFields[0].FieldName = "LEDGER_NAME";
        ...
        private void BindReportProperty()
        {
            ...
            {
                grpHeaderCC.Visible = grpFooterCC.Visible = grpFooterLedger.Visible = grpHeaderCostcentreCategoryName.Visible = Detail.Visible = true;
                grpHeadrLedgerName.Visible = true;
            }
            else
            {
                grpHeaderCostcentreCategoryName.Visible = false;
                grpHeaderCC.Visible = grpFooterCC.Visible = grpFooterLedger.Visible = grpHeadrLedgerName.Visible = Detail.Visible = true;
            }
            ...
            grpHeadrLedgerName.Level = 1;
            grpHeaderCC.Level = 0;
            if (this.ReportProperties.ShowByCostCentre == 1)
            {
                grpHeadrLedgerName.Level = 0;
                grpHeaderCC.Level = 1;
            }
            ...
            this.SetCurrencyFormat(MessageCatalog.ReportCommonTitle.DEBIT.ToString(), xrCosCapDebit);
            this.SetCurrencyFormat(MessageCatalog.ReportCommonTitle.CREDIT.ToString(), xrCosCapCredit);
            ...
            if (resultArgs.Success)
            {
                ...
                if (dtReport != null)
                {
                    ...
                    {
                        grpHeaderCC.Visible = grpFooterCC.Visible = grpFooterLedger.Visible = grpHeaderCostcentreCategoryName.Visible = Detail.Visible = ReportFooter.Visible = false;

                        // 09/07/2025, *Chinna
                        //  grpHeadrLedgerName.Visible = grpFooterLedger.Visible = false;
                        grpHeadrLedgerName.Visible = false;
                    }
                }
                SetReportBorder();
        ...
        private void SetReportBorder()
        {
            ...
            xrLedgerName.BackColor = xrCCName.BackColor = Color.Transparent;
            if (ReportProperties.ShowByCostCentre == 1)
            {
                ...
                xrTblLedger = AlignCostCentreTable(xrTblLedger);
            }
            else
            {
                xrLedgerName.BackColor = xrCosCapParticulars.BackColor;
                xrTblLedger = AlignCostCentreTable(xrTblLedger);
        ...
        private void AlignLedgerLabel()
        {
            xrTblLedger.Borders = BorderSide.Left | BorderSide.Right | BorderSide.Bottom | BorderSide.Top;
            xrTblLedger.BorderColor = ((int)BorderStyleCell.Regular == this.ReportProperties.ReportBorderStyle) ? System.Drawing.Color.Gainsboro : System.Drawing.Color.Black;
        }
            ...
            foreach (XRTableRow row in table.Rows)
            {
                ...
                foreach (XRTableCell cell in row)
                {
                    ...
                    if (ReportProperties.IncludeNarration != 1 && rowcount == 1)
                    {
                        ...
                        {
                            if (cellcount == 1)
                                cell.Borders = BorderSide.Right | BorderSide.Left | BorderSide.Bottom;  //BorderSide.Bottom
                            else
        ...
        {
            xrOPBLDr.Text = xrOPBLCr.Text = string.Empty;
        ...
        /// <summary>
        /// On 13/04/2020, to show CC category sums 
        /// 0 - 
        /// 1 - 
        /// 3 - 
        /// </summary>
        /// <param name="CCCategoryId"></param>
        private double GetCCCategorySum(Int32 CCCategoryId, int option)
        {
            double dopeningCr = 0;
            double dopeningDr = 0;
            double dsumCr = 0;
            ...
            if (dtCCOpeningBalanceAlone != null && dtCCOpeningBalanceAlone.Rows.Count > 0)
            {
                string filter = "COST_CENTRECATEGORY_ID=" + CCCategoryId;
                dopeningCr = Math.Abs(UtilityMember.NumberSet.ToDouble(dtCCOpeningBalanceAlone.Compute("SUM(OP_AMOUNT)", filter + " AND OP_AMOUNT < 0").ToString()));
                dopeningDr = UtilityMember.NumberSet.ToDouble(dtCCOpeningBalanceAlone.Compute("SUM(OP_AMOUNT)", filter + " AND OP_AMOUNT > 0").ToString());
            }
            ...
            if (this.DataSource != null)
            {
                ...
                string filter = "COST_CENTRECATEGORY_ID=" + CCCategoryId;
                dsumCr = UtilityMember.NumberSet.ToDouble(dtRptSource.Compute("SUM(CREDIT)", filter).ToString());
                dsumDr = UtilityMember.NumberSet.ToDouble(dtRptSource.Compute("SUM(DEBIT)", filter).ToString());
            }

            if (option == 0) //Debit
            {
                rtn = (dopeningDr + dsumDr);
            }
            else if (option == 1) //Debit
            {
                rtn = (dopeningCr + dsumCr);
            }
            else //For Closing
            {
                rtn = (dopeningDr + dsumDr) - (dopeningCr + dsumCr);
            }

            return rtn;
        }

        private void xrCCTotalBalance_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
        ...
        {
            GroupHeaderBand grpledgerHeader = sender as GroupHeaderBand;
            if (this.GetCurrentColumnValue(this.reportSetting1.Receipts.LEDGER_IDColumn.ColumnName) != null)
            {
                ...
                if (ledgerid > 0)
                {
                    xrTblLedger.TopF = 0; //xrTblLedger.TopF = 5
                    if (LedgerGroupNumber == 0)
                    {
                        xrTblLedger.TopF = 0;
                        grpledgerHeader.HeightF = 25;
                    }
                    else
                    {
                        // grpledgerHeader.HeightF = 52;
                        grpledgerHeader.HeightF = 25;
                    }
                }
        ...
        private void xrCCTotalBalance_SummaryGetResult(object sender, SummaryGetResultEventArgs e)
        {
            ...
            if ((CosDebitTotal - CosCreditTotal) >= 0)
            {
                if (this.AppSetting.EnableCCModeReports == 1)
                {
                    ...
                    // e.Result  = UtilityMember.NumberSet.ToNumber(Math.Abs(calcuvalues)).ToString()) != "0,00") ? "-" + UtilityMember.NumberSet.ToNumber(Math.Abs(calcuvalues)).ToString() : UtilityMember.NumberSet.ToNumber(Math.Abs(calcuvalues)).ToString();
        ...
        private void xrccCategorySumDr_SummaryGetResult(object sender, SummaryGetResultEventArgs e)
        {
            if (this.GetCurrentColumnValue("COST_CENTRECATEGORY_ID") != null)
            {
                Int32 ccCategoryId = UtilityMember.NumberSet.ToInteger(this.GetCurrentColumnValue("COST_CENTRECATEGORY_ID").ToString());
                e.Result = GetCCCategorySum(ccCategoryId, 0);
                e.Handled = true;
            }
        }

        private void xrccCategorySumCr_SummaryGetResult(object sender, SummaryGetResultEventArgs e)
        {
            if (this.GetCurrentColumnValue("COST_CENTRECATEGORY_ID") != null)
            {
                Int32 ccCategoryId = UtilityMember.NumberSet.ToInteger(this.GetCurrentColumnValue("COST_CENTRECATEGORY_ID").ToString());
                e.Result = GetCCCategorySum(ccCategoryId, 1);
                e.Handled = true;
            }
        }

        private void xrccCategorySumBalance_SummaryGetResult(object sender, SummaryGetResultEventArgs e)
        {
            if (this.GetCurrentColumnValue("COST_CENTRECATEGORY_ID") != null)
            {
                Int32 ccCategoryId = UtilityMember.NumberSet.ToInteger(this.GetCurrentColumnValue("COST_CENTRECATEGORY_ID").ToString());
                double cccategoryclosingbalance = GetCCCategorySum(ccCategoryId, 3);
                e.Result = UtilityMember.NumberSet.ToNumber(Math.Abs(cccategoryclosingbalance)) + (cccategoryclosingbalance < 0 ? " Cr" : " Dr");
                e.Handled = true;
            }
        }
        ...
        private void xrRowLedgerOpBalance_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            ...
            //On 22/12/2023, Hide Ledger Opening Balance
            rowLedgerOP.Visible = false;//(settingProperty.ShowCCOpeningBalanceInReports == 1);
        }

        private void xrSumLedgerDr_SummaryGetResult(object sender, SummaryGetResultEventArgs e)
        {
            if (this.DataSource != null && this.GetCurrentColumnValue("COST_CENTRECATEGORY_ID") != null)
            {
                //double ledgeropdr = UtilityMember.NumberSet.ToDouble(this.GetCurrentColumnValue("LEDGER_OP_DR").ToString());
                //e.Result = CosLedgerDebit + ledgeropdr;
                //e.Handled = true;

                DataTable dt = this.DataSource as DataTable;
                double ledgeropdr = UtilityMember.NumberSet.ToDouble(this.GetCurrentColumnValue("LEDGER_OP_DR").ToString());
                Int32 ccId = UtilityMember.NumberSet.ToInteger(this.GetCurrentColumnValue(reportSetting1.BudgetCostCentre.COST_CENTRE_IDColumn.ColumnName).ToString());
                Int32 lId = UtilityMember.NumberSet.ToInteger(this.GetCurrentColumnValue(reportSetting1.BUDGETVARIANCE.LEDGER_IDColumn.ColumnName).ToString());
                string fitler = reportSetting1.BUDGETVARIANCE.LEDGER_IDColumn.ColumnName + " = " + lId + " AND " +
                                reportSetting1.BudgetCostCentre.COST_CENTRE_IDColumn.ColumnName + " = " + ccId;
                double amt = UtilityMember.NumberSet.ToDouble(dt.Compute("SUM(" + reportSetting1.Ledger.DEBITColumn.ColumnName + ")", fitler).ToString());
                // 09/07/2025, *Chinna
                //e.Result = amt
                e.Result = CosLedgerDebit;
                e.Handled = true;
            }
        }

        private void xrSumLedgerCr_SummaryGetResult(object sender, SummaryGetResultEventArgs e)
        {
            if (this.DataSource != null && this.GetCurrentColumnValue("COST_CENTRECATEGORY_ID") != null)
            {
                /*double ledgeropcr = UtilityMember.NumberSet.ToDouble(this.GetCurrentColumnValue("LEDGER_OP_CR").ToString());
                e.Result = CosLedgerCredit + ledgeropcr;
                e.Handled = true;*/

                DataTable dt = this.DataSource as DataTable;
                double ledgeropdr = UtilityMember.NumberSet.ToDouble(this.GetCurrentColumnValue("LEDGER_OP_CR").ToString());
                Int32 ccId = UtilityMember.NumberSet.ToInteger(this.GetCurrentColumnValue(reportSetting1.BudgetCostCentre.COST_CENTRE_IDColumn.ColumnName).ToString());
                Int32 lId = UtilityMember.NumberSet.ToInteger(this.GetCurrentColumnValue(reportSetting1.BUDGETVARIANCE.LEDGER_IDColumn.ColumnName).ToString());
                string fitler = reportSetting1.BUDGETVARIANCE.LEDGER_IDColumn.ColumnName + " = " + lId + " AND " +
                                reportSetting1.BudgetCostCentre.COST_CENTRE_IDColumn.ColumnName + " = " + ccId;
                double amt = UtilityMember.NumberSet.ToDouble(dt.Compute("SUM(" + reportSetting1.Ledger.CREDITColumn.ColumnName + ")", fitler).ToString());
                // 09/07/2025, *Chinna
                //  e.Result = amt;
                e.Result = CosLedgerCredit;
                e.Handled = true;
            }
        }

        private void xrSumLedgerCLBalance_SummaryGetResult(object sender, SummaryGetResultEventArgs e)
        {
            if (this.DataSource != null && this.GetCurrentColumnValue("COST_CENTRECATEGORY_ID") != null)
            {
                /*double ledgeropcr = UtilityMember.NumberSet.ToDouble(this.GetCurrentColumnValue("LEDGER_OP_CR").ToString());
                double ledgeropdr = UtilityMember.NumberSet.ToDouble(this.GetCurrentColumnValue("LEDGER_OP_DR").ToString());
                double LedgerCCBalance = (CosLedgerDebit + ledgeropdr) - (CosLedgerCredit + ledgeropcr); ;
                e.Result = UtilityMember.NumberSet.ToNumber(Math.Abs(LedgerCCBalance)) + " " + (LedgerCCBalance < 0 ? "Cr" : "Dr");
                e.Handled = true;*/
                ...
                string fitler = reportSetting1.BUDGETVARIANCE.LEDGER_IDColumn.ColumnName + " = " + lId + " AND " +
                                reportSetting1.BudgetCostCentre.COST_CENTRE_IDColumn.ColumnName + " = " + ccId;
                double amtdr = UtilityMember.NumberSet.ToDouble(dt.Compute("SUM(" + reportSetting1.Ledger.DEBITColumn.ColumnName + ")", fitler).ToString());
                double amtcr = UtilityMember.NumberSet.ToDouble(dt.Compute("SUM(" + reportSetting1.Ledger.CREDITColumn.ColumnName + ")", fitler).ToString());

                //09/07/2025, *Chinna
                //double amt = amtdr - amtcr;
                double amt = CosLedgerDebit - CosLedgerCredit;
                e.Result = UtilityMember.NumberSet.ToNumber(Math.Abs(amt)) + " " + (amt < 0 ? "Cr" : "Dr");
                //   e.Result = UtilityMember.NumberSet.ToNumber(Math.Abs(amt)) + " " + (amt < 0 ? "-" : "");
                e.Handled = true;
            }
        }

        private void grpFooterLedger_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            if (this.GetCurrentColumnValue(this.reportSetting1.Receipts.LEDGER_IDColumn.ColumnName) != null)
            {
                Int32 ledgerid = UtilityMember.NumberSet.ToInteger(this.GetCurrentColumnValue(this.reportSetting1.Receipts.LEDGER_IDColumn.ColumnName).ToString());
                //On 22/12/2023, Hide Ledger Opening Balance
                //e.Cancel = true;// (ledgerid == 0 || settingProperty.ShowCCOpeningBalanceInReports == 0);
                // e.Cancel = true;// (ReportProperties.ShowByCostCentre == 0);
            }
        }
        ...
        /// <param name="e"></param>
        private void xrCCTotalDr_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            //if (this.AppSetting.EnableCCModeReports == 1)
            //{
            //    XRTableCell cell = (XRTableCell)sender;
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void xrCCTotalDr_PrintOnPage(object sender, PrintOnPageEventArgs e)
        {
            //if (this.AppSetting.EnableCCModeReports == 1)
            //{
            //    XRTableCell cell = (XRTableCell)sender;
        ...
        {
            if (this.ReportProperties.ReportId != "RPT-050")
            {
                string C1 = this.GetCurrentColumnValue("COST_CENTRE_NAME").ToString();
                XRTableCell cell = sender as XRTableCell;
                //if (C1 == "ANG Africa Angola Luanda")
                //{


                if (C1 == TempCostCentre && ReportProperties.ShowHorizontalLine == 0)
                {
                    cell.Borders = BorderSide.Left | BorderSide.Right | BorderSide.Bottom | BorderSide.Top;
                }
                else
                {
                ...
                //if (ReportProperties.ShowHorizontalLine == 1 && ReportProperties.ShowVerticalLine == 1)
                //{
                //    xrLedgerName.Borders = BorderSide.Left | BorderSide.Right | BorderSide.Bottom | BorderSide.Top;
                //}
                //else
                //{
                //    if ((int)BorderStyleCell.Bold == this.ReportProperties.ReportBorderStyle && (int)BorderStyleCell.Bold == this.ReportProperties.ColumnCaptionFontStyle)
                //    {
                //        xrLedgerName.Borders = BorderSide.Left | BorderSide.Right | BorderSide.Bottom | BorderSide.Top;
                //    }
                //    else
                //    {
                //        xrLedgerName.Borders = BorderSide.All;
                //    }
                //}
                //xrLedgerName.BorderColor = ((int)BorderStyleCell.Regular == this.ReportProperties.ReportBorderStyle) ? System.Drawing.Color.Gainsboro : System.Drawing.Color.Black;
            }
        }

        private void xrGrpCCOPCaption_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            if (this.ReportProperties.ReportId != "RPT-050")
            {
                //Int32 ledgerid = UtilityMember.NumberSet.ToInteger(this.GetCurrentColumnValue(this.reportSetting1.Receipts.LEDGER_IDColumn.ColumnName).ToString());
                //if (ledgerid != 0)
                //{
                //    xrGrpCCOPCaption.Borders = xrOPBLDr.Borders = xrOPBLCr.Borders = xrTableCell23.Borders = BorderSide.Left | BorderSide.Right;
                //}
                //else
                //{
                //    xrGrpCCOPCaption.Borders = xrOPBLDr.Borders = xrOPBLCr.Borders = xrTableCell23.Borders = BorderSide.Left | BorderSide.Right | BorderSide.Bottom;
                //}
            }
        }
    }
