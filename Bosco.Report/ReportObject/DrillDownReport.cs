...
namespace Bosco.Report.ReportObject
{
    public partial class DrillDownReport : Bosco.Report.Base.ReportHeaderBase
    {
        ...
        public DrillDownReport()
        {
            ...
            if (this.ReportProperties.DrillDownProperties != null && this.ReportProperties.DrillDownProperties.Count > 1)
            {
                ...
                if (dicDDProperties.ContainsKey(this.ReportParameters.DATE_AS_ONColumn.ColumnName) || BaseReportId == "RPT-031")
                {
                    ...
                    else if (RecentDrillingReportId == "RPT-030" || BaseReportId == "RPT-030" || RecentDrillingReportId == "RPT-240" || BaseReportId == "RPT-240") //TB report - Current and Closing // || RecentDrillingReportId == "RPT-240" || BaseReportId == "RPT-240"
                    {
        ...
        public void BindReceiptSource()
        {
            ...
            if (dvReceipt != null)
            {
                string rowFilter = "(CREDIT <>0 OR DEBIT <> 0)";
                // Trial Balance only (RPT-030 standard, RPT-240 portrait): exclude subgroup summaries
                bool isTrialBalanceDrill =
                    (BaseReportId == "RPT-030" || BaseReportId == "RPT-240" ||
                     RecentDrillingReportId == "RPT-030" || RecentDrillingReportId == "RPT-240");
                if (isTrialBalanceDrill)
                {
                    rowFilter += " AND (PARTICULAR_TYPE <> 'GROUP_SUMMARY' AND PARTICULAR_TYPE <> 'GROUP_SUMMARY_RECEIPTS' AND PARTICULAR_TYPE <> 'GROUP_SUMMARY_PAYMENTS')";
                }
                dvReceipt.RowFilter = rowFilter;
