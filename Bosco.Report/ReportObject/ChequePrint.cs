...
namespace Bosco.Report.ReportObject
{
    public partial class ChequePrint : Bosco.Report.Base.ReportHeaderBase
    {
            ...
            if (resultArgs.Success && resultArgs.DataSource.Table != null)
            {
                ...
                if (dtBankVoucher.Rows.Count > 0)
                {
                    ...
                    if (resultArgs.Success)
                    {
                        ...
                        {
                            crossUsingCode = UtilityMember.NumberSet.ToInteger(dtBankVoucher.Rows[0]["BENEFICIARY_CROSS_USING"].ToString());
                            if (crossUsingCode < 0 || crossUsingCode > 2)
                                crossUsingCode = 0;
                        }
                        xrlblcrossusing.Text = GetCrossUsingCaption(crossUsingCode);
        ...
        private void drChequePrinting_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            ...
            xrlblcrossusing.TopF = (float)DateTop;
            xrlblcrossusing.LeftF = (float)GetXtraReportSize(CrossUsingLeftMarginInches);
            xrlblcrossusing.WidthF = 120F;
            xrlblcrossusing.AutoWidth = false;
