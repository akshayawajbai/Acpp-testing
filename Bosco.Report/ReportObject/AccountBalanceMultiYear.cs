...
{
    public partial class AccountBalanceMultiYear : Report.Base.ReportBase
    {
        ResultArgs resultArgs = null;
        SettingProperty settingProperty = new SettingProperty();
        public double PeriodBalanceAmount { get; set; }
        public bool IsQuaterly { get; set; }
        ...
        public XRPivotGridStyles ApplyParentReportStyle
        {
            set
            {
                ...
                //On 16/04/2021
                fieldLEDGERGROUP.Appearance.FieldValue.Font = new Font(value.FieldHeaderStyle.Font, value.FieldHeaderStyle.Font.Style);
            }
        }
        ...
        public AccountBalanceMultiYear()
        {
            InitializeComponent();
        }
        ...
        public void BindBalance(bool isOpBalance)
        {
            ...
            int NoYears = this.ReportProperties.NoOfYears;

            if (isOpBalance)
            {
                DateTime OpeningDate = UtilityMember.DateSet.ToDate(this.ReportProperties.DateFrom, false);
                for (int i = 0; i <= NoYears; i++)
                {
                    dicMonthYear[UtilityMember.DateSet.ToDate(OpeningDate.ToShortDateString(), false)] =
                            UtilityMember.DateSet.ToDate(OpeningDate.AddDays(-1).ToShortDateString(), false);
                    OpeningDate = OpeningDate.AddYears(-1);
                }
                //Add Total
                //24/04/2020, To have proper opening balnace amount
                /*dicMonthYear[UtilityMember.DateSet.ToDate(this.ReportProperties.DateFrom, false).AddYears(1)] =
                    UtilityMember.DateSet.ToDate(settingProperty.BookBeginFrom, false).AddDays(-1);
                //UtilityMember.DateSet.ToDate(this.ReportProperties.DateFrom, false).AddDays(-1);*/
                var lastOpTotal = dicMonthYear.Values.Last();
                if (lastOpTotal <= UtilityMember.DateSet.ToDate(settingProperty.BookBeginFrom, false).AddDays(-1))
                {
                    lastOpTotal = UtilityMember.DateSet.ToDate(settingProperty.BookBeginFrom, false).AddDays(-1);
                }
                dicMonthYear[UtilityMember.DateSet.ToDate(this.ReportProperties.DateFrom, false).AddYears(1)] = lastOpTotal;
            }
            else
            {
                DateTime OpeningDate = UtilityMember.DateSet.ToDate(this.ReportProperties.DateFrom, false);
                for (int i = 0; i <= NoYears; i++)
                {
                    DateTime dtCLBalDate = UtilityMember.DateSet.ToDate(OpeningDate.AddYears(1).AddDays(-1).ToShortDateString(), false);
                    if (dtCLBalDate == UtilityMember.DateSet.ToDate(settingProperty.BookBeginFrom, false).AddDays(-1))
                    {
                        dicMonthYear[UtilityMember.DateSet.ToDate(OpeningDate.ToShortDateString(), false)] = dtCLBalDate.AddDays(-1);
                    }
                    else
                    {
                        dicMonthYear[UtilityMember.DateSet.ToDate(OpeningDate.ToShortDateString(), false)] = dtCLBalDate;
                    }
                ...
                //Add Total
                //24/04/2020, To have proper opening balnace amount
                //dicMonthYear[UtilityMember.DateSet.ToDate(this.ReportProperties.DateFrom, false).AddYears(1)] = UtilityMember.DateSet.ToDate(settingProperty.YearTo, false);
                dicMonthYear[UtilityMember.DateSet.ToDate(this.ReportProperties.DateFrom, false).AddYears(1)] = UtilityMember.DateSet.ToDate(this.ReportProperties.DateTo, false);
            }
            ...
            dtBalance.Columns.Add(reportSetting1.AccountBalance.AC_YEAR_NAMEColumn.ColumnName, typeof(string));
            dtBalance.Columns.Add(reportSetting1.AccountBalance.YEAR_FROMColumn.ColumnName, typeof(string));
            Int32 MonthRow = 1;
            foreach (KeyValuePair<DateTime, DateTime> dateKey in dicMonthYear)
            {
                ...
                //DateTime FinanceFromDate = UtilityMember.DateSet.GetFinancialYearByDate(monthYear);
                DateTime FinanceFromDate = UtilityMember.DateSet.GetFinancialYearByDate(monthYear, settingProperty.YearFrom);
                DateTime FinanceToDate = FinanceFromDate.AddMonths(12).AddDays(-1);
                ...
                if (dtBalMonth != null)
                {
                    if (dtBalMonth.Rows.Count > 0)
                    {
                        foreach (DataRow drBalMonth in dtBalMonth.Rows)
                        {
                            ...
                            drBalance[reportSetting1.AccountBalance.YEAR_FROMColumn.ColumnName] = FinanceFromDate;//01/04/2017";
                            ...
                            {
                                //drBalance[reportSetting1.AccountBalance.AC_YEAR_NAMEColumn.ColumnName] = "Opening (Total)";
                                drBalance[reportSetting1.AccountBalance.AC_YEAR_NAMEColumn.ColumnName] = MessageRender.GetMessage(MessageCatalog.Criteria.OPENING_TOTAL);
                            }
                            else if (xrPGAccountBalanceMulti.OptionsView.ShowRowHeaders && MonthRow == dicMonthYear.Count && IsOpeningBalance == false)
                            {
                                //drBalance[reportSetting1.AccountBalance.AC_YEAR_NAMEColumn.ColumnName] = "Closing (Total)";
                                drBalance[reportSetting1.AccountBalance.AC_YEAR_NAMEColumn.ColumnName] = MessageRender.GetMessage(MessageCatalog.Criteria.CLOSING_TOTAL);
                            }
                            else
                            {
                                drBalance[reportSetting1.AccountBalance.AC_YEAR_NAMEColumn.ColumnName] = FinanceFromDate.Year.ToString() + "-" + FinanceToDate.Year.ToString();
        ...
        public void BindBalanceQuartely(bool isOpBalance)
        {
            string dateFrom = ReportProperties.DateFrom;
            string dateTo = ReportProperties.DateTo;
            string balDate = "";
            ...
            List<Tuple<DateTime, DateTime>> quarterDates = ReportProperty.Current.DateSet.GetQuarterDates(date_from, date_to);
            int NoYears = quarterDates.Count;
            ...
            if (isOpBalance)
            {
                ...
                for (int i = 0; i <= NoYears; i++)
                {
                    dicMonthYear[UtilityMember.DateSet.ToDate(OpeningDate.ToShortDateString(), false)] =
                            UtilityMember.DateSet.ToDate(OpeningDate.AddDays(-1).ToShortDateString(), false);
                    OpeningDate = OpeningDate.AddMonths(3); ;// OpeningDate.AddYears(-1);
                }
                //Add Total
                //dicMonthYear[UtilityMember.DateSet.ToDate(this.ReportProperties.DateFrom, false).AddYears(1)] =
                //    UtilityMember.DateSet.ToDate(settingProperty.BookBeginFrom, false).AddDays(-1);
                var lastOpTotal = dicMonthYear.Values.Last();
                var lastOpTotal1 = dicMonthYear.Values.First();
                dicMonthYear[lastOpTotal.AddDays(1)] = lastOpTotal1;
            }
            else
            {
                DateTime OpeningDate = UtilityMember.DateSet.ToDate(this.ReportProperties.DateFrom, false);
                for (int i = 0; i <= NoYears; i++)
                {
                    DateTime dtCLBalDate = UtilityMember.DateSet.ToDate(OpeningDate.AddMonths(3).ToShortDateString(), false);
                    if (dtCLBalDate == UtilityMember.DateSet.ToDate(settingProperty.BookBeginFrom, false))
                    {
                        dicMonthYear[UtilityMember.DateSet.ToDate(OpeningDate.ToShortDateString(), false)] = dtCLBalDate.AddDays(-1);
                    }
                    else
                ...
                //24/04/2020, To have proper opening balnace amount
                //dicMonthYear[UtilityMember.DateSet.ToDate(this.ReportProperties.DateFrom, false).AddYears(1)] = UtilityMember.DateSet.ToDate(settingProperty.YearTo, false);
                var lastOpTotal = dicMonthYear.Keys.Last();
                //var lastOpTotal1 = dicMonthYear.Values.Last();
                var lastOpTotal1 = dicMonthYear.ElementAt(dicMonthYear.Values.Count - 2).Value;
                dicMonthYear[lastOpTotal] = lastOpTotal1;
            }
            ...
            foreach (KeyValuePair<DateTime, DateTime> dateKey in dicMonthYear)
            {
                ...
                //DateTime FinanceFromDate = UtilityMember.DateSet.GetFinancialYearByDate(monthYear);
                DateTime FinanceFromDate = UtilityMember.DateSet.GetFinancialYearByDate(monthYear, settingProperty.YearFrom);
                DateTime FinanceToDate = FinanceFromDate.AddMonths(3).AddDays(-1); //FinanceFromDate.AddMonths(12).AddDays(-1);
                ...
                if (dtBalMonth != null)
                {
                    if (dtBalMonth.Rows.Count > 0)
                    {
                        foreach (DataRow drBalMonth in dtBalMonth.Rows)
                        {
                            ...
                            drBalance[reportSetting1.AccountBalance.AMOUNTColumn.ColumnName] = amount;
                            drBalance[reportSetting1.AccountBalance.TRANS_MODEColumn.ColumnName] = transMode;
                            drBalance[reportSetting1.AccountBalance.YEAR_FROMColumn.ColumnName] = monthYear; // FinanceFromDate;//01/04/2017";
                            ...
                            {
                                int QuterRow = MonthRow;
                                if (monthYear <= this.UtilityMember.DateSet.ToDate(settingProperty.YearTo, false))
                                {
                                    QuterRow = UtilityMember.DateSet.GetQuarter(monthYear, settingProperty.YearFrom);
                                    drBalance[reportSetting1.AccountBalance.AC_YEAR_NAMEColumn.ColumnName] = UtilityMember.DateSet.GetQuaterMonthsFinancialYearByQuaterNo(QuterRow,
                                    settingProperty.YearFrom, settingProperty.YearTo);
                                }
                                else
                                {
                                    drBalance[reportSetting1.AccountBalance.AC_YEAR_NAMEColumn.ColumnName] = "Total";
                                }
            ...
            try
            {
                if (e.Field.Name == fieldACYEARNAME.Name)
                {
                    if (e.Value1 != null && e.Value2 != null)
                    {
                        ...
                        DateTime dt1 = DateTime.Parse(e.GetListSourceColumnValue(e.ListSourceRowIndex1, reportSetting1.AccountBalance.YEAR_FROMColumn.ColumnName).ToString());
                        DateTime dt2 = DateTime.Parse(e.GetListSourceColumnValue(e.ListSourceRowIndex2, reportSetting1.AccountBalance.YEAR_FROMColumn.ColumnName).ToString());
        ...
        private void xrPGAccountBalanceMulti_FieldValueDisplayText(object sender, DevExpress.XtraReports.UI.PivotGrid.PivotFieldDisplayTextEventArgs e)
        {
            ...
            else if (e.ValueType == DevExpress.XtraPivotGrid.PivotGridValueType.Value)
            {
        ...
        private void SetReportSetting()
        {
            ...
            if (xrPGAccountBalanceMulti.OptionsView.ShowRowHeaders && IsOpeningBalance)
            {
                ...
                //fieldLEDGERNAME.Caption = "Opening Balance";
                fieldLEDGERNAME.Caption = MessageRender.GetMessage(MessageCatalog.Criteria.OPENING_BALANCE);
                if (ReportProperties.ShowByLedger == 1 && ReportProperties.ShowByGroup == 0)
                {
                    fieldLEDGERGROUP.Caption = "Group";
                }
                else
                {
                    fieldLEDGERGROUP.Caption = MessageRender.GetMessage(MessageCatalog.Criteria.OPENING_BALANCE);
                }
            }
