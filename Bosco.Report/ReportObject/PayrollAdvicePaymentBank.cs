...
namespace Bosco.Report.ReportObject
{
    public partial class PayrollAdvicePaymentBank : ReportHeaderBase
    {
        ...
        private void ReportSetting()
        {
            ...
            setHeaderTitleAlignment();
            string accountNo = ReportProperty.Current.PayrollPaymentBankAccountNo;
            ...
            xrlblSubject.Text = "Payment Advice from " + ReportProperty.Current.PayrollProjectTitle + (string.IsNullOrWhiteSpace(accountNo) ? string.Empty : " (A/c No : " + accountNo + ")") +
                    " for the month of " + ReportProperty.Current.PayrollName;

            if (string.IsNullOrWhiteSpace(accountNo) || accountNo.Trim() == "0")
            {
                xrLblInfo.Text = string.Empty;
            }
            else
            {
            ...
            using (DataManager dataManager = new DataManager(SQLCommand.Payroll.PaymentAdviceBank, "PaymentAdviceBank"))
            {
                ...
                // #Dinesh 03-07-2026 - Filter staff mapped to selected company bank account
                dataManager.Parameters.Add("BANK_ACCOUNT_ID", ReportProperty.Current.PayrollPaymentBankAccountId);
                dataManager.DataCommandArgs.IsDirectReplaceParameter = true;
