...
namespace Bosco.Report.ReportObject
{
    public partial class BudgetAnnualRealizationLedgersCCwise : Bosco.Report.Base.ReportHeaderBase
    {
            ...
            try
            {
                ...
                if (resultArgs != null && resultArgs.Success && resultArgs.DataSource.Table != null)
                {
                    ...
                    string filter = "(" + this.reportSetting1.BUDGETVARIANCE.APPROVED_AMOUNTColumn.ColumnName + " > 0 " +
                        ...
                        // to show cr and dr filter verifications // bharath
                                    " OR " + this.reportSetting1.BUDGETVARIANCE.BUDGET_TRANS_MODEColumn.ColumnName + " = 'CR'" + ")";
                    ...
                    if (this.ReportProperties.IncludeAllLedger == 0)
                    {
                        filter = "(" + this.reportSetting1.BUDGETVARIANCE.APPROVED_AMOUNTColumn.ColumnName + " > 0 OR " + this.reportSetting1.BUDGETVARIANCE.ACTUAL_AMOUNTColumn.ColumnName + " > 0 OR " +
                            // to show only budget realized amount // bharath
                                      this.reportSetting1.BUDGETVARIANCE.BUDGET_TRANS_MODEColumn.ColumnName + " = 'CR' OR " +
        ...
        //Dinesh29-07-2026 - Detail % from Approved Budget and Due Months (event only, no extra column)
        private void xrtblPerctange_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            XRTableCell cell = sender as XRTableCell;
            double percentage = 0;
            DataRowView drvCurrentRow = this.GetCurrentRow() as DataRowView;
            if (drvCurrentRow != null)
            {
                double approvedBudget = UtilityMember.NumberSet.ToDouble(drvCurrentRow[reportSetting1.BUDGETVARIANCE.APPROVED_AMOUNTColumn.ColumnName].ToString());
                double dueMonths = UtilityMember.NumberSet.ToDouble(drvCurrentRow[reportSetting1.BUDGETVARIANCE.ACTUAL_AMOUNTColumn.ColumnName].ToString());
                percentage = GetPercentage(approvedBudget, dueMonths);
            }
            if (cell != null)
                cell.Text = FormatPercentage(percentage);
        }

        //Dinesh29-07-2026 - Income / Expenditure Total % = (Sum Due -1 Months / Sum Approved Budget) * 100
        private void xrtblPerTot_SummaryGetResult(object sender, SummaryGetResultEventArgs e)
        {
            double percentage = 0;
            try
            {
                if (this.DataSource != null && GetCurrentColumnValue(reportSetting1.BUDGETVARIANCE.BUDGET_TRANS_MODEColumn.ColumnName) != null)
                {
                    DataTable dtTable = this.DataSource as DataTable;
                    string transMode = GetCurrentColumnValue(reportSetting1.BUDGETVARIANCE.BUDGET_TRANS_MODEColumn.ColumnName).ToString();
                    string filter = reportSetting1.BUDGETVARIANCE.BUDGET_TRANS_MODEColumn.ColumnName + " = '" + transMode + "'";

                    if (GetCurrentColumnValue(reportSetting1.BUDGETVARIANCE.COST_CENTRECATEGORY_IDColumn.ColumnName) != null)
                    {
                        Int32 cccategoryid = UtilityMember.NumberSet.ToInteger(GetCurrentColumnValue(reportSetting1.BUDGETVARIANCE.COST_CENTRECATEGORY_IDColumn.ColumnName).ToString());
                        filter += " AND " + reportSetting1.BUDGETVARIANCE.COST_CENTRECATEGORY_IDColumn.ColumnName + "=" + cccategoryid;
                    }

                    if (this.ReportProperties.ShowByCostCentre == 1 && GetCurrentColumnValue(reportSetting1.BUDGETVARIANCE.COST_CENTRE_IDColumn.ColumnName) != null)
                    {
                        Int32 costCentreId = UtilityMember.NumberSet.ToInteger(GetCurrentColumnValue(reportSetting1.BUDGETVARIANCE.COST_CENTRE_IDColumn.ColumnName).ToString());
                        filter += " AND " + reportSetting1.BUDGETVARIANCE.COST_CENTRE_IDColumn.ColumnName + "=" + costCentreId;
                    }

                    object approvedObj = dtTable.Compute("SUM(" + reportSetting1.BUDGETVARIANCE.APPROVED_AMOUNTColumn.ColumnName + ")", filter);
                    object dueObj = dtTable.Compute("SUM(" + reportSetting1.BUDGETVARIANCE.ACTUAL_AMOUNTColumn.ColumnName + ")", filter);

                    double approvedBudget = (approvedObj == null || approvedObj == DBNull.Value) ? 0 : UtilityMember.NumberSet.ToDouble(approvedObj.ToString());
                    double dueMonths = (dueObj == null || dueObj == DBNull.Value) ? 0 : UtilityMember.NumberSet.ToDouble(dueObj.ToString());
            ...
            e.Result = FormatPercentage(percentage);
            e.Handled = true;
        }

        //Dinesh29-07-2026 - Balance % cell: empty only (no value)
        private void xrtblGrandPercTot_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            XRTableCell cell = sender as XRTableCell;
            if (cell != null)
        ...
        //Dinesh29-07-2026 - % = (Due Months / Approved Budget) * 100
        private double GetPercentage(double approvedBudget, double dueMonths)
        {
            if (approvedBudget <= 0)
                return 0;
            return Math.Round((dueMonths / approvedBudget) * 100, 1);
        }

        //Dinesh29-07-2026 - display format e.g. 34.6%
