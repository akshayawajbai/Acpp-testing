...
{
    public partial class IncomeandExpenditureYearwish : Bosco.Report.Base.ReportHeaderBase
    {
        IncomeExpanditueYearSubRp multiAbstractReceipts;
        IncomeExpanditueYearSubRp multiAbstractPayments;

        public IncomeandExpenditureYearwish()
        {
            InitializeComponent();
        ...
        private void LoadMultiAbstractReport()
        {
            ...
            multiAbstractReceipts = xrSubMultiAbstractReceipt.ReportSource as IncomeExpanditueYearSubRp;
            multiAbstractPayments = xrSubMultiAbstractPayment.ReportSource as IncomeExpanditueYearSubRp;
        ...
        private void AssignChartProperties(DataTable dtGrandReceipts, double TotalReceipts, DataTable dtGrandPayments, double TotalPayments)
        {
            if (grpFooterChartReport.Visible)
            {
                ...
                ChartSeries newChartSeriesProperties = new ChartSeries("Income", new string[] { "AMOUNT_RECEIPT" }, "AC_YEAR", dtRPChart);
                if (ReportProperty.Current.ChartInPercentage == 1)
                {
                    newChartSeriesProperties = new ChartSeries("Income", new string[] { "AMOUNT_RECEIPT_PERCENTAGE" }, "AC_YEAR", dtRPChart);
                }
                reportchart.ChartSeriesProperties.Add("Income", newChartSeriesProperties);

                newChartSeriesProperties = new ChartSeries("Expenditure", new string[] { "AMOUNT_PAYMENT" }, "AC_YEAR", dtRPChart);
                if (ReportProperty.Current.ChartInPercentage == 1)
                {
                    newChartSeriesProperties = new ChartSeries("Expenditure", new string[] { "AMOUNT_PAYMENT_PERCENTAGE" }, "AC_YEAR", dtRPChart);
                }
                reportchart.ChartSeriesProperties.Add("Expenditure", newChartSeriesProperties);
