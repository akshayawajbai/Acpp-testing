...
ALTER TABLE `fd_account` DROP INDEX `idx_fd_ac_fd_ac_no`;

ALTER TABLE `voucher_master_trans` DROP INDEX `FK_VOUCHER_MASTER_CREATED_BY`
, DROP INDEX `FK_VOUCHER_MASTER_MODIFIED_BY`,
 DROP INDEX `FK_voucher_master_trans_PROJECT_ID`,
 ADD INDEX `FK_voucher_master_trans_PROJECT_ID`
 USING BTREE(`PROJECT_ID`, `VOUCHER_DATE`, `VOUCHER_TYPE`);

-- Salamon Raj on 30.10.2014
ALTER TABLE `fd_account` ADD COLUMN `BRANCH_ID` VARCHAR(45) NOT NULL DEFAULT '0' AFTER `NOTES`,
 DROP PRIMARY KEY,
 ADD PRIMARY KEY(`FD_ACCOUNT_ID`, `BRANCH_ID`);

ALTER TABLE `fd_renewal` ADD COLUMN `BRANCH_ID` INTEGER UNSIGNED NOT NULL DEFAULT 0 AFTER `IS_DELETED`;

ALTER TABLE `ledger_balance` DROP PRIMARY KEY,
 ADD PRIMARY KEY(`BALANCE_DATE`, `PROJECT_ID`, `LEDGER_ID`, `BRANCH_ID`);

ALTER TABLE `master_donaud` ADD COLUMN `BRANCH_ID` INTEGER UNSIGNED NOT NULL DEFAULT 0 AFTER `PAN`;

ALTER TABLE `master_ledger` ADD COLUMN `IS_BRANCH_LEDGER` INTEGER UNSIGNED NOT NULL DEFAULT 0 AFTER `ACCESS_FLAG`;


ALTER TABLE `voucher_master_trans` DROP PRIMARY KEY,
 ADD PRIMARY KEY(`VOUCHER_ID`, `BRANCH_ID`);

ALTER TABLE `voucher_trans` DROP PRIMARY KEY,
 ADD PRIMARY KEY(`VOUCHER_ID`, `SEQUENCE_NO`, `BRANCH_ID`);

ALTER TABLE `voucher_master_trans` DROP FOREIGN KEY `FK_VOUCHER_MASTER_CREATED_BY`,
 DROP FOREIGN KEY `FK_VOUCHER_MASTER_MODIFIED_BY`;

-- Ended

-- MICHAEL R ON 30-10-2014

ALTER TABLE  `budget_master` DROP FOREIGN KEY `FK_budget_master_budget_id`;
...
ALTER TABLE `master_state` ADD UNIQUE `UNQ_STATE_NAME`(`STATE_NAME`);

ALTER TABLE `master_insti_perference` DROP COLUMN `COUNTRY`,
 ADD COLUMN `COUNTRY_ID` INTEGER AFTER `STATE`,
 ADD CONSTRAINT `FK_COUNTRY_ID` FOREIGN KEY `FK_COUNTRY_ID` (`COUNTRY_ID`)
    REFERENCES `master_country` (`COUNTRY_ID`)
    ON DELETE RESTRICT
    ON UPDATE RESTRICT;

ALTER TABLE `master_executive_committee` DROP COLUMN `STATE`,
 ADD COLUMN `STATE_ID` INTEGER UNSIGNED NOT NULL DEFAULT 0 AFTER `PLACE`,
 ADD CONSTRAINT `FK_STATE_ID` FOREIGN KEY `FK_STATE_ID` (`STATE_ID`)
    REFERENCES `master_state` (`STATE_ID`)
    ON DELETE RESTRICT
    ON UPDATE RESTRICT;

ALTER TABLE `master_insti_perference` ADD COLUMN `OTHER_ASSOCIATION_NATURE` VARCHAR(100) AFTER `DENOMINATION`,
 ADD COLUMN `OTHER_DENOMINATION` VARCHAR(100) NOT NULL DEFAULT '' AFTER `OTHER_ASSOCIATION_NATURE`;

ALTER TABLE `master_insti_perference` MODIFY COLUMN `OTHER_DENOMINATION` VARCHAR(100) CHARACTER SET utf8 COLLATE utf8_general_ci;


ALTER TABLE `master_insti_perference` MODIFY COLUMN `ASSOCIATIONNATURE` INTEGER COMMENT '0-Cultural,1-Ecomomic,2-Educational,3-Religious,4-Social',
 MODIFY COLUMN `DENOMINATION` INTEGER COMMENT '0-Hindu,1-Sikh,2-Muslim,3-Christian,4-Buddhist,5-Others';

ALTER TABLE `master_donaud` ADD COLUMN `CUSTOMERID` INTEGER UNSIGNED NOT NULL DEFAULT 0 AFTER `BRANCH_ID`;

ALTER TABLE  `master_insti_perference` ADD COLUMN `FCRINO` VARCHAR(30) AFTER `OTHER_DENOMINATION`,
 ADD COLUMN `FCRIREGDATE` DATETIME AFTER `FCRINO`,
 ADD COLUMN `EIGHTYGNO` VARCHAR(30) AFTER `FCRIREGDATE`;


-- Michael  R on 10-11-2014
...
INSERT INTO `master_country` (`COUNTRY_ID`,`COUNTRY`,`COUNTRY_CODE`,`CURRENCY_CODE`,`CURRENCY_SYMBOL`,`CURRENCY_NAME`) VALUES 
 ...
 (56,'United Kingdom','GBR','GBP','£','UK Pound Sterling'),
...
INSERT INTO `master_country` (`COUNTRY_ID`,`COUNTRY`,`COUNTRY_CODE`,`CURRENCY_CODE`,`CURRENCY_SYMBOL`,`CURRENCY_NAME`) VALUES 
 ...
 (89,'Costa Rica','CRI','CRC','¢','Costa Rican Colon'),
 (90,'Morocco','MAR','MAD','?.?.?','Moroccan Dirham'),
 (91,'Ireland','IRL','EUR','€','Euro'),
 (92,'Panama','PAN','PAB','B/.','Panamanian Balboa'),
 (93,'Principality of Monaco','MCO','EUR','€','Euro');
INSERT INTO `master_country` (`COUNTRY_ID`,`COUNTRY`,`COUNTRY_CODE`,`CURRENCY_CODE`,`CURRENCY_SYMBOL`,`CURRENCY_NAME`) VALUES 
...
ALTER TABLE `activitiy_rights` ADD COLUMN `ENUMTYPE` VARCHAR(100) NOT NULL DEFAULT '' AFTER `OBJECT_NAME`;


ALTER TABLE `voucher_number_format` ADD COLUMN `PROJECT_ID` INTEGER UNSIGNED NOT NULL DEFAULT 0 AFTER `VOUCHER_YEAR`;

ALTER TABLE  `fd_renewal` DROP FOREIGN KEY `FK_FD_RENEWAL_FD_ACCOUNT_ID`;

ALTER TABLE  `fd_renewal` DROP INDEX `IndexFDACCOUNT`;

ALTER TABLE `fd_renewal` ADD CONSTRAINT `FK_fd_Account_id` FOREIGN KEY `FK_fd_Account_id` (`FD_ACCOUNT_ID`)
    REFERENCES `fd_account` (`FD_ACCOUNT_ID`)
    ON DELETE RESTRICT
    ON UPDATE RESTRICT;

ALTER TABLE `fd_renewal` ADD COLUMN `IS_DELETED` INTEGER UNSIGNED NOT NULL DEFAULT 1 COMMENT '1-ACTIVE, 0-DELETED' AFTER `STATUS`;


ALTER TABLE `ledger_balance` ADD COLUMN `BRANCH_ID` INTEGER UNSIGNED NOT NULL DEFAULT 0 AFTER `TRANS_FLAG`;

ALTER TABLE `voucher_cc_trans` ADD COLUMN `BRANCH_ID` INTEGER UNSIGNED NOT NULL DEFAULT 0 AFTER `SEQUENCE_NO`;

ALTER TABLE `voucher_master_trans` ADD COLUMN `BRANCH_ID` INTEGER UNSIGNED NOT NULL DEFAULT 0 AFTER `NAME_ADDRESS`;

ALTER TABLE `voucher_trans` ADD COLUMN `BRANCH_ID` INTEGER UNSIGNED NOT NULL DEFAULT 0 AFTER `STATUS`;
...
-- DB Changes by Praveen on 09.10.2014
...
ALTER TABLE `master_cost_centre_category` CHANGE COLUMN `COST_CENTRE_NAME` `COST_CENTRE_CATEGORY_NAME`
VARCHAR(45) CHARACTER SET latin1 COLLATE latin1_swedish_ci NOT NULL DEFAULT '';

CREATE TABLE `CostCategory_CostCentre` (
  `COST_CATEGORY_ID` INTEGER UNSIGNED NOT NULL AUTO_INCREMENT,
  `COST_CENTRE_ID` INTEGER UNSIGNED NOT NULL DEFAULT 0,
  PRIMARY KEY(`COST_CATEGORY_ID`)
)
ENGINE = InnoDB;

ALTER TABLE `costcategory_costcentre` MODIFY COLUMN `COST_CATEGORY_ID` INTEGER UNSIGNED NOT NULL DEFAULT 0,
 DROP PRIMARY KEY;

 
 INSERT INTO MASTER_COST_CENTRE_CATEGORY(COST_CENTRECATEGORY_ID, COST_CENTRE_CATEGORY_NAME) VALUES (1, 'Primary');

-- Currency
 update master_setting set Value='र' where setting_name='Currency';
 update master_country set currency_symbol='र' where country='India';
 update master_country set currency_symbol='र' where country='India';
 INSERT INTO master_setting(VALUE,setting_name) VALUES('Primary','Location');
...
INSERT INTO `activitiy_rights` (`ACTIVITY_ID`,`PARENT_ID`,`OBJECT_NAME`,`ENUMTYPE`,`OBJECT_TYPE`,`OBJECT_SUB_TYPE`) VALUES 
 (16,205,'Legal Entity','LegalEntity','Settings','Legal Entity'),
...
INSERT INTO `activitiy_rights` (`ACTIVITY_ID`,`PARENT_ID`,`OBJECT_NAME`,`ENUMTYPE`,`OBJECT_TYPE`,`OBJECT_SUB_TYPE`) VALUES 
 ...
 (67,89,'Audit Info','Audit Info','Masters','Audit Info'),
 ...
 (72,89,'Governing Members','GoverningMembers','Masters','Governing Members'),
 (73,72,'Create Governing Member','CreateGoverningMember','Masters','Governing Members');
INSERT INTO `activitiy_rights` (`ACTIVITY_ID`,`PARENT_ID`,`OBJECT_NAME`,`ENUMTYPE`,`OBJECT_TYPE`,`OBJECT_SUB_TYPE`) VALUES 
 (74,72,'Edit Governing Member','EditGoverningMember','Masters','Governing Members'),
 (75,72,'Delete Governing Member','DeleteGoverningMember','Masters','Governing Members'),
 (76,72,'Print Governing Member','PrintGoverningMember','Masters','Governing Members'),
 (77,89,'Auditor','Auditor','Masters','Auditor'),
...
INSERT INTO `activitiy_rights` (`ACTIVITY_ID`,`PARENT_ID`,`OBJECT_NAME`,`ENUMTYPE`,`OBJECT_TYPE`,`OBJECT_SUB_TYPE`) VALUES 
 ...
 (121,89,'Budget','Budget','Finance','Budget'),
 (122,121,'Create Budget','CreateBudget','Finance','Budget'),
 (123,121,'Edit Budget','EditBudget','Finance','Budget'),
 (124,121,'Delete Budget','DeleteBudget','Finance','Budget'),
 (125,121,'Print Budget','PrintBudget','Finance','Budget'),
...
INSERT INTO `activitiy_rights` (`ACTIVITY_ID`,`PARENT_ID`,`OBJECT_NAME`,`ENUMTYPE`,`OBJECT_TYPE`,`OBJECT_SUB_TYPE`) VALUES 
 ...
 (180,72,'View Governing Members','ViewGoverningMembers','Masters','Governing Members'),
...
INSERT INTO `activitiy_rights` (`ACTIVITY_ID`,`PARENT_ID`,`OBJECT_NAME`,`ENUMTYPE`,`OBJECT_TYPE`,`OBJECT_SUB_TYPE`) VALUES 
 ...
 (188,121,'View Budget','ViewBudget','Finance','Budget'),
...
INSERT INTO `activitiy_rights` (`ACTIVITY_ID`,`PARENT_ID`,`OBJECT_NAME`,`ENUMTYPE`,`OBJECT_TYPE`,`OBJECT_SUB_TYPE`) VALUES 
 ...
 (214,213,'View Receipt and Payments','ViewReceiptPayments','Finance','Dash Board'),
 (215,213,'Show FD Alert','ShowFDAlert','Finance','Dash Board'),
 (216,213,'Show Bank Reconciliation','ShowBankReconciliation','Finance','Dash Board'),
 (217,213,'Show Project Details','ShowProjectDetails','Finance','Dash Board'),
 ...
 (221,205,'Import Data','ImportData','Data Utility','Activities'),
 (222,205,'Export Data','ExportData','Data Utility','Activities');
INSERT INTO `activitiy_rights` (`ACTIVITY_ID`,`PARENT_ID`,`OBJECT_NAME`,`ENUMTYPE`,`OBJECT_TYPE`,`OBJECT_SUB_TYPE`) VALUES 
...
INSERT INTO activitiy_rights(ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)
VALUES(218,205,'Import Masters','ShowImportMasters','Data Utility','Activities'),
(219,205,'Export Vouchers','ShowExportVouchers','Data Utility','Activities'),
(220,205,'Map Ledgers','ShowMapLedgers','Data Utility','Activities');

ALTER TABLE `master_project` ADD COLUMN `CUSTOMERID` INTEGER UNSIGNED NOT NULL DEFAULT 0 AFTER `DELETE_FLAG`;

ALTER TABLE `voucher_number_format` ADD COLUMN `MONTH` VARCHAR(45) NOT NULL DEFAULT '' COMMENT 'Applicable from' AFTER `NUMBER_FORMAT`,
 ADD COLUMN `VOUCHER_MONTH` INTEGER UNSIGNED NOT NULL DEFAULT 0 COMMENT 'Current Voucher Month' AFTER `MONTH`,
 ADD COLUMN `DURATION` INTEGER UNSIGNED NOT NULL DEFAULT 0 COMMENT 'Reset Month' AFTER `VOUCHER_MONTH`;

ALTER TABLE `voucher_master_trans` MODIFY COLUMN `VOUCHER_NO` VARCHAR(25) CHARACTER SET utf8 COLLATE utf8_general_ci;

ALTER TABLE `voucher_number_format` ADD COLUMN `VOUCHER_YEAR` INTEGER UNSIGNED NOT NULL DEFAULT 0 AFTER `DURATION`;

ALTER TABLE  `fd_renewal` ADD COLUMN `INTEREST_TYPE` INTEGER UNSIGNED NOT NULL DEFAULT 0 AFTER `INTEREST_RATE`;

ALTER TABLE  `fd_account` ADD COLUMN `BANK_LEDGER_ID` INTEGER UNSIGNED NOT NULL DEFAULT 0 AFTER `LEDGER_ID`;

ALTER TABLE `master_ledger` ADD COLUMN `ACCESS_FLAG` INTEGER(10) UNSIGNED NOT NULL DEFAULT 0 AFTER `STATUS`;

ALTER TABLE `voucher_number_format` ADD COLUMN `PROJECT_ID` INTEGER UNSIGNED NOT NULL DEFAULT 0 AFTER `VOUCHER_YEAR`;

ALTER TABLE `activitiy_rights` ADD COLUMN `ENUMTYPE` VARCHAR(100) NOT NULL DEFAULT '' AFTER `OBJECT_NAME`;

-- DBChanges for Data Synchronization on -04-07-2014
...
-- DBChanges by Salamon on -15-07-2014

-- DBChanges by Michael R on -24-07-2014
-- ALTER TABLE `voucher_cc_trans` DROP PRIMARY KEY;

-- DB Changes by Michael R on 26-08-2014
...
CREATE TABLE `project_branch` (
  ...
  `BRANCH_ID` INTEGER UNSIGNED NOT NULL DEFAULT 0,
  CONSTRAINT `FK_project_branch_master_project` FOREIGN KEY `FK_project_branch_master_project` (`PROJECT_ID`)
    REFERENCES `master_project` (`PROJECT_ID`)
    ON DELETE RESTRICT
    ON UPDATE RESTRICT,
  CONSTRAINT `FK_project_branch_branch_office` FOREIGN KEY `FK_project_branch_branch_office` (`BRANCH_ID`)
...
-- Ended
-- Michael R on 17-10-2014 Remove Index of FD Account Number
...
ALTER TABLE  `master_insti_perference` ADD COLUMN `FCRINO` VARCHAR(30) AFTER `OTHER_DENOMINATION`,
 ...
 INSERT INTO `master_state` (`STATE_NAME`,`COUNTRY_ID`) VALUES
 (' Andhra Pradesh ',1),
...
ALTER TABLE `master_executive_committee` ADD COLUMN `CUSTOMERID` INTEGER UNSIGNED NOT NULL DEFAULT 0 AFTER `NOTES`;

-- Salamon 12-11-2014
ALTER TABLE `master_insti_perference` ADD COLUMN `LEDGER_ID` INTEGER UNSIGNED NOT NULL DEFAULT 0 AFTER `EIGHTYGNO`;

-- Sanjeev	on 12-11-2014

CREATE TABLE `master_audit_type` (
  `AUDIT_TYPE_ID` INTEGER UNSIGNED NOT NULL AUTO_INCREMENT,
  `AUDIT_TYPE` VARCHAR(50) NOT NULL DEFAULT '',
  PRIMARY KEY(`AUDIT_TYPE_ID`)
)
ENGINE = InnoDB;

ALTER TABLE `master_executive_committee` DROP COLUMN `STATE_ID`;

ALTER TABLE `master_donaud` DROP COLUMN `STATE_ID`;

ALTER TABLE `master_insti_perference`  DROP COLUMN `STATE_ID`;

ALTER TABLE  `master_donaud` ADD COLUMN `STATE_ID` INTEGER UNSIGNED AFTER `CUSTOMERID`;

ALTER TABLE  `master_insti_perference` MODIFY COLUMN `CUSTOMERID` INTEGER UNSIGNED NOT NULL,
 ADD COLUMN `STATE_ID` INTEGER UNSIGNED AFTER `LEDGER_ID`;


ALTER TABLE  `master_executive_committee` ADD COLUMN `STATE_ID` INTEGER UNSIGNED AFTER `CUSTOMERID`;

-- Sugan	on 12-11-2014

ALTER TABLE `master_project` ADD COLUMN `CONTRIBUTION_ID` INTEGER UNSIGNED NOT NULL DEFAULT 0 COMMENT 'FC6 Purpose Id' AFTER `CUSTOMERID`;

-- Sanjeev on 13-11-2014

ALTER TABLE  `master_auditing_info` CHANGE COLUMN `AUDIT_TYPE` `AUDIT_TYPE_ID` INTEGER UNSIGNED NOT NULL DEFAULT 0,
 ADD CONSTRAINT  `FK_AUDIT_TYPE_ID` FOREIGN KEY `FK_AUDIT_TYPE_ID` (`AUDIT_TYPE_ID`)
    REFERENCES `master_audit_type` (`AUDIT_TYPE_ID`)
    ON DELETE RESTRICT
    ON UPDATE RESTRICT;
...
-- Sanjeev on 14/11/2014
ALTER TABLE `master_audit_type` ADD UNIQUE `UNQ_AUDIT_TYPE`(`AUDIT_TYPE`);

ALTER TABLE `master_insti_perference` MODIFY COLUMN `CUSTOMERID` INTEGER UNSIGNED NOT NULL AUTO_INCREMENT;

-- Praveen on 17/11/2014

ALTER TABLE `master_insti_perference` MODIFY COLUMN `CUSTOMERID` INTEGER UNSIGNED NOT NULL DEFAULT 0,
 MODIFY COLUMN `ASSOCIATIONNATURE` VARCHAR(100) COMMENT '0-Cultural,1-Ecomomic,2-Educational,3-Religious,4-Social';
...
-- Michael R on 25-11-2014

ALTER TABLE  `master_insti_perference` MODIFY COLUMN `ASSOCIATIONNATURE` VARCHAR(15) CHARACTER SET utf8 COLLATE utf8_general_ci COMMENT '0-Cultural,1-Ecomomic,2-Educational,3-Religious,4-Social';


ALTER TABLE `master_donaud` CHANGE COLUMN `STATE` `STATE_ID` INTEGER UNSIGNED;

UPDATE MASTER_LEDGER SET IS_BANK_INTEREST_LEDGER=1 WHERE LEDGER_ID=10;
UPDATE MASTER_LEDGER SET ACCESS_FLAG=0 WHERE LEDGER_ID=11;

UPDATE VOUCHER_MASTER_TRANS SET CREATED_ON=VOUCHER_DATE,MODIFIED_ON=VOUCHER_DATE;

-- Michael R on 25-11-2014

ALTER TABLE  `master_insti_perference` MODIFY COLUMN `ASSOCIATIONNATURE` VARCHAR(15) CHARACTER SET utf8 COLLATE utf8_general_ci COMMENT '0-Cultural,1-Ecomomic,2-Educational,3-Religious,4-Social';

-- Michael R 05-12-2014

ALTER TABLE `master_headoffice_ledger` DROP FOREIGN KEY `MASTER_LEDGER_GROUP`;

ALTER TABLE `voucher_master_trans`
 ADD COLUMN `CLIENT_REFERENCE_ID` INTEGER UNSIGNED NOT NULL DEFAULT 0 AFTER `BRANCH_ID`,
 ADD COLUMN `CLIENT_CODE` VARCHAR(15)  DEFAULT '' AFTER `CLIENT_REFERENCE_ID`;

UPDATE MASTER_LEDGER_GROUP SET MAIN_GROUP_ID=4 WHERE GROUP_ID=31;
...
-- Michael R on 23-01-2015
update
    voucher_master_trans
    set created_on=voucher_date ,modified_on=voucher_date where created_on is null and modified_on is null;



-- Michael R on 10-02-2015
  --  drop table tds_company_deductors;
  -- drop table tds_booking;
  -- drop table tds_booking_detail;
  -- drop table tds_credtiors_profile;
  -- drop table tds_deduction;
  -- drop table tds_deduction_detail;
  -- drop table tds_party_payment;
  -- drop table tds_party_payment_detail;
  -- drop table tds_payment;
  -- drop table tds_payment_detail;
  -- drop table tds_tax_rate;
  -- drop table tds_policy;
  -- drop table tds_nature_payment;
  -- drop table tds_duty_taxtype;
  -- drop table tds_dedutee_type;
  -- drop table tds_deductee_type;
  -- drop table tds_section;
...
ALTER TABLE `headoffice_mapped_ledger` ADD PRIMARY KEY(`LEDGER_ID`, `HEADOFFICE_LEDGER_ID`);

-- Salamon 16-02-2015
ALTER TABLE `master_bank` MODIFY COLUMN `BANK_CODE` VARCHAR(10) CHARACTER SET utf8 COLLATE utf8_general_ci
, DROP INDEX `UNQ_BANK_CODE`;
...
ALTER TABLE `master_cost_centre` MODIFY COLUMN `ABBREVATION` VARCHAR(10) CHARACTER SET utf8 COLLATE utf8_general_ci
, DROP INDEX `UNQ_ABBREVATION`;

ALTER TABLE `master_ledger` MODIFY COLUMN `LEDGER_CODE` VARCHAR(15) CHARACTER SET utf8 COLLATE utf8_general_ci
, DROP INDEX `UNQ_ABBRE`;

ALTER TABLE `master_ledger_group` MODIFY COLUMN `GROUP_CODE` VARCHAR(10) CHARACTER SET utf8 COLLATE utf8_general_ci
, DROP INDEX `UNQ_ABBRE`;

ALTER TABLE `master_project` MODIFY COLUMN `PROJECT_CODE` VARCHAR(15) CHARACTER SET utf8 COLLATE utf8_general_ci
, DROP INDEX `unq_ProjectCode`;

update master_ledger set access_flag=0;

-- Michael R on 19-02-2015
CREATE TABLE `Master_Lock_Type` (
  `LOCK_TYPE_ID` INTEGER UNSIGNED NOT NULL AUTO_INCREMENT,
...
CREATE TABLE `Master_Lock_Trans` (
  `LOCK_TRANS_ID` INTEGER UNSIGNED NOT NULL AUTO_INCREMENT,
  `LOCK_TYPE_ID` INTEGER UNSIGNED NOT NULL DEFAULT 0,
  `PROJECT_ID` INTEGER UNSIGNED NOT NULL DEFAULT 0,
  ...
  CONSTRAINT `PK_Master_Lock_Trans` FOREIGN KEY `PK_Master_Lock_Trans` (`LOCK_TYPE_ID`)
    REFERENCES `master_lock_type` (`LOCK_TYPE_ID`)
    ON DELETE RESTRICT
    ON UPDATE RESTRICT
)
ENGINE = InnoDB;

-- Michael R on 24-02-2015
ALTER TABLE `voucher_master_trans` ADD CONSTRAINT `FK_voucher_master_trans_PROJECT_ID` FOREIGN KEY `FK_voucher_master_trans_PROJECT_ID` (`PROJECT_ID`)
    REFERENCES `master_project` (`PROJECT_ID`)
    ON DELETE RESTRICT
    ON UPDATE RESTRICT;

 -- Added by Carmel Raj on 07-05-2015
UPDATE MASTER_NATURE SET NATURE='Incomes' WHERE NATURE='Income';

UPDATE MASTER_LEDGER_GROUP SET LEDGER_GROUP='Incomes' WHERE LEDGER_GROUP='Income';

UPDATE MASTER_LEDGER_GROUP SET LEDGER_GROUP='Duties & Taxes' WHERE LEDGER_GROUP='Duties and Taxes';

update activitiy_rights set parent_id=89 where parent_id=21;
update activitiy_rights set parent_id=205 where parent_id=1;
update activitiy_rights set parent_id=89 where activity_id=5;
update activitiy_rights set parent_id=205 where parent_id=151;
delete from activitiy_rights where activity_id in(1,21,151);
update activitiy_rights set parent_id=89 where activity_id in(208,209);
update activitiy_rights set object_name='Map Accounts' where activity_id in(5);
update activitiy_rights set object_name='Refresh Ledger Balance' where activity_id in(208);
update activitiy_rights set object_name='Regenarate Voucher Number' where activity_id in(209);
update activitiy_rights set object_name='Settings' where activity_id in(2);
update activitiy_rights set object_name='Manage User Security' where activity_id in(164);
update activitiy_rights set object_name='User Rights' where activity_id in(162);
update activitiy_rights set object_name='Utilites' where activity_id in(205);
update activitiy_rights set object_name='Migration' where activity_id in(212);


ALTER TABLE `budget_ledger` ADD COLUMN `NARRATION` VARCHAR(150) AFTER `TRANS_MODE`;

ALTER TABLE `master_bank_account` ADD COLUMN `BRANCH_ID` INTEGER UNSIGNED NOT NULL DEFAULT 0 AFTER `IS_FCRA_ACCOUNT`;

ALTER TABLE `voucher_master_trans` ADD COLUMN `CREATED_BY_NAME` VARCHAR(100) NOT NULL DEFAULT '' AFTER `CLIENT_CODE`,
 ADD COLUMN `MODIFIED_BY_NAME` VARCHAR(100) NOT NULL DEFAULT '' AFTER `CREATED_BY_NAME`;

 ALTER TABLE `tds_company_deductors` ADD COLUMN `FULL_NAME` VARCHAR(500) NOT NULL DEFAULT '' AFTER `EMAIL`;

 UPDATE TDS_SECTION SET CODE='94C' WHERE TDS_SECTION_ID=33 AND SECTION_NAME='Contractors & Sub-Contractors';

 UPDATE TDS_NATURE_PAYMENT SET PAYMENT_CODE='94C' WHERE NATURE_PAY_ID=32 AND NAME='Payments to Contractors(Other than Advertisements)';

 
 ALTER TABLE `voucher_trans` ADD COLUMN `LOCATION_ID` INTEGER UNSIGNED NOT NULL DEFAULT 0 AFTER `BRANCH_ID`,
 DROP PRIMARY KEY,
 ADD PRIMARY KEY(`VOUCHER_ID`, `SEQUENCE_NO`, `BRANCH_ID`,`LOCATION_ID`);
 
 ALTER TABLE `voucher_master_trans` ADD COLUMN `LOCATION_ID` INTEGER UNSIGNED NOT NULL DEFAULT 0 AFTER `BRANCH_ID`,
 DROP PRIMARY KEY,
 ADD PRIMARY KEY(`VOUCHER_ID`,`BRANCH_ID`, `LOCATION_ID`);

 ALTER TABLE `voucher_cc_trans` ADD COLUMN `LOCATION_ID` INTEGER UNSIGNED NOT NULL DEFAULT 0 AFTER `BRANCH_ID`;


ALTER TABLE `fd_account` ADD COLUMN `LOCATION_ID` INTEGER UNSIGNED NOT NULL DEFAULT 0 AFTER `BRANCH_ID`,
 DROP PRIMARY KEY,
 ADD PRIMARY KEY(`FD_ACCOUNT_ID`, `BRANCH_ID`, `LOCATION_ID`);
...
ALTER TABLE `fd_renewal` ADD COLUMN `LOCATION_ID` INTEGER UNSIGNED NOT NULL DEFAULT 0 AFTER `BRANCH_ID`,
 ...
 UPDATE activitiy_rights SET ENUMTYPE='CreateVoucher' WHERE ACTIVITY_ID=48;


INSERT INTO ACTIVITIY_RIGHTS (ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)
VALUES(266,90,'Insert Receipt Voucher','InsertReceiptVoucher','Finance','Receipt');
INSERT INTO ACTIVITIY_RIGHTS (ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)
VALUES(267,97,'Insert Payment Voucher','InsertPaymentVoucher','Finance','Payment');
INSERT INTO ACTIVITIY_RIGHTS (ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)
VALUES(268,103,'Insert Contra Voucher','InsertContraVoucher','Finance','Contra');
INSERT INTO ACTIVITIY_RIGHTS (ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)
VALUES(269,90,'Show Nagative Balance','ShowNagativeBalance','Finance','Receipt');
INSERT INTO ACTIVITIY_RIGHTS (ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)
VALUES(270,97,'Show Nagative Balance','ShowNagativeBalance','Finance','Payment');
INSERT INTO ACTIVITIY_RIGHTS (ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)
VALUES(271,103,'Show Nagative Balance','ShowNagativeBalance','Finance','Contra');

update ACTIVITIY_RIGHTS SET OBJECT_SUB_TYPE='Budget Period' WHERE ACTIVITY_ID IN(121,122,123,124,125,188);

-- Michael  R on 13-07-2015
UPDATE ACTIVITIY_RIGHTS SET PARENT_ID=272 WHERE ACTIVITY_ID IN(67,77,16,72);
UPDATE ACTIVITIY_RIGHTS SET OBJECT_NAME ='Governing Bodies' WHERE ACTIVITY_ID=72;
UPDATE ACTIVITIY_RIGHTS SET OBJECT_NAME ='Create Governing Bodies' WHERE ACTIVITY_ID=73;
UPDATE ACTIVITIY_RIGHTS SET OBJECT_NAME ='Edit Governing Bodies' WHERE ACTIVITY_ID=74;
UPDATE ACTIVITIY_RIGHTS SET OBJECT_NAME ='Delete Governing Bodies' WHERE ACTIVITY_ID=75;
UPDATE ACTIVITIY_RIGHTS SET OBJECT_NAME ='Print Governing Bodies' WHERE ACTIVITY_ID=76;
UPDATE ACTIVITIY_RIGHTS SET OBJECT_NAME ='View Governing Bodies' WHERE ACTIVITY_ID=180;

-- Salamon 16.07.2015
ALTER TABLE `costcategory_costcentre` ADD PRIMARY KEY(`COST_CATEGORY_ID`, `COST_CENTRE_ID`);


-- Michael R on 21-07-2015
INSERT INTO `activitiy_rights` (`ACTIVITY_ID`,`PARENT_ID`,`OBJECT_NAME`,`ENUMTYPE`,`OBJECT_TYPE`,`OBJECT_SUB_TYPE`) VALUES 
...
INSERT INTO `activitiy_rights` (`ACTIVITY_ID`,`PARENT_ID`,`OBJECT_NAME`,`ENUMTYPE`,`OBJECT_TYPE`,`OBJECT_SUB_TYPE`) VALUES 
 ...
 UPDATE ACTIVITIY_RIGHTS SET ENUMTYPE='ShowReceiptNagativeBalance' WHERE ACTIVITY_ID IN(269);
UPDATE ACTIVITIY_RIGHTS SET ENUMTYPE='ShowPaymentNagativeBalance' WHERE ACTIVITY_ID IN(270);
UPDATE ACTIVITIY_RIGHTS SET ENUMTYPE='ShowContraNagativeBalance' WHERE ACTIVITY_ID IN(271);


-- Michael R on 28-07-2015
UPDATE activitiy_rights SET OBJECT_NAME ='Budget Period' WHERE ACTIVITY_ID =121;
UPDATE activitiy_rights SET OBJECT_NAME ='Create Budget Period' WHERE ACTIVITY_ID =122;
UPDATE activitiy_rights SET OBJECT_NAME ='Edit Budget Period' WHERE ACTIVITY_ID =123;
UPDATE activitiy_rights SET OBJECT_NAME ='Delete Budget Period' WHERE ACTIVITY_ID =124;
UPDATE activitiy_rights SET OBJECT_NAME ='Print Budget Period' WHERE ACTIVITY_ID =125;
UPDATE activitiy_rights SET OBJECT_NAME ='View Budget Period' WHERE ACTIVITY_ID =188;

-- Michael R  on 30-07-2015
ALTER TABLE  `fd_renewal` DROP FOREIGN KEY `FK_FD_ACCOUNT_ID`;

ALTER TABLE  `fd_renewal` DROP INDEX `IndexFDACCOUNT`;


ALTER TABLE  `fd_renewal` DROP PRIMARY KEY,
 ADD PRIMARY KEY(`FD_RENEWAL_ID`);

ALTER TABLE  `fd_account` DROP PRIMARY KEY,
 ADD PRIMARY KEY(`FD_ACCOUNT_ID`);
...
CREATE TABLE `portal_amendment_message` (
  ...
  `LEDGER_NAME` varchar(45) NOT NULL DEFAULT '',
  `PROJECT` varchar(45) NOT NULL DEFAULT '',
  `VOUCHER_NO` int(10) unsigned NOT NULL DEFAULT '0',
  `VOUCHER_TYPE` varchar(45) NOT NULL DEFAULT '',
...
CREATE TABLE  `portal_datasyn_message` (
 ...
 DELETE FROM MASTER_LEDGER_GROUP WHERE LEDGER_GROUP='Duties & Taxes' and group_id <> 24;
update MASTER_LEDGER_GROUP set LEDGER_GROUP='Duties & Taxes' where group_id=24;

-- Praveen 11-08-2015 This is to make the columns NULL value
ALTER TABLE `portal_amendment_message` MODIFY COLUMN `PROJECT` VARCHAR(100);
ALTER TABLE `portal_amendment_message` MODIFY COLUMN `VOUCHER_ID` INTEGER UNSIGNED;
ALTER TABLE `portal_amendment_message` MODIFY COLUMN `AMENDMENT_DATE` DATETIME;
ALTER TABLE `portal_amendment_message` MODIFY COLUMN `VOUCHER_DATE` DATETIME;
ALTER TABLE `portal_amendment_message` MODIFY COLUMN `LEDGER_NAME` VARCHAR(45);
ALTER TABLE `portal_amendment_message` MODIFY COLUMN `VOUCHER_NO` INTEGER UNSIGNED;
ALTER TABLE `portal_amendment_message` MODIFY COLUMN `VOUCHER_TYPE` VARCHAR(45);
ALTER TABLE `portal_amendment_message` MODIFY COLUMN `AMOUNT` INTEGER UNSIGNED;
...
ALTER TABLE `fd_renewal` DROP INDEX `IndexFDACCOUNT`,
 DROP FOREIGN KEY `FK_FD_ACCOUNT_ID`;
...
ALTER TABLE `fd_account` DROP PRIMARY KEY,
 ...
 -- Salamon 21.09.2015
ALTER TABLE `master_headoffice_ledger` DROP INDEX `UNQ_ABBRE`;
...
UPDATE FD_RENEWAL SET FD_TYPE=
CASE WHEN RENEWAL_TYPE='IRI' OR RENEWAL_TYPE='ACI' THEN
'RN'
ELSE 'WD' END WHERE FD_TYPE='';

-- Praveen 02-11-2015

UPDATE MASTER_LEDGER_GROUP SET LEDGER_GROUP='', ACCESS_FLAG=2
WHERE GROUP_ID=14;
UPDATE MASTER_LEDGER_GROUP SET LEDGER_GROUP='Deposits (Asset)', ACCESS_FLAG=0
WHERE GROUP_ID=49;
UPDATE MASTER_LEDGER_GROUP SET LEDGER_GROUP='Fixed Deposits', ACCESS_FLAG=2
WHERE GROUP_ID=14;


-- Praveen 31-10-2015

ALTER TABLE `master_voucher` ADD COLUMN `IS_NARRATION_ENABLED` INTEGER UNSIGNED NOT NULL DEFAULT 0 AFTER `NOTE`;

ALTER TABLE `voucher_trans` ADD COLUMN `NARRATION` VARCHAR(500) AFTER `STATUS`;

UPDATE MASTER_LEDGER_GROUP SET MAIN_GROUP_ID=PARENT_GROUP_ID WHERE PARENT_GROUP_ID>0;

ALTER TABLE `master_donaud` MODIFY COLUMN `NAME` VARCHAR(150) CHARACTER SET utf8 COLLATE utf8_general_ci;

DROP TABLE IF EXISTS INKINDTRANS;

ALTER TABLE `master_headoffice_ledger` DROP FOREIGN KEY `FK_GROUP_ID`;

UPDATE MASTER_BANK_ACCOUNT MBA, MASTER_LEDGER ML SET MBA.ACCOUNT_NUMBER=ML.LEDGER_NAME WHERE MBA.LEDGER_ID=ML.LEDGER_ID AND ML.GROUP_ID=12;
...
-- (Donor Form DB Changes) - 28.01.2016
ALTER TABLE `master_donaud` ADD COLUMN `INSTITUTIONAL_TYPE_ID` INTEGER UNSIGNED NOT NULL DEFAULT 0 AFTER `STATE_ID`,
 ADD COLUMN `REGISTRATION_TYPE_ID` INTEGER UNSIGNED NOT NULL DEFAULT 0 AFTER `INSTITUTIONAL_TYPE_ID`,
 ADD COLUMN `TITLE` VARCHAR(20) AFTER `REGISTRATION_TYPE_ID`,
 ADD COLUMN `GENDER` INTEGER UNSIGNED COMMENT 'MALE FEMALE' AFTER `TITLE`,
 ADD COLUMN `DOB` DATETIME AFTER `GENDER`,
 ADD COLUMN `LANGUAGE` VARCHAR(100) AFTER `DOB`,
 ADD COLUMN `RELIGION` VARCHAR(100) AFTER `LANGUAGE`,
 ADD COLUMN `REFERRED_STAFF` VARCHAR(100) AFTER `RELIGION`,
 ADD COLUMN `OCCUPATION` VARCHAR(100) AFTER `REFERRED_STAFF`,
 ADD COLUMN `ORG_EMPLOYED` VARCHAR(100) AFTER `OCCUPATION`,
 ADD COLUMN `STATUS` INTEGER UNSIGNED NOT NULL DEFAULT 1 AFTER `ORG_EMPLOYED`,
 ADD COLUMN `PAYMENT_MODE_ID` INTEGER UNSIGNED NOT NULL DEFAULT 0 AFTER `STATUS`,
 ADD COLUMN `MARITAL_STATUS` VARCHAR(100) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL DEFAULT '0' AFTER `PAYMENT_MODE_ID`,
 ADD COLUMN `ANNIVERSARY_DATE` VARCHAR(100) AFTER `MARITAL_STATUS`,
 ADD COLUMN `REASON_FOR_ACTIVE` VARCHAR(200) AFTER `ANNIVERSARY_DATE`,
 ADD COLUMN `LASTNAME` VARCHAR(100) AFTER `REASON_FOR_ACTIVE`,
 ADD COLUMN `CURRENT_BIRTHDAY_DATE_EMAIL` DATETIME AFTER `LASTNAME`,
 ADD COLUMN `BIRTHDAY_WISHING_DATE_EMAIL` DATETIME AFTER `CURRENT_BIRTHDAY_DATE_EMAIL`,
 ADD COLUMN `CURRENT_MARRIAGE_DATE_EMAIL` DATETIME AFTER `BIRTHDAY_WISHING_DATE_EMAIL`,
 ADD COLUMN `MARRIAGE_WISHING_DATE_EMAIL` DATETIME AFTER `CURRENT_MARRIAGE_DATE_EMAIL`,
 ADD COLUMN `APPEAL_SENT_DATE_EMAIL` DATETIME AFTER `MARRIAGE_WISHING_DATE_EMAIL`,
 ADD COLUMN `CURRENT_BIRTHDAY_DATE_SMS` DATETIME AFTER `APPEAL_SENT_DATE_EMAIL`,
 ADD COLUMN `BIRTHDAY_WISHING_DATE_SMS` DATETIME AFTER `CURRENT_BIRTHDAY_DATE_SMS`,
 ADD COLUMN `CURRENT_MARRIAGE_DATE_SMS` DATETIME AFTER `BIRTHDAY_WISHING_DATE_SMS`,
 ADD COLUMN `MARRIAGE_WISHING_DATE_SMS` DATETIME AFTER `CURRENT_MARRIAGE_DATE_SMS`,
 ADD COLUMN `APPEAL_SENT_DATE_SMS` DATETIME AFTER `MARRIAGE_WISHING_DATE_SMS`;

ALTER TABLE `master_donaud` MODIFY COLUMN `NAME` VARCHAR(100) CHARACTER SET utf8 COLLATE utf8_general_ci,
 MODIFY COLUMN `ADDRESS` VARCHAR(300) CHARACTER SET utf8 COLLATE utf8_general_ci,
 ADD UNIQUE `UNQ_NAMEADDRESS`(`NAME`, `LASTNAME`, `COUNTRY_ID`, `PLACE`);
...
INSERT INTO `master_donaud_reg_type`(REGISTRATION_TYPE_ID, REGISTRATION_TYPE)
VALUES(1,'ECS') ON DUPLICATE KEY UPDATE REGISTRATION_TYPE_ID=1,REGISTRATION_TYPE='ECS';

INSERT INTO `master_donaud_reg_type`(REGISTRATION_TYPE_ID, REGISTRATION_TYPE)
VALUES(2,'Company') ON DUPLICATE KEY UPDATE REGISTRATION_TYPE_ID=2,REGISTRATION_TYPE='Company';

INSERT INTO `master_donaud_reg_type`(REGISTRATION_TYPE_ID, REGISTRATION_TYPE)
VALUES(3,'Educational Institution') ON DUPLICATE KEY UPDATE REGISTRATION_TYPE_ID=3,REGISTRATION_TYPE='Educational Institution';

INSERT INTO `master_donaud_reg_type`(REGISTRATION_TYPE_ID, REGISTRATION_TYPE)
VALUES(4,'Religious Reference') ON DUPLICATE KEY UPDATE REGISTRATION_TYPE_ID=4,REGISTRATION_TYPE='Religious Reference';

INSERT INTO `master_donaud_reg_type`(REGISTRATION_TYPE_ID, REGISTRATION_TYPE)
VALUES(5,'Foreign') ON DUPLICATE KEY UPDATE REGISTRATION_TYPE_ID=5,REGISTRATION_TYPE='Foreign';
...
INSERT INTO `master_donaud_ins_type`(INSTITUTIONAL_TYPE_ID, INSTITUTIONAL_TYPE)
VALUES(1,'School') ON DUPLICATE KEY UPDATE INSTITUTIONAL_TYPE_ID=1,INSTITUTIONAL_TYPE='School';

INSERT INTO `master_donaud_ins_type`(INSTITUTIONAL_TYPE_ID, INSTITUTIONAL_TYPE)
VALUES(2,'College') ON DUPLICATE KEY UPDATE INSTITUTIONAL_TYPE_ID=2,INSTITUTIONAL_TYPE='College';

INSERT INTO `master_donaud_ins_type`(INSTITUTIONAL_TYPE_ID, INSTITUTIONAL_TYPE)
VALUES(3,'Non-Government Organization') ON DUPLICATE KEY UPDATE INSTITUTIONAL_TYPE_ID=3,INSTITUTIONAL_TYPE='Non-Government Organization';

INSERT INTO `master_donaud_ins_type`(INSTITUTIONAL_TYPE_ID, INSTITUTIONAL_TYPE)
VALUES(4,'Corporate') ON DUPLICATE KEY UPDATE INSTITUTIONAL_TYPE_ID=4,INSTITUTIONAL_TYPE='Corporate';

INSERT INTO `master_donaud_ins_type`(INSTITUTIONAL_TYPE_ID, INSTITUTIONAL_TYPE)
VALUES(5,'Religious Community') ON DUPLICATE KEY UPDATE INSTITUTIONAL_TYPE_ID=5,INSTITUTIONAL_TYPE='Religious Community';

INSERT INTO `master_donaud_ins_type`(INSTITUTIONAL_TYPE_ID, INSTITUTIONAL_TYPE)
VALUES(6,'Parish') ON DUPLICATE KEY UPDATE INSTITUTIONAL_TYPE_ID=6,INSTITUTIONAL_TYPE='Parish';
...
INSERT INTO `master_donaud_pay_mode`(PAYMENT_MODE_ID, PAYMENT_MODE)
VALUES(1,'Cash') ON DUPLICATE KEY UPDATE PAYMENT_MODE_ID=1,PAYMENT_MODE='Cash';

INSERT INTO `master_donaud_pay_mode`(PAYMENT_MODE_ID, PAYMENT_MODE)
VALUES(2,'Cheque') ON DUPLICATE KEY UPDATE PAYMENT_MODE_ID=2,PAYMENT_MODE='Cheque';

INSERT INTO `master_donaud_pay_mode`(PAYMENT_MODE_ID, PAYMENT_MODE)
VALUES(3,'Demand Draft') ON DUPLICATE KEY UPDATE PAYMENT_MODE_ID=3,PAYMENT_MODE='Demand Draft';

INSERT INTO `master_donaud_pay_mode`(PAYMENT_MODE_ID, PAYMENT_MODE)
VALUES(4,'Direct Debit') ON DUPLICATE KEY UPDATE PAYMENT_MODE_ID=4,PAYMENT_MODE='Direct Debit';


-- Payroll tables and DB Changes------------------------------------------------------------------------------------------------------------------------------------
...
INSERT INTO `prcomponent` (`COMPONENTID`,`COMPONENT`,`DESCRIPTION`,`TYPE`,`DEFVALUE`,`LINKVALUE`,`EQUATION`,`EQUATIONID`,`MAXSLAP`,`COMPROUND`,`IFCONDITION`,`SHOWINBROWSE`,`RELATEDCOMPONENTS`,`ISEDITABLE`,`ACCESS_FLAG`) VALUES 
 (13,'BASIC','Basic Pay','0','0','Basicpay','','','0.00','1.00','0',0,'',0,1),
 (14,'DA','Dearness Allowance','0','1653','','','','0.00','1.00','0',0,'',0,1),
 (15,'HRA','House Rent Allowance','0','0','','<BASIC>*40/100','<13>*40/100~0~0','0.00','1.00','1',0,'ê13ê',0,1),
 (16,'PF WAGES','Basic and Dearness Allowance','1','0','','<BASIC>+<DA>','<13>+<14>~0~0','0.00','1.00','1',0,'ê13ê14ê',0,1),
 (17,'PF','Provident Fund','1','0','','<PF WAGES>*12/100','<16>*12/100~0~0','0.00','1.00','1',0,'ê16ê',0,1),
 (18,'PT','Professional Tax','1','0','','','','0.00','1.00','0',0,'',0,1),
 (19,'GROSS WAGES','GROSS SALARY','0','0','','<BASIC>+<DA>+<HRA>','<13>+<14>+<15>~0~0','0.00','1.00','1',0,'ê13ê14ê15ê',0,1),
 (20,'DEDUCTIONS','DEDUCTIONS','1','0','','<PF>+<PT>','<17>+<18>~0~0','0.00','1.00','1',0,'ê17ê18ê',0,1),
 (21,'NETPAY','Net Payable Amount to the Employee','0','0','','<GROSS WAGES>-<DEDUCTIONS>','<19>-<20>~0~0','0.00','1.00','1',0,'ê19ê20ê',0,1);
INSERT INTO `prcomponent` (`COMPONENTID`,`COMPONENT`,`DESCRIPTION`,`TYPE`,`DEFVALUE`,`LINKVALUE`,`EQUATION`,`EQUATIONID`,`MAXSLAP`,`COMPROUND`,`IFCONDITION`,`SHOWINBROWSE`,`RELATEDCOMPONENTS`,`ISEDITABLE`,`ACCESS_FLAG`) VALUES
 (22,'NAME','Name of the Employee','2','0','Name','','','0.00','1.00','0',0,'',0,1),
 (23,'DESIGNATION','DESIGNATION','2','0','Designation','','','0.00','1.00','0',0,'',0,1);
/*!40000 ALTER TABLE `prcomponent` ENABLE KEYS */;


INSERT INTO `master_ledger` (`LEDGER_ID`,`LEDGER_CODE`,`LEDGER_NAME`,`GROUP_ID`,`LEDGER_TYPE`,`LEDGER_SUB_TYPE`,`BANK_ACCOUNT_ID`,`IS_COST_CENTER`,`IS_TDS_LEDGER`,`NOTES`,`IS_BANK_INTEREST_LEDGER`,`SORT_ID`,`STATUS`,`ACCESS_FLAG`,`IS_BRANCH_LEDGER`) VALUES
 (1001,'1001','Staff Loan',11,'GN','GN',0,0,0,'',0,0,0,0,0);

INSERT INTO `master_ledger` (`LEDGER_ID`,`LEDGER_CODE`,`LEDGER_NAME`,`GROUP_ID`,`LEDGER_TYPE`,`LEDGER_SUB_TYPE`,`BANK_ACCOUNT_ID`,`IS_COST_CENTER`,`IS_TDS_LEDGER`,`NOTES`,`IS_BANK_INTEREST_LEDGER`,`SORT_ID`,`STATUS`,`ACCESS_FLAG`,`IS_BRANCH_LEDGER`) VALUES
 (1002,'1002','Interest on Staff Loan',5,'GN','GN',0,0,0,'',0,0,0,0,0);
...
CREATE TABLE `payroll_range_formula` (
  ...
  `COMPONENTID` INTEGER UNSIGNED NOT NULL DEFAULT 0,
  `LINK_COMPONENT_ID` INTEGER UNSIGNED NOT NULL DEFAULT 0,
...
UPDATE PRCOMPONENT SET relatedcomponents =replace(relatedcomponents,'Ãª','ê');
...
ALTER TABLE `payroll_range_formula` DROP COLUMN `ID`,
 DROP PRIMARY KEY;
...
ALTER TABLE `stfpersonal` ADD COLUMN `YOS` DOUBLE AFTER `ACCOUNT_NUMBER`;
ALTER TABLE `stfpersonal` MODIFY COLUMN `YOS` DECIMAL(15,2);
ALTER TABLE `stfpersonal` MODIFY COLUMN `YOS` FLOAT;

INSERT INTO `princome` (`INCOME_ID`,`INCOME_NAME`) VALUES
 (2,'YOS');

 -- Network specific tables and DB Changes-------------------------------------------------------------------------------------------------------------------
...
INSERT INTO `master_donor_letters` (`LETTER_TYPE_ID`,`LETTER_NAME`) VALUES
 (1,'Thanksgiving'),
 (2,'Appeal'),
 (3,'NewsLetter'),
 (4,'Anniversary'),
...
ALTER TABLE `voucher_master_trans` ADD COLUMN `DONOR_MAIL_STATUS` INTEGER UNSIGNED NOT NULL DEFAULT 0 AFTER `CLIENT_CODE`,
 ADD COLUMN `DONOR_MAIL_SENT_DATE` DATETIME AFTER `DONOR_MAIL_STATUS`,
 ADD COLUMN `DONOR_SMS_STATUS` INTEGER UNSIGNED NOT NULL DEFAULT 0 AFTER `DONOR_MAIL_SENT_DATE`,
 ADD COLUMN `DONOR_SMS_SENT_DATE` DATETIME AFTER `DONOR_SMS_STATUS`;

 INSERT INTO ACTIVITIY_RIGHTS(ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)VALUES(312,0,'Network','Network','Network','Network') ON DUPLICATE KEY UPDATE PARENT_ID= 0,OBJECT_NAME='Network',ENUMTYPE='Network',OBJECT_TYPE='Network',OBJECT_SUB_TYPE='Network';
INSERT INTO ACTIVITIY_RIGHTS(ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)VALUES(313,312,'Prospect','Prospect','Network','Prospect') ON DUPLICATE KEY UPDATE PARENT_ID= 312,OBJECT_NAME='Prospect',ENUMTYPE='Prospect',OBJECT_TYPE='Network',OBJECT_SUB_TYPE='Prospect';
INSERT INTO ACTIVITIY_RIGHTS(ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)VALUES(314,313,'Create Prospect','CreateProspect','Network','Prospect') ON DUPLICATE KEY UPDATE PARENT_ID= 313,OBJECT_NAME='Create Prospect',ENUMTYPE='CreateProspect',OBJECT_TYPE='Network',OBJECT_SUB_TYPE='Prospect';
INSERT INTO ACTIVITIY_RIGHTS(ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)VALUES(315,313,'Edit Prospect','EditProspect','Network','Prospect') ON DUPLICATE KEY UPDATE PARENT_ID= 313,OBJECT_NAME='Edit Prospect',ENUMTYPE='EditProspect',OBJECT_TYPE='Network',OBJECT_SUB_TYPE='Prospect';
INSERT INTO ACTIVITIY_RIGHTS(ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)VALUES(316,313,'Delete Prospect','DeleteProspect','Network','Prospect') ON DUPLICATE KEY UPDATE PARENT_ID= 313,OBJECT_NAME='Delete Prospect',ENUMTYPE='DeleteProspect',OBJECT_TYPE='Network',OBJECT_SUB_TYPE='Prospect';
INSERT INTO ACTIVITIY_RIGHTS(ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)VALUES(317,313,'Print Prospect','PrintProspect','Network','Prospect') ON DUPLICATE KEY UPDATE PARENT_ID= 313,OBJECT_NAME='Print Prospect',ENUMTYPE='PrintProspect',OBJECT_TYPE='Network',OBJECT_SUB_TYPE='Prospect';
INSERT INTO ACTIVITIY_RIGHTS(ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)VALUES(318,313,'View Prospect','ViewProspect','Network','Prospect') ON DUPLICATE KEY UPDATE PARENT_ID= 313,OBJECT_NAME='View Prospect',ENUMTYPE='ViewProspect',OBJECT_TYPE='Network',OBJECT_SUB_TYPE='Prospect';
INSERT INTO ACTIVITIY_RIGHTS(ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)VALUES(319,313,'Convert Prospect','ConvertProspect','Network','Prospect') ON DUPLICATE KEY UPDATE PARENT_ID= 313,OBJECT_NAME='Convert Prospect',ENUMTYPE='ConvertProspect',OBJECT_TYPE='Network',OBJECT_SUB_TYPE='Prospect';
INSERT INTO ACTIVITIY_RIGHTS(ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)VALUES(320,313,'Import Prospect','ImportProspect','Network','Prospect') ON DUPLICATE KEY UPDATE PARENT_ID= 313,OBJECT_NAME='Import Prospect',ENUMTYPE='ImportProspect',OBJECT_TYPE='Network',OBJECT_SUB_TYPE='Prospect';
INSERT INTO ACTIVITIY_RIGHTS(ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)VALUES(321,312,'Mail Template','MailTemplate','Network','Mail Template') ON DUPLICATE KEY UPDATE PARENT_ID= 312,OBJECT_NAME='Mail Template',ENUMTYPE='MailTemplate',OBJECT_TYPE='Network',OBJECT_SUB_TYPE='Mail Template';
INSERT INTO ACTIVITIY_RIGHTS(ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)VALUES(322,321,'Create/Modify Thanksgivng','CreateThanksgivingMail','Network','Mail Template') ON DUPLICATE KEY UPDATE PARENT_ID= 321,OBJECT_NAME='Create/Modify Thanksgivng',ENUMTYPE='CreateThanksgivingMail',OBJECT_TYPE='Network',OBJECT_SUB_TYPE='Mail Template';
INSERT INTO ACTIVITIY_RIGHTS(ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)VALUES(323,321,'Create/Modify Appeal','CreateAppealMail','Network','Mail Template') ON DUPLICATE KEY UPDATE PARENT_ID= 321,OBJECT_NAME='Create/Modify Appeal',ENUMTYPE='CreateAppealMail',OBJECT_TYPE='Network',OBJECT_SUB_TYPE='Mail Template';
INSERT INTO ACTIVITIY_RIGHTS(ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)VALUES(324,321,'Create/Modify Newsletter','CreateNewsletterMail','Network','Mail Template') ON DUPLICATE KEY UPDATE PARENT_ID= 321,OBJECT_NAME='Create/Modify Newsletter',ENUMTYPE='CreateNewsletterMail',OBJECT_TYPE='Network',OBJECT_SUB_TYPE='Mail Template';
INSERT INTO ACTIVITIY_RIGHTS(ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)VALUES(325,321,'Create/Modify Anniversary','CreateAnniversaryMail','Network','Mail Template') ON DUPLICATE KEY UPDATE PARENT_ID= 321,OBJECT_NAME='Create/Modify Anniversary',ENUMTYPE='CreateAnniversaryMail',OBJECT_TYPE='Network',OBJECT_SUB_TYPE='Mail Template';
INSERT INTO ACTIVITIY_RIGHTS(ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)VALUES(326,321,'Create/Modify Feast Day','CreateFeastDayMail','Network','Mail Template') ON DUPLICATE KEY UPDATE PARENT_ID= 321,OBJECT_NAME='Create/Modify Feast Day',ENUMTYPE='CreateFeastDayMail',OBJECT_TYPE='Network',OBJECT_SUB_TYPE='Mail Template';
INSERT INTO ACTIVITIY_RIGHTS(ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)VALUES(327,312,'SMS Template','SMSTemplate','Network','SMS Template') ON DUPLICATE KEY UPDATE PARENT_ID= 312,OBJECT_NAME='SMS Template',ENUMTYPE='SMSTemplate',OBJECT_TYPE='Network',OBJECT_SUB_TYPE='SMS Template';
INSERT INTO ACTIVITIY_RIGHTS(ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)VALUES(328,327,'Create/Modify Thanksgivng','CreateThanksgivingSMS','Network','SMS Template') ON DUPLICATE KEY UPDATE PARENT_ID= 327,OBJECT_NAME='Create/Modify Thanksgivng',ENUMTYPE='CreateThanksgivingSMS',OBJECT_TYPE='Network',OBJECT_SUB_TYPE='SMS Template';
INSERT INTO ACTIVITIY_RIGHTS(ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)VALUES(329,327,'Create/Modify Appeal','CreateAppealSMS','Network','SMS Template') ON DUPLICATE KEY UPDATE PARENT_ID= 327,OBJECT_NAME='Create/Modify Appeal',ENUMTYPE='CreateAppealSMS',OBJECT_TYPE='Network',OBJECT_SUB_TYPE='SMS Template';
INSERT INTO ACTIVITIY_RIGHTS(ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)VALUES(330,327,'Create/Modify Anniversary','CreateAnniversarySMS','Network','SMS Template') ON DUPLICATE KEY UPDATE PARENT_ID= 327,OBJECT_NAME='Create/Modify Anniversary',ENUMTYPE='CreateAnniversarySMS',OBJECT_TYPE='Network',OBJECT_SUB_TYPE='SMS Template';
INSERT INTO ACTIVITIY_RIGHTS(ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)VALUES(331,327,'Create/Modify Feast Day','CreateFeastDaySMS','Network','SMS Template') ON DUPLICATE KEY UPDATE PARENT_ID= 327,OBJECT_NAME='Create/Modify Feast Day',ENUMTYPE='CreateFeastDaySMS',OBJECT_TYPE='Network',OBJECT_SUB_TYPE='SMS Template';
INSERT INTO ACTIVITIY_RIGHTS(ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)VALUES(332,312,'Thanksgiving - Mail','ThanksgivingMail','Network','Thanksgiving - Mail') ON DUPLICATE KEY UPDATE PARENT_ID= 312,OBJECT_NAME='Thanksgiving - Mail',ENUMTYPE='ThanksgivingMail',OBJECT_TYPE='Network',OBJECT_SUB_TYPE='Thanksgiving - Mail';
INSERT INTO ACTIVITIY_RIGHTS(ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)VALUES(333,332,'Preview Thanksgiving Mail','PreviewThanksgivingMail','Network','Thanksgiving - Mail') ON DUPLICATE KEY UPDATE PARENT_ID= 332,OBJECT_NAME='Preview Thanksgiving Mail',ENUMTYPE='PreviewThanksgivingMail',OBJECT_TYPE='Network',OBJECT_SUB_TYPE='Thanksgiving - Mail';
INSERT INTO ACTIVITIY_RIGHTS(ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)VALUES(334,332,'Send Thanksgiving Mail','SendThanksgivingMail','Network','Thanksgiving - Mail') ON DUPLICATE KEY UPDATE PARENT_ID= 332,OBJECT_NAME='Send Thanksgiving Mail',ENUMTYPE='SendThanksgivingMail',OBJECT_TYPE='Network',OBJECT_SUB_TYPE='Thanksgiving - Mail';
INSERT INTO ACTIVITIY_RIGHTS(ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)VALUES(335,312,'Appeal - Mail','AppealMail','Network','Appeal - Mail') ON DUPLICATE KEY UPDATE PARENT_ID= 312,OBJECT_NAME='Appeal - Mail',ENUMTYPE='AppealMail',OBJECT_TYPE='Network',OBJECT_SUB_TYPE='Appeal - Mail';
INSERT INTO ACTIVITIY_RIGHTS(ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)VALUES(336,335,'Preview Appeal Mail','PreviewAppealMail','Network','Appeal - Mail') ON DUPLICATE KEY UPDATE PARENT_ID= 335,OBJECT_NAME='Preview Appeal Mail',ENUMTYPE='PreviewAppealMail',OBJECT_TYPE='Network',OBJECT_SUB_TYPE='Appeal - Mail';
INSERT INTO ACTIVITIY_RIGHTS(ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)VALUES(337,335,'Send Appeal Mail','SendAppealMail','Network','Appeal - Mail') ON DUPLICATE KEY UPDATE PARENT_ID= 335,OBJECT_NAME='Send Appeal Mail',ENUMTYPE='SendAppealMail',OBJECT_TYPE='Network',OBJECT_SUB_TYPE='Appeal - Mail';
INSERT INTO ACTIVITIY_RIGHTS(ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)VALUES(338,312,'Newsletter - Mail','NewsletterMail','Network','Newsletter - Mail') ON DUPLICATE KEY UPDATE PARENT_ID= 312,OBJECT_NAME='Newsletter - Mail',ENUMTYPE='NewsletterMail',OBJECT_TYPE='Network',OBJECT_SUB_TYPE='Newsletter - Mail';
INSERT INTO ACTIVITIY_RIGHTS(ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)VALUES(339,338,'Preview Newsletter Mail','PreviewNewsletterMail','Network','Newsletter - Mail') ON DUPLICATE KEY UPDATE PARENT_ID= 338,OBJECT_NAME='Preview Newsletter Mail',ENUMTYPE='PreviewNewsletterMail',OBJECT_TYPE='Network',OBJECT_SUB_TYPE='Newsletter - Mail';
INSERT INTO ACTIVITIY_RIGHTS(ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)VALUES(340,338,'Send Newsletter Mail','SendNewsletterMail','Network','Newsletter - Mail') ON DUPLICATE KEY UPDATE PARENT_ID= 338,OBJECT_NAME='Send Newsletter Mail',ENUMTYPE='SendNewsletterMail',OBJECT_TYPE='Network',OBJECT_SUB_TYPE='Newsletter - Mail';
INSERT INTO ACTIVITIY_RIGHTS(ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)VALUES(341,312,'Anniversary - Mail','AnniversaryMail','Network','Anniversary - Mail') ON DUPLICATE KEY UPDATE PARENT_ID= 312,OBJECT_NAME='Anniversary - Mail',ENUMTYPE='AnniversaryMail',OBJECT_TYPE='Network',OBJECT_SUB_TYPE='Anniversary - Mail';
INSERT INTO ACTIVITIY_RIGHTS(ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)VALUES(342,341,'Preview Anniversary Mail','PreviewAnniversaryMail','Network','Anniversary - Mail') ON DUPLICATE KEY UPDATE PARENT_ID= 341,OBJECT_NAME='Preview Anniversary Mail',ENUMTYPE='PreviewAnniversaryMail',OBJECT_TYPE='Network',OBJECT_SUB_TYPE='Anniversary - Mail';
INSERT INTO ACTIVITIY_RIGHTS(ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)VALUES(343,341,'Send Anniversary Mail','SendAnniversaryMail','Network','Anniversary - Mail') ON DUPLICATE KEY UPDATE PARENT_ID= 341,OBJECT_NAME='Send Anniversary Mail',ENUMTYPE='SendAnniversaryMail',OBJECT_TYPE='Network',OBJECT_SUB_TYPE='Anniversary - Mail';
INSERT INTO ACTIVITIY_RIGHTS(ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)VALUES(344,312,'Feast - Mail','FeastMail','Network','Feast - Mail') ON DUPLICATE KEY UPDATE PARENT_ID= 312,OBJECT_NAME='Feast - Mail',ENUMTYPE='FeastMail',OBJECT_TYPE='Network',OBJECT_SUB_TYPE='Feast - Mail';
INSERT INTO ACTIVITIY_RIGHTS(ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)VALUES(345,344,'Preview Feast Mail','PreviewFeastMail','Network','Feast - Mail') ON DUPLICATE KEY UPDATE PARENT_ID= 344,OBJECT_NAME='Preview Feast Mail',ENUMTYPE='PreviewFeastMail',OBJECT_TYPE='Network',OBJECT_SUB_TYPE='Feast - Mail';
INSERT INTO ACTIVITIY_RIGHTS(ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)VALUES(346,344,'Send Feast Mail','SendFeastMail','Network','Feast - Mail') ON DUPLICATE KEY UPDATE PARENT_ID= 344,OBJECT_NAME='Send Feast Mail',ENUMTYPE='SendFeastMail',OBJECT_TYPE='Network',OBJECT_SUB_TYPE='Feast - Mail';
INSERT INTO ACTIVITIY_RIGHTS(ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)VALUES(347,312,'Thanksgiving - SMS','ThanksgivingSMS','Network','Thanksgiving - SMS') ON DUPLICATE KEY UPDATE PARENT_ID= 312,OBJECT_NAME='Thanksgiving - SMS',ENUMTYPE='ThanksgivingSMS',OBJECT_TYPE='Network',OBJECT_SUB_TYPE='Thanksgiving - SMS';
INSERT INTO ACTIVITIY_RIGHTS(ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)VALUES(348,347,'Preview Thanksgiving SMS','PreviewThanksgivingSMS','Network','Thanksgiving - SMS') ON DUPLICATE KEY UPDATE PARENT_ID= 347,OBJECT_NAME='Preview Thanksgiving SMS',ENUMTYPE='PreviewThanksgivingSMS',OBJECT_TYPE='Network',OBJECT_SUB_TYPE='Thanksgiving - SMS';
INSERT INTO ACTIVITIY_RIGHTS(ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)VALUES(349,347,'Send Thanksgiving SMS','SendThanksgivingSMS','Network','Thanksgiving - SMS') ON DUPLICATE KEY UPDATE PARENT_ID= 347,OBJECT_NAME='Send Thanksgiving SMS',ENUMTYPE='SendThanksgivingSMS',OBJECT_TYPE='Network',OBJECT_SUB_TYPE='Thanksgiving - SMS';
INSERT INTO ACTIVITIY_RIGHTS(ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)VALUES(350,312,'Appeal - SMS','AppealSMS','Network','Appeal - SMS') ON DUPLICATE KEY UPDATE PARENT_ID= 312,OBJECT_NAME='Appeal - SMS',ENUMTYPE='AppealSMS',OBJECT_TYPE='Network',OBJECT_SUB_TYPE='Appeal - SMS';
INSERT INTO ACTIVITIY_RIGHTS(ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)VALUES(351,350,'Preview Appeal SMS','PreviewAppealSMS','Network','Appeal - SMS') ON DUPLICATE KEY UPDATE PARENT_ID= 350,OBJECT_NAME='Preview Appeal SMS',ENUMTYPE='PreviewAppealSMS',OBJECT_TYPE='Network',OBJECT_SUB_TYPE='Appeal - SMS';
INSERT INTO ACTIVITIY_RIGHTS(ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)VALUES(352,350,'Send Appeal SMS','SendAppealSMS','Network','Appeal - SMS') ON DUPLICATE KEY UPDATE PARENT_ID= 350,OBJECT_NAME='Send Appeal SMS',ENUMTYPE='SendAppealSMS',OBJECT_TYPE='Network',OBJECT_SUB_TYPE='Appeal - SMS';
INSERT INTO ACTIVITIY_RIGHTS(ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)VALUES(353,312,'Anniversary - SMS','AnniversarySMS','Network','Anniversary - SMS') ON DUPLICATE KEY UPDATE PARENT_ID= 312,OBJECT_NAME='Anniversary - SMS',ENUMTYPE='AnniversarySMS',OBJECT_TYPE='Network',OBJECT_SUB_TYPE='Anniversary - SMS';
INSERT INTO ACTIVITIY_RIGHTS(ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)VALUES(354,353,'Preview Anniversary SMS','PreviewAnniversarySMS','Network','Anniversary - SMS') ON DUPLICATE KEY UPDATE PARENT_ID= 353,OBJECT_NAME='Preview Anniversary SMS',ENUMTYPE='PreviewAnniversarySMS',OBJECT_TYPE='Network',OBJECT_SUB_TYPE='Anniversary - SMS';
INSERT INTO ACTIVITIY_RIGHTS(ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)VALUES(355,353,'Send Anniversary SMS','SendAnniversarySMS','Network','Anniversary - SMS') ON DUPLICATE KEY UPDATE PARENT_ID= 353,OBJECT_NAME='Send Anniversary SMS',ENUMTYPE='SendAnniversarySMS',OBJECT_TYPE='Network',OBJECT_SUB_TYPE='Anniversary - SMS';
INSERT INTO ACTIVITIY_RIGHTS(ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)VALUES(356,312,'Feast - SMS','FeastSMS','Network','Feast - SMS') ON DUPLICATE KEY UPDATE PARENT_ID= 312,OBJECT_NAME='Feast - SMS',ENUMTYPE='FeastSMS',OBJECT_TYPE='Network',OBJECT_SUB_TYPE='Feast - SMS';
INSERT INTO ACTIVITIY_RIGHTS(ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)VALUES(357,356,'Preview Feast SMS','PreviewFeastSMS','Network','Feast - SMS') ON DUPLICATE KEY UPDATE PARENT_ID= 356,OBJECT_NAME='Preview Feast SMS',ENUMTYPE='PreviewFeastSMS',OBJECT_TYPE='Network',OBJECT_SUB_TYPE='Feast - SMS';
INSERT INTO ACTIVITIY_RIGHTS(ACTIVITY_ID, PARENT_ID, OBJECT_NAME, ENUMTYPE, OBJECT_TYPE, OBJECT_SUB_TYPE)VALUES(358,356,'Send Feast SMS','SendFeastSMS','Network','Feast - SMS') ON DUPLICATE KEY UPDATE PARENT_ID= 356,OBJECT_NAME='Send Feast SMS',ENUMTYPE='SendFeastSMS',OBJECT_TYPE='Network',OBJECT_SUB_TYPE='Feast - SMS';
