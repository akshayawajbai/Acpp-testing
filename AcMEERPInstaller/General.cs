...
namespace AcMEERPInstaller
{
    public static class General
    {
        ...
        // Chinna 09/02/2026, 
        // Inside General class in AcMEERPInstaller\General.cs
        /// <summary>
        /// Returns true if license is SDBANG (HEAD_OFFICE_CODE decrypts to "SDBANG"). For SDBANG, BRANCH_OFFICE_NAME, InstituteName, ADDRESS, SocietyName are plain text.
        /// </summary>
        private static bool IsLicenseFileSDBANG(DataTable dtLicenseInfo, DataRow drLicense)
        {
            if (dtLicenseInfo == null || drLicense == null || !dtLicenseInfo.Columns.Contains("HEAD_OFFICE_CODE"))
                return false;

            string raw = drLicense["HEAD_OFFICE_CODE"].ToString().Trim().ToUpper();
            if (raw == "SDBANG")
            {
                General.WriteLog("License: SDBANG detected (HEAD_OFFICE_CODE plain)");
                return true;
            }

            try
            {
                string dec = objdec.DecryptString(drLicense["HEAD_OFFICE_CODE"].ToString());
                bool isSDBANG = (dec != null && dec.Trim().ToUpper() == "SDBANG");
                if (isSDBANG)
                    General.WriteLog("License: SDBANG detected (HEAD_OFFICE_CODE decrypted)");
                return isSDBANG;
            }
            catch (Exception ex)
            {
                General.WriteLog("License: IsLicenseFileSDBANG check failed - " + ex.Message);
                return false;
            }
        }
        ...
        public static string GetLicInstituteName(string licencekay)
        {
            ...
            try
            {
                if (File.Exists(licencekay))
                {
                    ...
                    if (dsLicenseInfo != null && dsLicenseInfo.Tables.Count > 0)
                    {
                        ...
                        {
                            DataRow drLicense = dtLicenseInfo.Rows[0];
                            bool isSDBANG = IsLicenseFileSDBANG(dtLicenseInfo, drLicense);

                            if (dtLicenseInfo.Columns.Contains("InstituteName") &&
                                !string.IsNullOrEmpty(drLicense["InstituteName"].ToString()))
                            {
                                string value = drLicense["InstituteName"].ToString();
                                if (isSDBANG)
                                {
                                    instituteName = value;
                                    General.WriteLog("License: InstituteName used as plain (SDBANG)");
                                }
                                else
                                {
                                    instituteName = objdec.DecryptString(value);
                                }
                            }
                        }
                    }
            ...
            {
                General.WriteLog("Could not read License Info - " + ex.Message);
            }
            finally { }
