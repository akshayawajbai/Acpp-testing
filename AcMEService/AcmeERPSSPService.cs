...
namespace AcMEService
{
    public class AcmeERPSSPService : IAcmeERPSSPService
    {
        ...
        //bharath 23/05/2026 new Url for Sacred-Heart sdbinm inmshc
        private const string LeapShcTransactionUrl = "https://www.leapshc.org:8443/shcacmedata/getTransactionDetails.jsp";
        ...
        public string APIPostMaster(string ManagementCode, string JsonScriptMaster)
        {
            ...
            else
            {
               ...
               //urlName = string.Format("http://192.168.1.122:8000/api/acme/store/data");
               urlName = string.Format("https://economato.sdb.org/api/acme/store/data");
                //urlName = string.Format("https://6348ee27be7c.ngrok-free.app/api/acme/store/data");
            }
        ...
        /// <param name="JsonScript"></param>
        /// <returns></returns>
        public string APIExportBudget(string ManagementCode, string JsonScriptBudget)
        {
            string status = "";

            string Data = "{\"params\": " + JsonScriptBudget + "}";
            string urlName = string.Empty;

            ServicePointManager.SecurityProtocol = SecurityProtocolType.Tls | SecurityProtocolType.Ssl3;
            ServicePointManager.SecurityProtocol = (SecurityProtocolType)3072;

            //urlName = string.Format("https://6348ee27be7c.ngrok-free.app/api/acme/budgets");
            urlName = string.Format("https://economato.sdb.org/api/acme/budgets");
        ...
        public string APIGetVoucher(string DateFrom, string DateTo, string ManagementCode, string JsonScriptMaster, string projectname = "", string ProjectId = "")
        {
            ...
            if (!setting.IS_SDB_RMG)
            {
                ...
                AcMELog.WriteLog(" fetching Higrade URL Alone " + HiGradeURL);
                ...
                AcMELog.WriteLog(" fetching URL " + urlName);
            ...
            else
            {
                ...
                //urlName = string.Format("https://6348ee27be7c.ngrok-free.app/api/acme/transaction/data/" + DateTime.Parse(DateFrom).Date.ToString("dd-MM-yyyy") + "/" + DateTime.Parse(DateTo).Date.ToString("dd-MM-yyyy") + "/" + projectname);
        ...
        //bharath 23/05/2026 new Voucher Post Api for Sacred-Heart sdbinm inmshc
        /// <summary>
        /// Gets transaction details from Leap SHC (POST). Same argument order as <see cref="APIGetVoucher"/>; only dates and project name are sent to the server.
        /// </summary>
        /// <param name="DateFrom">From date (e.g. dd/MM/yyyy from UI); sent as <c>from_date</c> (yyyy-MM-dd).</param>
        /// <param name="DateTo">To date; sent as <c>to_date</c> (yyyy-MM-dd).</param>
        /// <param name="jsonScriptMaster">Unused for Leap SHC; kept for call-site compatibility with <see cref="APIGetVoucher"/>.</param>
        /// <param name="projectName">Project name; sent as request parameter <c>account</c>.</param>
        /// <param name="projectId">Unused for Leap SHC; kept for call-site compatibility.</param>
        public string APIGetVoucherLeapSHC(string DateFrom, string DateTo, string jsonScriptMaster, string projectName, string projectId = "")
        {
            string Status = "";
            const string leapUser = "admin";
            const string leapPassword = "DBSHCTPT322@";

            DateTime fromDt;
            DateTime toDt;
            if (!TryParseLeapShcDate(DateFrom, out fromDt) || !TryParseLeapShcDate(DateTo, out toDt))
            {
                Status = "Status: Could not proceed. Get Voucher (Leap SHC)... Invalid from/to date.";
                AcMELog.WriteLog("Leap SHC getTransactionDetails: invalid dates DateFrom=" + DateFrom + " DateTo=" + DateTo);
                return Status;
            }

            string fromDate = fromDt.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture);
            string toDate = toDt.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture);

            // Leap SHC expects account exactly as stored (no wrapping quotes). Quotes in the value break matching and return empty Vouchers.
            string accountForRequest = NormalizeLeapShcAccount(projectName);

            string postData =
                "username=" + Uri.EscapeDataString(leapUser) +
                "&password=" + Uri.EscapeDataString(leapPassword) +
                "&account=" + Uri.EscapeDataString(accountForRequest) +
                "&from_date=" + Uri.EscapeDataString(fromDate) +
                "&to_date=" + Uri.EscapeDataString(toDate);
            ...
            try
            {
                AcMELog.WriteLog("Leap SHC getTransactionDetails POST started for account: " + accountForRequest + " from_date=" + fromDate + " to_date=" + toDate);
                HttpWebRequest request = (HttpWebRequest)WebRequest.Create(LeapShcTransactionUrl);
                request.Method = "POST";
                ...
                using (Stream srData = response.GetResponseStream())
                using (StreamReader reader = new StreamReader(srData))
                {
                    Status = reader.ReadToEnd();
                }

                AcMELog.WriteLog("Leap SHC getTransactionDetails POST completed.");
            }
            catch (Exception ex)
            {
                AcMELog.WriteLog("Leap SHC getTransactionDetails: " + ex.Message);
                Status = "Status: Could not proceed. Get Voucher (Leap SHC)..." + ex.Message;
            }

            return Status;
        }

        private static string NormalizeLeapShcAccount(string name)
        {
        ...
        private static bool TryParseLeapShcDate(string input, out DateTime date)
        {
            input = (input ?? string.Empty).Trim();
            string[] formats = { "dd/MM/yyyy", "d/M/yyyy", "dd/M/yyyy", "d/MM/yyyy", "yyyy-MM-dd", "dd-MM-yyyy" };
        ...
        public string APIUpdateGetVouchers(string ManagementCode, string JsonSavedScriptTransaction)
        {
            ...
            try
            {
                ...
                if (setting.BranchOfficeCode == "sdbinminmshc")
                {
                    status = "Success";
                    return status;
                }
                ...
                else
                {
                    ...
                    //urlName = string.Format("https://6348ee27be7c.ngrok-free.app/api/acme/transaction/update");
        ...
        /// <returns></returns>
        /// 
        //bharath 27/02/2026 Json Ledger with Sub json changed 
        public static DataTable ConvertJsonStringDatatble(string json)
        {
            JObject root = JObject.Parse(json);
            JArray vouchers = (JArray)root["Vouchers"];

            JArray finalArray = new JArray();

            foreach (JObject voucher in vouchers)
            {
                JObject cleanObject = new JObject();

                int ledgerFlag = 0;
                string ledgerNames = "";
                string ledgerAmounts = "";

                foreach (JProperty prop in voucher.Properties())
                {
                    if (prop.Name == "LEDGER" && prop.Value.Type == JTokenType.Array)
                    {
                        JArray ledgerArray = (JArray)prop.Value;

                        if (ledgerArray.Count > 1)
                            ledgerFlag = 1;
                        else
                            ledgerFlag = 0;

                        foreach (JObject ledger in ledgerArray)
                        {
                            if (ledgerNames != "")
                                ledgerNames += ", ";
                            ...
                            if (ledger["LEDGER"] != null)
                                ledgerNames += ledger["LEDGER"].ToString();

                            if (ledger["LEDGER_AMOUNT"] != null)
                                ledgerAmounts += ledger["LEDGER_AMOUNT"].ToString();
                        }
                    }
                    else if (prop.Name == "LEDGER" && prop.Value.Type == JTokenType.String)
                    {
                        ledgerNames = prop.Value.ToString();
                        ledgerFlag = 0;
                    }
                    else if (prop.Name == "LEDGER_AMOUNT" && prop.Value.Type != JTokenType.Array)
                    {
                        ledgerAmounts = prop.Value.ToString();
                    }
                    else if (prop.Value.Type != JTokenType.Array &&
                             prop.Name != "LEDGER")
                    {
                        cleanObject.Add(prop.Name, prop.Value);
                    }
                }

                cleanObject["LEDGER"] = ledgerNames;
                cleanObject["LEDGER_AMOUNT"] = ledgerAmounts;
                cleanObject["LEDGER_FLAG"] = ledgerFlag;

                finalArray.Add(cleanObject);
            }

            return JsonConvert.DeserializeObject<DataTable>(finalArray.ToString());
        }
        ...
        public static DataTable ConvertJsonStringDatatbleforDecimal(string json)
        {
            ...
            foreach (JObject row in SourceDataArray.Children<JObject>())
            {
                ...
                foreach (JProperty column in row.Properties())
                {
                    if (stringColumns.Contains(column.Name))
                    {
                        NewObjectClean.Add(column.Name, column.Value.ToString());
                        continue;
                    }
        ...
        /// <summary>
        /// GET cost centres/users JSON from ACME endpoint.
        /// If overrideUrl is given, it is used; otherwise we pick based on env settings.
        /// </summary>
        /// bharath
        public string APIGetCostCentres()
        {
            string result = "";
            //string urlName = "https://6348ee27be7c.ngrok-free.app/api/acme/users";
            string urlName = "https://economato.sdb.org/api/acme/users";
            ...
            {
                AcMELog.WriteLog("Could not proceed APIGetCostCentres: " + ex.Message);
                result = "";
            }
