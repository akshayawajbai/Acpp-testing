...
namespace Payroll.Model.UIModel
{
    public class clsprCompBuild : SystemBase
    {
        ...
        /// <returns></returns>
        private static double ParseModifyStaffAlterAmount(string s)
        {
            if (PayrollSdbangMoneyParse.Enabled)
                return PayrollSdbangMoneyParse.ParseDouble(s);
            return double.Parse(s);
        }

        private static string NormalizeModifyStaffTempValue(string s, bool daysOrTextField)
        {
            if (string.IsNullOrEmpty(s) || daysOrTextField || !PayrollSdbangMoneyParse.Enabled)
                return s;
            return PayrollSdbangMoneyParse.ParseDecimal(s).ToString("G29", CultureInfo.InvariantCulture);
        }

        // 06/04/2026, chinna — PayrollSdbangMoneyParse / Global Setting currency:
        // - Helpers below: parse formula strings with settings when IS_SDBANG; legacy double.Parse when off.
        // - Rounding: SDBANG uses DecimalPlaces + invariant ToString for next formula step (comma-decimal safe).
        // - Earning/LOP pro-rate, text TryParse, GetProcessedValue rounding, EvaluateExpression+max slab updated.

        /// <summary>
        /// Parse a component/formula string as a number. When SDBANG is off, behavior matches legacy (thread culture).
        /// When on, uses Global Setting separators (Angola, India, etc.).
        /// </summary>
        private static double ParseCompPipelineAmount(string s)
        {
            if (string.IsNullOrWhiteSpace(s))
                return 0;
            string t = s.Trim();
            if (PayrollSdbangMoneyParse.Enabled)
                return PayrollSdbangMoneyParse.ParseDouble(t);
            return double.Parse(t);
        }

        private static bool TryParseCompPipelineAmount(string s, out double result)
        {
            result = 0;
            if (string.IsNullOrWhiteSpace(s))
                return false;
            ...
            if (PayrollSdbangMoneyParse.Enabled)
            {
                if (!PayrollSdbangMoneyParse.LooksLikeNumericAmountString(t))
                    return false;
                result = PayrollSdbangMoneyParse.ParseDouble(t);
                return true;
            }
            return double.TryParse(t, out result);
        }

        private static int GetCurrencyDecimalPlacesForRounding()
        {
            if (!PayrollSdbangMoneyParse.Enabled)
                return 2;
            try
            {
                ...
                if (sp != null && !string.IsNullOrEmpty(sp.DecimalPlaces))
                {
                    int n;
                    if (int.TryParse(sp.DecimalPlaces, out n) && n >= 0 && n <= 15)
                        return n;
                }
            }
            catch
        ...
        /// <summary>
        /// After ceiling/floor/round, format for the next formula step. SDBANG: invariant so comma decimals do not break the chain.
        /// </summary>
        private static string FormatCompPipelineAmountRounded(double value, int nCompRnd)
        {
            if (!PayrollSdbangMoneyParse.Enabled)
            {
                if (nCompRnd == 0)
            ...
            int dec = GetCurrencyDecimalPlacesForRounding();
            if (nCompRnd == 0)
            ...
            return Math.Round(value, dec, MidpointRounding.AwayFromZero).ToString("F" + dec, CultureInfo.InvariantCulture);
        }

        private static string FormatDoubleForCompPipeline(double value)
        {
            if (!PayrollSdbangMoneyParse.Enabled)
            ...
            for (int i = 0; i < nMax; i++)
            {
                ...
                if (Strings.Trim(dvComponent.Table.Rows[0]["linkvalue"] + "") != "")
                {
                    ...
                    if (sFldName.Length >= 6 && sFldName.Substring(0, 6).ToUpper() == "LOAN :")
                    {
                        ...
                        bModifyStaffComponent = AlterLoanDue(nLoanId, ParseModifyStaffAlterAmount(sAlterVal), sStaffName);
                    ...
                    else if (sFldName == "BASIC PAY" || sFldName == "BASICPAY") //Alter Basic Pay
                    {
                        ...
                        UpdateDefaultValue(NormalizeModifyStaffTempValue(sAlterVal, false), nCompId);
                                ...
                                sFldName == "TOTALDAYSINPAYMONTH") //Alter no of paying salary days on 15/05/2019
                    {
                        ...
                        if (ParseModifyStaffAlterAmount(sAlterVal) <= daysinmonth)
                        {
                            //2. Update given no of salary days
                            nType = int.Parse(dvComponent.Table.Rows[0]["TYPE"] + "");
                            UpdateDefaultValue(NormalizeModifyStaffTempValue(sAlterVal, true), nCompId);
                            if (!DontProcess)
                            {
                                ProcessComponent(nPayRollId, sGroupId, "", nStaffId + "", false, null, true, ResetFromStaffProfile, true);
                            }
                ...
                else // if (Strings.Trim(dvComponent.Table.Rows[0]["EQUATIONID"] + "" )== "")  //Fixed Value
                {
                    ...
                    {
                        nType = int.Parse(dvComponent.Table.Rows[0]["TYPE"] + "");
                        bool daysField = CompName.Trim().ToUpper() == PayRollDefaultComponent.WORKINGDAYS.ToString().ToUpper() ||
                            CompName.Trim().ToUpper() == PayRollDefaultComponent.LEAVEDAYS.ToString().ToUpper() ||
                            CompName.Trim().ToUpper() == PayRollDefaultComponent.LOPDAYS.ToString().ToUpper();
                        UpdateDefaultValue(NormalizeModifyStaffTempValue(sAlterVal, daysField), nCompId);
                        if (!DontProcess)
                        {
                            ProcessComponent(nPayRollId, sGroupId, "", nStaffId + "", false, null, true, ResetFromStaffProfile, true);
                        }
        ...
        public string ProcessCompRangeValue(string compId, string lnkCompValue)
        {
            ...
            if (result.Success && result.DataSource.Table.Rows.Count > 0)
            {
                ...
                foreach (DataRow dr in dtRangeList.Rows)
                {
                    ...
                    TypeId = commem.NumberSet.ToInteger(dr["TYPE_ID"].ToString());//Dinesh
                    ...
                    // Old and New One 14/02/2026
                    if (!setting.IS_SDBANG)
                    {
                        if (lnkcomamount >= MinVal) //Added By Dinesh 30-10-2025
                        {
                            double taxableAmount = Math.Min(lnkcomamount, MaxVal) - MinVal;
                            if (taxableAmount < 0)
                                taxableAmount = 0;
                            double slabTax = 0;
                            if (TypeId == 0 || TypeId == null)
                            {
                                slabTax = RangeMaxSlab;
                            }
                            else if (TypeId == 1)
                            {
                                slabTax = taxableAmount * (RangeMaxSlab / 100);
                            }
                            totalTax += slabTax;
                        }
                    }
                    else
                    {
                        if (lnkcomamount >= MinVal && lnkcomamount <= MaxVal)
                        {
                            if (TypeId == 0)//Fixed Slab
                            {
                                totalTax = RangeMaxSlab;
                            }
                            else if (TypeId == 1)
                            {
                               double taxableAmount = lnkcomamount - MinVal;
                               totalTax = taxableAmount * (RangeMaxSlab / 100);
                            }
                            break;
                        }
                    }
                ...
                ptrtnVal = Math.Round(totalTax, 0).ToString("0");
            }
        ...
        public string ProcessCompRangeValueByConsolidated(string compId, string lnkCompValue)
        {
            ...
            if (result.Success && result.DataSource.Table.Rows.Count > 0)
            {
                ...
                foreach (DataRow dr in dtRangeList.Rows)
                {
                    ...
                    RangeMaxSlab = commem.NumberSet.ToDouble(dr["MAX_SLAB"].ToString());
                    //Dinesh
                    TypeId = commem.NumberSet.ToInteger(dr["TYPE_ID"].ToString());
                    //Commend By Dinesh 25-10-2025
                    ...
                    if (lnkcomamount >= MinVal) //Dinesh 
                    {
                        ...
                        if (TypeId == 0 || TypeId == null)
                        {
                            rangeamt = RangeMaxSlab;
                        }
                        else if (TypeId == 1)
                        {
                            rangeamt = rangebalanceamt * (RangeMaxSlab / 100);
                        }
                        ptrtnVal += rangeamt;
                    }
                }
        ...
        //added Dinesh
        //public bool IsManualZeroSalaryDays()
        //{
        //    if (PayingSalaryDaysCompId <= 0)
        //    {
        //        return false;
        //    }
        //    if (!RecordsExistsinPrStattTemp(PayingSalaryDaysCompId))
        //    {
        //        return false;
        //    }

        //    double SalaryDays = UtilityMember.NumberSet.ToDouble(GetDefaultValue("", PayingSalaryDaysCompId));
        ...
        private double ParseBasicPayFromStaffRow(string basicPayText)
        {
            if (string.IsNullOrEmpty(basicPayText))
                return 0.0;
            if (PayrollSdbangMoneyParse.Enabled)
                return PayrollSdbangMoneyParse.ParseDouble(basicPayText);
            return double.Parse(basicPayText + "");
        }

        private double ParseScaleOfPaySegment(string segment)
        {
            if (PayrollSdbangMoneyParse.Enabled)
                return PayrollSdbangMoneyParse.ParseDouble(segment);
            return UtilityMember.NumberSet.ToDouble(segment);
        }
        ...
        private double GetBasicPay()
        {
            ...
            if (dvStaff[0]["BasicPay"].ToString() != "")
                dBPay = ParseBasicPayFromStaffRow(dvStaff[0]["BasicPay"].ToString());
            else dBPay = 0.0;
            dCurBP = dBPay;
            ...
            if (sScaleofPay != "")
            {
                ...
                if (aScaleofPay.GetUpperBound(0) == 2)
                {
                    ...
                    if (ParseScaleOfPaySegment(aScaleofPay[1].ToString()) > 0) //if (int.Parse(aScaleofPay[1].ToString()) > 0)
                    {
                        if (ParseScaleOfPaySegment(aScaleofPay[2].ToString()) <= 0) //if (int.Parse(aScaleofPay[2].ToString()) <= 0)
                        {
                            dBPay = dBPay + ParseScaleOfPaySegment(aScaleofPay[1].ToString());//int.Parse(aScaleofPay[1].ToString());
                        }
                        else
                        {
                            //if (dBPay + int.Parse(aScaleofPay[1].ToString()) <= int.Parse(aScaleofPay[2].ToString()))
                            if (dBPay + ParseScaleOfPaySegment(aScaleofPay[1].ToString()) <= ParseScaleOfPaySegment(aScaleofPay[2].ToString()))
                            {
                                dBPay = dBPay + ParseScaleOfPaySegment(aScaleofPay[1].ToString());// int.Parse(aScaleofPay[1].ToString());
                            }
                        }
            ...
            double dMaxSlab, int nCompRnd, bool bApplySalaryDays)
        {
            ...
            EndLine:
                ...
                //sVal1= dRetVal +""; the existing vb code
                sVal1 = nEvalExpr.ToString();
                // 06/04/2026, chinna — SDBANG: parse Excel result + format invariant; max slab same pattern
                {
                    double evalAmt = ParseCompPipelineAmount(sVal1);
                    double rounded = RoundValue(evalAmt, nCompRnd);
                    sVal1 = FormatDoubleForCompPipeline(rounded);
                }
                //Check the Maximum Slab Value
                if (dMaxSlab > 0)
                {
                    double slabCmp = ParseCompPipelineAmount(sVal1);
                    sVal1 = (slabCmp > dMaxSlab) ? FormatDoubleForCompPipeline(dMaxSlab) : sVal1;
                }
                ...
                foreach (DataRow dr in dtRange.Rows)
                {
                    if (dr.RowState != DataRowState.Deleted)
                    {
                        using (DataManager dManager = new DataManager(SQLCommand.Payroll.InsertRangeCondtions))
                        {
                            ...
                            //Added By Dinesh 25-10-2025
                            dManager.Parameters.Add(dtPayrollRange.TYPE_IDColumn, TypeId);
        ...
        public double GetTotalPayrollProcessingDays()
        {
            ...
            try
            {
                ...
                if (result.Success && result.DataSource.Table != null)
                {
                    ...
                    PayrollProcessingDays = (dtPayrollTo.Date - dtPayrollFrom.Date).TotalDays + 1;
                }
                else
                {
