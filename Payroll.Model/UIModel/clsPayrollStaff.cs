...
namespace Payroll.Model.UIModel
{
    public class clsPayrollStaff : SystemBase
    {
        ...
        // #Dinesh 03-07-2026 - Company bank account id and bank name
        public Int32 CompanyBankAccountId { get; set; }
            ...
            if (Convert.ToString(StaffId) != "")
            {
                ...
                try
                {
                    ...
                    if (resultArgs.Success && resultArgs.RowsAffected > 0)
                    {
                        ...
                        // #Dinesh 03-07-2026 - Fetch company bank account id and bank name
                        CompanyBankAccountId = UtilityMember.NumberSet.ToInteger(dtTable.Rows[0]["BANK_ACCOUNT_ID"].ToString());
                        CompanyBankName = dtTable.Rows[0]["BANK_NAME"].ToString();
                    }
                }
                                ...
                                if (resultArgs.Success)
                                {
                                    ...
                                    if (resultArgs.Success)
                                    {
                                        ...
                                        using (clsPrGroupStaff groupStaff = new clsPrGroupStaff())
                                        {
                                            ...
                                            // #Dinesh 03-07-2026 - Map company bank account to pay group
                                            groupStaff.CompanyBankAccountId = CompanyBankAccountId;
                                            groupStaff.CompanyBankName = CompanyBankName;
        ...
        // 06/04/2026, chinna — SDBANG: STFSERVICE.PAY varchar invariant F2 / ParseDecimal for money fields
        private string MapPayColumnForDb(string pay)
        {
            return PayrollSdbangMoneyParse.Enabled ? PayrollSdbangMoneyParse.PayVarcharForDb(pay) : pay;
        }

        private decimal MapPayrollMoneyDecimal(string text)
        {
            return PayrollSdbangMoneyParse.Enabled ? PayrollSdbangMoneyParse.ParseDecimal(text) : this.UtilityMember.NumberSet.ToDecimal(text);
        }
                                    ...
                                    string payingsalarydays, string uan)
        {
            if (Option.Equals((int)clsPayrollConstants.PAYROLL_STAFF_INSERT) && RtnStaffid != "0")
            {
                using (DataManager dataManagerNew = new DataManager(SQLCommand.Payroll.PayrollStaffserviceInsert))
                {
                    ...
                    dataManagerNew.Parameters.Add(dtStaff.PAYColumn, MapPayColumnForDb(pay));
                    ...
                    dataManagerNew.Parameters.Add(dtStaff.EARNING1Column, MapPayrollMoneyDecimal(earning1));
                    dataManagerNew.Parameters.Add(dtStaff.EARNING2Column, MapPayrollMoneyDecimal(earning2));
                    dataManagerNew.Parameters.Add(dtStaff.EARNING3Column, MapPayrollMoneyDecimal(earning3));
                    dataManagerNew.Parameters.Add(dtStaff.DEDUCTION1Column, MapPayrollMoneyDecimal(deduction1));
                    dataManagerNew.Parameters.Add(dtStaff.DEDUCTION2Column, MapPayrollMoneyDecimal(deduction2));
                    dataManagerNew.Parameters.Add(dtStaff.PAYING_SALARY_DAYSColumn, MapPayrollMoneyDecimal(payingsalarydays));
                    dataManagerNew.DataCommandArgs.ActiveSQLAdapterType = SQLAdapterType.PayrollSQL;
