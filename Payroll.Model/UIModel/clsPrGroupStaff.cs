                ...
                for (int i = 0; i < aStaffId.Length; i++, MaxSortOrder++)
                {
                    ...
                    if (resultArgs.Success && resultArgs.DataSource.Table != null)
                    {
                        ...
                        using (DataManager dataManager = new DataManager(alreadyMaped ? SQLCommand.Payroll.UpdatePRStaffGroup : SQLCommand.Payroll.InsertPRStaffGroup))
                        {
                            ...
                            // #Dinesh 03-07-2026 - Save company bank account id and bank name
                            dataManager.Parameters.Add("BANK_ACCOUNT_ID", CompanyBankAccountId);
                            dataManager.Parameters.Add("BANK_NAME", CompanyBankName ?? string.Empty, DataType.String);
                        ...
                        if (resultArgs.Success && !alreadyMaped)
                        {
                            using (DataManager dataMapManager = new DataManager(SQLCommand.Payroll.InsertPRStaffGroupForAllMonths))
                            {
                                ...
                                // #Dinesh 03-07-2026 - Save company bank account id and bank name
                                dataMapManager.Parameters.Add("BANK_ACCOUNT_ID", CompanyBankAccountId);
                                dataMapManager.Parameters.Add("BANK_NAME", CompanyBankName ?? string.Empty, DataType.String);
                        ...
                        // #Dinesh03-08-2026 - Bank mapping is one-time for staff; keep same bank in all months until manually cleared
                        if (resultArgs.Success)
                        {
                            using (DataManager dataBankManager = new DataManager(SQLCommand.Payroll.UpdatePRStaffGroupBankForAllMonths))
                            {
                                dataBankManager.DataCommandArgs.ActiveSQLAdapterType = SQLAdapterType.PayrollSQL;
                                dataBankManager.Database = dm.Database;
                                dataBankManager.Parameters.Add(dtGrade.STAFFIDColumn, nStaffId);
                                dataBankManager.Parameters.Add("BANK_ACCOUNT_ID", CompanyBankAccountId);
                                dataBankManager.Parameters.Add("BANK_NAME", CompanyBankName ?? string.Empty, DataType.String);
                                dataBankManager.DataCommandArgs.IsDirectReplaceParameter = true;
                                resultArgs = dataBankManager.UpdateData();
                            }
                        }
