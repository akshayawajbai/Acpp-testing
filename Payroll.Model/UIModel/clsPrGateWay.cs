...
namespace Payroll.Model.UIModel
{
    public class clsPrGateWay : SystemBase
    {
        ...
        // #Dinesh03-08-2026 - Auto fix old empty bank mapping months for client build (no manual SQL)
        private static bool isStaffBankSyncedForOldMonths = false;
        public ResultArgs SyncStaffBankMappingForOldMonths()
        {
            if (isStaffBankSyncedForOldMonths)
            {
            ...
            using (DataManager dataManager = new DataManager(SQLCommand.Payroll.SyncPRStaffGroupBankForOldMonths))
            {
                dataManager.DataCommandArgs.ActiveSQLAdapterType = SQLAdapterType.PayrollSQL;
                resultArgs = dataManager.UpdateData();
                if (resultArgs.Success)
                {
                    isStaffBankSyncedForOldMonths = true;
                }
            }
            return resultArgs;
        }
