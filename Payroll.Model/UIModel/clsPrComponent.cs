...
namespace Payroll.Model.UIModel
{
    public class clsPrComponent : SystemBase
    {
        ...
        // 06/04/2026, chinna — SDBANG: browse grid display/parse with Global Setting separators
        private string FormatPayrollBrowseMoneyCell(double tmpValue)
        {
            if (PayrollSdbangMoneyParse.Enabled)
                return PayrollSdbangMoneyParse.FormatMoneyForDisplay((decimal)tmpValue);
            return this.UtilityMember.NumberSet.ToNumber(tmpValue);
        }

        private bool TryParsePayrollCompValueForBrowseGrid(string raw, out double tmpValue)
        {
            tmpValue = 0;
            ...
            if (PayrollSdbangMoneyParse.Enabled)
            {
                if (!PayrollSdbangMoneyParse.LooksLikeNumericAmountString(raw))
                    return false;
                tmpValue = PayrollSdbangMoneyParse.ParseDouble(raw);
                return true;
            }
            double myDec;
            if (!double.TryParse(raw, out myDec))
                return false;
            tmpValue = this.UtilityMember.NumberSet.ToDouble(raw);
            return true;
        }
            ...
            try
            {
                ...
                if (dvGrpComp.Count == 0)
                {
                    ...
                    using (DataManager dataManager = new DataManager(SQLCommand.Payroll.PrcompMonthAdd))
                    {
                       ...
                       // dataManager.Parameters.Add(dtCompMonth.MAXSLABColumn, dt.Rows[0][5].ToString());//Commeded By Dinesh 19-02-2026
                        dataManager.Parameters.Add(dtCompMonth.MAXSLABColumn, NumberSet.ToDouble(dt.Rows[0][5].ToString())); //Adde By Dinesh 19-02-2026

                        //strQuery = strQuery.Replace("<COMP_ORDER>", "0");
                        dataManager.Parameters.Add(dtCompMonth.COMP_ORDERColumn, compOrder );

                        //strQuery = strQuery.Replace("<COMPROUND>", dt.Rows[0][6].ToString());
                        //dataManager.Parameters.Add(dtCompMonth.COMPROUNDColumn, dt.Rows[0][6].ToString());//Commeded By Dinesh 19-02-2026
                        int compound = NumberSet.ToInteger(dt.Rows[0][6].ToString());////Added By Dinesh 19-02-2026
                        dataManager.Parameters.Add(dtCompMonth.COMPROUNDColumn, compound); //Added By Dinesh
        ...
        public List<string> GetAcMeERPPayRoll(string nPayRollId, int ProjectId, long nSalaryGroup, DevExpress.XtraGrid.GridControl objGrid, bool bEdit)
        {
            ...
            if (dtGrp.Rows.Count > 0) //creating dynamic data table to view the staff processed component details according to count..
            {
                ...
                //-------------------------------------------------------------------------------------------------------------
                //Added By Dinesh Purpose : ComponetId Based  Bind On LOP Days Calculation 27-05-2026
                DataTable dtPayRollCompID = new DataTable();
                int PAyRollCompId = 0;
                if (dtGrp.Rows.Count > 0)
                {
                    using (DataManager dataManger = new DataManager(SQLCommand.Payroll.FetchPayrollComponetIdandName))
                    {
                        ...
                        if (resultArgs.Success)
                        {
                            dtPayRollCompID = resultArgs.DataSource.Table;

                            if (dtPayRollCompID != null && dtPayRollCompID.Rows.Count > 0)
                            {
                                PAyRollCompId = Convert.ToInt32(dtPayRollCompID.Rows[0]["COMPONENTID"]);
                            }
                        }
                    }
                }
                            ...
                            if (dtGrp1.Rows.Count > 0)
                            {
                                for (int J = 0; J < dtGrp1.Rows.Count + 1; J++)
                                {
                                    ...
                                    else if (dtGrp1.Rows[J]["component"].ToString() == strFields[J])
                                    {
                                        try
                                        {
                                            bool Result = TryParsePayrollCompValueForBrowseGrid(dtGrp1.Rows[J]["CompValue"].ToString(), out tmpValue);
                                            if (Result)
                                            {
                                                ...
                                                else if (dtGrp1.Rows[J]["LinkValue"].ToString().ToUpper() == "EMPLOYEENO")
                                                {
                                                    dtPayroll.Rows[i][J] = Convert.ToInt32(tmpValue).ToString();
                                                }
                                                else if (dtGrp1.Rows[J]["LinkValue"].ToString().ToUpper() == PayRollExtraPayInfo.PAYING_SALARY_DAYS.ToString().Replace("_","").ToString())
                                                {
                                                    dtPayroll.Rows[i][J] = tmpValue.ToString();
                                                }
                                                else if (dtGrp1.Rows[J]["LinkValue"].ToString().ToUpper() == "TOTALDAYSINPAYMONTH")
                                                {
                                                    dtPayroll.Rows[i][J] = tmpValue.ToString();
                                                }
                                                else if (dtGrp1.Rows[J]["componentid"] != DBNull.Value && Convert.ToInt32(dtGrp1.Rows[J]["componentid"]) == PAyRollCompId)
                                                {
                                                    dtPayroll.Rows[i][J] = Convert.ToInt32(tmpValue).ToString(); //Added By Dinesh : Purpose Of Rounded of LOP Days Only 
                                                }
                                                else
                                                {
                                                    dtPayroll.Rows[i][J] = FormatPayrollBrowseMoneyCell(tmpValue);
                                                }
                                            }
                                            else
                                            {
                                        ...
                                        foreach (string item in strFields)
                                        {
                                            if (dtGrp1.Rows[J]["component"].ToString() == item)
                                            {
                                                ...
                                                if (Result)
                                                {
                                                    ...
                                                    {
                                                        dtPayroll.Rows[i][compIndex] = FormatPayrollBrowseMoneyCell(tmpValue);
                                                    }
