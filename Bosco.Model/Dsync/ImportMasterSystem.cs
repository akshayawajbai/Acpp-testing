                    ...
                    foreach (DataRow drRowLedgerGroup in dtLedgerGroup.Rows)
                    {
                        using (LedgerGroupSystem MasterLedgerGroup = new LedgerGroupSystem())
                        {
                            ...
                            if (!string.IsNullOrEmpty(MasterLedgerGroup.Group))
                            {
                                ...
                                // 17/02/2026, added DisplayLedgerGroup from portal - insert/update when column exists
                                if (dtLedgerGroup.Columns.Contains(this.AppSchema.LedgerGroup.DISPLAY_LEDGER_GROUPColumn.ColumnName))
                                {
                                    object displayValue = drRowLedgerGroup[this.AppSchema.LedgerGroup.DISPLAY_LEDGER_GROUPColumn.ColumnName];
                                    MasterLedgerGroup.DisplayLedgerGroup = (displayValue != null && displayValue != DBNull.Value) ? displayValue.ToString() : string.Empty;
                                }
                        ...
                        using (LedgerSystem ledgerSystem = new LedgerSystem())
                        {
                            ...
                            if (!string.IsNullOrEmpty(LedgerName))
                            {
                                ...
                                if (ledgerSystem.GroupId != (int)FixedLedgerGroup.BankAccounts)
                                {
                                    ...
                                    // 17/02/2026, added DisplayLedgerName from portal - insert/update when column exists
                                    if (dtLedger.Columns.Contains(this.AppSchema.Ledger.DISPLAY_LEDGER_NAMEColumn.ColumnName))
                                    {
                                        object displayValue = drLedger[this.AppSchema.Ledger.DISPLAY_LEDGER_NAMEColumn.ColumnName];
                                        ledgerSystem.DisplayLedgerName = (displayValue != null && displayValue != DBNull.Value) ? displayValue.ToString() : string.Empty;
                                    }
        ...
        public ResultArgs SaveMasterHeadOfficeProjects(DataTable dtProjects)
        {
            ...
            foreach (DataRow drProject in dtProjects.Rows)
            {
                using (ProjectSystem project = new ProjectSystem())
                {
                    ...
                    // 17/02/2026, added DisplayProject from portal - insert/update when column exists
                    if (dtProjects.Columns.Contains(this.AppSchema.Project.DISPLAY_PROJECTColumn.ColumnName))
                    {
                        object displayValue = drProject[this.AppSchema.Project.DISPLAY_PROJECTColumn.ColumnName];
                        project.DisplayProject = (displayValue != null && displayValue != DBNull.Value) ? displayValue.ToString() : string.Empty;
                    }
                    if (!string.IsNullOrEmpty(ProjectName))
                    {
