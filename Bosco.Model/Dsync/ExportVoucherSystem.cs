...
namespace Bosco.Model.Dsync
{
    public class ExportVoucherSystem : SystemBase
    {
                                                ...
                                                if (resultArgs.Success)
                                                {
                                                    ...
                                                    if (resultArgs.Success)
                                                    {
                                                        ...
                                                        if (resultArgs.Success)
                                                        {
                                                            ...
                                                            if (resultArgs.Success)
                                                            {
                                                                resultArgs = FetchCountryExchangeCurrency(); // Country Currency Details
                                                                if (resultArgs.Success)
                                                                {
        ...
        /// </summary>
        /// <returns></returns>
        private ResultArgs FetchCountryExchangeCurrency()
        {
            try
            {
                AcMELog.WriteLog("FetchCountryCurrencyRate Started.");
                using (DataManager dataManager = new DataManager(SQLCommand.ExportVouchers.FetchCountryCurrencyExchangeRate))
                {
                    dataManager.Parameters.Add(this.AppSchema.ExportVouchers.DATE_FROMColumn, DateFrom);
                    dataManager.Parameters.Add(this.AppSchema.ExportVouchers.DATE_TOColumn, DateTo);
                    dataManager.DataCommandArgs.IsDirectReplaceParameter = true;
            ...
            if (resultArgs.Success)
            {
                AcMELog.WriteLog("FetchCountryCurrencyRate Ended.");
                resultArgs.DataSource.Table.TableName = COUNTRY_CURRENCY_TABLE_NAME;
                dsTransaction.Tables.Add(resultArgs.DataSource.Table);
            }
            else
            {
                resultArgs.Message = "Error in FetchCountryCurrencyRate: " + resultArgs.Message;
            }
            return resultArgs;
        }
