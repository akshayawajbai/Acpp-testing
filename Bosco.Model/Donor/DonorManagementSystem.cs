                    ...
                    if (!string.IsNullOrEmpty(ProjectIds))
                    {
                        ...
                        if (resultArgs.Success)
                        {
                            ...
                            if (dtTrans != null && dtTrans.Rows.Count > 0)
                            {
                                ...
                                { 
                                    //bharath 17/06/2026
                                    //For SDB_RMG, delete only EMTV vouchers and skip other client codes.
                                    if (this.IS_SDB_RMG)
                                    {
                                        string clientCode = Convert.ToString(drTrans["CLIENT_CODE"]).Trim();

                                        if (!clientCode.Equals("EMTV", StringComparison.OrdinalIgnoreCase))
                                        {
                                            continue;
                                        }
                                    }
                    ...
                    using (DataManager dm = new DataManager())
                    {
                        foreach (DataRow drTrans in dtTransaction.Rows)
                        {
                            using (VoucherTransactionSystem voucherTransaction = new VoucherTransactionSystem())
                            {
                                ...
                                if (voucherTransaction.DonorId > 0 ||IS_SDB_RMG)
                                {
                                    voucherTransaction.ContributionType = "F";
                                    //voucherTransaction.CurrencyCountryId = voucherTransaction.ExchageCountryId = 1;
                                    //bharath udeg set for local Currency set
                                    int countryId = 1;
                                    if (dtTransaction.Columns.Contains("Country"))
                                    {
                                        string countryName = string.Empty;

                                        if (drTrans["Country"] != DBNull.Value)
                                        {
                                            countryName = drTrans["Country"].ToString().Trim();
                                        }
                                        else
                                        {
                                            if (IS_SDB_RMG)
                                        ...
                                        if (!string.IsNullOrEmpty(countryName))
                                        {
                                            ...
                                            {
                                                int tempId = countrysystem.GetCountryId(countryName);
                                                ...
                                                {
                                                    resultArgs.Message = "Country is not available. The required Country is '" + countryName + "'";
                                                }
                                            }
                                        }
                                    }
                                ...
                                else if ((!string.IsNullOrEmpty(DonorName)) && voucherTransaction.DonorId == 0)
                                {
                                    ...
                                    //bharath 12/03/2026 if sdbrmg prupose is not need
                                else if (voucherTransaction.DonorId > 0 && voucherTransaction.PurposeId == 0 && !IS_SDB_RMG)
                                {
                                                    ...
                                                    if (resultArgs.Success)
                                                    {
                                                        ...
                                                        if (resultArgs.Success)
                                                        {
                                                            ...
                                                            if (resultArgs.Success)
                                                            {
                                                                ...
                                                                //bharath to inset cost center 28/02/2026 
                                                                if (dtTransaction.Columns.Contains("Cost Centre") &&
                                                                    !string.IsNullOrWhiteSpace(drTrans["Cost Centre"].ToString()))
                                                                {
                                                                    string costCentreName = drTrans["Cost Centre"].ToString().Trim();
                                                                    ...
                                                                    if (costCentreId == 0)
                                                                    {
                                                                        resultArgs.Message = "Cost Centre is not available. The required Cost Centre is '"
                                                                                              + costCentreName + "'";
                                                                    ...
                                                                    if (costCentreId != 0)
                                                                        using (CostCentreSystem costcentre = new CostCentreSystem())
                                                                        {
                                                                            ...
                                                                            costcentre.ProjectId = voucherTransaction.ProjectId;
                                                                            resultArgs = costcentre.CheckCostCentreMapped();
                                                                            if (resultArgs.Success && resultArgs.DataSource.Sclar.ToInteger == 0)
                                                                            {
                                                                                resultArgs.Message = "Cost Centre '" + costCentreName + "' is not mapped to the selected Project. Please map it before importing the voucher.";
                                                                                resultArgs.Success = false;
                                                                                break;
                                                                            }
                                                                        }
                                                                    ...
                                                                    string tableName = "0LDR" + GeneralLedgerId;

                                                                    DataTable dtCC = new DataTable(tableName);
                                                                    dtCC.Columns.Add("COST_CENTRE_ID", typeof(int));
                                                                    dtCC.Columns.Add("AMOUNT", typeof(decimal));

                                                                    dtCC.Rows.Add(costCentreId, GeneralLedgerAmout);
                                                                    dsCostCentre.Tables.Add(dtCC);

                                                                    voucherTransaction.CostCenterInfo = dsCostCentre;
                                                                }
                                                                else
                                                                {
                                                                    voucherTransaction.CostCenterInfo = null;
                                                                }
