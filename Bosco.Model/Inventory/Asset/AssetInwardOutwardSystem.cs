...
namespace Bosco.Model
{
    public class AssetInwardOutwardSystem : SystemBase
    {
        ...
        public int assetCashBankEntry { get; set; }
        ...
        public int SequenceNo { get; set; } //Added By //---Dinesh
        public int SourceId { get; set; } //Added By //---Dinesh
        public int ledgerId { get; set; } //Added By //---Dinesh
        public int CrDetailId { get; set; }//Added By //---Dinesh
        ...
        public DataSet FetchAssetInOutDetailsByFlag()
        {
            ...
            try
            {
                    ...
                    && resultArgs.DataSource.Table.Rows.Count > 0)
                {
                    ...
                    resultArgs = FetchAssetPaymentDetail(InOutId);
                    if (resultArgs != null && resultArgs.Success && resultArgs.DataSource.Table != null && resultArgs.DataSource.Table.Rows.Count > 0)
                    {
                        resultArgs.DataSource.Table.TableName = "Payment Detail";
                        dsAssetInOut.Tables.Add(resultArgs.DataSource.Table);
                        dsAssetInOut.Relations.Add(dsAssetInOut.Tables[2].TableName, dsAssetInOut.Tables[0].Columns[this.AppSchema.AssetInOut.IN_OUT_IDColumn.ColumnName], dsAssetInOut.Tables[2].Columns[this.AppSchema.AssetInOut.IN_OUT_IDColumn.ColumnName]);
                    }
        ...
        public ResultArgs FetchAssetPaymentDetail(string InOutID)
        {
            using (DataManager dataManger = new DataManager(SQLCommand.AssetInOut.FetchAssetPaymentDetail))
            {
                dataManger.Parameters.Add(this.AppSchema.AssetInOut.IN_OUT_IDColumn, InOutID);
        ...
        //        {
        //            SequenceNo = assetSeqNo++; //---Dinesh

        //            //SourceId = drInwardOutward[this.AppSchema.AssetInOut.SOURCEColumn.ColumnName] != null ?  //---Dinesh
        //            //    this.NumberSet.ToInteger(drInwardOutward[this.AppSchema.AssetInOut.SOURCEColumn.ColumnName].ToString()) : 0;
        //            if (dtinoutword.Columns.Contains(this.AppSchema.AssetInOut.SOURCEColumn.ColumnName))
        //            {
        //                SourceId = NumberSet.ToInteger(drInwardOutward[this.AppSchema.AssetInOut.SOURCEColumn.ColumnName].ToString());
        //            }
        //            else
        //            {
        //                SourceId = 0;
        //            }
        //            ItemId = (drInwardOutward[this.AppSchema.AssetInOut.ITEM_IDColumn.ColumnName] != DBNull.Value) ?
        ...
        private ResultArgs SaveInOutDetails(DataManager dmanager)
        {
            using (DataManager dataManager = new DataManager(InoutDetailId == 0 ? SQLCommand.AssetInOut.SaveAssetInOutDetail : SQLCommand.AssetInOut.UpdateAssetInOutDetail))
            {
                ...
                dataManager.Parameters.Add(this.AppSchema.AssetInOut.SEQUENCE_NOColumn, SequenceNo); //---Dinesh
                dataManager.Parameters.Add(this.AppSchema.AssetInOut.SOURCEColumn, SourceId);//---Dinesh
                dataManager.DataCommandArgs.IsDirectReplaceParameter = true;
                resultArgs = dataManager.UpdateData();
            }
            return resultArgs;
        ...
        //added By Dinesh 30-12-2025 :purpose Of Insert All Cr Row Details 
        private ResultArgs SaveAssetCrDetails(DataManager CrManger)
        {
            int assetSeqNo = 1;
            if (dtinoutword != null && dtinoutword.Rows.Count > 0)
            {
                foreach (DataRow drInwardOutward in dtinoutword.Rows)
                {
                    SequenceNo = assetSeqNo++;

                    SourceId = drInwardOutward[this.AppSchema.AssetInOutCr.SOURCEColumn.ColumnName] != null ?
                        this.NumberSet.ToInteger(drInwardOutward[this.AppSchema.AssetInOutCr.SOURCEColumn.ColumnName].ToString()) : 0;

                    if (SourceId == 2)
                        continue;

                    CrDetailId = drInwardOutward[this.AppSchema.AssetInOutCr.CR_DETAIL_IDColumn.ColumnName] != null ?
                        this.NumberSet.ToInteger(drInwardOutward[this.AppSchema.AssetInOutCr.CR_DETAIL_IDColumn.ColumnName].ToString()) : 0;

                    ledgerId = drInwardOutward[this.AppSchema.AssetInOutCr.LEDGER_IDColumn.ColumnName] != DBNull.Value ?
                        this.NumberSet.ToInteger(drInwardOutward[this.AppSchema.AssetInOutCr.LEDGER_IDColumn.ColumnName].ToString()) : 0;

                    Amount = drInwardOutward[this.AppSchema.AssetInOutCr.AMOUNTColumn.ColumnName] != DBNull.Value ?
                        this.NumberSet.ToDouble(drInwardOutward[this.AppSchema.AssetInOutCr.AMOUNTColumn.ColumnName].ToString()) : 0;

                    resultArgs = SaveAssetCrDetail(CrManger);
                    if (!resultArgs.Success)
                        break;
                }
            }
        ...
        //added By Dinesh 30-12-2025 :purpose Of Insert All Cr Row Details 
        private ResultArgs SaveAssetCrDetail(DataManager dManager)
        {
            using (DataManager dataManger = new DataManager(CrDetailId == 0 ? SQLCommand.AssetInOut.SaveAssetCrDetail : SQLCommand.AssetInOut.UpdateAssetCrDetail))
            {
                dataManger.Database = dManager.Database;
                dataManger.Parameters.Add(this.AppSchema.AssetInOutCr.CR_DETAIL_IDColumn, CrDetailId, true);
                dataManger.Parameters.Add(this.AppSchema.AssetInOutCr.IN_OUT_IDColumn, InoutId);
                dataManger.Parameters.Add(this.AppSchema.AssetInOutCr.LEDGER_IDColumn, ledgerId);
                dataManger.Parameters.Add(this.AppSchema.AssetInOutCr.SOURCEColumn, SourceId);
                dataManger.Parameters.Add(this.AppSchema.AssetInOutCr.SEQUENCE_NOColumn, SequenceNo);
                dataManger.Parameters.Add(this.AppSchema.AssetInOutCr.AMOUNTColumn, Amount);
        ...
        //Added By Dinesh 05-01-2026 :Purpose of Delete all Cr Rows in Asset_Cr_detail,tbl
        public ResultArgs DeleteCrEntries(int inOutId, int ledgerId)
        {
            using (DataManager dtManger = new DataManager(SQLCommand.AssetInOut.DeleteAssetCrDetail))
            {
                dtManger.Parameters.Add(this.AppSchema.AssetInOutCr.IN_OUT_IDColumn, inOutId);
                dtManger.Parameters.Add(this.AppSchema.AssetInOutCr.LEDGER_IDColumn, ledgerId);
                dtManger.DataCommandArgs.IsDirectReplaceParameter = true;
                resultArgs = dtManger.UpdateData();
            }
            return resultArgs;
        }
        ...
        {
            // 30/01/2026, chinna: use upsert (insert or update) instead of delete+insert so existing rows are updated
            if (InoutDetailId <= 0 || ItemDetailId <= 0)
            {
                resultArgs.Success = false;
                resultArgs.Message = "Cannot save asset transaction: detail or item ID is missing. Save detail and item first.";
                return resultArgs;
            }
        ...
        private bool AssetTransRowExists(DataManager dmanager)
        {
            using (DataManager dataManager = new DataManager(SQLCommand.AssetInOut.CheckAssetTransRowExists))
            {
                dataManager.Database = dmanager.Database;
                dataManager.Parameters.Add(this.AppSchema.AssetInOut.IN_OUT_DETAIL_IDColumn, InoutDetailId);
                dataManager.Parameters.Add(this.AppSchema.ASSETItem.ITEM_DETAIL_IDColumn, ItemDetailId);
            ...
            int cnt = resultArgs.DataSource.Sclar.ToInteger;
            if (cnt > 0)
                return true;
            else
                return false;
        ...
        private ResultArgs SaveFixedAssetTrans(DataManager dmanager)
        {
            try
            {
                // 30/01/2026, chinna: when row already exists (InoutDetailId there) only UPDATE to avoid FK/duplicate; when new only INSERT. Use IN_OUT_DETAIL_ID and ITEM_DETAIL_ID only.
                bool rowExists = AssetTransRowExists(dmanager);

                if (rowExists)
                {
                    using (DataManager updateManager = new DataManager(SQLCommand.AssetInOut.UpdateAssetTrans))
                    {
                        updateManager.Database = dmanager.Database;
                        updateManager.Parameters.Add(this.AppSchema.AssetInOut.IN_OUT_DETAIL_IDColumn, InoutDetailId);
                        updateManager.Parameters.Add(this.AppSchema.ASSETItem.ITEM_DETAIL_IDColumn, ItemDetailId);
                        updateManager.Parameters.Add(this.AppSchema.AssetInOut.DEPRECIATION_AMOUNTColumn, DeprAmount);
                        updateManager.Parameters.Add(this.AppSchema.AssetInOut.AMOUNTColumn, AssetAmount);
                        updateManager.Parameters.Add(this.AppSchema.AssetInOut.GAIN_AMOUNTColumn, GainAmount);
                        updateManager.Parameters.Add(this.AppSchema.AssetInOut.LOSS_AMOUNTColumn, LossAmount);
                        resultArgs = updateManager.UpdateData();
                    }
                }
                else
                {
            ...
            catch (Exception ex)
            {
                ...
                if (msg.IndexOf("foreign key", StringComparison.OrdinalIgnoreCase) >= 0 ||
                    ...
                    msg.IndexOf("1452", StringComparison.Ordinal) >= 0)
                    resultArgs.Message = "Asset transaction could not be saved: reference to detail or item is invalid. Ensure the movement detail and asset item are saved in the same document.";
                else
        ...
        public ResultArgs SaveAssetInwardOutward()
        {
            try
            {
                using (DataManager dataManager = new DataManager())
                {
                    ...
                    {
                        // 30/01/2026, chinna: only delete when new OP (InoutId==0); when updating (InoutId>0) do not delete
                        if (InoutId == 0)
                    ...
                    if (resultArgs.Success)
                    {
                        ...
                        if (resultArgs.Success)
                        {
                            ...
                            if (resultArgs.Success) //added By Dinesh :Purpose of Insert Asset Cr Details 
                            {
                                //resultArgs = SaveAssetCrDetails(dataManager);
                            }
                        }
                    }
                    if (!resultArgs.Success)
        ...
        // added by Dinesh -
        private bool IsCashOrBankLedger(int ledgerId)
        {
            using (LedgerSystem ledgerSystem = new LedgerSystem())
            {
                ledgerSystem.LedgerId = ledgerId;

                int groupId = ledgerSystem.FetchLedgerGroupById(); // 

                return (groupId == 12 || groupId == 13);
            }
        }

        private bool IsCashBankOrSundryLedger(int ledgerId) //Added By Dinesh 09-04-2026 Asset Part Payment 
        {
            using (LedgerSystem ledgerSystem = new LedgerSystem())
            {
                ledgerSystem.LedgerId = ledgerId;
        ...
        public ResultArgs SaveAssetVouchers(DataManager dm)
        {
            try
            {
                ...
                using (VoucherTransactionSystem voucherTransaction = new VoucherTransactionSystem())
                {
                    ...
                    //    ? (Int32)DefaultVoucherTypes.Journal : (Int32)DefaultVoucherTypes.Receipt);  //Commed By Dinesh 30-03-2026
                    voucherTransaction.VoucherType = (Flag == AssetInOut.PU.ToString() || Flag == AssetInOut.AMC.ToString() || Flag == AssetInOut.INS.ToString() ? DefaultVoucherTypes.Payment.ToString()
                                       : Flag == AssetInOut.IK.ToString() || Flag == AssetInOut.DN.ToString() || Flag == AssetInOut.DS.ToString()
                                       ? VoucherSubTypes.JN.ToString() : DefaultVoucherTypes.Receipt.ToString());
                    voucherTransaction.VoucherDefinitionId = (Flag == AssetInOut.PU.ToString() || Flag == AssetInOut.AMC.ToString() || Flag == AssetInOut.INS.ToString()
                        ? (Int32)DefaultVoucherTypes.Payment : Flag == AssetInOut.IK.ToString() || Flag == AssetInOut.DN.ToString() || Flag == AssetInOut.DS.ToString()
                        ? (Int32)DefaultVoucherTypes.Journal : (Int32)DefaultVoucherTypes.Receipt);
                    if (Flag == AssetInOut.DS.ToString())
                    {
                        foreach (DataRow dr in dtCashBank.Rows)
                        {
                            int ledgerId = NumberSet.ToInteger(dr["LEDGER_ID"].ToString());
                            if (IsCashOrBankLedger(ledgerId))
                            {
                    ...
                    if (Flag == AssetInOut.PU.ToString()) //Added By Dinesh 09-04-2026 Asset Part Payment 
                    {
                        foreach (DataRow dr in dtCashBank.Rows)
                        {
                            int ledgerId = NumberSet.ToInteger(dr["LEDGER_ID"].ToString());
                            if (IsCashBankOrSundryLedger(ledgerId))
                            {
                    ...
                    voucherTransaction.CashBankEntry = assetCashBankEntry == 1 ? 1 : 0;
                    ...
                    else
                    {
                        ...
                        if (EnumurableInoutword.Count() > 0)
                        {
                            ...
                            if (dtAssetVoucher != null && dtAssetVoucher.Rows.Count > 0)
                            {
                                foreach (DataRow drLedger in dtAssetVoucher.Rows)
                                {
                                    ...
                                    GROUPID = drLedger["GROUP_ID"] != DBNull.Value ? Convert.ToInt32(drLedger["GROUP_ID"]) : 0; //30-03-2026
                                    SourceId = drLedger["SOURCE"] != DBNull.Value ? Convert.ToInt32(drLedger["SOURCE"]) : 0; //30-03-2026
                                    if (Flag == AssetInOut.DS.ToString() && voucherTransaction.VoucherType == DefaultVoucherTypes.Receipt.ToString())
                                    {
                                        if (GROUPID == 12 || GROUPID == 13 || SourceId == 2)
                                            continue;
                                    }
                        ...
                        //added by Dinesh :
                        bool hasNormalLedger = dtCashBank.AsEnumerable().Any(row => row["GROUP_ID"] != DBNull.Value && !new int[] { 12, 13 }.Contains(Convert.ToInt32(row["GROUP_ID"])));
                        if (Flag == AssetInOut.SL.ToString() || Flag == AssetInOut.DS.ToString() && hasNormalLedger) // if (Flag == AssetInOut.SL.ToString())
                        {
                            dTransDRAmount = this.NumberSet.ToDecimal(this.TransInfo.Table.Compute("SUM(" + "AMOUNT" + ")", "SOURCE" + "=" + (int)TransSource.Dr).ToString());
                            dTransCRAmount = this.NumberSet.ToDecimal(this.TransInfo.Table.Compute("SUM(" + "AMOUNT" + ")", "SOURCE" + "=" + (int)TransSource.Cr).ToString());
                        }
                        ...
                        if (this.AllowMultiCurrency == 1)
                        {
                            if (this.TransInfo != null)
                            {
                                ...
                                {
                                    drv.BeginEdit();
                                    if (!this.TransInfo.Table.Columns.Contains(voucherTransaction.AppSchema.VoucherTransaction.EXCHANGE_RATEColumn.ColumnName))
                                    {
                                        TransInfo.Table.Columns.Add("EXCHANGE_RATE", typeof(decimal));
                                        TransInfo.Table.Columns.Add("LIVE_EXCHANGE_RATE", typeof(decimal));
                                    }
                                    drv[voucherTransaction.AppSchema.VoucherTransaction.EXCHANGE_RATEColumn.ColumnName] = AssetExchangeRate;
                                    drv[voucherTransaction.AppSchema.VoucherTransaction.LIVE_EXCHANGE_RATEColumn.ColumnName] = AssetLiveExchangeRate;
                                    drv.EndEdit();
                            ...
                            if (this.CashTransInfo != null)
                            {
                                ...
                                {
                                    drvCash.BeginEdit();
                                    if (!this.CashTransInfo.Table.Columns.Contains(voucherTransaction.AppSchema.VoucherTransaction.EXCHANGE_RATEColumn.ColumnName))
                                    {
                                        CashTransInfo.Table.Columns.Add("EXCHANGE_RATE", typeof(decimal));
                                        CashTransInfo.Table.Columns.Add("LIVE_EXCHANGE_RATE", typeof(decimal));
                                    }
                                    drvCash[voucherTransaction.AppSchema.VoucherTransaction.EXCHANGE_RATEColumn.ColumnName] = AssetExchangeRate;
                                    drvCash[voucherTransaction.AppSchema.VoucherTransaction.LIVE_EXCHANGE_RATEColumn.ColumnName] = AssetLiveExchangeRate;
                                    drvCash.EndEdit();
        ...
        private ResultArgs ConstructLedgerTransaction()
        {
            ...
            try
            {
                if (dtinoutword != null && dtinoutword.Rows.Count > 0)
                {
                    ...
                    //    }
                    //}
                    if (dtinoutword.Columns.Contains("LEDGER_ID") && dtinoutword.Columns.Contains("GROUP_ID"))
                    {
                        IEnumerable<DataRow> fillteredRows = dtinoutword.Rows.Cast<DataRow>()
                            .Where(row =>
                            {
                                int groupId = NumberSet.ToInteger(row["GROUP_ID"].ToString());
                                int ledgerId = NumberSet.ToInteger(row["LEDGER_ID"].ToString());
                                string itemId = row["ITEM_ID"] == DBNull.Value ? "" : row["ITEM_ID"].ToString();

                                return !string.IsNullOrEmpty(itemId) || (string.IsNullOrEmpty(itemId) && groupId == 26 && ledgerId > 0);
                            });
                        if (fillteredRows.Any())
                        {
                            dtinoutword = fillteredRows.CopyToDataTable();
                        }
                        else
                        {
                            dtinoutword = dtinoutword.Clone();
                        }
                    }
                    else if (dtinoutword.Rows.Count > 0 && dtinoutword.Columns.Contains("LEDGER_ID"))
                    {
                        IEnumerable<DataRow> EnumurableInoutword = dtinoutword.Rows.Cast<DataRow>().Where(row => string.IsNullOrEmpty(row["ITEM_ID"].ToString()) ? false : true);
                        if (EnumurableInoutword.Count() > 0)
                        {
