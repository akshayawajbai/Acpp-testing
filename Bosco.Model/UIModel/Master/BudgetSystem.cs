...
namespace Bosco.Model.UIModel
{
    ...
    public class BudgetSystem : SystemBase
    {
        ...
        public ResultArgs GetBudgetSourceAPI(int BudgetId)
        {
            this.BudgetId = BudgetId;
            resultArgs = FetchBudgetDetailsByIdExportData(BudgetId);
            return resultArgs;
        }
        ...
        private ResultArgs FetchBudgetDetailsByIdExportData(int BudgetId)
        {
            using (DataManager dataManager = new DataManager(SQLCommand.Budget.FetchByIdtoUdeg))
            {
                dataManager.Parameters.Add(this.AppSchema.Budget.BUDGET_IDColumn, BudgetId);
                dataManager.DataCommandArgs.IsDirectReplaceParameter = true;
                resultArgs = dataManager.FetchData(DataSource.DataSet);
            }
            return resultArgs;
        }
        ...
        public ResultArgs FetchBudgetProjectsLookup()
        {
            using (DataManager dataManager = new DataManager(SQLCommand.Budget.FetchBudgetProjectforLookup))
            {
                ...
                //string value = DisplayInputLanguage;
                dataManager.DataCommandArgs.IsDirectReplaceParameter = true;
                resultArgs = dataManager.FetchData(DataSource.DataTable);
            }
            return resultArgs;
