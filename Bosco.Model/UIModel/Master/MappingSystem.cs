...
namespace Bosco.Model.UIModel
{
    public class MappingSystem : SystemBase
    {
        ...
        const string AMOUNT_FC = "AMOUNT_FC";
        const string OP_EXCHANGE_RATE = "OP_EXCHANGE_RATE";
        ...
        public ResultArgs FetchProjectsLookup(string yearto = "")
        {
            using (DataManager dataManager = new DataManager(SQLCommand.Mapping.FetchProjectforLookup))
            {
                ...
                //bharath 18/02/2026 language update
        ...
        public ResultArgs LoadProjectMappingGrid()
        {
            ...
            {
                // UcAccountMapping.FetchMappedLedgers() sets LedgerId only — LedgerGroupId stays 0 until the user changes group on the form.
                // Resolve group from MASTER_LEDGER so ?GROUP_ID / sundry FC flags match on second load (e.g. UcAccountMapping_Load after edit).
                int effectiveLedgerGroupId = LedgerGroupId > 0 ? LedgerGroupId : 0;
                if (effectiveLedgerGroupId == 0 && LedgerId > 0)
                {
                    using (LedgerSystem ledgerSystem = new LedgerSystem(LedgerId))
                    {
                        effectiveLedgerGroupId = ledgerSystem.GroupId;
                    }
                }
                ...
                dataManager.Parameters.Add(this.AppSchema.Ledger.GROUP_IDColumn, effectiveLedgerGroupId);
                dataManager.Parameters.Add(this.AppSchema.Ledger.LEDGER_IDColumn, LedgerId);
                //18-02-2026 Added By Dinesh 
                dataManager.Parameters.Add(this.AppSchema.Project.DISPLAY_INPUT_LANColumn, SettingProperty.Current.DisplayInputLanguage == "1" ? 1 : 0);
                // 14-05-2026, Option 2: BSGRMG + sundry debtors/creditors — SQL uses AMOUNT_FC for OP when CUR_COUNTRY_ID > 0 (same rule shape as cash/bank/FD groups 12,13,14).
                dataManager.Parameters.Add("IS_BSGRMG", this.IS_BSGRMG ? 1 : 0);
                dataManager.Parameters.Add("LEDGER_GROUP_IS_SUNDRY_FC",
                    (effectiveLedgerGroupId == (int)TDSDefaultLedgers.SundryDebtors || effectiveLedgerGroupId == (int)TDSDefaultLedgers.SunderyCreditors) ? 1 : 0);
                dataManager.DataCommandArgs.IsDirectReplaceParameter = true;
                resultArgs = dataManager.FetchData(DataSource.DataTable);
            }
            return resultArgs;
        ...
        public ResultArgs LoadProjectCostCentreMappingGrid(int Id)
        {
            using (DataManager dataManager = new DataManager(SQLCommand.Mapping.LoadProjectCostCentreGrid))
            {
                ...
                //Added By Dinesh C :Purpose of Multi lang Process
                dataManager.Parameters.Add(this.AppSchema.Project.DISPLAY_INPUT_LANColumn, SettingProperty.Current.DisplayInputLanguage == "1" ? 1 : 0);
                dataManager.DataCommandArgs.IsDirectReplaceParameter = true;
                resultArgs = dataManager.FetchData(DataSource.DataTable);
            }
        ...
        {
            // return this.AllowMultiCurrency == 1 && (this.IS_SDBAFE || this.IS_BSGRMG);
            return this.AllowMultiCurrency == 1 && (this.IS_BSGRMG);
        }

        private int ResolveMappingRowLedgerId(DataRow dr)
        {
            if (dr == null || dr.Table == null)
                return 0;
            string ledgerIdColumn = this.AppSchema.Ledger.LEDGER_IDColumn.ColumnName;
            if (dr.Table.Columns.Contains(ledgerIdColumn) && dr[ledgerIdColumn] != DBNull.Value
                && !string.IsNullOrWhiteSpace(dr[ledgerIdColumn].ToString()))
            {
                return this.NumberSet.ToInteger(dr[ledgerIdColumn].ToString());
            }
            return LedgerId > 0 ? LedgerId : 0;
        }

        /// <summary>
        /// BSGRMG: general-ledger OP in FC (incl. sundry creditors/debtors). Rate conversion is only in BalanceSystem.UpdateOpBalance.
        /// </summary>
        private bool IsBSGRMGGeneralLedgerCurrencyRow(DataRow dr)
        {
            if (!IsSDBAFEMultiCurrencyGeneralLedgerMode() || dr == null || dr.Table == null)
                return false;

            Int32 groupId = 0;
            Int32 currencyCountryId = 0;
            string ledgerGroup = string.Empty;
            string groupIdColumn = this.AppSchema.Ledger.GROUP_IDColumn.ColumnName;
            string curCountryColumn = this.AppSchema.Ledger.CUR_COUNTRY_IDColumn.ColumnName;

            if (dr.Table.Columns.Contains(groupIdColumn) && dr[groupIdColumn] != DBNull.Value)
                groupId = this.NumberSet.ToInteger(dr[groupIdColumn].ToString());
            if (dr.Table.Columns.Contains(curCountryColumn) && dr[curCountryColumn] != DBNull.Value)
                currencyCountryId = this.NumberSet.ToInteger(dr[curCountryColumn].ToString());
            if (dr.Table.Columns.Contains("GROUP") && dr["GROUP"] != DBNull.Value)
                ledgerGroup = dr["GROUP"].ToString();

            Int32 ledgerId = ResolveMappingRowLedgerId(dr);
            if (ledgerId > 0 && (groupId == 0 || currencyCountryId == 0))
            {
                using (LedgerSystem ledgerSystem = new LedgerSystem(ledgerId))
                {
                    if (groupId == 0)
                    ...
                    if (currencyCountryId == 0)
                        currencyCountryId = ledgerSystem.LedgerCurrencyCountryId;
                }
            }
            ...
            if (groupId == (Int32)FixedLedgerGroup.BankAccounts ||
                groupId == (Int32)FixedLedgerGroup.Cash ||
                groupId == (Int32)FixedLedgerGroup.FixedDeposit)
            {
                return false;
            }

            if (groupId == (Int32)TDSDefaultLedgers.SundryDebtors || groupId == (Int32)TDSDefaultLedgers.SunderyCreditors)
                return true;

            return ledgerGroup == "General";
        }

        private bool IsEditableGeneralLedgerCurrencyRow(DataRow dr)
        {
            return IsBSGRMGGeneralLedgerCurrencyRow(dr);
        }

        /// <summary>
        /// BSGRMG general/sundry: read FC from grid only. Local = FC × rate is done once in BalanceSystem.UpdateOpBalance.
        /// </summary>
        private void GetLedgerOpeningAmounts(DataRow dr, out double amountLocal, out double amountFC)
        {
            amountLocal = this.NumberSet.ToDouble(dr[AMOUNT].ToString());
            amountFC = 0;

            if (!IsBSGRMGGeneralLedgerCurrencyRow(dr) || amountLocal <= 0)
                return;

            if (dr.Table.Columns.Contains(AMOUNT_FC))
            {
                amountFC = this.NumberSet.ToDouble(dr[AMOUNT_FC].ToString());
                if (amountFC <= 0)
                    amountFC = amountLocal;
            }
            else
            {
                amountFC = amountLocal;
            ...
            amountLocal = amountFC;
        }

        private ResultArgs UpdateLedgerCurrencyForOpeningBalance(DataRow dr)
        {
            ResultArgs updateResult = new ResultArgs();
            updateResult.Success = true;

            if (!IsEditableGeneralLedgerCurrencyRow(dr))
            {
                return updateResult;
            }

            double enteredAmount = dr.Table.Columns.Contains(AMOUNT_FC) ?
                this.NumberSet.ToDouble(dr[AMOUNT_FC].ToString()) :
                this.NumberSet.ToDouble(dr[AMOUNT].ToString());
            if (enteredAmount <= 0 || !dr.Table.Columns.Contains(this.AppSchema.Ledger.CUR_COUNTRY_IDColumn.ColumnName))
            {
                return updateResult;
            }

            Int32 currencyCountryId = this.NumberSet.ToInteger(dr[this.AppSchema.Ledger.CUR_COUNTRY_IDColumn.ColumnName].ToString());
            if (currencyCountryId == 0)
            {
                return updateResult;
            }
            ...
            if (dr.Table.Columns.Contains(OP_EXCHANGE_RATE))
            {
                exchangeRate = this.NumberSet.ToDouble(dr[OP_EXCHANGE_RATE].ToString());
                if (exchangeRate == 0)
                {
                    exchangeRate = 1;
                }
            }
            else if (dr.Table.Columns.Contains(EXCHANGE_RATE))
            {
                exchangeRate = this.NumberSet.ToDouble(dr[EXCHANGE_RATE].ToString());
                if (exchangeRate == 0)
                {
                    exchangeRate = 1;
                }
            ...
            Int32 ledgerId = this.NumberSet.ToInteger(dr[this.AppSchema.Ledger.LEDGER_IDColumn.ColumnName].ToString());
            using (LedgerSystem ledgerSystem = new LedgerSystem())
            {
                ledgerSystem.FillLedgerProperties(ledgerId);
                ledgerSystem.LedgerCurrencyCountryId = currencyCountryId;
                ledgerSystem.LedgerCurrencyOPExchangeRate = exchangeRate;
                updateResult = ledgerSystem.SaveLedgerDetails();
            }
        ...
        private ResultArgs MapLedger(DataManager dataManagers, DataTable LedgerIds, int ProjectId)
        {
            if (LedgerIds != null && LedgerIds.Rows.Count > 0)
            {
                foreach (DataRow dr in LedgerIds.Rows)
                {
                    ...
                    //if (ledgerId == 1059)
                    //{
                    double openingAmountLocal = 0;
                    double openingAmountFC = 0;
                    GetLedgerOpeningAmounts(dr, out openingAmountLocal, out openingAmountFC);

                    using (BalanceSystem balanceSystem = new BalanceSystem())
                    {

                        if (balanceSystem.HasBalance(ProjectId, ledgerId))
                            balanceSystem.UpdateOpBalance(OpeningBalanceDate, ProjectId, ledgerId, openingAmountLocal, dr[TRANS_MODE].ToString(), TransactionAction.EditBeforeSave, openingAmountFC);
                    }
                    using (DataManager dataManager = new DataManager(SQLCommand.Mapping.ProjectLedgerMappingAdd))
                    {
                        ...
                        if (resultArgs.Success && resultArgs.RowsAffected > 0)
                        {
                            ...
                            {
                                resultArgs = UpdateLedgerCostCentreDistributionAmount(ProjectId, ledgerId, dataManager);
                            }

                            if (resultArgs.Success && dtProjectLedgerApplicableDetails != null && dtProjectLedgerApplicableDetails.Rows.Count > 0)
                            {
                                resultArgs = UpdateProjectLedgerApplicable(MapForm.Ledger, ProjectId, ledgerId, dataManager);
                            }

                            if (resultArgs.Success && IsBSGRMGGeneralLedgerCurrencyRow(dr))
                            {
                                resultArgs = UpdateLedgerCurrencyForOpeningBalance(dr);
                            }
                            ...
                            if (resultArgs.Success)
                            {
                                ...
                                {
                                    if (balanceSystem.HasBalance(ProjectId, ledgerId))
                                        balanceSystem.UpdateOpBalance(OpeningBalanceDate, ProjectId, ledgerId, openingAmountLocal, dr[TRANS_MODE].ToString(), TransactionAction.EditAfterSave, openingAmountFC);
                                    else if (openingAmountFC > 0 || this.NumberSet.ToDouble(dr[AMOUNT].ToString()) > 0)
                                    {
                                        balanceSystem.UpdateOpBalance(OpeningBalanceDate, ProjectId, ledgerId, openingAmountLocal, dr[TRANS_MODE].ToString(), TransactionAction.New, openingAmountFC);
                                    }
                                }
                                //}
                            }
            ...
            if (dtMappingLedger != null && dtMappingLedger.Rows.Count > 0)
            {
                ...
                if (resultArgs != null && resultArgs.Success && FDTransType != LedgerTypes.FD.ToString())
                {
                    foreach (DataRow dr in dtLedgerProjectMapping.Rows)
                    {
                        ...
                        int projectId = NumberSet.ToInteger(dr[this.AppSchema.Project.PROJECT_IDColumn.ColumnName].ToString());
                        using (BalanceSystem balanceSystem = new BalanceSystem())
                        {
                            if (balanceSystem.HasBalance(projectId, LedgerId))
                                resultArgs = balanceSystem.UpdateOpBalance(OpeningBalanceDate, projectId, LedgerId, openingAmountLocal, dr[TRANS_MODE].ToString(), TransactionAction.Cancel, openingAmountFC);
                            resultArgs = balanceSystem.UpdateOpBalance(OpeningBalanceDate, projectId, LedgerId, openingAmountLocal, dr[TRANS_MODE].ToString(), TransactionAction.New, openingAmountFC);
