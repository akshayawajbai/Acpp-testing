...
namespace Bosco.Model.UIModel
{
    public class ProjectCatogorySystem : SystemBase
    {
        ...
        public ResultArgs ChangeProjectCategoryName(string oldCategoryName, string newCategoryName)
        {
            ResultArgs resultArgs = new ResultArgs();

            try
            {
                // Escape any single quotes safely
                string safeNew = newCategoryName.Replace("'", "''");
                string safeOld = oldCategoryName.Replace("'", "''");

                using (DataManager dataMember = new DataManager(SQLCommand.ProjectCatogory.ChangeProjectCategoryNameInMaster))
                {
                    dataMember.Parameters.Add("PROJECT_CATOGORY_NAME", "'" + safeNew + "'");
                    dataMember.Parameters.Add("OLD_PROJECT_CATOGORY_NAME", "'" + safeOld + "'");
