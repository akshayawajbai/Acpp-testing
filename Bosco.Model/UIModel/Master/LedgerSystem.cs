...
namespace Bosco.Model.UIModel
{
    public class LedgerSystem : SystemBase
    {
        ...
        // 17/02/2026, added DisplayLedgerName for second-level language (e.g. French)
        public string DisplayLedgerName { get; set; }
        ...
        /// <summary>
        /// Chinna 10/04/2026: When 1, FetchLedgerByGroup returns all bank ledgers (GROUP_ID 12) with project names; non-bank remains filtered by ProjectId. Default 0 preserves existing behaviour for all callers.
        /// </summary>
        public int BankAllProjects { get; set; }

        /// <summary>
        /// Substring matched against MASTER_LEDGER.LEDGER_NAME (LIKE %value%) with ACCESS_FLAG = 2 for inter-project internal adjustment. Default matches ledger names such as "INTERNAL ADJUSTMENT".
        /// </summary>
        public string InternalAdjustmentLedgerNameMatch { get; set; }

        public const string DefaultInternalAdjustmentLedgerNameMatch = "INTERNAL TRANSFER";
        ...
        public int ProjectCategoryIdThirdParty { get; set; }
        ...
        //Added By Dinesh 18-05-2026 Purpose of Enable for province Ledgers
        public ResultArgs FetchLedgerByProvinceLedge()
        {
            using (DataManager dataManger = new DataManager(SQLCommand.LedgerBank.FetchProvinceLedgers))
            {
                dataManger.Parameters.Add(this.AppSchema.Ledger.DISPLAY_INPUT_LANColumn, SettingProperty.Current.DisplayInputLanguage == "1" ? 1 : 0);
                resultArgs = dataManger.FetchData(DataSource.DataTable);
            }
            return resultArgs;
        }

        public ResultArgs FetchCashBankSundryLedger()  //added By Diensh 09-04-2026
        {
            using (DataManager dataManager = new DataManager(SQLCommand.LedgerBank.FetchCasBankSundry))
            {
        ...
        public ResultArgs FetchLedgerDetailsIntegration()
        {
            using (DataManager dataManager = new DataManager(SQLCommand.LedgerBank.FetchAllIntegration))
            {
                ...
                if (ProjectCategoryIdThirdParty > 0)
                {
                    dataManager.Parameters.Add(this.AppSchema.ProjectCatogory.PROJECT_CATOGORY_IDColumn, ProjectCategoryIdThirdParty);
                }
        ...
        public ResultArgs FetchCCDetailsIntegration()
        {
            using (DataManager dataManager = new DataManager(SQLCommand.LedgerBank.FetchCCIntegraion))
            {
                if (!IsFullRightsReservedUser)
                {
                    dataManager.Parameters.Add(this.AppSchema.UserRole.USERROLE_IDColumn, this.LoginUserRoleId);
        ...
        public ResultArgs FetchBankAccountDetails()
        {
            ...
            {
                //18-02-2026 added By Dinesh C : purpose of Additonal Language
        ...
        public ResultArgs FetchLedgerByGroup()
        {
            using (DataManager dataManager = new DataManager(SQLCommand.LedgerBank.FetchLedgerByGroup))
            {
                ...
                dataManager.Parameters.Add(this.AppSchema.Project.DISPLAY_INPUT_LANColumn, SettingProperty.Current.DisplayInputLanguage == "1" ? 1 : 0);
                // Chinna 10/04/2026: optional journal cash/bank — all banks + project labels (see LedgerSQL FetchLedgerByGroup)
                dataManager.Parameters.Add(this.AppSchema.BankAccount.BANK_ALLColumn, BankAllProjects);
                dataManager.DataCommandArgs.IsDirectReplaceParameter = true;
        ...
        public ResultArgs SaveLedgerDetails()
        {
            using (DataManager dataManager = new DataManager((LedgerId == 0) ? SQLCommand.LedgerBank.Add : SQLCommand.LedgerBank.Update))
            {
                ...
                dataManager.Parameters.Add(this.AppSchema.Ledger.LEDGER_NAMEColumn, LedgerName);
                //dataManager.Parameters.Add(this.AppSchema.Ledger.DISPLAY_LEDGER_NAMEColumn, SettingProperty.Current.DisplayInputLanguage == "1" ? DisplayLedgerName : (object)DBNull.Value);
                // Store Display Ledger Name; save NULL when Display Input Language is disabled or the value is empty.
                //23/06/2026 bharath
                dataManager.Parameters.Add(this.AppSchema.Ledger.DISPLAY_LEDGER_NAMEColumn, (SettingProperty.Current.DisplayInputLanguage == "1" && !string.IsNullOrWhiteSpace(Convert.ToString(DisplayLedgerName))) ? (object)DisplayLedgerName.Trim() : DBNull.Value);
        ...
        public ResultArgs SaveLedgerDetailsByImportMaster()
        {
            using (DataManager dataManager = new DataManager((LedgerId == 0) ? SQLCommand.LedgerBank.Add : SQLCommand.LedgerBank.UpdateByHeadOffice))
            {
                ...
                dataManager.Parameters.Add(this.AppSchema.Ledger.LEDGER_NAMEColumn, LedgerName);
                // 17/02/2026, added DisplayLedgerName for insert and update (second-level language)
                object displayLedgerNameValue = (string.IsNullOrEmpty(DisplayLedgerName) ? (object)DBNull.Value : DisplayLedgerName);
                dataManager.Parameters.Add(this.AppSchema.Ledger.DISPLAY_LEDGER_NAMEColumn, displayLedgerNameValue);
                dataManager.Parameters.Add(this.AppSchema.Ledger.GROUP_IDColumn, GroupId);
                dataManager.Parameters.Add(this.AppSchema.Ledger.LEDGER_TYPEColumn, LedgerType);
                dataManager.Parameters.Add(this.AppSchema.Ledger.LEDGER_SUB_TYPEColumn, LedgerSubType);
                dataManager.Parameters.Add(this.AppSchema.Ledger.BUDGET_GROUP_IDColumn, BudgetGroupId);
        ...
        public void FillLedgerProperties(int LedgerId)
        {
            ...
            if (resultArgs.DataSource.Table != null && resultArgs.DataSource.Table.Rows.Count > 0)
            {
                ...
                DisplayLedgerName = resultArgs.DataSource.Table.Rows[0][this.AppSchema.Ledger.DISPLAY_LEDGER_NAMEColumn.ColumnName].ToString();
        ...
        /// <summary>Inter-project journal: primary project id and group for a ledger (bank uses MIN(PROJECT_ID) when multiple maps).</summary>
        public ResultArgs FetchBankLedgerProjectContextForInterJournal(int ledgerId)
        {
            using (DataManager dataManager = new DataManager(SQLCommand.LedgerBank.FetchBankLedgerProjectForInterJournal))
            {
                dataManager.Parameters.Add(this.AppSchema.Ledger.LEDGER_IDColumn, ledgerId);
                resultArgs = dataManager.FetchData(DataSource.DataTable);
            }
        ...
        /// <summary>System-only internal adjustment ledger (ACCESS_FLAG = 2, LEDGER_NAME LIKE %InternalAdjustmentLedgerNameMatch%).</summary>
        public ResultArgs FetchInternalAdjustmentLedgerByProjectId(int projectId)
        {
            string nameMatch = string.IsNullOrWhiteSpace(InternalAdjustmentLedgerNameMatch)
                ? DefaultInternalAdjustmentLedgerNameMatch
                : InternalAdjustmentLedgerNameMatch.Trim();
            using (DataManager dataManager = new DataManager(SQLCommand.LedgerBank.FetchInternalAdjustmentLedgerByProject))
            {
                dataManager.Parameters.Add(this.AppSchema.Project.PROJECT_IDColumn, projectId);
                dataManager.Parameters.Add(this.AppSchema.Ledger.LEDGER_NAMEColumn, nameMatch);
                resultArgs = dataManager.FetchData(DataSource.DataTable);
            }
            return resultArgs;
        }

        /// <summary>Returns LEDGER_ID or 0 if none.</summary>
        public int GetInternalAdjustmentLedgerIdForProject(int projectId)
        {
            ResultArgs ra = FetchInternalAdjustmentLedgerByProjectId(projectId);
            if (ra != null && ra.Success && ra.DataSource.Table != null && ra.DataSource.Table.Rows.Count > 0
                && ra.DataSource.Table.Rows[0]["LEDGER_ID"] != DBNull.Value)
                return NumberSet.ToInteger(ra.DataSource.Table.Rows[0]["LEDGER_ID"].ToString());
            return 0;
        }
        ...
        public ResultArgs UpdateLedgerOptionForProvincezero() //Dinesh
        {
            using (DataManager dataManger = new DataManager(SQLCommand.LedgerBank.UpdateLedgerOptionProvincezero))
            {
                resultArgs = dataManger.UpdateData();
            }
            return resultArgs;
        }
        public ResultArgs UpdateLedgerOptionFroProvinceOne(int LedgerId) //Dinesh
        {
            using (DataManager dataMember = new DataManager(SQLCommand.LedgerBank.UpdateLedgerProvinceOne))
            {
                dataMember.Parameters.Add(this.AppSchema.Ledger.LEDGER_IDColumn, LedgerId);
                resultArgs = dataMember.UpdateData();
            }
        ...
        public ResultArgs FetchLedgerIdByLedgerName(string LedgerName, bool IsCashBankLedger)
        {
            ...
            {
                //bharath 08/12/2025 for ledger name {'} comes.
        ...
        public ResultArgs FetchLedgerByLedgerGroup()
        {
            ...
            {
                dataManager.Parameters.Add(this.AppSchema.LedgerGroup.GROUP_IDColumn, GroupId);
                //19-02-2026 Added By Dinesh : Purpose of Multi Language Process
                dataManager.Parameters.Add(this.AppSchema.Project.DISPLAY_INPUT_LANColumn, SettingProperty.Current.DisplayInputLanguage == "1" ? 1 : 0);
                dataManager.DataCommandArgs.IsDirectReplaceParameter = true;
                resultArgs = dataManager.FetchData(DataSource.DataTable);
            }
        ...
        public ResultArgs ChangeLedgerGroupName(string oldGroupName, string newGroupName)
        {
            ResultArgs resultArgs = new ResultArgs();

            try
            {
                string safeNew = newGroupName.Replace("'", "''");
                string safeOld = oldGroupName.Replace("'", "''");

                using (DataManager dataMember = new DataManager(SQLCommand.LedgerBank.ChangeLedgerGroupNameInMaster))
                {
                    dataMember.Parameters.Add("LEDGER_GROUP", "'" + safeNew + "'");
                    dataMember.Parameters.Add("OLD_LEDGER_GROUP", "'" + safeOld + "'");
                    dataMember.DataCommandArgs.IsDirectReplaceParameter = true;
        ...
        public ResultArgs ChangeLedgerNames(string oldLedgerName, string newLedgerName)
        {
            ResultArgs resultArgs = new ResultArgs();

            try
            {
                string safeNew = newLedgerName.Replace("'", "''");
                string safeOld = oldLedgerName.Replace("'", "''");

                using (DataManager dataMember = new DataManager(SQLCommand.LedgerBank.ChangeLedgerNameInMasterAOS))
                {
                    dataMember.Parameters.Add("LEDGER_NAME", "'" + safeNew + "'");
                    dataMember.Parameters.Add("OLD_LEDGER_NAME", "'" + safeOld + "'");
                    dataMember.DataCommandArgs.IsDirectReplaceParameter = true;
