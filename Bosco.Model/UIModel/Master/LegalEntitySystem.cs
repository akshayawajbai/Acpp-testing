    ...
    public class LegalEntitySystem : SystemBase
    {
        ...
        public ResultArgs ApplyLicensePeriod(string licensePath)
        {
            ...
            try
            {
                if (File.Exists(licensePath))
                {
                    ...
                    // Fix XML declaration order if needed and read with UTF-8 encoding
                    // XML declaration must have encoding before standalone: <?xml version="1.0" encoding="UTF-8" standalone="yes"?>
                    string xmlContent = System.IO.File.ReadAllText(licensePath, System.Text.Encoding.UTF8);
                    // Fix malformed XML declaration if encoding comes after standalone
                    //if (xmlContent.StartsWith("<?xml version=\"1.0\" standalone=\"yes\" encoding=\"UTF-8\"?>"))
                    //{
                    //    xmlContent = xmlContent.Replace("<?xml version=\"1.0\" standalone=\"yes\" encoding=\"UTF-8\"?>",
                    //        "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?>");
                    //}
                    // Read XML from corrected string
                    ...
                    if (ds != null && ds.Tables[0].Rows.Count != 0)
                    {
                        ...
                        using (UserSystem userSystem = new UserSystem())
                        {
                            ...
                            foreach (DataRow dr in dtLicense.Rows)
                            {
                                ...
                                if (dr.Table.Columns.Contains(userSystem.AppSchema.LicenseDataTable.AUDITOR_RIGHTSColumn.ColumnName))
                                {
                                    SettingProperty.Current.AuditorRights = dr[userSystem.AppSchema.LicenseDataTable.AUDITOR_RIGHTSColumn.ColumnName] != DBNull.Value ? this.NumberSet.ToInteger(dr[userSystem.AppSchema.LicenseDataTable.AUDITOR_RIGHTSColumn.ColumnName].ToString()) : 0;
                                }
        ...
        private DataView DecryptLicenseDetails(DataView dvLicense)
        {
            if (dvLicense != null && dvLicense.Table.Rows.Count > 0)
            {
                using (UserSystem userSystem = new UserSystem())
                {
                    // chinna 23/01/2026 – Decrypt ALLOW_MULTI_CURRENCY first to determine if Base64 decryption should be used
                    //Int32 allowMultiCurrency = 0;
                    //if (dvLicense.Table.Columns.Contains(userSystem.AppSchema.LicenseDataTable.ALLOW_MULTI_CURRENCYColumn.ColumnName))
                    //{
                    //    string encryptedMultiCurrency = dvLicense.Table.Rows[0][userSystem.AppSchema.LicenseDataTable.ALLOW_MULTI_CURRENCYColumn.ColumnName].ToString();
                    //    string decryptedMultiCurrency = CommonMethod.Decrept(encryptedMultiCurrency);
                    //    allowMultiCurrency = this.NumberSet.ToInteger(decryptedMultiCurrency);
                    //    dvLicense.Table.Rows[0][userSystem.AppSchema.LicenseDataTable.ALLOW_MULTI_CURRENCYColumn.ColumnName] = decryptedMultiCurrency;
                    //}
                    ...
                    bool isSDBANG = (dvLicense.Table.Rows[0][this.AppSchema.LicenseDataTable.HEAD_OFFICE_CODEColumn.ColumnName].ToString().Trim().ToUpper() == "SDBANG");

                    string rawSocietyName = dvLicense.Table.Rows[0][userSystem.AppSchema.LicenseDataTable.SocietyNameColumn.ColumnName].ToString();
                    dvLicense.Table.Rows[0][this.AppSchema.LicenseDataTable.SocietyNameColumn.ColumnName] = isSDBANG ? rawSocietyName : CommonMethod.Decrept(rawSocietyName);

                    string rawInstituteName = dvLicense.Table.Rows[0][userSystem.AppSchema.LicenseDataTable.InstituteNameColumn.ColumnName].ToString();
                    dvLicense.Table.Rows[0][this.AppSchema.LicenseDataTable.InstituteNameColumn.ColumnName] = isSDBANG ? rawInstituteName : CommonMethod.Decrept(rawInstituteName);

                    string rawAddress = dvLicense.Table.Rows[0][userSystem.AppSchema.LegalEntity.ADDRESSColumn.ColumnName].ToString();
                    dvLicense.Table.Rows[0][this.AppSchema.LegalEntity.ADDRESSColumn.ColumnName] = isSDBANG ? rawAddress : CommonMethod.Decrept(rawAddress);
                    ...
                    if (dvLicense.Table.Columns.Contains(userSystem.AppSchema.LicenseDataTable.AUDITOR_RIGHTSColumn.ColumnName))
                    {
                        dvLicense.Table.Rows[0][userSystem.AppSchema.LicenseDataTable.AUDITOR_RIGHTSColumn.ColumnName] = CommonMethod.Decrept(dvLicense.Table.Rows[0][userSystem.AppSchema.LicenseDataTable.AUDITOR_RIGHTSColumn.ColumnName].ToString());
                    }
                    ...
                    // ALLOW_MULTI_CURRENCY already decrypted at the beginning of this method --- 03-02-2026
                    if (dvLicense.Table.Columns.Contains(userSystem.AppSchema.LicenseDataTable.ALLOW_MULTI_CURRENCYColumn.ColumnName))
                    {
