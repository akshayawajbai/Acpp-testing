...
namespace Bosco.Model.UIModel
{
    public class CostCentreSystem : SystemBase
    {
        ...
        public int ApiCostCentreId { get; set; }

        public ResultArgs IsCostCentreExistsByApiId(int apiId)
        {
            using (DataManager dm = new DataManager(SQLCommand.CostCentre.FetchCostCentreIdByApiId))
            {
                dm.Parameters.Add("THIRD_PARTY_COST_CENTRE_ID", apiId);
                return dm.FetchData(DataSource.Scalar);
            }
        }
        public ResultArgs IndividualSaveCostCentreApi()
        {
            using (DataManager dataManager = new DataManager(SQLCommand.CostCentre.AddApi))
            {
                dataManager.Parameters.Add(this.AppSchema.CostCentre.ABBREVATIONColumn, CostCentreAbbrevation);
                dataManager.Parameters.Add(this.AppSchema.CostCentre.COST_CENTRE_NAMEColumn, CostCentreName);
                dataManager.Parameters.Add(this.AppSchema.CostCentre.NOTESColumn, string.Empty);
                dataManager.Parameters.Add("THIRD_PARTY_COST_CENTRE_ID", ApiCostCentreId);

                resultArgs = dataManager.UpdateData();
            }
            return resultArgs;
        }
        public ResultArgs UpdateCostCentreAllByApiId()
        {
            using (DataManager dm = new DataManager(SQLCommand.CostCentre.UpdateAllByApiId))
            {
                dm.Parameters.Add(this.AppSchema.CostCentre.ABBREVATIONColumn, this.CostCentreAbbrevation);
                dm.Parameters.Add(this.AppSchema.CostCentre.COST_CENTRE_NAMEColumn, this.CostCentreName);
                dm.Parameters.Add(this.AppSchema.CostCentre.NOTESColumn, this.Notes ?? string.Empty);
                dm.Parameters.Add("THIRD_PARTY_COST_CENTRE_ID", this.ApiCostCentreId);
                return dm.UpdateData();
            }
        }
        public ResultArgs FetchCostCentreIdMap()
        {
            using (DataManager dataManager = new DataManager(SQLCommand.CostCentre.FetchIdMap))
            {
                resultArgs = dataManager.FetchData(DataSource.DataTable);
            }
            return resultArgs;
        }
        public ResultArgs FetchVoucherCostCentreUsage()
        {
            using (DataManager dataManager = new DataManager(SQLCommand.CostCentre.FetchVoucherUsage))
            {
                resultArgs = dataManager.FetchData(DataSource.DataTable);
            }
            return resultArgs;
        }
        public ResultArgs DeleteUnusedCostCentre(int costCentreId)
        {
            using (DataManager dataManager = new DataManager(SQLCommand.CostCentre.DeleteUnusedCostCentre))
            {
                dataManager.Parameters.Add(this.AppSchema.CostCentre.COST_CENTRE_IDColumn, costCentreId);
                resultArgs = dataManager.UpdateData();
            }
