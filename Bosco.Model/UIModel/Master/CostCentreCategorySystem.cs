...
namespace Bosco.Model.UIModel
{
    public class CostCentreCategorySystem : SystemBase
    {
        ...
        //bharath 27/08/2025
        //perform cleanup of mappings in DB
        public ResultArgs DeleteCostCategoryMappings(int CostCategoryId)
        {
            using (DataManager dataManager = new DataManager(SQLCommand.CostCentreCategory.DeleteMappings))
            {
                dataManager.Parameters.Add("COST_CATEGORY_ID", CostCategoryId);
                resultArgs = dataManager.UpdateData();
            }
            return resultArgs;
        }
        ...
        //bharath
        public int ApiCostCentreCategoryId { get; set; }

        public ResultArgs IsCostCentreCategoryExistsByApiId(int apiCatId)
        {
            using (DataManager dm = new DataManager(SQLCommand.CostCentreCategory.FetchCostCentreCategoryIdByApiId))
            {
                dm.Parameters.Add("THIRD_PARTY_COST_CENTRE_CATEGORY_ID", apiCatId);
                return dm.FetchData(DataSource.Scalar);
            }
        }

        public ResultArgs SaveCostCentreCatogoryDetailsapi()
        {
            using (var dm = new DataManager(
                (CostCentreCategoryId == 0)
                    ? SQLCommand.CostCentreCategory.AddWithApi
                    : SQLCommand.CostCentreCategory.UpdateWithApi))
            {
                dm.Parameters.Add(this.AppSchema.CostCentreCategory.COST_CENTRECATEGORY_IDColumn, CostCentreCategoryId, true);
                dm.Parameters.Add(this.AppSchema.CostCentreCategory.COST_CENTRE_CATEGORY_NAMEColumn, CostCentreCategoryName);
                dm.Parameters.Add("THIRD_PARTY_COST_CENTRE_CATEGORY_ID", ApiCostCentreCategoryId);
            ...
            {
                if (CostCentreCategoryId == 0 && resultArgs.RowUniqueId != null)
                    CostCentreCategoryId = this.NumberSet.ToInteger(resultArgs.RowUniqueId.ToString());
            }
