...
namespace Bosco.Model.UIModel
{
    public class ProjectSystem : SystemBase
    {
        ...
        // 17/02/2026, added DisplayProject for second-level language (e.g. French)
        ...
        public ResultArgs FetchProjectlistDetails()
        {
            using (DataManager dataManager = new DataManager(SQLCommand.Mapping.FetchProjectforLookup))
            {
                ...
                dataManager.DataCommandArgs.IsDirectReplaceParameter = true; //24-02-2026 Purpose of Second Leval Language
                resultArgs = dataManager.FetchData(DataSource.DataTable);
            }
            return resultArgs;
        }
        ...
        public ResultArgs SaveProjectDetails(DataManager projectDataManager)
        {
            ...
            using (DataManager dataManager = new DataManager(ProjectId == 0 ? SQLCommand.Project.Add : SQLCommand.Project.Update))
            {
                ...
                // 17/02/2026, added DisplayProject for insert and update (second-level language)
                object displayProjectValue = (string.IsNullOrEmpty(DisplayProject) ? (object)DBNull.Value : DisplayProject);
                dataManager.Parameters.Add(this.AppSchema.Project.DISPLAY_PROJECTColumn, displayProjectValue);
        ...
        public ResultArgs FetchProjectsIntegration()
        {
            using (DataManager dataManager = new DataManager(SQLCommand.Project.FetchProjectsIntegration))
            {
                ...
                if (ProjectCategroyId > 0)
                {
                    dataManager.Parameters.Add(this.AppSchema.ProjectCatogory.PROJECT_CATOGORY_IDColumn, ProjectCategroyId);
                }
        ...
        public ResultArgs ChangeProjectName(string oldProjectName, string newProjectName)
        {
            ResultArgs resultArgs = new ResultArgs();

            try
            {
                string safeNew = newProjectName.Replace("'", "''");
                string safeOld = oldProjectName.Replace("'", "''");

                using (DataManager dataMember = new DataManager(SQLCommand.Project.ChangeProjectNameInMaster))
                {
                    dataMember.Parameters.Add("PROJECT", "'" + safeNew + "'");
                    dataMember.Parameters.Add("OLD_PROJECT", "'" + safeOld + "'");
