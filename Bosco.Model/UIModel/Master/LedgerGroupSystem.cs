...
namespace Bosco.Model.UIModel
{
    public class LedgerGroupSystem : SystemBase
    {
        ...
        // 17/02/2026, added DisplayLedgerGroup for second-level language (e.g. French)
        public string DisplayLedgerGroup { get; set; }
        ...
        public ResultArgs SaveLedgerGroupDetails()
        {
            using (DataManager dataManager = new DataManager((GroupId == 0) ? SQLCommand.LedgerGroup.Add : SQLCommand.LedgerGroup.Update))
            {
                ...
                // 17/02/2026, added DisplayLedgerGroup for insert and update (second-level language)
                object displayLedgerGroupValue = (string.IsNullOrEmpty(DisplayLedgerGroup) ? (object)DBNull.Value : DisplayLedgerGroup);
                dataManager.Parameters.Add(this.AppSchema.LedgerGroup.DISPLAY_LEDGER_GROUPColumn, displayLedgerGroupValue);
        ...
        public ResultArgs GetLedgerGroupSource()
        {
            using (DataManager dataManager = new DataManager(SQLCommand.LedgerGroup.FetchAll))
            {
                ...
                dataManager.Parameters.Add(this.AppSchema.LedgerGroup.DISPLAY_INPUT_LANColumn , SettingProperty.Current.DisplayInputLanguage == "1" ? 1 : 0);
        ...
        public ResultArgs GetLedgerList()
        {
            ...
            //bharath 19/11/2025 to hide cash ledger in multicurrency database
            if (AllowMultiCurrency == 1 && GroupIds.Contains("13") && resultArgs != null && resultArgs.DataSource != null && resultArgs.DataSource.Table.Rows.Count > 0)
            {
