...
namespace Bosco.Model.UIModel.Master
{
    public class AuditLockTransSystem : SystemBase
    {
        ...
        public ResultArgs SaveAuditTrans()
        {
            using (DataManager dataManager = new DataManager(LockTransId == 0 ? SQLCommand.AuditLockTrans.AddAuditTrans : SQLCommand.AuditLockTrans.UpdateAuditTrans))
            {
                ...
                if (resultArgs != null && resultArgs.Success)
                {
                    if (LockTransId == 0)
                    {
                        LockTransId = GetLastLockTransId();
                    }
                }
            }
            return resultArgs;
        ...
        public int GetLastLockTransId()  //Added By Dinesh 13-05-2026
        {
            int lockTransId = 0;
            using (DataManager dataManager = new DataManager(SQLCommand.AuditLockTrans.GetLastLockTransId))
            {
                resultArgs = dataManager.FetchData(DataSource.DataTable);
                if (resultArgs != null && resultArgs.DataSource.Table != null && resultArgs.DataSource.Table.Rows.Count > 0)
                {
                    lockTransId = Convert.ToInt32
                    (
                        resultArgs.DataSource.Table.Rows[0]["LOCK_TRANS_ID"]
                    );
                }
            }
            return lockTransId;
        ...
        public ResultArgs UpdateAuditUsers(int LockTransId, int userId)  //Added By Dinesh 13-05-2026
        {
            using (DataManager datamanger = new DataManager(SQLCommand.AuditLockTrans.AddUsers))
            {
                datamanger.Parameters.Add(this.AppSchema.AuditLockTransType.LOCK_TRANS_IDColumn, LockTransId);
                datamanger.Parameters.Add(this.AppSchema.User.USER_IDColumn, userId);
                resultArgs = datamanger.UpdateData();
            }
            return resultArgs;
        }

        public ResultArgs UpdateUser(int LockTransId, int userId)  //Added By Dinesh 13-05-2026
        {
            using (DataManager datamanger = new DataManager(SQLCommand.AuditLockTrans.UpdateUsers))
            {
                datamanger.Parameters.Add(this.AppSchema.AuditLockTransType.LOCK_TRANS_IDColumn, LockTransId);
                datamanger.Parameters.Add(this.AppSchema.User.USER_IDColumn, userId);
                resultArgs = datamanger.UpdateData();
        ...
        public ResultArgs UpdateUserMain(int LockTransId, int userId)  //Added By Dinesh 13-05-2026
        {
            using (DataManager datamanger = new DataManager(SQLCommand.AuditLockTrans.UpdateUsersMain))
            {
                datamanger.Parameters.Add(this.AppSchema.AuditLockTransType.LOCK_TRANS_IDColumn, LockTransId);
                datamanger.Parameters.Add(this.AppSchema.User.USER_IDColumn, userId);
                resultArgs = datamanger.UpdateData();
        ...
        public ResultArgs DeleteAuditUsers(int lockTransId)  //Added By Dinesh 13-05-2026
        {
            using (DataManager dataManager =
                new DataManager(SQLCommand.AuditLockTrans.DeleteUsers))
            {
                dataManager.Parameters.Add
                (
                    this.AppSchema.AuditLockTransType.LOCK_TRANS_IDColumn,
                    lockTransId
                );
        ...
        public ResultArgs FetchAuditUsers(int LockTransId)  //Added By Dinesh 13-05-2026
        {
            using (DataManager dataManager = new DataManager(SQLCommand.AuditLockTrans.FetchUsers))
            {
                dataManager.Parameters.Add(this.AppSchema.AuditLockTransType.LOCK_TRANS_IDColumn, LockTransId);
                resultArgs = dataManager.FetchData(DataSource.DataTable);
            }
        ...
        {
            int userId = Convert.ToInt32(this.LoginUserId);
            using (DataManager dataManager = new DataManager(SQLCommand.AuditLockTrans.FetchAuditLockDetailsForProjectAndDate))
            {
                ...
                dataManager.Parameters.Add(this.AppSchema.User.USER_IDColumn, userId); //added By Dinesh 
                dataManager.DataCommandArgs.IsDirectReplaceParameter = true;
                resultArgs = dataManager.FetchData(DataSource.DataTable);
            }
            return resultArgs;
