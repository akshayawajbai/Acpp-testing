                        ...
                        if (TallyResponseDataSet.Tables["LEDGER"] != null)
                        {
                            ...
                            if (resultarg.Success)
                            {
                                ...
                                if (dtTallyAddressList != null)
                                {
                                    ...
                                    {
                                        // Chinna, 05/05/2026 — legacy was // resultdtAddress = dtTallyAddressList; normalize rows when ADDRESS.LIST has no LEDGER_ID (e.g. Prime uses ledgeridcolum).
                                        resultdtAddress.Rows.Clear();
                                        string addressCol = null;
                                        if (dtTallyAddressList.Columns.Contains("ADDRESS_TEXT"))
                                            addressCol = "ADDRESS_TEXT";
                                        else if (dtTallyAddressList.Columns.Contains("ADDRESS"))
                                            addressCol = "ADDRESS";
                                        foreach (DataRow src in dtTallyAddressList.Rows)
                                        {
                                            int lid = 0;
                                            if (dtTallyAddressList.Columns.Contains("LEDGER_ID") && src["LEDGER_ID"] != DBNull.Value)
                                                lid = Convert.ToInt32(src["LEDGER_ID"], System.Globalization.CultureInfo.InvariantCulture);
                                            else if (dtTallyAddressList.Columns.Contains(ledgeridcolum) && src[ledgeridcolum] != DBNull.Value)
                                                lid = Convert.ToInt32(src[ledgeridcolum], System.Globalization.CultureInfo.InvariantCulture);
                                            string addr = string.Empty;
                                            if (addressCol != null && src[addressCol] != DBNull.Value)
                                                addr = src[addressCol].ToString();
                                            resultdtAddress.LoadDataRow(new object[] { lid, addr }, true);
                                        }
                                    }
                                }
                                //Address ------------------------------------------------------------------------------------
