...
{
    public partial class frmMysoreBudgetView : frmFinanceBase
    {
        ...
        #region Constructor
        public frmMysoreBudgetView()
        {
            InitializeComponent();
        }
        public frmMysoreBudgetView(BudgetType type)
            : this()
        {
            budgetType = type;
        }
        ...
        private void LoadProject()
        {
            try
            {
                using (MappingSystem mappingSystem = new MappingSystem())
                {
                    ...
                    glkpProjectDetails.Properties.DataSource = null;
                    if (resultArgs.Success && resultArgs.DataSource.Table.Rows.Count > 0)
                    {
                        using (CommonMethod SelectAll = new CommonMethod())
                        {
                            this.UtilityMember.ComboSet.BindGridLookUpCombo(glkpProjectDetails, resultArgs.DataSource.Table, mappingSystem.AppSchema.Project.PROJECTColumn.ColumnName, mappingSystem.AppSchema.Project.PROJECT_IDColumn.ColumnName);
                            glkpProjectDetails.EditValue = projectId != 0 ? projectId : glkpProjectDetails.Properties.GetKeyValue(0);
                        }
                    }
                }
            }
                    ...
                    if (IsTwoMonthBudget)
                    {
                        ...
                        if (dt != null && dt.Rows.Count > 0)
                        {
                            ...
                            { 
                                ...
                                //Implemantaion
                                ShowMessageBox(this.GetMessage(MessageCatalog.Common.GENERATE_SELECT_TWO_MONTHS_BUDGET));
                            }
                        }
                    }
                    else
        ...
        private void LoadBudget()
        {
            ...
            if (budgetType.Equals(BudgetType.BudgetByAnnualYear))
            {
                using (BudgetSystem AnnualBudgetAll = new BudgetSystem())
                {
                    ...
                    if (resultArgs.Success && resultArgs.DataSource.Table.Rows.Count > 0)
                    {
                        int prjId = glkpProjectDetails.EditValue != null ? this.UtilityMember.NumberSet.ToInteger(glkpProjectDetails.EditValue.ToString()) : 0;
                        resultArgs.DataSource.Table.DefaultView.RowFilter = "PROJECT_ID =" + prjId;
                        BudgetInfo = resultArgs.DataSource.Table.DefaultView.ToTable();
                        gcBudget.DataSource = BudgetInfo;
        ...
        private void SetAnnualBudgetForm(int BudgetId, int projectId)
        {
            try
            {
                    ...
                    if (BudgetId > 0)
                    {
                        ...
                        if (dt != null && dt.Rows.Count > 0)
                        {
                            ...
                            else
                            {
                                ...
                                //Implemantaion
                                ShowMessageBox(this.GetMessage(MessageCatalog.Common.GENERATE_THIS_IS_NOT_MONTHS_BUDGET));
                            }
                        }
                    }
                    else
                ...
                {
                    frmMysoreBudgetAdd frmBudget = new frmMysoreBudgetAdd(BudgetId, month2budgetId, projectId, IsTwoMonthBudget);
                            ...
                            using (BudgetSystem budgetSystem = new BudgetSystem())
                            {
                                if (IsTwoMonthBudget)
                                {
                                    ...
                                    if (dt != null && dt.Rows.Count > 0)
                                    {
                                        ...
                                        if (dv.Count == 2)
                                        {
                                            budgetids = dv[0]["BUDGET_ID"].ToString();
                                            budgetids += "," + dv[1]["BUDGET_ID"].ToString();
                                        }
                                    }

                                    string[] monthsbudget = budgetids.Split(',');
                                    if (monthsbudget.Length == 2)
                                    {
                                        budgetSystem.TwoMonthIds = monthsbudget;
                                    }
                                    resultArgs = budgetSystem.RemoveMysoreBudgetDetails();
                                }
                                else
                                {
        ...
        private bool HasTransaction()
        {
            ...
            using (BudgetSystem budget = new BudgetSystem())
            {
                ...
                {   
                    ...
                    if (this.ShowConfirmationMessage(this.GetMessage(MessageCatalog.Common.GENERATE_TRANSACTION_ARE_AVALIABLE_DO_YOU_WANT_DELETE), MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        NotHasTrans = true;
                    }
                    else
        ...
        private void glkpProjectDetails_EditValueChanged(object sender, EventArgs e)
        {
            LoadBudget();
        }
    }
