                ...
                if (gvCostCentreCategory.RowCount != 0)
                {
                    if (costCentreCategoryId != 0)
                    {
                        using (CostCentreCategorySystem Costcentrecategorysystem = new CostCentreCategorySystem())
                        {
                            ...
                            {
                                //bharath 27/08/2025
                                //first remove any orphaned cost category mappings before checking dependencies
                                resultArgs = Costcentrecategorysystem.DeleteCostCategoryMappings(costCentreCategoryId);
                                if (!resultArgs.Success)
                                {
                                    this.ShowMessageBoxError("Unable to cleanup orphan mappings for this category.");
                                    return;
                                }
