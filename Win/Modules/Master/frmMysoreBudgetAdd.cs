...
{
    public partial class frmMysoreBudgetAdd : frmFinanceBaseAdd
    {
        #region Variables
        ...
        public frmMysoreBudgetAdd(int BudgetId, int month2budgetId, int projectid, bool isTwoMonthBudget)
            : this()
        {
                ...
                using (BudgetSystem budgetSystem = new BudgetSystem())
                {
                    ...
                    else
                    {
                       ...
                       // if (ShowConfirmationMessage("Budget is already made for this this month. Are you sure to show Not-Budgeted Month?", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == System.Windows.Forms.DialogResult.Yes)
                        if (ShowConfirmationMessage(this.GetMessage(MessageCatalog.Common.GENERATE_BUDGET_IS_ALREADY_MADE_THIS_MONTH), MessageBoxButtons.YesNo, MessageBoxIcon.Question) == System.Windows.Forms.DialogResult.Yes)
                        {
                            if (Month1BudgetId == 0)
                                ClearControls();
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            if (IsFormValid())
            {
                ...
                if (IsTwoMonthBudget)
                {
                    //dvRecords.RowFilter = "(PROPOSED_CURRENT_YR>0 OR APPROVED_CURRENT_YR>0) OR (M2_PROPOSED_AMOUNT>0 OR M2_APPROVED_AMOUNT>0)";
                    dvRecords.RowFilter = "(PROPOSED_CURRENT_YR>0 OR APPROVED_CURRENT_YR>0) OR (M2_PROPOSED_AMOUNT>0 OR M2_APPROVED_AMOUNT>0)";
                }
                else
                {
                    ...
                    using (BudgetSystem budgetSystem = new BudgetSystem())
                    {
                        ...
                        if (resultArgs != null && resultArgs.Success)
                        {
                            ...
                            if (Month1BudgetId == 0)
                            {
                                ...
                                Month1BudgetId = budgetSystem.M1BudgetId;
                                Month2BudgetId = budgetSystem.M2BudgetId;
                                lcImportBudget.Visibility = lcExportBudget.Visibility = DevExpress.XtraLayout.Utils.LayoutVisibility.Always;
                            }
                        }
                ...
                else
                {
                    ...
                    {  
                        ...
                        // ShowMessageBox("Fill Proposed Amount for any one of the Ledgers for both Months");
                         ShowMessageBox(this.GetMessage(MessageCatalog.Common.GENERATE_LEDGERS_FOR_BOTH_MONTHS));
                    }
                    else
                    {    
                        //Dinesh 04/07/2025
                        //ShowMessageBox("Fill Proposed Amount for any one of the Ledgers");
                        ShowMessageBox(this.GetMessage(MessageCatalog.Common.GENERATE_PROPOSED_AMOUNT_FOR_ANY_MONTH));
                    }
                }
            }
        }
        ...
        private void peReplaceValue_Click(object sender, EventArgs e)
        {
            ...
            {   
                ...
                if (ShowConfirmationMessage(this.GetMessage(MessageCatalog.Common.GENERATE_ASSIGN_PROPOSED_AMOUNT_APPROVED_AMOUNT), MessageBoxButtons.YesNo, MessageBoxIcon.Question) == System.Windows.Forms.DialogResult.Yes)
                {
                    //All other Ledgers
            ...
            {   
                ...
                MessageRender.ShowMessage(this.GetMessage(MessageCatalog.Common.GENERATE_BUDGET_LEDGERS_ARE_NOT_AVAILABLE));
            }
        }
        ...
        private void LoadMonths()
        {
            ...
            {   
                ...
                this.ShowMessageBoxWarning(this.GetMessage(MessageCatalog.Common.GENERATE_PROBLEMS_OCCURED));
            }
        }
        ...
        private void SetTitle()
        {
            ...
            colNRLedger.Width = colRecLedger.Width;
            colNRPreviousApproved.Width = colRecPreviousApproved.Width;
            colNRActual.Width = colRecActual.Width;
            colNRBalance.Width = colRecBalance.Width;
            colNRProposed.Width = colRecProposed.Width;
            colNRCurrentApproved.Width = colRecCurrentApproved.Width;
            colNRNextMonthProposals.Width = colRecNextMonthProposed.Width;
            colNextMonthCurrentApproved.Width = colRecNextMonthCurrentApproved.Width;
            colNRNarration.Width = colRecNarration.Width - 25;
        ...
        private void FetchBudgetDetails()
        {
            using (BudgetSystem budgetAnnual = new BudgetSystem())
            {
                ...
                budgetAnnual.ProjectId = grdProject.EditValue != null ? UtilityMember.NumberSet.ToInteger(grdProject.EditValue.ToString()) : 0;
        ...
        private bool IsFormValid()
        {
            ...
            {  
                ...
                this.ShowMessageBox(this.GetMessage(MessageCatalog.Common.GENERATE_INVALID_MONTH));
        ...
        private void InsertNewSubLeger(BudgetMainGroup budgetgroup, string SubLedgerName, double PropsedAmount, double NextMonthPropsedAmount, string Narration)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(SubLedgerName))
                {
                    ...
                    Int32 isalloted = (Month1BudgetId == 0 || Month2BudgetId == 0 ? 0 : 1);
                    ...
                    if (!isSubLedgerExists)
                    {
                        ...
                        else if (budgetgroup == BudgetMainGroup.NonRecurring)
                        {
                            ...
                            drNewSubLedger["IS_ALLOTED"] = 1; // (Month1BudgetId == 0 ? 0 : 1);
                    ...
                    else
                    {
                        ...
                        {
                            //dtNonRecBudgetSubwithLedgerRecords.DefaultView[0]["IS_ALLOTED"] = (Month1BudgetId == 0 ? 0 : 1);
                            dtNonRecBudgetSubwithLedgerRecords.DefaultView[0]["IS_ALLOTED"] = 1;
                            dtNonRecBudgetSubwithLedgerRecords.DefaultView[0]["PROPOSED_CURRENT_YR"] = PropsedAmount;
                            dtNonRecBudgetSubwithLedgerRecords.DefaultView[0]["APPROVED_CURRENT_YR"] = 0.00;
                            dtNonRecBudgetSubwithLedgerRecords.DefaultView[0]["M2_PROPOSED_AMOUNT"] = 0.00;
                            dtNonRecBudgetSubwithLedgerRecords.DefaultView[0]["M2_APPROVED_AMOUNT"] = 0.00;
                            if (IsTwoMonthBudget)
                            {
                                dtNonRecBudgetSubwithLedgerRecords.DefaultView[0]["M2_PROPOSED_AMOUNT"] = NextMonthPropsedAmount;
                                dtNonRecBudgetSubwithLedgerRecords.DefaultView[0]["M2_APPROVED_AMOUNT"] = 0.00;
                            }
                            dtNonRecBudgetSubwithLedgerRecords.AcceptChanges();
        ...
        {    
            ...
            if (this.ShowConfirmationMessage(this.GetMessage(MessageCatalog.Common.GENERATE_EXPORT_BUDGET_TO_HEAD_OFFICE_PORTAL), MessageBoxButtons.YesNo, MessageBoxIcon.Question) == System.Windows.Forms.DialogResult.Yes)
            {
                //Dinesh 04/07/2025
               // this.ShowWaitDialog("Uploading Budget from Head Office Portal");
                this.ShowWaitDialog(this.GetMessage(MessageCatalog.Common.GENERATE_UPLOADING_BUDGET_FROM_HEAD_OFFICE));
                ResultArgs result = this.UploadBudgetToAcmeerpPortal(Month1BudgetId, !IsTwoMonthBudget);
                this.CloseWaitDialog();
                if (result.Success)
                {
                    if (IsTwoMonthBudget)
                    {
                        result = this.UploadBudgetToAcmeerpPortal(Month2BudgetId, true);
                    }
                    if (result.Success)
                    {  
                        //Dinesh 04/07/2025
                        //this.ShowMessageBox("Budget has been uploaded to Head Office Portal, It will be approved by Head Office");
                        this.ShowMessageBox(this.GetMessage(MessageCatalog.Common.GENERATE_APPROVED_BY_HEADOFFICE));
                    }
                }
        ...
        private void btnImportBudget_Click(object sender, EventArgs e)
        {
            ...
            if (this.ShowConfirmationMessage(this.GetMessage(MessageCatalog.Common.GENERATE_IMPORT_BUDGET_TO_HEAD_OFFICE_PORTAL), MessageBoxButtons.YesNo, MessageBoxIcon.Question) == System.Windows.Forms.DialogResult.Yes)
            {
                //Dinesh 04/07/2025
                //this.ShowWaitDialog("Downloading Budget from Head Office Portal");
                this.ShowWaitDialog(this.GetMessage(MessageCatalog.Common.GENERATE_DOWNLOADING_BUDGET_HEAD_OFFICE_PORTAL));
                ...
                if (result.Success)
                {
                    ...
                    {  
                        ...
                        this.ShowMessageBox(this.GetMessage(MessageCatalog.Common.GENERATE_APPROVED_HEAD_OFFICE_PORTAL_IN));
                    }
                ...
                if (ledgerid > 0)
                {
                    ...
                    else //For Main Ledgers
                    {
                        ...
                        { 
                            ...
                            msg = this.GetMessage(MessageCatalog.Common.GENERATE_SUB_LEDGERS_ARE_AVAILABLE);
                        }
