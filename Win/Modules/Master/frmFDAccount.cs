...
namespace ACPP.Modules.Master
{
    public partial class frmFDAccount : frmFinanceBaseAdd
    {
        ...
        public frmFDAccount(int fdRenewalId, int FDAccountId, FDTypes FdTypes, DataTable dtMasters, DataTable dtRenewval, bool frmRprt = false, int maxRenId = 0)
            : this()
        {
        ...
        {
            bool isEditable = (FDRenewalId == MaxRenewalId);

            cboInterestMode.Enabled = isEditable;
            glkpInterestLedger.Enabled = isEditable;
            glkpBankInterestLedger.Enabled = isEditable;
            dteRenewalOn.Enabled = isEditable;
            txtFDReceiptNo.Enabled = isEditable;
            cboInsType.Enabled = isEditable;
            txtFdInterestRate.Enabled = isEditable;
            dteFDMaturityDate.Enabled = isEditable;
            txtExpectedMaturityRenewalValue.Enabled = isEditable;
            txtRenewalExpectedInteres.Enabled = isEditable;
        }
        #endregion
                ...
                if (isValidCurrency() && ValidateFDAccountDetails())
                {
                    using (FDAccountSystem fdAccountSystem = new FDAccountSystem())
                    {
                        ...
                        if (this.AppSetting.AllowMultiCurrency == 1)
                        {
                            ...
                            fdAccountSystem.LiveExchangeRate = this.UtilityMember.NumberSet.ToDecimal(txtLiveExchangeRate.Text);
                ...
                if (glkpLedgers.GetSelectedDataRow() != null)
                {
                    ...
                    using (LedgerSystem ledgersystem = new LedgerSystem())
                    {
                        if (drv != null)
                        {
                            ...
                            txtLiveExchangeRate.Tag = txtExchangeRate.Text;
                            // 29/01/2025, in order to subscribe live exchange rate - exchange and live same
                            // lblLiveExchangeRate.Text = txtExchangeRate.Text;
                            txtExchangeRate.Enabled = false;
                            txtLiveExchangeRate.Enabled = false;
                        }
                    }
                }
            }
        ...
        private bool isValidCurrency()
        {
            ...
            if (this.AppSetting.AllowMultiCurrency == 1)
            {
                ...
                { //On 04/09/2024, If multi currency enabled, all the currency details must be filled
                    ...
                    //Implemantaion
                    MessageRender.ShowMessage(this.GetMessage(MessageCatalog.Common.GENERAL_AS_MULTI_CURRENCY_ENABLED_DETAIL_SHOULD_BE_FILLED));
                ...
                if (isValid)
                {
                    ...
                    if (amt != UtilityMember.NumberSet.ToDouble(txtCurrencyAmount.Text))
                    {
                        ...
                        MessageRender.ShowMessage(this.GetMessage(MessageCatalog.Common.GENERAL_MISMATCHING_CURRENCY_AMOUNT));
        ...
        private bool ValidateFDAccountDetails()
        {
            ...
            // 22/07/2025, *Chinna
            DataTable dtMasterValidate = new DataTable();
            DataTable dtRenewalValidate = new DataTable();
            using (FDAccountSystem fdAccountSystem = new FDAccountSystem())
            {
                DataSet dsFDRenewal = fdAccountSystem.GetFDMasterRenewalDetails(FDAccountId);
                if (dsFDRenewal.Tables.Count > (int)YesNo.Yes)
                {
                    dtMasterValidate = dsFDRenewal.Tables[0];
                    dtRenewalValidate = dsFDRenewal.Tables[1];
                }
            }
            ...
            if (fdTypes == FDTypes.IN || fdTypes == FDTypes.OP)
            {
                ...
                if (HasFDReInvestment() > 0)
                {
                    if (cboFDScheme.SelectedIndex == 0)
                    {
                        ...
                        //Implemantaion
                        this.ShowMessageBox(this.GetMessage(MessageCatalog.Common.GENERAL_RE_INVESTMENT_IS_MADE));
            ...
            if (isFDAccount && fdTypes == FDTypes.IN)
            {
                ...
                else if (string.IsNullOrEmpty(deCreatedDate.Text) || deCreatedDate.DateTime == DateTime.MinValue)
                {
                    ...
                    this.ShowMessageBox(this.GetMessage(MessageCatalog.Common.GENERAL_CREATE_ON_IS_EMPTY));
                    this.SetBorderColorForDateTimeEdit(deCreatedDate);
                    isFDAccount = false;
                    deCreatedDate.Focus();
                }
                else if (deCreatedDate.DateTime > this.UtilityMember.DateSet.ToDate(this.AppSetting.YearTo, false))
                {
                    ...
                    this.ShowMessageBox(this.GetMessage(MessageCatalog.Common.GENERAL_CREATE_ON_TRANSACTION_PERIOD));
                    this.SetBorderColorForDateTimeEdit(deCreatedDate);
                    isFDAccount = false;
                    deCreatedDate.Focus();
                }
            ...
            if (fdTypes == FDTypes.RIN)
            {
                ...
                if (string.IsNullOrEmpty(dePostDate.Text.Trim()) || dePostDate.DateTime == DateTime.MinValue)
                {
                    ...
                    this.ShowMessageBox(this.GetMessage(MessageCatalog.Common.GENERAL_RE_INVETMENT_DATA_EMPTY));
                    this.SetBorderColorForDateTimeEdit(dteClosedOn);
                    isFDAccount = false;
                    dePostDate.Focus();
                }
                else if (dePostDate.DateTime > this.UtilityMember.DateSet.ToDate(this.AppSetting.YearTo, false))
                {
                    ...
                    //Implemantaion
                    this.ShowMessageBox(this.GetMessage(MessageCatalog.Common.GENERAL_RE_INVESTMENT_DATE_TRANSACTION_PERIOD));
                ...
                else if (string.IsNullOrEmpty(txtWithdrawAmount.Text.Trim()))
                {
                    ...
                    //Implemantaion
                    this.ShowMessageBox(this.GetMessage(MessageCatalog.Common.GENERAL_RE_INVESTMENT_AMOUNT_IS_EMPTY));
                ...
                else if ((this.UtilityMember.DateSet.ToDate(lblRINDate.Text, false) > dePostDate.DateTime) && (!(lblRINDate.Text.Equals("0"))))
                {
                    ...
                    //Implemantaion
                    this.ShowMessageBox(this.GetMessage(MessageCatalog.Common.GENERAL_RE_INVESTMENT_DATE_SHOULD_GREATER_THAN));
                    isFDAccount = false;
                    dePostDate.Focus();
                    return isFDAccount;
                }
                else if (lblDateofMaturity.Visibility != LayoutVisibility.Never && this.UtilityMember.DateSet.ValidateDate(this.UtilityMember.DateSet.ToDate(this.MaturityDate, false), dePostDate.DateTime))
                {
                    ...
                    //Implemantaion
                    this.ShowMessageBox(this.GetMessage(MessageCatalog.Common.GENERAL_RE_INVETSMENT_DATE_LESS_MATURITY_DATE));
                    isFDAccount = false;
                    dePostDate.Focus();
                    return isFDAccount;
                }
                else if (!(string.IsNullOrEmpty(lblAssignLastRenewedOn.Text.Trim())))
                {
                    if (lblAssignLastRenewedOn.Text != "0")
                    {
                        if (!(dePostDate.DateTime >= this.UtilityMember.DateSet.ToDate(lblAssignLastRenewedOn.Text, false)))
                        {
                            ...
                            //Implemantaion
                            this.ShowMessageBox(this.GetMessage(MessageCatalog.Common.GENERAL_RE_INVESTMENT_DATE_RENEWAL_DATE));
                            isFDAccount = false;
                            dePostDate.Focus();
                            return isFDAccount;
                        }
                    ...
                    using (FDAccountSystem fdsystem = new FDAccountSystem())
                    {
                        ...
                        if (fdsystem.GetNoOfPostInterestsCount() > 0)
                        {
                            if ((!(dePostDate.DateTime == PostInterestCreatedDate)) && dePostDate.DateTime <= PostInterestCreatedDate)
                            {
                                ...
                                this.ShowMessageBox(this.GetMessage(MessageCatalog.Common.GENERAL_RE_INVESTMENT_DATE_POST_INTREST_DATE));
                                isFDAccount = false;
                                dePostDate.Focus();
                                return isFDAccount;
                            }
            ...
            if (fdTypes == FDTypes.RN)
            {
                ...
                if (isFDAccount)
                {
                    ...
                    {   //On 13/12/2023, For Post Interest accmulated, to alert if FD Amount is reduced
                        ...
                        //Implemantaion
                        if (this.ShowConfirmationMessage(this.GetMessage(MessageCatalog.Common.GENERAL_SINCE_FIXED_DEPOSIT_TRANS_MODE_CREDIT), MessageBoxButtons.YesNo, MessageBoxIcon.Question)
                          == System.Windows.Forms.DialogResult.No)
                        {
                            isFDAccount = false;
                    ...
                    if (FDRenewalId == 0)
                    {
                        if (!this.UtilityMember.DateSet.ValidateDate(this.UtilityMember.DateSet.ToDate(this.MaturityDate, false), dteRenewalOn.DateTime))
                        {
                            ...
                            //Implemantaion
                            msg = this.GetMessage(MessageCatalog.Common.GENERAL_RENEWAL_DATE_LAST_MATURITY_DATE);
                    ...
                    if (FDRenewalId != 0)
                    {
                        string maturityDateToValidate = string.Empty;
                        if (dtMasterValidate != null && dtMasterValidate.Rows.Count > 0 && dtMasterValidate.Rows[0]["FD_MATURITY_DATE"] != DBNull.Value)
                        {
                            // Use database value from dtMasterValidate
                            maturityDateToValidate = dtMasterValidate.Rows[0]["FD_MATURITY_DATE"].ToString();
                            if (!this.UtilityMember.DateSet.ValidateDate(this.UtilityMember.DateSet.ToDate(maturityDateToValidate, false), dteRenewalOn.DateTime))
                            {
                                //On 08/10/2021, To show affected date in message
                                string msg = this.GetMessage(MessageCatalog.Master.FDLedger.FD_RENEWAL_DATE_NOT_GREATER_THAN_MATURITY_DATE);
                                //Dinesh 07/07/2025
                                ...
                                //Implemantaion
                                msg = this.GetMessage(MessageCatalog.Common.GENERAL_RENEWAL_DATE_LAST_MATURITY_DATE);
                                msg += " (" + this.UtilityMember.DateSet.ToDate(maturityDateToValidate.ToString()) + ")";
                                this.ShowMessageBox(msg);
                                isFDAccount = false;
                                deCreatedDate.Focus();
                                return isFDAccount;
                    ...
                    if (CheckRenewalDuplicate() > 0)
                    {
                        ...
                        //Implemantaion
                        string msg = this.GetMessage(MessageCatalog.Common.GENERAL_FD_RENEWAL_PARTIAL_WITHDRAWAL) + dteRenewalOn.DateTime.ToShortDateString();
                        this.ShowMessageBox(msg);
                        isFDAccount = false;
                        dteRenewalOn.Focus();
                    }
                    ...
                    // 
                    //--------------------------------------------------
                    if (dtRenewalValidate != null && dtRenewalValidate.Rows.Count > 0)
                    {
                        // Get all active renewals (status = 1) for all types (RN, OP, IN, PWD, WD) ordered by FD_RENEWAL_ID
                        var activeRenewalRows = dtRenewalValidate.AsEnumerable()
                            .Where(r =>
                                r["STATUS"] != null && r["STATUS"] != DBNull.Value && r["STATUS"].ToString() == "1" &&
                                (r["FD_TYPE"] != null && r["FD_TYPE"] != DBNull.Value &&
                                 (r["FD_TYPE"].ToString() == "RN"))
                            )
                            .OrderBy(r =>
                            {
                                object idVal = r["FD_RENEWAL_ID"];
                                if (idVal == null || idVal == DBNull.Value) return int.MaxValue;
                                int parsedId;
                                return int.TryParse(idVal.ToString(), out parsedId) ? parsedId : int.MaxValue;
                            })
                            .ToList();

                        // Check if any active PWD/WD exists - if exists, no editing allowed
                        bool hasActivePwdOrWd = dtRenewalValidate.AsEnumerable().Any(r =>
                        {
                            object typeObj = r["FD_TYPE"];
                            string type = (typeObj != null && typeObj != DBNull.Value) ? typeObj.ToString() : "";
                            object statusObj = r["STATUS"];
                            string status = (statusObj != null && statusObj != DBNull.Value) ? statusObj.ToString() : "";
                            return (type == "PWD" || type == "WD") && status == "1";
                        });

                        if (hasActivePwdOrWd)
                        {
                            this.ShowMessageBox("Active Withdrawal or Partial Withdrawal exists. No editing allowed for any renewal entries.");
                            isFDAccount = false;
                            dteRenewalOn.Focus();
                            return isFDAccount;
                        }

                        // Get current renewal's new start and end dates
                        DateTime newStartDate = dteRenewalOn.DateTime;
                        DateTime newEndDate = dteFDMaturityDate.DateTime;

                        // Check for any overlap with other active renewals (excluding current renewal being edited)
                        bool hasOverlap = activeRenewalRows.Any(r =>
                        {
                            if (r["FD_RENEWAL_ID"] == null || r["FD_RENEWAL_ID"] == DBNull.Value) return false;

                            int rowId;
                            if (!int.TryParse(r["FD_RENEWAL_ID"].ToString(), out rowId) || rowId == FDRenewalId)
                                return false;

                            // Additional status check to ensure only active entries are considered
                            object statusObj = r["STATUS"];
                            if (statusObj == null || statusObj == DBNull.Value || statusObj.ToString() != "1")
                                return false;

                            DateTime rowStart = DateTime.MinValue;
                            DateTime rowEnd = DateTime.MinValue;

                            object startVal = r["RENEWAL_DATE"];
                            object endVal = r["MATURITY_DATE"];

                            if (startVal != null && startVal != DBNull.Value)
                                DateTime.TryParse(startVal.ToString(), out rowStart);
                            if (endVal != null && endVal != DBNull.Value)
                                DateTime.TryParse(endVal.ToString(), out rowEnd);

                            // Check for overlap: new period overlaps with existing period
                            // Allow same-day renewals (when maturity date equals renewal date)
                            // But prevent actual overlaps where periods intersect
                            return (newStartDate < rowEnd && newEndDate > rowStart);
                        });

                        if (hasOverlap)
                        {
                            this.ShowMessageBox("The renewal period overlaps with another active renewal. Please adjust the dates to avoid overlap.");
                            isFDAccount = false;
                            dteRenewalOn.Focus();
                            return isFDAccount;
                        }
            ...
            if (fdTypes == FDTypes.POI)
            {
                ...
                else if (dePostDate.DateTime > this.UtilityMember.DateSet.ToDate(this.AppSetting.YearTo, false))
                {
                    ...
                    //Implemnataion
                    this.ShowMessageBox(this.GetMessage(MessageCatalog.Common.GENERAL_POST_DATE_SHOULD_BE_TRANSACTION_PERIOD));
                    this.SetBorderColorForDateTimeEdit(dePostDate);
                    isFDAccount = false;
                    dePostDate.Focus();
                }
                ...
                else if (txtRenewalInterestRate.Text == "0.00")
                {
                    ...
                    //Implemnataion
                    this.ShowMessageBox(this.GetMessage(MessageCatalog.Common.GENERAL_ACTUAL_INTREAST_AMOUNT_THEN_ZERO));
                    this.SetBorderColor(txtRenewalInterestRate);
                    isFDAccount = false;
                    txtRenewalInterestRate.Focus();
                }
            ...
            if ((fdTypes == FDTypes.OP || fdTypes == FDTypes.IN) && isFDAccount)
            {
                ...
                if (fdTypes == FDTypes.OP)
                {
                    ...
                    //if (this.UtilityMember.DateSet.ValidateDate(deDateOfMaturity.DateTime, this.UtilityMember.DateSet.ToDate(this.AppSetting.YearFrom, false))) 
                    {
                        ...
                        //Implemantaion
                        this.ShowMessageBox(this.GetMessage(MessageCatalog.Common.GENERAL_FD_MATURITY_DATE_FINANCIAL));
                        isFDAccount = false;
                        deDateOfMaturity.Focus();
                    }
                    else if (EntryFDInvestmentType != (int)FDInvestmentType.MutualFund && deDateOfMaturity.DateTime < this.UtilityMember.DateSet.ToDate(this.AppSetting.BookBeginFrom, false))
                        ...
                        (string.IsNullOrEmpty(txtMutualFolioNo.Text) || UtilityMember.NumberSet.ToDouble(txtBaseNAVNoOfUnits.Text) == 0 || UtilityMember.NumberSet.ToDouble(txtBaseNAVPerUnit.Text) == 0))
                    {
                        ...
                        //Implemantaion
                        this.ShowMessageBox(this.GetMessage(MessageCatalog.Common.GENERAL_FILL_MUTUAL_DETAILS_PROPERLY));
                        isFDAccount = false;
            ...
            if ((fdTypes == FDTypes.OP || fdTypes == FDTypes.IN) && isFDAccount)
            {
                ...
                //(this.UtilityMember.DateSet.ValidateDate(deDateOfMaturity.DateTime, deCreatedDate.DateTime))
                {
                    ...
                    //Implemantaion
                    this.ShowMessageBox(this.GetMessage(MessageCatalog.Common.GENERAL_THE_MATURITY_DATE_GREATER_THEN_FINANCIAL_YEAR));
                    this.SetBorderColorForDateTimeEdit(deDateOfMaturity);
                    isFDAccount = false;
                    deDateOfMaturity.Focus();
                    return isFDAccount;
                        ...
                        (string.IsNullOrEmpty(txtMutualFolioNo.Text) || UtilityMember.NumberSet.ToDouble(txtBaseNAVNoOfUnits.Text) == 0 || UtilityMember.NumberSet.ToDouble(txtBaseNAVPerUnit.Text) == 0))
                {
                    ...
                    // this.ShowMessageBox("Fill Mutual Fund details properly.");
                    //Implemantaion
                    this.ShowMessageBox(this.GetMessage(MessageCatalog.Common.GENERAL_FILL_MUTUAL_FUNDS_DETAILS_PROPERLY));
                    isFDAccount = false;
                    txtMutualFolioNo.Focus();
                }
            }
            ...
            if (fdTypes == FDTypes.WD)
            {
                ...
                if (isFDAccount && WithdrwalReceiptTransVoucherMethod != (int)TransactionVoucherMethod.Automatic)
                {
                    if (string.IsNullOrEmpty(txtWithdrwalReceiptVNo.Text) && UtilityMember.NumberSet.ToDouble(txtRenewalInterestRate.Text) > 0)
                    {
                        ...
                        //Implemantaion
                        this.ShowMessageBox(this.GetMessage(MessageCatalog.Common.GENERAL_FIXED_DEPOSIT_WITHDRAWAL_RECEIPT_VOUCHER));
                ...
                if (isFDAccount)
                {
                    ...
                    else if (dteClosedOn.DateTime > this.UtilityMember.DateSet.ToDate(this.AppSetting.YearTo, false))
                    {
                        ...
                        //Implemantaion
                        this.ShowMessageBox(this.GetMessage(MessageCatalog.Common.GENERAL_WITHDRAWAL_ON_SHOULD_BE_PERIOD));
                        this.SetBorderColorForDateTimeEdit(dteClosedOn);
                        isFDAccount = false;
                        dteClosedOn.Focus();
                    }
            ...
            if (isFDAccount && UtilityMember.NumberSet.ToDouble(txtTDSAmount.Text) > 0 && UtilityMember.NumberSet.ToDouble(txtRenewalInterestRate.Text) == 0)
            {
                ...
                //Implemantaion
                this.ShowMessageBox(this.GetMessage(MessageCatalog.Common.GENERAL_CANT_PROCESS_TDS));
                isFDAccount = false;
                txtTDSAmount.Focus();
            }
            else if (isFDAccount && cbPenaltyMode.SelectedIndex > 0 && lcPenaltyAmount.Visibility == LayoutVisibility.Always && UtilityMember.NumberSet.ToDouble(txtPenaltyAmount.Text) > 0)
            {
                if (glkpPenaltyLedger.EditValue == null || this.UtilityMember.NumberSet.ToInteger(glkpPenaltyLedger.EditValue.ToString()) == 0)
                {
                    ...
                    //Implemnataion
                    this.ShowMessageBox(this.GetMessage(MessageCatalog.Common.GENERAL_PENALTY_LEDGER_IS_EMPTY));
                    isFDAccount = false;
            ...
            if (this.AppSetting.AllowMultiCurrency == 1)
            {
                if (FDIntrestVoucherId > 0 || FDVoucherId > 0 || VoucherId > 0)
                {
                    ...
                    if (vid > 0)
                    {
                        ...
                        {
                            //bharath 07/04/2026 decimal value (15,4)
                            txtExchangeRate.Text = vsystem.ExchangeRate.ToString("0.00##");
                            txtExchangeRate.Tag = vsystem.ExchangeRate.ToString("0.00##");
                            // 29/01/2025, in order to subscribe live exchange rate - exchange and live same
                            // lblLiveExchangeRate.Text = txtExchangeRate.Text;
                            ...
                            txtLiveExchangeRate.Text = vsystem.LiveExchangeRate.ToString("0.00##");
                            txtLiveExchangeRate.Tag = vsystem.ExchangeRate.ToString("0.00##");
                    ...
                    if (fdAccountId != 0)
                    {
                        using (FDAccountSystem fdAccountSystem = new FDAccountSystem(fdAccountId))
                        {
                            ...
                            if (this.AppSetting.AllowMultiCurrency == 1)
                            {
                                ...
                                txtLiveExchangeRate.Text = fdTypes == FDTypes.OP ? fdAccountSystem.ExchangeRate.ToString() : fdAccountSystem.LiveExchangeRate.ToString();
                                txtExchangeRate.Tag = txtExchangeRate.Text;
                                // 29/01/2025, in order to subscribe live exchange rate - exchange and live same
                                // lblLiveExchangeRate.Text = txtExchangeRate.Text;
        ...
        private void CalculateInterestAmount()
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        //chinna
                        string dad = dteFDMaturityDate.DateTime != DateTime.MinValue.Date ? dteFDMaturityDate.DateTime.ToShortDateString() + " : " + this.UtilityMember.NumberSet.ToCurrency(0) : " : " + this.UtilityMember.NumberSet.ToCurrency(0);
                    }
                    else
                    {
                        //chinna
                    ...
                    using (FDAccountSystem fdAccountSystem = new FDAccountSystem())
                    {
                        ...
                        if (resultArgsTransDetails.Success && resultArgsTransDetails.DataSource.Table != null && resultArgsTransDetails.DataSource.Table.Rows.Count != 0)
                        {
                            ...
                            else if (RenewalType == FDRenewalTypes.WDI.ToString() || RenewalType == FDRenewalTypes.PWD.ToString())
                            {
                                ...
                                //FDAmount = Math.Round((FDAmount + Accumlated + reInvested) - withdrawals, 2);
        ...
        private void CheckFDTransDateAndLockFDModification()
        {
            ...
            if (FDTypes.OP != fdTypes)
            {
                               ...
                               && FDTransDate <= this.UtilityMember.DateSet.ToDate(this.AppSetting.YearTo, false))))
                {
                    ...
                    //Implemnataion
                    this.ShowMessageBox(this.GetMessage(MessageCatalog.Common.GENERAL_CHANGE_FINANCIAL_YERA_ON_RENEWAL));
                    this.Close();
                }
            }
            //-------------------------------------------------------------------------------------------------------------------
            ...
            try
            {
                using (FDAccountSystem fdaccountsystem = new FDAccountSystem())
                {
                    ...
                    if (rtn)
                    {
                        ...
                        //Implemantaion
                        this.ShowMessageBox(this.GetMessage(MessageCatalog.Common.GENERAL_AS_FIXED_DEPOSITE_IS_ALREDY_CLOSED));
                    }
                }
            }
            catch (Exception ex)
            ...
            try
            {
                if (fdAccountId != 0 && FDRenewalId != 0)
                {
                    ...
                    if ((FDAmount - WithdrawAmount) < (Withdrawals + ((this.UtilityMember.NumberSet.ToDouble(txtWithdrawAmount.Text)) - WithdrawAmount)))
                    {
                        ...
                        //Implemantaion
                        this.ShowMessageBox(this.GetMessage(MessageCatalog.Common.GENERAL_WITHDRAWAL_AMOUNT_LESS_THEN_BALANCE_AMOUNT));
                ...
                {
                    //  11/09/2025, *Chinna
                    FDAmount = Math.Round(FDAmount, 2);
                    double withdrawAmount = this.UtilityMember.NumberSet.ToDouble(txtWithdrawAmount.Text);

                    if (FDAmount < withdrawAmount)
                    {
                        this.ShowMessageBox(this.GetMessage(MessageCatalog.Master.BreakUp.FD_WITHDRAWALAMOUNT_EXCEED));
                        isExceed = true;
                    }

                    // commande before 11/09/2025
