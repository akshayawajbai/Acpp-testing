...
namespace ACPP.Modules.Master
{
    public partial class frmMapProjectLedger : frmFinanceBaseAdd
    {
        ...
        private bool isUpdatingLedgerCurrency = false;
        private readonly Dictionary<int, CurrencyRowDetails> currencyDetailsCache = new Dictionary<int, CurrencyRowDetails>();
        #endregion
        ...
        private class CurrencyRowDetails
        {
        ...
        {
            InitializeComponent();
            gvLedgerDetail.CellValueChanged += gvLedgerDetail_CellValueChanged;
        }

        // 30/03/2026, Enable this new general-ledger multicurrency logic only for SDBAFE/BSGRMG.
        private bool IsSDBAFEMultiCurrencyGeneralLedgerMode()
        {
            // return this.AppSetting.AllowMultiCurrency == 1 && (this.AppSetting.IS_SDBAFE || this.AppSetting.IS_BSGRMG);
            return this.AppSetting.AllowMultiCurrency == 1 && (this.AppSetting.IS_BSGRMG);
        }
        ...
        private void frmMapProjectLedger_Load(object sender, EventArgs e)
        {
            ...
            gvColCurrencyGeneralLedger.Visible = IsSDBAFEMultiCurrencyGeneralLedgerMode();
        ...
        private void xtcProject_SelectedPageChanged(object sender, DevExpress.XtraTab.TabPageChangedEventArgs e)
        {
            ...
            {   //For Currency Exchange Rate
                ...
                LoadCurrencyCountry();
                //bharath load Currency name
                LoadCurrencyList();
                gvColCurrencyGeneralLedger.Visible = true;
            }
        ...
        private void gvLedgerDetail_ShowingEditor(object sender, System.ComponentModel.CancelEventArgs e)
        {
            ...
            if (gvLedgerDetail.GetRowCellValue(gvLedgerDetail.FocusedRowHandle, colLedgerId) != null)
            {
                ...
                GroupId = drLedger != null ? UtilityMember.NumberSet.ToInteger(drLedger["GROUP_ID"].ToString()) : 0;

                if (gvLedgerDetail.FocusedColumn == gvColCurrencyGeneralLedger)
                {
                    if (!IsSDBAFEMultiCurrencyGeneralLedgerMode() || GroupId == 12 || GroupId == 13 || GroupId == 14)
                    {
                        e.Cancel = true;
                        return;
                    }
                ...
                if (gvLedgerDetail.GetRowCellValue(gvLedgerDetail.FocusedRowHandle, colLedgerId.FieldName).ToString() != string.Empty)
                {
                    ...
                    if (dtTransactionFD != null && dtTransactionFD.Rows.Count > 0)
                    {
                        ...
                        else
                        {
                            ...
                            else if (gvLedgerDetail.FocusedColumn.Name.Equals(grdChkCol.Name) && IsNotCCLedgerDistributed(Lid))
                            {
                                ...
                                //Implemantaion
                                this.ShowMessageBox(this.GetMessage(MessageCatalog.Common.GENERAL_AS_LEDGERS_ARE_MAPPED_WITH_COST_CENTRE));
                            }
                        }
                    }
                    else
                    {
                        ...
                        else if (this.AppSetting.CostCeterMapping == 1 && IsNotCCLedgerDistributed(Lid))
                        {
                            ...
                            //ShowMessageBox("As Ledger(s) are mapped with Cost Centre, Can't unmap Ledger");
                            //implemantaion
                            ShowMessageBox(this.GetMessage(MessageCatalog.Common.GENERAL_AS_LEDGERS_ARE_MAPPED_WITH_COST_CENTRE));
                        }
                    }
                }
            }
        ...
        {
            SyncGeneralLedgerMultiCurrencyRow(e.RowHandle);
            UpdateBalanceLedger();
        }

        private void gvLedgerDetail_CellValueChanged(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            if (isUpdatingLedgerCurrency || !IsSDBAFEMultiCurrencyGeneralLedgerMode() || e.RowHandle < 0)
            {
                return;
            }

            if (e.Column == colOPBalance)
            {
                SyncGeneralLedgerMultiCurrencyRow(e.RowHandle);
                UpdateBalanceLedger();
                return;
            ...
            if (e.Column != gvColCurrencyGeneralLedger)
            {
                return;
            }
            ...
            if (e.Value != null)
            {
                countryId = UtilityMember.NumberSet.ToInteger(e.Value.ToString());
            }
            if (countryId == 0)
            {
                object currentValue = gvLedgerDetail.GetRowCellValue(e.RowHandle, gvColCurrencyGeneralLedger);
                countryId = currentValue == null ? 0 : UtilityMember.NumberSet.ToInteger(currentValue.ToString());
            }
            if (countryId == 0)
            {
                return;
            ...
            string currencyDisplay = string.Empty;
            double exchangeRate = 1;
            LoadCurrencyDetailsByCountry(countryId, ref currencyDisplay, ref exchangeRate);

            isUpdatingLedgerCurrency = true;
            try
            {
                gvLedgerDetail.SetRowCellValue(e.RowHandle, "CUR_COUNTRY_ID", countryId);
                if (!string.IsNullOrEmpty(currencyDisplay))
                {
                    gvLedgerDetail.SetRowCellValue(e.RowHandle, "CURRENCY", currencyDisplay);
                }
                gvLedgerDetail.SetRowCellValue(e.RowHandle, "EXCHANGE_RATE", exchangeRate == 0 ? 1 : exchangeRate);
                SyncGeneralLedgerMultiCurrencyRow(e.RowHandle);
                UpdateBalanceLedger();
            }
            finally
            {
                isUpdatingLedgerCurrency = false;
            }
        }

        // 30/03/2026, Use the same FY-based exchange rate fetch as frmLedgerDetailAdd when currency changes.
        private void LoadCurrencyDetailsByCountry(int countryId, ref string currencyDisplay, ref double exchangeRate)
        {
            if (countryId <= 0)
            {
                return;
            ...
            CurrencyRowDetails cachedDetails;
            if (currencyDetailsCache.TryGetValue(countryId, out cachedDetails))
            {
                currencyDisplay = cachedDetails.CurrencyDisplay;
                exchangeRate = cachedDetails.ExchangeRate;
                return;
            }
            ...
            try
            {
                using (CountrySystem countrySystem = new CountrySystem())
                {
                    ...
                    {
                        string currencySymbol = countrySystem.CurrencySymbol ?? string.Empty;
                        string country = countrySystem.CountryName ?? string.Empty;
                        string currencyName = countrySystem.CurrencyName ?? string.Empty;
                        currencyDisplay = string.Concat("( ", currencySymbol, " : ", country, ") - ", currencyName);
                        exchangeRate = countrySystem.ExchangeRate == 0 ? 1 : countrySystem.ExchangeRate;
                        currencyDetailsCache[countryId] = new CurrencyRowDetails()
                        {
        ...
        private void btnMoveOut_Click(object sender, EventArgs e)
        {
            ...
            //Implementaion
            this.ShowWaitDialog(this.GetMessage(MessageCatalog.Common.GENERAL_CHECKING_AND_UNMAPPING_LEDGERS));
        ...
        {
            CommitGeneralLedgerCurrencyEdits();
        ...
        private void LoadLedgerDetails()
        {
            try
            {
                using (MappingSystem mappingSystem = new MappingSystem())
                {
                    ...
                    //bharath to get local country id 
                    string local_countryid = this.AppSetting.Country;
                    DataTable dt = resultArgs.DataSource.Table;
                    UpdateCurrentCountryForEmptyLedgers(dt, local_countryid, mappingSystem);
        ...
        //bharath to set currency into data table
        private void UpdateCurrentCountryForEmptyLedgers(DataTable ledgerTable, string localCountryId, MappingSystem mappingSystem)
        {
            if (ledgerTable == null || mappingSystem == null || string.IsNullOrWhiteSpace(localCountryId))
            {
                return;
            }


            Int32 currentCountryId;
            if (!Int32.TryParse(localCountryId, out currentCountryId))
            {
                return;
            }

            string currentCountryIdColumn = mappingSystem.AppSchema.Ledger.CUR_COUNTRY_IDColumn.ColumnName;
            if (!ledgerTable.Columns.Contains(currentCountryIdColumn))
            {
                return;
            }

            string currencyColumn = mappingSystem.AppSchema.Country.CURRENCYColumn.ColumnName; // "CURRENCY" (from MappingSQL.LoadAllLedgers)
            bool hasCurrencyColumn = ledgerTable.Columns.Contains(currencyColumn);
            string localCurrencyDisplay = string.Empty;
            if (hasCurrencyColumn)
            {
                using (CountrySystem countryCurrency = new CountrySystem())
                {
                    var currencyResult = countryCurrency.FetchCountryCurrencyExchangeRateByCountryDate(
                        currentCountryId,
                        UtilityMember.DateSet.ToDate(AppSetting.YearFrom, false)
                    );

                    if (currencyResult != null && currencyResult.Success)
                    {
                        string sym = countryCurrency.CurrencySymbol ?? string.Empty;
                        string country = countryCurrency.CountryName ?? string.Empty;
                        string name = countryCurrency.CurrencyName ?? string.Empty;
                        localCurrencyDisplay = string.Concat("( ", sym, " : ", country, ")  - ", name);
                    }
                }
            }
            ...
            {
                if (row[currentCountryIdColumn] == DBNull.Value || string.IsNullOrWhiteSpace(row[currentCountryIdColumn].ToString()))
                {
                    row[currentCountryIdColumn] = currentCountryId;
                    if (hasCurrencyColumn && (row[currencyColumn] == DBNull.Value || string.IsNullOrWhiteSpace(row[currencyColumn].ToString())))
                    {
                        row[currencyColumn] = localCurrencyDisplay;
                    }
                }
            }
        }
            ...
            try
            {
                ...
                if (dtBalanceUpdate != null && dtBalanceUpdate.Rows.Count > 0)
                {
                    ...
                    if (this.AppSetting.AllowMultiCurrency == 1)
                    {
                        ...
                        r.Field<string>("GROUP") == "General").Sum(r => (r.Field<decimal?>("AMOUNT") ?? 0) * (r.Field<decimal?>("EXCHANGE_RATE") ?? 0));
            ...
            try
            {
                using (MappingSystem mappingSystem = new MappingSystem())
                {
                    if (grdlProjectName != null)
                    {
                        ...
                        {
                            UpdateCurrentCountryForEmptyLedgers(resultArgs.DataSource.Table, local_countryid, mappingSystem);
                            ApplyGeneralLedgerMultiCurrencyValues(resultArgs.DataSource.Table);
        ...
        private void ShowSelectedLedgerCount()
        {
            ResourceManager rm = new ResourceManager("ACPP.Modules.Master.frmMapProjectLedger", typeof(frmMapProjectLedger).Assembly);

            // Fallback to default English text if the resource key is missing
            string selectedLedgersText = rm.GetString("SELECTED_LEDGERS", CultureInfo.CurrentUICulture) ?? "Selected Ledger(s)";

            lgrpSelectedLedgers.Text = selectedLedgersText;
            lblSelecteLedgersCount.Text = "0";

            if (gvLedgerDetail.RowCount > 0)
            {
                lgrpSelectedLedgers.Text = gvLedgerDetail.RowCount.ToString() + " " + selectedLedgersText;
                lblSelecteLedgersCount.Text = "# " + gvLedgerDetail.RowCount.ToString();
            }
        }
        ...
        {
            ResourceManager rm = new ResourceManager("ACPP.Modules.Master.frmMapProjectLedger", typeof(frmMapProjectLedger).Assembly);
            string availableLedgersText = rm.GetString("AVAILABLE_LEDGERS", CultureInfo.CurrentUICulture) ?? "Available Ledger(s)";

            // Default when no rows
            lgrpAvailableLedgers.Text = availableLedgersText;

            if (gvAvailableLedgerDetails.RowCount > 0)
            {
                // Combine row count with the localized label
                lgrpAvailableLedgers.Text = gvAvailableLedgerDetails.RowCount.ToString() + " " + availableLedgersText;
            }
        }
                ...
                if (gcAvailableCostCentre.DataSource != null && projectSystem.dtCostCenterIDCollection != null)
                {
                    if ((gcAvailableCostCentre.DataSource as DataTable).Rows.Count > 0 || projectSystem.dtCostCenterIDCollection.Rows.Count > 0)
                    {
                        ...
                        else if (AppSetting.CostCeterMapping == 1 && CCTotalDistributedAmount != CCMappLedgerOpAmount)
                        {
                            ...
                            //Implenation
                            MessageRender.ShowMessage(this.GetMessage(MessageCatalog.Common.GENERAL_LEDGER_OPENING_AMOUNT_IS_NOT_YET));
                    ...
                    if ((gcGenAvailable.DataSource as DataTable).Rows.Count > 0 || projectSystem.dtGeneralateIDCollection.Rows.Count > 0)
                    {
                        if (glkpGenLedger.EditValue != null)
                        {
                            ...
                            if (resultArgs.Success)
                            {
                                ...
                                //Implemantaion
                                ShowSuccessMessage(this.GetMessage(MessageCatalog.Common.GENERAL_GENERALATE_MAPPING_IS_SUCCESSFUL));
                ...
                if (dtMappedPurpose != null && dtMappedPurpose.Rows.Count > 0)
                {
                    ...
                    foreach (DataRowView drv in dtPurposeList.DefaultView)
                    {
                        ...
                        if (opCCDistributedAmount > 0 && opamount != opCCDistributedAmount)
                        {
                            ...
                            //Implemantaion
                            MessageRender.ShowMessage("Purpose '" + purposename + "'" + this.GetMessage(MessageCatalog.Common.GENERAL_IS_NOT_YET_FULLY_DISTRIBUTED_WITH_COST_CENTRE));
                            break;
                        }
                    }
                }
        ...
        // 30/03/2026, For frmMapProjectLedger general-ledger maintenance:
        // keep shared SQL behavior unchanged and normalize only this form's editable amount/rate.
        private void ApplyGeneralLedgerMultiCurrencyValues(DataTable dtMappedLedgers)
        {
            if (!IsSDBAFEMultiCurrencyGeneralLedgerMode() || dtMappedLedgers == null || dtMappedLedgers.Rows.Count == 0)
            {
                return;
            }

            if (!dtMappedLedgers.Columns.Contains("AMOUNT_FC") || !dtMappedLedgers.Columns.Contains("OP_EXCHANGE_RATE"))
            {
                return;
            }
            ...
            {
                Int32 groupId = dtMappedLedgers.Columns.Contains("GROUP_ID") ? UtilityMember.NumberSet.ToInteger(dr["GROUP_ID"].ToString()) : 0;
                string ledgerGroup = dtMappedLedgers.Columns.Contains("GROUP") ? dr["GROUP"].ToString() : string.Empty;
                Int32 currencyCountryId = dtMappedLedgers.Columns.Contains("CUR_COUNTRY_ID") ? UtilityMember.NumberSet.ToInteger(dr["CUR_COUNTRY_ID"].ToString()) : 0;

                bool isEditableGeneralLedger = currencyCountryId > 0 &&
                    ledgerGroup == "General" &&
                    groupId != 12 && groupId != 13 && groupId != 14;

                if (isEditableGeneralLedger)
                {
                    dr["AMOUNT"] = UtilityMember.NumberSet.ToDouble(dr["AMOUNT_FC"].ToString());
                    dr["EXCHANGE_RATE"] = UtilityMember.NumberSet.ToDouble(dr["OP_EXCHANGE_RATE"].ToString());
                }
            }
        }

        // 30/03/2026, Keep this SDBAFE-only general-ledger multicurrency behavior isolated from existing users.
        private bool IsEditableGeneralLedgerMultiCurrencyRow(DataRow drLedger)
        {
            if (!IsSDBAFEMultiCurrencyGeneralLedgerMode() || drLedger == null || drLedger.Table == null ||
                !drLedger.Table.Columns.Contains("GROUP_ID") || !drLedger.Table.Columns.Contains("GROUP") ||
                !drLedger.Table.Columns.Contains("CUR_COUNTRY_ID"))
            {
                return false;
            }

            Int32 groupId = UtilityMember.NumberSet.ToInteger(drLedger["GROUP_ID"].ToString());
            string ledgerGroup = drLedger["GROUP"].ToString();
            Int32 currencyCountryId = UtilityMember.NumberSet.ToInteger(drLedger["CUR_COUNTRY_ID"].ToString());

            return currencyCountryId > 0 &&
                ledgerGroup == "General" &&
                groupId != 12 && groupId != 13 && groupId != 14;
        }

        // 30/03/2026, Keep the entered FC amount and selected country-based rate in this form's row state only.
        private void SyncGeneralLedgerMultiCurrencyRow(int rowHandle)
        {
            if (!IsSDBAFEMultiCurrencyGeneralLedgerMode() || rowHandle < 0)
            {
                return;
            }

            DataRow drLedger = gvLedgerDetail.GetDataRow(rowHandle);
            if (drLedger == null || drLedger.Table == null)
            {
                return;
            }

            if (!drLedger.Table.Columns.Contains("GROUP_ID") || !drLedger.Table.Columns.Contains("GROUP") ||
                !drLedger.Table.Columns.Contains("CUR_COUNTRY_ID") || !drLedger.Table.Columns.Contains("AMOUNT_FC") ||
                !drLedger.Table.Columns.Contains("OP_EXCHANGE_RATE"))
            {
                return;
            }

            if (!IsEditableGeneralLedgerMultiCurrencyRow(drLedger))
            {
                return;
            }

            Int32 currencyCountryId = UtilityMember.NumberSet.ToInteger(drLedger["CUR_COUNTRY_ID"].ToString());
            string currencyDisplay = drLedger.Table.Columns.Contains("CURRENCY") ? drLedger["CURRENCY"].ToString() : string.Empty;
            double exchangeRate = drLedger.Table.Columns.Contains("EXCHANGE_RATE") ?
                UtilityMember.NumberSet.ToDouble(drLedger["EXCHANGE_RATE"].ToString()) : 1;

            LoadCurrencyDetailsByCountry(currencyCountryId, ref currencyDisplay, ref exchangeRate);

            if (drLedger.Table.Columns.Contains("CURRENCY"))
            {
                drLedger["CURRENCY"] = currencyDisplay;
            }
            if (drLedger.Table.Columns.Contains("EXCHANGE_RATE"))
            {
                drLedger["EXCHANGE_RATE"] = exchangeRate;
            }

            drLedger["AMOUNT_FC"] = UtilityMember.NumberSet.ToDouble(drLedger["AMOUNT"].ToString());
            drLedger["OP_EXCHANGE_RATE"] = exchangeRate;
        }

        // 30/03/2026, Commit focused grid edits before save so MASTER_LEDGER and balance use the selected row currency rate.
        private void CommitGeneralLedgerCurrencyEdits()
        {
            if (!IsSDBAFEMultiCurrencyGeneralLedgerMode() || gvLedgerDetail == null)
            {
                return;
            }

            gvLedgerDetail.CloseEditor();
            gvLedgerDetail.PostEditor();
            gvLedgerDetail.UpdateCurrentRow();

            for (int rowHandle = 0; rowHandle < gvLedgerDetail.RowCount; rowHandle++)
            {
                SyncGeneralLedgerMultiCurrencyRow(rowHandle);
            }
        }
        ...
        private bool ValidateVoucher()
        {
            ...
            if (dtProVouchers != null && dtProVouchers.Rows.Count > 0)
            {
                if (!(ValidateProjectVouchers(dtProVouchers)))
                {
                    ...
                    //Implemantaion
                    this.ShowMessageBox(this.GetMessage(MessageCatalog.Common.GENERAL_CAN_NOT_UNMAP_BASIC_VOUCHER));
                ...
                else if (!(isExistTransaction()))
                {
                    ...
                    //Implemantaion
                    this.ShowMessageBox(this.GetMessage(MessageCatalog.Common.GENERAL_VOUCHER_TRANSACTION_CAN_NOT_UNMAP_VOUCHER_TYPE));
            ...
            if (isProject && ProjectId == 0)
            {
                ...
                //Implemantaion
                this.ShowMessageBox(this.GetMessage(MessageCatalog.Common.GENERAL_SELECT_PROJECT));
                isProject = false;
                                ...
                                if (IsFCPurpose)
                                {
                                    if (dtAllProjectPurposeDistributed != null && dtAllProjectPurposeDistributed.Rows.Count > 0)
                                    {
                                        ...
                                        if (!Distributedfully)
                                        {
                                            ...
                                            //Implemataion
                                            ShowMessageBox(this.GetMessage(MessageCatalog.Common.GENERAL_CANT_UNMAP_PURPOSE));
            ...
            try
            {
                ...
                if (dtMappedItems != null)
                {
                    ...
                    if (CheckedItems.Count() > 0)
                    {
                        ...
                        //Implemantaion
                        this.ShowMessageBox(this.GetMessage(MessageCatalog.Common.GENERAL_DEFAULT_LEDGERS_USED_LEDGERS_WILLNOT_BE_UNMAPPED));
                        this.ShowMessageBox(this.GetMessage(MessageCatalog.Common.GENERAL_PROCESSING_UNMAP_SELECTED_LEDGER));
                    ...
                    foreach (DataRowView drv in dtPurposeList.DefaultView)
                    {
                        ...
                        if (opCCDistributedAmount > 0 && opamount != opCCDistributedAmount)
                        {
                            ...
                            if (opamount == 0 && opCCDistributedAmount > 0)
                            {
                                ...
                                //Implemantaion
                                MessageRender.ShowMessage("Ledger '" + ledgername + "'" + this.GetMessage(MessageCatalog.Common.GENERAL_OPENING_BALANCE_IS_ALREADY_DISTRIBUTED));
                            }
                            else
                            {
                                //Dinesh 04/07/2025
                                ...
                                //Implemantaion
                                MessageRender.ShowMessage("Ledger '" + ledgername + "'" + this.GetMessage(MessageCatalog.Common.GENERAL_OPENING_BALANCE_IS_NOT_YET_FULLY_DISTRIBUTED_WITH_COST_CENTRE));
                            }
                            break;
                        }
                    }
                ...
                if (dtMappedItems != null)
                {
                    ...
                    if (CheckedItems.Count() > 0)
                    {
                        ...
                        using (VoucherTransactionSystem voucherTransaction = new VoucherTransactionSystem())
                        {
                            ...
                            if (resultArgs != null)
                            {
                                //int BudgetCount = this.UtilityMember.NumberSet.ToInteger(resultArgs.DataSource.Table.Rows[0]["LEDGER_ID"].ToString());
                                //bharath 21/03/2026 
                                int BudgetCount = 0;
                                if (resultArgs.DataSource != null && resultArgs.DataSource.Table != null && resultArgs.DataSource.Table.Rows.Count > 0)
                                {
                                    BudgetCount = this.UtilityMember.NumberSet.ToInteger(resultArgs.DataSource.Table.Rows[0]["LEDGER_ID"].ToString());
                                }
                                if (BudgetCount == 0)
                                {
                                    if (dtUnMappedItems != null)
            ...
            using (LedgerSystem ledgersystem = new LedgerSystem())
            {
                if (gvLedgerDetail.GetRowCellValue(gvLedgerDetail.FocusedRowHandle, colLedgerId) != null)
                {
                    ...
                    else
                    {
                        ...
                        //Implemantaion
                        this.ShowMessageBox(this.GetMessage(MessageCatalog.Common.GENERAL_DEFAULT_LEDGERS_CANT_BE_MODIFIED));
                    ...
                    if (dtDefinedCurrencyExchangeRate != null && dtDefinedCurrencyExchangeRate.Rows.Count > 0)
                    {
                        ...
                        if (dt.DefaultView.Count == 0)
                        {
                            ...
                            using (MappingSystem mappingsystem = new MappingSystem())
                            {
                                ...
                                //Implemantaion
                                this.ShowWaitDialog(this.GetMessage(MessageCatalog.Common.GENERAL_UPDATING_EXCHANGE_RAT));
                                ...
                                if (result.Success)
                                {
                                    ...
                                    //Implemantaion
                                    ShowSuccessMessage(this.GetMessage(MessageCatalog.Common.GENERAL_EXCHANGE_RATE_UPDATEING));
                        ...
                        else
                        {
                            ...
                            //Implemantaion
                            this.ShowMessageBoxWarning(this.GetMessage(MessageCatalog.Common.GENERAL_ALL_MAPPED_CURRENCY_SHOULD_HAVE_EXCHANGE_RATE));
                        }
                    }
                    else
                    {
        ...
        /// <summary>
        /// Load currency dropdown list (only lists CURRENCY values, no ledger-specific filtering).
        /// </summary>
        private void LoadCurrencyList()
        {
            try
            {
                ...
                {
                    resultArgs = countrySystem.FetchCountryCurrencyExchangeRateByFY();
                    if (resultArgs != null && resultArgs.Success && resultArgs.DataSource != null && resultArgs.DataSource.Table != null)
                    {
                        DataTable dtCurrencyCountry = resultArgs.DataSource.Table;
                        if (dtCurrencyCountry.Rows.Count > 0)
                        {
                            // Only show currencies which are mapped for the FY (exchange rate > 0).
                            string exchangeRateFilter = countrySystem.AppSchema.Country.EXCHANGE_RATEColumn.ColumnName + " > 0";
                            DataView dv = dtCurrencyCountry.DefaultView;
                            dv.RowFilter = exchangeRateFilter;
                            DataTable dtDistinctCurrency = dv.ToTable();
                            if (!dtDistinctCurrency.Columns.Contains("CURRENCY_DISPLAY"))
                            {
                                dtDistinctCurrency.Columns.Add("CURRENCY_DISPLAY", typeof(string));
                            }

                            foreach (DataRow drCurrency in dtDistinctCurrency.Rows)
                            {
                                string currencySymbol = drCurrency.Table.Columns.Contains("CURRENCY_SYMBOL") ? drCurrency["CURRENCY_SYMBOL"].ToString() : string.Empty;
                                string country = drCurrency.Table.Columns.Contains("COUNTRY") ? drCurrency["COUNTRY"].ToString() : string.Empty;
                                string currencyName = drCurrency.Table.Columns.Contains("CURRENCY_NAME") ? drCurrency["CURRENCY_NAME"].ToString() : drCurrency[countrySystem.AppSchema.Country.CURRENCYColumn.ColumnName].ToString();
                                drCurrency["CURRENCY_DISPLAY"] = string.Concat("( ", currencySymbol, " : ", country, ") - ", currencyName);
                            }

                            this.UtilityMember.ComboSet.BindRepositoryItemGridLookUpEdit(
                                rglkpCurrencylist,
                                dtDistinctCurrency,
                                "CURRENCY_DISPLAY",
                                countrySystem.AppSchema.Country.COUNTRY_IDColumn.ColumnName);
                        }
                    }
                }
            }
        ...
        private void btnCurrencyMoveOut_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (isCantUnmap)
                {
                    ...
                    //Implemantaion
                    MessageRender.ShowMessage(this.GetMessage(MessageCatalog.Common.GENERAL_AS_VOUCHER_CURRENCY_YOU_CANT_BE_UNMAPPED));
                }
            }
            catch (Exception ex)
            {
