...
namespace ACPP.Modules.Master
{
    public partial class frmAddAnnualBudget : frmFinanceBaseAdd
    {
        ...
        private void EnforceBudgetFeatuersByLicenseKeys()
        {
            ...
            // sdbrmg details Chinna* 20/06/2025
            if (this.AppSetting.IS_SDB_RMG)
            {
                lcExportBudget.Visibility = DevExpress.XtraLayout.Utils.LayoutVisibility.Always;
                btnExportBudget.Text = "Export Budget";
            }
        }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            if (IsFormValid())
            {
                ...
                // 26/08/2025. Chinna
                string filter;
                if (this.AppSetting.IS_BSG_ESP || this.AppSetting.IS_SDB_RMG)
                {
                    filter = string.Empty;
                }
                else if (this.AppSetting.IS_SDB_INM)
                {
                    filter = "(PROPOSED_CURRENT_YR > 0 OR APPROVED_CURRENT_YR > 0 OR ACTUAL > 0)";
                }
                else
                {
                    filter = "(PROPOSED_CURRENT_YR > 0 OR APPROVED_CURRENT_YR > 0)";
                }

                dvRecords.RowFilter = filter;
                dtAnnualBudget = dvRecords.ToTable();

                //commanded by chinna 26-08-2025
                ...
                using (BudgetSystem budgetSystem = new BudgetSystem())
                {
                    ...
                    if (resultArgs.Success && resultArgs.DataSource.Table.Rows.Count > 0)
                    {
                       // this.UtilityMember.ComboSet.BindGridLookUpCombo(glkpProject, resultArgs.DataSource.Table, budgetSystem.AppSchema.Project.PROJECTColumn.ColumnName, budgetSystem.AppSchema.Project.PROJECT_IDColumn.ColumnName);
                        this.UtilityMember.ComboSet.BindGridLookUpCombo(glkpProject, resultArgs.DataSource.Table, budgetSystem.AppSchema.Project.PROJECT_NAMEColumn.ColumnName, budgetSystem.AppSchema.Project.PROJECT_IDColumn.ColumnName);
        ...
        private void btnImportBudget_Click(object sender, EventArgs e)
        {
            ...
            if (this.ShowConfirmationMessage("Are you sure to import Budget from Head Office Portal ?", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == System.Windows.Forms.DialogResult.Yes)
            {
                ...
                if (result.Success)
                {
                    ...
                    this.ShowMessageBox(this.GetMessage(MessageCatalog.Common.GENRAL_BUDGET_APPROVED_HOF));
                    if (gvProject.GetSelectedRows().Length > 0)
                    {
                        AssignValues();
                        AssignProjectIds = GridselectedProjectIds;
