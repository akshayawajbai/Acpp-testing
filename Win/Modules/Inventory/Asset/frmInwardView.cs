...
namespace ACPP.Modules.Asset
{
    public partial class frmInwardView : frmFinanceBase
    {
            ...
            try
            {
                ...
                using (AssetInwardOutwardSystem InwardOutwardVoucherSystem = new AssetInwardOutwardSystem())
                {
                    ...
                    if (dsAssetInwardOutward != null && dsAssetInwardOutward.Tables.Count > 0)
                    {
                        ...
                        gvAssetPaymentDetails.Columns["IN_OUT_ID"].Visible = false;
                    }
                    else
                    {
        ...
        private void rbtnInwardPaymentAddScreen_Click(object sender, EventArgs e)
        {
            try
            {
                int ProjectId = this.UtilityMember.NumberSet.ToInteger(glkpProject.EditValue.ToString());
                string ProjectName = glkpProject.Text.ToString();
                int rowHandla = gvPurchaseView.FocusedRowHandle;
                int currecnyId = gvPurchaseView.FocusedRowHandle;
                DateTime dateTime = Convert.ToDateTime(gvPurchaseView.GetRowCellValue(rowHandla, "IN_OUT_DATE"));
                int CurrencyId = Convert.ToInt32(gvPurchaseView.GetRowCellValue(rowHandla, "CURRENCY_COUNTRY_ID"));
                ApplyRecentPrjectDetails(ProjectId);
                frmTransactionMultiAdd paymenAdd = new frmTransactionMultiAdd(dateTime.ToString("yyyy-MM-dd"), ProjectId, ProjectName, 0, 1, false);
                paymenAdd.CurrencyId = CurrencyId;
                paymenAdd.ShowDialog();   
        ...
        private void ShowPaymentForm(int VoucherID, bool duplicate = false)
        {   // private void ShowForm(int VoucherID, bool duplicate = false)
            

        }

        private void gcPurchaseView_Click(object sender, EventArgs e)
        {
