...
using Bosco.DAO.Data;//Added by Dinesh 05-01-2026
...
namespace ACPP.Modules.Asset
{
    public partial class frmInwardVoucherAdd : frmFinanceBaseAdd
    {
        ...
        //    {
        //        amountValue = 0;
        //        amountValue = gvPurchase.GetFocusedRowCellValue(colAmount) != null ? this.UtilityMember.NumberSet.ToInteger(gvPurchase.GetFocusedRowCellValue(colAmount).ToString()) : 0;
        //        return amountValue;
        //    }
        ...
        // 19/12/2025, Chinna.
        // Calculate summary as CR - DR (not sum) to match transaction screen behavior
        private double AmountSummaryVal
        {
            get { return GetTransSummaryAmount(); }
        }
        ...
        #region Dinesh Property  08-12-2025 ****************************

        public int CrDetailId
        {
            get { return gvPurchase.GetFocusedRowCellValue(colCRDetailId) != null ? this.UtilityMember.NumberSet.ToInteger(gvPurchase.GetFocusedRowCellValue(colCRDetailId).ToString()) : 0; }
        }
        ...
        private DataTable dtAssetOnly { get; set; }

        private DataTable dtLedgerOnly { get; set; }

        private int AssetItemId  //purpose of Hide Quentity Column in Ledger item 
        {
            get
            {
                int assetItemId = 0;

                object itemTypeObj = gvPurchase.GetRowCellValue(gvPurchase.FocusedRowHandle, "ITEM_TYPE");
                object itemIdObj = gvPurchase.GetRowCellValue(gvPurchase.FocusedRowHandle, "ITEM_ID");

                if (itemTypeObj == null || itemIdObj == null)
                {
                    return 0;
                }
                int itemType = this.UtilityMember.NumberSet.ToInteger(itemTypeObj.ToString());
                int itemId = this.UtilityMember.NumberSet.ToInteger(itemIdObj.ToString());
                if (itemType == 1)
                {
                    assetItemId = itemId;
                }
        ...
        private bool AllowZeroValuedCashBankLedger
        {
            get
            {
                bool allowzerovaluedledger = (this.AppSetting.AllowZeroValuedCashBankVoucherEntry &&
                                    (rgVoucherType.SelectedIndex == 0 || rgVoucherType.SelectedIndex == 1));
        ...
        private void GenerationList()
        {
            ...
            if (gvPurchase.FocusedColumn == colQuantity || gvPurchase.FocusedColumn == colViewDetails || gvPurchase.FocusedColumn == colAmount)
            {
                ...
                bool isCRMode = gvPurchase.GetFocusedRowCellValue(colSource) != null && Convert.ToInt32(gvPurchase.GetFocusedRowCellValue(colSource)) == (int)Source.To;
                double CrAmount = gvPurchase.GetFocusedRowCellValue(colAmount) != null ? this.UtilityMember.NumberSet.ToDouble(gvPurchase.GetFocusedRowCellValue(colAmount).ToString()) : 0;
                ...
                if (!string.IsNullOrEmpty(gvPurchase.GetFocusedRowCellValue(colAssItemID).ToString()) &&
                    ...
                    || !string.IsNullOrEmpty(gvPurchase.GetFocusedRowCellValue(colAmount).ToString()))
                {
                    gvPurchase.CloseEditor();   //that point is Start for Quentity added and Amount validated Part 
                    ...
                    if (dtFilterdAsset != null && dtFilterdAsset.Rows.Count > 0)
                    {
                        ...
                        if (isCRMode && CrAmount > 0)
                        {
                            gvPurchase.UpdateTotalSummary();
                            CalculateFirstRowValue();
                            gvPurchase.FocusedColumn = gvPurchase.Columns["ASSET_NAME"];
                            return;
                        }
                        if (Quantity > 0)
                        {
                            ...
                            if (AssetItemList.Dialogresult == DialogResult.OK)
                            {
                                ...
                                if (amount > 0)
                                {
                                    ...
                                    gvPurchase.UpdateTotalSummary();
                                    CalculateFirstRowValue();   //Calculate all Values for Grid 
                                    gvPurchase.FocusedColumn = gvPurchase.Columns["0"];//sudhakar["0"];
                                }
                            }
                            else
                            {
        ...
        void RealColumnEditPurchaseAmount_EditValueChanged(object sender, System.EventArgs e)
        {
            ...
            //    string Balance = GetLedgerBalanceValues(dtTrans, ItemId);
            //    if(Balance != string.Empty)
            //    {
            //        gvPurchase.SetRowCellValue(gvPurchase.FocusedRowHandle, colAmount, Balance);
            //    }
            //}
            ...
            if (ValidatePurchaseDetials())
            {
                try
                {
                    using (AssetInwardOutwardSystem inwardvouchersystem = new AssetInwardOutwardSystem())
                    {
                        ...
                        if (!dtSource.Columns.Contains("CR_DETAIL_ID")) //Added By Dinesh 30-12-2025
                        {
                            dtSource.Columns.Add("CR_DETAIL_ID", typeof(int));
                        }
                        dtSource.AcceptChanges();
                        dtFilteredRows = dtSource.Rows.Cast<DataRow>().Where(row => !row.ItemArray.All(field => field is System.DBNull)).CopyToDataTable();
                        double debitAmount = GetSafeSum(dtSource, "SOURCE = 2");//Added By Dinesh 30-12-2025
                        double creditAmount = GetSafeSum(dtSource, "SOURCE = 1");//Added By Dinesh 30-12-2025
                        //inwardvouchersystem.TotalAmount = UtilityMember.NumberSet.ToDouble(dtSource.Compute("SUM(AMOUNT)", string.Empty).ToString());
                        inwardvouchersystem.TotalAmount = debitAmount - creditAmount; //
                        ...
                        inwardvouchersystem.dtAssetVoucher = ConstructGeneralLedgerSource(dtFilteredRows, ucAssetJournal1.DtCashBank); //Added By Dinesh 15-04-2026
        ...
        private double GetSafeSum(DataTable dt, string filter)  //Added By Dinesh 30-12-2025
        {
            object val = dt.Compute("SUM(AMOUNT)", filter);
        ...
        private DataTable ConstructGeneralLedgerSource(DataTable dtFilteredRows, DataTable dtCashBank)
        {
            ...
            bool hasCashBankLedger = dtBankTrans.AsEnumerable().Any(row => row["GROUP_ID"] != DBNull.Value && new int[] { 12, 13 }.Contains(Convert.ToInt32(row["GROUP_ID"])));
            if (Flag == AssetInOut.PU && hasCashBankLedger)
            {
            ...
            else
            {
                ...
                if (!dtLedgers.Columns.Contains("GROUP_ID"))
                    dtLedgers.Columns.Add("GROUP_ID", typeof(int));
                ...
                if (!dtBankTrans.Columns.Contains("GROUP_ID"))
                    dtBankTrans.Columns.Add("GROUP_ID", typeof(int));
                ...
                dtLedgers.AcceptChanges();
                foreach (DataRow sourceRow in dtBankTrans.Rows)
                {
                    DataRow newRow = dtLedgers.NewRow();
                    foreach (DataColumn col in dtBankTrans.Columns)
                    {
                        if (dtLedgers.Columns.Contains(col.ColumnName))
                            newRow[col.ColumnName] = sourceRow[col.ColumnName];
                    }
                    dtLedgers.Rows.Add(newRow);
                }
                dtLedgers.AcceptChanges();
            }
            return dtLedgers;
        ...
        private void frmInwardVoucherAdd_Load(object sender, EventArgs e)
        {
            ...
            // 19/12/2025, Chinna.
            // Set summary type to Custom to enable CR-DR calculation in grid footer
            colAmount.SummaryItem.SummaryType = DevExpress.Data.SummaryItemType.Custom;
        ...
        private bool ValidatePurchaseDetials()
        {
            ...
            { //On 04/09/2024, If multi currency enabled, all the currency details must be filled
                ...
                // Implementation
                MessageRender.ShowMessage(this.GetMessage(MessageCatalog.Common.GENERAL_MULTI_CURRENCY_OPTION_IS_FILLED));
                glkpCurrencyCountry.Select();
                glkpCurrencyCountry.Focus();
                isPurchaseTrue = false;
            }
        ...
        //            resultArgs = assetItemSystem.FetchAssetItemDetailsLocation();
        //            dtAssetItem = resultArgs.DataSource.Table;
        //            if (dtAssetItem != null && dtAssetItem.Rows.Count > 0)
        //            {
        //                dtAssetOnly = dtAssetItem.Select("ITEM_TYPE = 1").CopyToDataTable();
        //                dtLedgerOnly = dtAssetItem.Select("ITEM_TYPE = 2").CopyToDataTable();
        ...
        private void CalculateFirstRowValue()
        {
            try
            {
                ...
                {
                    double Amount = ucAssetJournal1.BankAmount;//asset.le BankTransSummaryVal
                    if (Amount >= 0)
                    {
                        double dAmount = 0.0;
                        if (BankTransSummaryVal <= AmountSummaryVal)
        ...
        private string GetLedgerBalanceValues(DataTable dtTrans, int LedgerId)
        {
            ...
            try
            {
                if (dtTrans != null)
                {
                    ...
                    //balanceamount = GetCurBalance(LedgerId,NewValue);
                }
            }
            catch (Exception ex)
            {
        ...
        //private string GetCurBalance(int ItemId,double NewValue)
        //{
        ...
        //    //BalanceProperty Balance = FetchCurrentBalance(ItemId);
        ...
        //    //dCalculateCurBal = CurBal - NewValue;
            ...
            try
            {
                ...
                if (InwardId > 0)
                {
                    ...
                    using (AssetInwardOutwardSystem InwardOutwardSystem = new AssetInwardOutwardSystem())
                    {
                        ...
                        resultArgs = InwardOutwardSystem.FetchAssetInOutDetailById();  //Dinesh  Fetch all Asset Purches details 
                        if (resultArgs != null && resultArgs.Success)
                        {
                            ...
                            if (!resultArgs.DataSource.Table.Columns.Contains("SOURCE")) //Added by Dinesh 30-12-2025
                            {
        ...
        private DataTable BindSource(DataTable dt)
        {
            ...
            {
                if (dr["SOURCE"] == DBNull.Value || string.IsNullOrEmpty(dr["SOURCE"].ToString()))
                {
                    int sourceId = (Flag == AssetInOut.PU || Flag == AssetInOut.IK) ? (int)Source.By : (int)Source.To;
                    dr["SOURCE"] = sourceId;
                }
        ...
        private void DeleteRowTransaction()
        {
            ...
            if (Quantity > 0) //Modifiyed By Dinesh 06-01-2026
            {
            ...
            {
                int rowNum = gvPurchase.FocusedRowHandle;
                int inOutId = Convert.ToInt32(gvPurchase.GetRowCellValue(rowNum, "IN_OUT_ID"));
                int ledgerId = Convert.ToInt32(gvPurchase.GetRowCellValue(rowNum, "LEDGER_ID"));

                AssetInwardOutwardSystem inOutSystem = new AssetInwardOutwardSystem();
                ResultArgs result = inOutSystem.DeleteCrEntries(inOutId, ledgerId);
            }
            gvPurchase.DeleteRow(gvPurchase.FocusedRowHandle);
            gvPurchase.UpdateCurrentRow();
            gcPurchase.RefreshDataSource();
        ...
        private void gvPurchase_ShowingEditor(object sender, CancelEventArgs e)
        {
            ...
            // Dinesh
            //if (gvPurchase.IsFirstRow && gvPurchase.FocusedColumn == colSource)
            //{
            //    e.Cancel = true;
            //}
            //bool isCRMode = gvPurchase.GetFocusedRowCellValue(colSource) != null && Convert.ToInt32(gvPurchase.GetFocusedRowCellValue(colSource)) == (int)Source.To;
            //if (gvPurchase.FocusedColumn == colAmount)
            //{
            //    if (!isCRMode)
            //    {
            //        e.Cancel = true;
            //    }
            //}
            //if (gvPurchase.FocusedColumn == colQuantity && isCRMode)
            //{
            //    e.Cancel = true;
            //}
        }
        ...
        private void rglkpAssetName_Popup(object sender, EventArgs e)
        {
            //GridLookUpEdit editor = sender as GridLookUpEdit;
            //int rowHandle = gvPurchase.FocusedRowHandle;

            //object transType = gvPurchase.GetRowCellValue(rowHandle, "SOURCE");
            //string type = (transType == null) ? "" : transType.ToString().Trim();
            //ApplyAssetLedgerFilter(editor, type);
        }

        private void ApplyAssetLedgerFilter(GridLookUpEdit editor, string transMode)
        {
            //try
            //{
            //    if (editor == null)
            ...
            //    if (transMode == "2")
            //    {
            //        popupView.ActiveFilterString = "ITEM_TYPE = 1";
            //    }
            //    else if (transMode == "1")
            //    {
            //        popupView.ActiveFilterString = "ITEM_TYPE = 2";
            //    }
            //    else
            //    {
            //        editor.ClosePopup();
        ...
        private void gvPurchase_CellValueChanged(object sender, CellValueChangedEventArgs e)
        {
        ...
        private void gcPurchase_KeyDown(object sender, KeyEventArgs e)
        {

        }

        private void rglkpSource_KeyDown(object sender, KeyEventArgs e)
        {

        }

        private void gvPurchase_CustomSummaryCalculate(object sender, DevExpress.Data.CustomSummaryEventArgs e)
        {
        ...
        private double GetTransSummaryAmount()
        {
            ...
            if (dtTrans != null && dtTrans.Rows.Count > 0)
            {
                ...
                // For Asset Purchase/In-Kind: CR - DR (similar to Receipt/Contra)
                dAmount = DrAmt - CrAmt;
            }
            return dAmount;
        }
        ...
        {
            bool isValid = true;
            if (rgVoucherType.SelectedIndex != 2)
            {
                DataTable dtTrans = gcPurchase.DataSource as DataTable;
                DataView dv = new DataView(dtTrans);
                dv.RowFilter = "(ITEM_ID>0 OR AMOUNT>0)";
                if (dv.Count > 0)
                {
                    double dAmt = GetTransSummaryAmount();

                    if (dAmt <= 0 || dtTrans.Rows.Count <= 1)
                    {
                        if (rgVoucherType.SelectedIndex == 0)
                        {
        ...
        private bool IsValidaTransactionRow()
        {
            ...
            try
            {
                IsLedgerValid = (LedgerId > 0 || ItemId > 0 && LedgerAmount > 0);
            }
            catch (Exception ex)
            {
                MessageRender.ShowMessage(ex.ToString(), true);
        ...
        private void ApplyModeAndFocus(bool isCRMode)
        {
            gvPurchase.PostEditor();
            gvPurchase.UpdateCurrentRow();
            ...
            if (isCRMode)
            {
                ...
                colAmount.OptionsColumn.ReadOnly = false;

                gvPurchase.FocusedColumn = colAmount;
                gvPurchase.ShowEditor();
            }
            else
            {
        ...
        private void rglkpAssetName_Leave(object sender, EventArgs e)
        {
            ...
            //            string balanceText = GetLedgerBalanceValues(dtTrans, ItemId);
            ...
            //                if (parts.Length >= 1) 
            //                {
            //                    double amount = UtilityMember.NumberSet.ToDouble(parts[0]);

            //                    gvPurchase.SetRowCellValue(gvPurchase.FocusedRowHandle, colAmount, amount);
            //                }
            //            }
            //            CalculateFirstRowValue();
            //        }
