...
namespace ACPP.Modules.Asset.Transactions
{
    public partial class frmAssetOutward : frmFinanceBaseAdd
    {
            ...
            try
            {
                if (ValidSalesDetails())
                {
                    using (AssetInwardOutwardSystem assetInwardOutwardSystem = new AssetInwardOutwardSystem())
                    {
                        ...
                        if (this.AppSetting.AllowMultiCurrency == 1)
                        {
                            assetInwardOutwardSystem.assetCashBankEntry = (isMultiCashBankEntry && rgTransactionType.SelectedIndex == 1) ? 1 : 0;
                        }
                        else
                        {
                            assetInwardOutwardSystem.assetCashBankEntry = 0;
                        }
        ...
        private DataTable ConstructGeneralLedgerSource(DataTable dtFilteredRows, DataTable dtCashBank)
        {
            ...
            if (rgTransactionType.SelectedIndex == 0 || rgTransactionType.SelectedIndex == 1 && hasCashBankLedger)
            {
                DataView dv = dtBankTrans.AsDataView();
                //dv.RowFilter = "GROUP_ID NOT IN (12,13)"; //Commend by Dinesh 
                dv.RowFilter = "GROUP_ID NOT IN (12,13) AND SOURCE <> 2";  //added Dinesh Purpose of Remove SOURCE ID 2
        ...
        {
            // ShowAssetListForm();
            if (e.Column.FieldName == "AMOUNT")  //Dinesh 
            {
        ...
        private bool IsValidCashBankGrid()
        {
            ...
            if (dv.Count > 0)
            {
                foreach (DataRowView drSales in dv)
                {
                    ...
                    ledid = this.UtilityMember.NumberSet.ToInteger(drSales["LEDGER_ID"].ToString());
                    GroupId = GetLedgerGroupId(ledid);
        ...
        private void SetBackColorByAssetVoucherType()
        {
            if (rgTransactionType.SelectedIndex == 0)
            {
                ...
                ResourceManager rm = new ResourceManager("ACPP.Modules.Asset.Transactions.frmAssetOutward", typeof(frmAssetOutward).Assembly);
                string amountCaption = rm.GetString("AMOUNT_CAPTION", CultureInfo.CurrentUICulture) ?? "Amount";
                gvInOutAdd.Columns[colAmount.FieldName].Caption = amountCaption;
            }
        ...
        private void gvInOutAdd_RowUpdated(object sender, DevExpress.XtraGrid.Views.Base.RowObjectEventArgs e)
        {
            this.BeginInvoke(new Action(() =>  //Dinesh
            {
                gvInOutAdd.PostEditor();
                gvInOutAdd.UpdateCurrentRow();
