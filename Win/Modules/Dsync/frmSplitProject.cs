                ...
                if (isBulkImport)
                {
                    ...
                    if (!string.IsNullOrEmpty(FYSplitFolderPath))
                    {
                        ...
                        if (files.Length > 0)
                        {
                            ...
                            // Implementation
                            if(this.ShowConfirmationMessage(this.GetMessage(MessageCatalog.Common.GENERAL_FEW_FILLES_ARE_ALREDY_AVAILABLE_IN_PATH),
                                MessageBoxButtons.YesNo, MessageBoxIcon.Question) == System.Windows.Forms.DialogResult.Yes)
        ...
        private ResultArgs ExportXML(DataSet dsTransaction)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    // Implementation
                    resultArgs.Message = this.GetMessage(MessageCatalog.Common.GENERAL_NO_DATA_AVAILABLE);
                }
            }
            catch (Exception ex)
            {
