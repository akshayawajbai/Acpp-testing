...
namespace ACPP.Modules.UIControls
{
    public partial class UcAccountMapping : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        private bool bsgesp
        {
            get
            {
                return Bosco.Utility.ConfigSetting.SettingProperty.Current.IS_BSGRMG;
            }
        }
        #endregion
        ...
        private void BindGridView(DataTable dtProject)
        {
            ...
            // ✅ Ensure COUNTRY_ID column exists (VERY IMPORTANT)
            if (!dtProject.Columns.Contains("COUNTRY_ID"))
                dtProject.Columns.Add("COUNTRY_ID", typeof(int));

            // ✅ Assign SELECT values safely
            foreach (DataRow dr in dtProject.Rows)
            {
                dr[SELECT_COL] = dr["SELECT_TEMP"];

                // 🔥 Prevent NULL issue (important for lookup)
                if (dr["COUNTRY_ID"] == DBNull.Value)
                    dr["COUNTRY_ID"] = 0;
            }

            DataView dvOrderBy = new DataView(dtProject);
            dvOrderBy.Sort = SELECT_COL + " DESC";
            ...
            if (ProjectId != 0)
            {
                ...
                {
                    if (row["PROJECT_ID"].ToString() == ProjectId.ToString())
                    {
                        row[SELECT_COL] = 1;
                    }
                }
            ...
            DataTable dtFinal = dvOrderBy.ToTable();


            if (!dtFinal.Columns.Contains("COUNTRY_ID"))
                dtFinal.Columns.Add("COUNTRY_ID", typeof(int));

            gcMapProject.DataSource = GetMappingDetails = dtFinal;
            dtAllSelectedProjects = GetMappingDetails;


            dvOrderBy.RowFilter = SELECT_COL + "=0";

            if (dvOrderBy.ToTable().Rows.Count == 0)
                CheckAllVisible = true;
        ...
        //                rglkpCurrencylist.DataSource = dtCurrencies;
        //                //rglkpCurrencylist.DisplayMember = countrySystem.AppSchema.Country.CURRENCYColumn.ToString();
        //                //rglkpCurrencylist.ValueMember = countrySystem.AppSchema.Country.COUNTRY_IDColumn.ToString();
        //                rglkpCurrencylist.DisplayMember = "CURRENCY";
        //                rglkpCurrencylist.ValueMember = "COUNTRY_ID";
        //            }
        //        }
        //    }
        //    catch (Exception)
            ...
            try
            {
                using (CountrySystem countrySystem = new CountrySystem())
                {
                    ...
                    if (resultArgs.Success)
                    {
                        ...
                        dtCurrencies.DefaultView.RowFilter =
                            countrySystem.AppSchema.Country.EXCHANGE_RATEColumn.ColumnName + " > 0";
                        ...
                        rglkpCurrencylist.PopulateViewColumns();

                        foreach (DevExpress.XtraGrid.Columns.GridColumn col in rglkpCurrencylist.View.Columns)
                        {
                            col.Visible = false;
                        }

                        rglkpCurrencylist.View.Columns["CURRENCY"].Visible = true;


                        rglkpCurrencylist.View.BestFitColumns();
                    }
                }
            }
            catch (Exception)
