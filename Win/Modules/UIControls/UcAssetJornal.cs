...
namespace ACPP.Modules.UIControls
{
    public partial class UcAssetJournal : UserControl
    {
        ...
        //27-03-2025 Validate for Cas/Bank Data 
        private void MoveToNarration(KeyEventArgs e)
        {
            gvBank.CloseEditor();
            e.Handled = true;
        ...
        private void gcBank_ProcessGridKey(object sender, KeyEventArgs e)
        {
            try
            {
                ...
                if (!(e.KeyCode == Keys.Enter || e.KeyCode == Keys.Tab) || e.Shift || e.Alt)
                    return;

                bool isBank = CashBankGroupId == (int)FixedLedgerGroup.BankAccounts;
                bool isDS = Flag == AssetInOut.DS;
                bool isLastRow = gvBank.IsLastRow;

                object ledgerObj = gvBank.GetFocusedRowCellValue(colCashLedger);
                object amtObj = gvBank.GetFocusedRowCellValue(colCashAmount);

                decimal amount = (amtObj == null || amtObj == DBNull.Value) ? 0 : Convert.ToDecimal(amtObj);

                bool isAmountMatched = BankTransSummaryVal == PurchaseTransSummary;

                bool isEmptyRow =
                    (ledgerObj == null || ledgerObj == DBNull.Value || string.IsNullOrWhiteSpace(ledgerObj.ToString())) &&
                    (amtObj == null || amtObj == DBNull.Value || amount == 0);

                gvBank.PostEditor();
                gvBank.CloseEditor();
                gvBank.UpdateCurrentRow();
                //  NEW FIX: EMPTY NEW ROW → GO NARRATION
                if (isEmptyRow && isLastRow)
                {
                    MoveToNarration(e);
                    e.Handled = true;
                    e.SuppressKeyPress = true;
                ...
                if (isBank)
                {
                    var col = gvBank.FocusedColumn;

                    if (col == colCashLedger)
                    {
                        gvBank.FocusedColumn = colCashAmount;
                        gvBank.ShowEditor();
                    }
                    else if (col == colCashAmount)
                    {
                        gvBank.FocusedColumn = colCashCheque;
                        gvBank.ShowEditor();
                    }
                    else if (col == colCashCheque)
                    {
                        gvBank.FocusedColumn = colMaterializedOn;
                        gvBank.ShowEditor();
                    }
                    else if (col == colMaterializedOn)
                    {
                        if (!isLastRow)
                        {
                            gvBank.MoveNext();
                ...
                // CASH / NORMAL FLOW
                if (gvBank.FocusedColumn == colCashLedger)
                {
                    gvBank.FocusedColumn = colCashAmount;
                    gvBank.ShowEditor();
                ...
                if (gvBank.FocusedColumn == colCashAmount)
                {
                    if (!isLastRow)
                    {
                        gvBank.MoveNext();
                ...
                // SHIFT + TAB BACK
                if (gvBank.IsFirstRow && gvBank.FocusedColumn == colCashLedger && e.Shift && e.KeyCode == Keys.Tab)
                {
        ...
        private void rglkpCashLedger_Validating(object sender, CancelEventArgs e)
        {
            ...
            if (gridLKPEdit.EditValue != null)
            {
                ...
                gvBank.SetFocusedRowCellValue(colLedgerGroupId, CashBankGroupId);
                //Validate for Asset Amount and Cash Grid Amount  24-03-2026 

                if (PurchaseTransSummary > 0 && BankTransSummaryVal <= PurchaseTransSummary && LedgerAmount < 1)
                {
                    double Amt = Math.Abs(BankTransSummaryVal - PurchaseTransSummary);
        ...
        private void gvBank_RowCellStyle(object sender, DevExpress.XtraGrid.Views.Grid.RowCellStyleEventArgs e)
        {
            try
            {
                ...
                rtxtCashAmount.ReadOnly = false; //true / false - Modified By 20/11/2025  Enable Cash and Bank split in the Purchase and Sales Screen
                colCashAmount.OptionsColumn.AllowEdit = true;
            }
            catch (Exception ex)
            {
                MessageRender.ShowMessage(ex.ToString());
            ...
            try
            {
                if (this.Flag != AssetInOut.OP)
                {
                    using (LedgerSystem ledgerSystem = new LedgerSystem())
                    {
                        ...
                        //{
                        resultArgs = ledgerSystem.FetchCashBankSundryLedger(); //FetchCashBankLedger ->this is old One 09-04-2026 Dinesh #New FetchCashBankSundryLedger();
                        ...
                        if (this.Settingproperty.AllowMultiCurrency == 1)
                        {
                            ...
                            if (this.Flag == AssetInOut.PU)
                            {
                                //Int32 currencycountry = CurrencyCountryId;
                                //dtLedgers.DefaultView.RowFilter = ledgerSystem.AppSchema.Ledger.CUR_COUNTRY_IDColumn.ColumnName + " = " + currencycountry;
                                Int32 currencycountry = CurrencyCountryId;
                                string curColumn = ledgerSystem.AppSchema.Ledger.CUR_COUNTRY_IDColumn.ColumnName;
                                string grpColumn = ledgerSystem.AppSchema.Ledger.GROUP_IDColumn.ColumnName;
                                dtLedgers.DefaultView.RowFilter = "((" + grpColumn + " IN (12,13) AND " + curColumn + " = " + currencycountry + ") " +
                                                                   "OR (" + grpColumn + " NOT IN (12,13)))";
                            }
                            else
                            {
                                Int32 currencycountry = CurrencyCountryId;
                        ...
                        else if (this.Flag == AssetInOut.DS)
                        {
                            ...
                            DataTable dtDisposal = LoadDisposalLedgers();
                            DataTable dtCashBank = null;

                            //  Only fetch & merge CashBank when MultiCurrency is enabled
                            if (this.Settingproperty.AllowMultiCurrency == 1)
                            {
                                resultArgs = ledgerSystem.FetchCashBankLedger();
                                dtCashBank = (resultArgs != null && resultArgs.Success) ? resultArgs.DataSource.Table : null;

                                // Apply currency filter
                                if (dtCashBank != null)
                                {
                                    Int32 currencycountry = CurrencyCountryId;
                                    dtCashBank.DefaultView.RowFilter =
                                        ledgerSystem.AppSchema.Ledger.CUR_COUNTRY_IDColumn.ColumnName + " = " + currencycountry;

                                    dtCashBank = dtCashBank.DefaultView.ToTable();
                                }

                                // Merge ONLY in MultiCurrency
                                if (dtDisposal != null && dtCashBank != null)
                                {
                                    dtDisposal.Merge(dtCashBank);
                                    dtLedgers = dtDisposal;
                                }
                                else if (dtDisposal != null)
                                {
                                    dtLedgers = dtDisposal;
                                }
                                else
                                {
                                    dtLedgers = dtCashBank;
                                }
                            }
                            else
                            {
                                //Normal Currency → NO merge
                                dtLedgers = dtDisposal;
                            }
                        }
            ...
            try
            {
                ...
                else if (gvBank.RowCount == 1)
                {
                    if (LedgerAmount > 0 || CashLedgerId > 0)
                    {
                        if (XtraMessageBox.Show("Delete this entry?", "Acme.erp", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                        {
                            ConstructCashTransEmptySournce();  //DS Flag Added By Dinesh 27-03-2026
                            int sourceId = (Flag == AssetInOut.PU || Flag == AssetInOut.DS || Flag == AssetInOut.IK) ? (int)Source.To : (int)Source.By;
                            gvBank.SetRowCellValue(gvBank.FocusedRowHandle, colCashSource, sourceId);
                            gvBank.FocusedColumn = colCashLedger;
                        }
                    }
                    ...
                    using (VoucherTransactionSystem voucherSystem = new VoucherTransactionSystem())
                    {
                        ...
                        if (resultArgs != null && resultArgs.Success)
                        {
                            ...
                            else
                            {
                                ...
                                else if (Flag == AssetInOut.PU)  //Added  By Dinesh 31-12-2025
                                {
                                    DataView dvBank = new DataView(resultArgs.DataSource.Table);
                                    dvBank.RowFilter = "GROUP_ID IN (12,13,26)";
                                    gcBank.DataSource = dvBank.ToTable();
                                    gvBank.ExpandAllGroups();
        ...
        private void gcBank_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter || e.KeyCode == Keys.Tab)
            {
                if (gvBank.FocusedColumn == colCashAmount)
                {
                    // Editor value commit pannum
                    gvBank.PostEditor();

                    // Validation fail na stop
                    if (!gvBank.IsEditorFocused)
                    {
                        e.Handled = true;
                        return;
                    }
        ...
        private void gcBank_Validated(object sender, EventArgs e)
        {

        }

        private void gvBank_RowCellClick(object sender, RowCellClickEventArgs e)
        {
            if (e.Column == colDeleteCashBank)
            {
                DeleteTransaction();
            }
        }
