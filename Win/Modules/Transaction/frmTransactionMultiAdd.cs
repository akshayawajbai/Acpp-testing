...
namespace ACPP.Modules.Transaction
{
    public partial class frmTransactionMultiAdd : frmFinanceBaseAdd
    {
        ...
        // CHANGE (17/04/2026): Hold voucher’s booking GST invoice id until lookup is visible; core getter still requires Always.
        private Int32 _bookingGstInvoiceIdPendingFromVoucher;
        ...
        /// <summary>
        /// Single entry mode applies only from 01/04/2026; earlier voucher dates use double-entry behaviour.
        /// </summary>
        private bool IsEnableSingleModeApplicable
        {
            get
            {
                return this.AppSetting.EnableTransMode == "1"
                    && this.AppSetting.EnableSingleMode == "1"
                    && dtTransactionDate.DateTime >= UtilityMember.DateSet.ToDate(SettingProperty.Enforce_Single_Mode_FY, false);
            }
        }
        ...
        public int CurrencyId { get; set; } //added By Dinesh 16-04-2026
        ...
        private VoucherEntryMethod TransacationGridNewItem
        {
            set
            {
                if (value == VoucherEntryMethod.Multi)
                {
                    ...
                    // Default: Receipt/Contra = CR (To), Payment = DR (By)
                    int sourceId = (rgTransactionType.SelectedIndex == 0 || rgTransactionType.SelectedIndex == 2) ? (int)Source.To : (int)Source.By;

                    // EnableSingleMode: Receipt row2+ = DR, Payment row2+ = CR (first row stays locked CR/DR)
                    //if (this.AppSetting.EnableTransMode == "1" && this.AppSetting.EnableSingleMode == "1")
                    if (IsEnableSingleModeApplicable)
                    {
                        if (rgTransactionType.SelectedIndex == 0)
                            sourceId = (int)Source.By;   // Receipt → DR
                        else if (rgTransactionType.SelectedIndex == 1)
                            sourceId = (int)Source.To;   // Payment → CR
                    }

                    gvTransaction.SetRowCellValue(gvTransaction.FocusedRowHandle, colSource, sourceId);

                    // Source locked in single mode for Receipt/Payment → focus Ledger
                    //if (this.AppSetting.EnableTransMode == "1" && this.AppSetting.EnableSingleMode == "1"
                    if (IsEnableSingleModeApplicable
        ...
        private bool EnableLedgerPan = false;
        private bool EnableLedgerGST = false;
            ...
            set
            {
                ...
                using (VoucherSystem vouchersystem = new VoucherSystem())
                {
                    ...
                    if (result.Success && result.DataSource.Table != null && result.DataSource.Table.Rows.Count > 0)
                    {
                        ...
                        //bharath 04/06/2026 Ledger PAN and GST set
                        int PanEnabled = vouchersystem.NumberSet.ToInteger(dtVoucherTypeDetail.Rows[0][vouchersystem.AppSchema.Voucher.IS_PAN_ENABLEDColumn.ColumnName].ToString());
                        int GSTEnabled = vouchersystem.NumberSet.ToInteger(dtVoucherTypeDetail.Rows[0][vouchersystem.AppSchema.Voucher.IS_GST_ENABLEDColumn.ColumnName].ToString());
                        EnableLedgerPan = PanEnabled == 0 ? false : true;
                        EnableLedgerGST = GSTEnabled == 0 ? false : true;
        ...
        // On 20/01/2026, Chinna - Updated: IsGeneralInvolice should be true when any currency is selected (not just local)
        // Currency filtering happens in C# code after fetching data, not here
            ...
            get
            {
                ...
                if (rtn)
                {
                    if (this.AppSetting.AllowMultiCurrency == 1)
                        // Return true if any currency is selected (VoucherCurrencyCountryId > 0)
                        // Currency filtering will be done in C# code after fetching data
                        rtn = (VoucherCurrencyCountryId > 0);
        ...
        {
            // I used to lock the inmpdo means i locked 24-25 accounting year
            // 06/01/2026

            // Check if INMPDO and specific accounting year period exists - lock transaction add screen
            //if (this.AppSetting.IS_SDB_PDO)
            //{
            //    using (AccouingPeriodSystem accountingSystem = new AccouingPeriodSystem())
            //    {
            //        // Check if "2024-04-01 00:00:00" to "2025-03-31 00:00:00" exists in accounting_year table
            //        if (accountingSystem.FetchAccountingYearByDate() > 0)
            //        {
            //            this.ShowMessageBoxError("Transaction entry is locked.");
            //            this.Close();
            //            return;
            //        }
            //    }
            ...
            LoadTransBasic();
            //bharath
            ResourceManager rm = new ResourceManager("ACPP.Modules.Transaction.frmTransactionMultiAdd", typeof(frmTransactionMultiAdd).Assembly);

            string receiptLabel = rm.GetString("rgTransactionType.Properties.Items1");
            rgTransactionType.Properties.Items[0].Description = receiptLabel;
            rgTransactionType.Refresh();
            if (VoucherId != 0)
            {
        ...
        private void enforceCurrencyDetails(bool visible)
        {
            ...
            //lcLiveExchangeRate.Visibility = LayoutVisibility.Never;
            lciLiveExchangeRate.Visibility = (this.AppSetting.AllowMultiCurrency == 1 || visible ? LayoutVisibility.Always : LayoutVisibility.Never);
                ...
                if (IsValidEntry())
                {
                    ...
                    using (VoucherTransactionSystem voucherTransaction = new VoucherTransactionSystem())
                    {
                        ...
                        if (dtVoucherImages != null && dtVoucherImages.Rows.Count > 0)
                        {
                            ...
                            this.ShowWaitDialog(this.GetMessage(MessageCatalog.Common.GENERAL_ATTACHING_VOUSHER_FILE));
                        }
                        ...
                        if (resultArgs.Success)
                        {
                            ...
                            {
                                 //Bharath 06/07/2026, For Payemnt voucher, skip voucher print when both voucher print and cheque printing are enabled
                                if (!(rgTransactionType.SelectedIndex == 1 && this.AppSetting.EnableChequePrinting == "1"))
                                {
                            ...
                            //bharath 20/07/2026 hide cheque print for contra
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(glkpDonor.Text))
                {
                    ...
                    if (dr != null)
                    {
                        ...
                        // bharath 08/06/2026
                        //Populate PAN number from selected donor details
                        txtPanNo.Text = dr["PAN"].ToString();
            ...
            try
            {
                ...
                if (row != null)
                {
                    ...
                    if (this.AppSetting.AllowMultiCurrency == 1)
                    {
                        ...
                        txtLiveExchangeRate.Text = txtExchangeRate.Text;
        ...
        {
            //bharath 07/04/2026
            txtLiveExchangeRate.Properties.Mask.UseMaskAsDisplayFormat = false;
        ...
        private void txtLiveExchangeRate_EditValueChanged(object sender, EventArgs e)
        {
            if (this.UtilityMember.NumberSet.ToDouble(txtLiveExchangeRate.Text) >= 0)
            {
                if (this.AppSetting.AllowMultiCurrency == 1)
                {
                    if (gvTransaction.FocusedRowHandle >= 0)
                        gvTransaction.SetRowCellValue(gvTransaction.FocusedRowHandle, colLiveExchangeAmount, UtilityMember.NumberSet.ToDecimal(txtLiveExchangeRate.Text));
                    if (gvBank.FocusedRowHandle >= 0)
                        gvBank.SetRowCellValue(gvBank.FocusedRowHandle, colCashBankLiveExchangeAmount, UtilityMember.NumberSet.ToDecimal(txtLiveExchangeRate.Text));
                    ApplyVoucherCurrencyToLedgers();
                }
            }
            else
        ...
        private void txtExchangeRate_Validating(object sender, CancelEventArgs e)
        {
            ...
            //bharath 07/04/2026
            txtExchangeRate.Properties.Mask.UseMaskAsDisplayFormat = false;
        }
        ...
        private void rglkpLedger_Leave(object sender, EventArgs e)
        {
            if (LedgerId > 0)
            {
                ...
                if (this.AppSetting.AllowMultiCurrency == 1)
                {
                    ...
                    // 29/01/2025, in order to subscribe live exchange rate
                    gvTransaction.SetRowCellValue(gvTransaction.FocusedRowHandle, colLiveExchangeAmount, txtLiveExchangeRate.Text);
                    gvTransaction.UpdateCurrentRow();
                }
                ...
                if (dAmt.Amount > 0)
                {
                        ...
                        (Balance.TransMode == TransactionMode.DR.ToString() && rgTransactionType.SelectedIndex == 1))
                    {
                        if (dAmt.Amount < (Balance.Amount))
                        {
                            ...
                            this.ShowMessageBox(this.GetMessage(MessageCatalog.Common.GENERAL_THE_TRANSACTION_APPROVED));
                        }
        ...
        private void gcTransaction_ProcessGridKey(object sender, KeyEventArgs e)
        {
            ...
            if ((e.KeyCode == Keys.Enter || e.KeyCode == Keys.Return || e.KeyCode == Keys.Tab) && !e.Shift && !e.Alt && !e.Control &&
                (gvTransaction.FocusedColumn == colAmount || gvTransaction.FocusedColumn == colValueDate || gvTransaction.FocusedColumn == colCheque ||
                 gvTransaction.FocusedColumn == colReferenceNumber || gvTransaction.FocusedColumn == colLedgerNarration || gvTransaction.FocusedColumn == colLedgerPan || gvTransaction.FocusedColumn == colLedgerGST||gvTransaction.FocusedColumn == colGSTLedgerClass ||
                 (AppSetting.IS_DIOMYS_DIOCESE && AppSetting.EnableSubLedgerVouchers == "1" && gvTransaction.FocusedColumn == colLedger)))
            {
                ...
                if (TransEntryMethod == VoucherEntryMethod.Single)
                {
                    if (rgTransactionType.SelectedIndex != 2)
                    {
                        ...
                        if (gvTransaction.FocusedColumn == colAmount && (!colLedgerNarration.Visible && !colLedgerPan.Visible && !colLedgerGST.Visible && !colGSTLedgerClass.Visible))
                        {
                            canFoucsCashTrnasaction = true;
                        }
                        else if (gvTransaction.FocusedColumn == colLedgerNarration
                             || gvTransaction.FocusedColumn == colLedgerPan //bharath 04/06/2026 Ledger PAN and GST set Focus
                             || gvTransaction.FocusedColumn == colLedgerGST)
                        {
                            canFoucsCashTrnasaction = true;
                        }
                ...
                if ((LedgerId > 0 && LedgerAmount > 0)
                    ...
                    && (!colLedgerNarration.Visible || gvTransaction.FocusedColumn == colLedgerNarration || gvTransaction.FocusedColumn == colLedgerPan || gvTransaction.FocusedColumn == colLedgerGST)
        ...
        private void gvTransaction_ShowingEditor(object sender, CancelEventArgs e)
        {
            try
            {
                ...
                //bharath 23/07/2026 for single entry option
                // EnableSingleMode: lock CR/DR on all Receipt/Payment rows (same as first row)
                if (gvTransaction.FocusedColumn == colSource
                    //&& this.AppSetting.EnableTransMode == "1"
                    //&& this.AppSetting.EnableSingleMode == "1"
                    && this.AppSetting.EnableSingleMode == "1"
                    && (rgTransactionType.SelectedIndex == 0 || rgTransactionType.SelectedIndex == 1))
                {
        ...
        private void rglkpCashLedger_Validating(object sender, CancelEventArgs e)
        {
            ...
            if (gridLKPEdit.EditValue != null)
            {
                ...
                if (this.AppSetting.AllowMultiCurrency == 1)
                {
                    ...
                    gvBank.SetRowCellValue(gvBank.FocusedRowHandle, colCashBankLiveExchangeAmount, txtLiveExchangeRate.Text);
        ...
        private void LoadTransBasic()
        {
            ...
            FetchDateDuration();
            ShowHideGSTPAN();
            //glkpCurrencyCountry.EditValue = CurrencyId;
        ...
        private void ConstructTransEmptySource()
        {
            ...
            //bharath 04/06/2026 Ledger PAN and GST set
            dtTransaction.Columns.Add("PAN_NUMBER", typeof(string));
            dtTransaction.Columns.Add("GST_NUMBER", typeof(string));
            ...
            dtTransaction.Columns.Add("BENEFICIARY_NAME", typeof(string));
            dtTransaction.Columns.Add("BENEFICIARY_ACC_NO", typeof(string));
            dtTransaction.Columns.Add("BENEFICIARY_BANK_BRANCH", typeof(string));
            dtTransaction.Columns.Add("BENEFICIARY_CHEQUE_IN_FAVOUR", typeof(string));
            dtTransaction.Columns.Add("BENEFICIARY_CROSS_USING", typeof(string));
        ...
        private void ConstructCashTransEmptySournce()
        {
            ...
            dtCashTransaction.Columns.Add("BENEFICIARY_NAME", typeof(string));
            dtCashTransaction.Columns.Add("BENEFICIARY_ACC_NO", typeof(string));
            dtCashTransaction.Columns.Add("BENEFICIARY_BANK_BRANCH", typeof(string));
            dtCashTransaction.Columns.Add("BENEFICIARY_CHEQUE_IN_FAVOUR", typeof(string));
            dtCashTransaction.Columns.Add("BENEFICIARY_CROSS_USING", typeof(string));
            dtCashTransaction.Rows.Add(dtCashTransaction.NewRow());
        ...
        private void VisibleTransBankAdditionalFields(bool Visible)
        {
            ...
            if (EnableLedgerNarration)
            {
                ...
                colLedgerNarration.VisibleIndex = visibleIndex++;
            }
            else
            {
            ...
            if (EnableLedgerPan)
            {
                colLedgerPan.Visible = true;
                colLedgerPan.VisibleIndex = visibleIndex++;
            }
            else
            {
                colLedgerPan.Visible = false;
            }

            // GST
            if (EnableLedgerGST)
            {
                colLedgerGST.Visible = true;
                colLedgerGST.VisibleIndex = visibleIndex++;
            }
            else
            {
                colLedgerGST.Visible = false;
            }

            colCheque.VisibleIndex = visibleIndex++;
            colValueDate.VisibleIndex = visibleIndex++;
        ...
        private bool IsValidEntry()
        {
            ...
            else if (!IsValidSingleModeEntry()) //bharath 24/07/2026
            {
                isValid = false;
            }
            ...
            { //On 26/08/2024, If multi currency enabled, all the currency details must be filled
                ...
                MessageRender.ShowMessage(this.GetMessage(MessageCatalog.Common.GENERAL_ALL_CURRENCY_DETAIL_FILLED));
                glkpCurrencyCountry.Select();
                glkpCurrencyCountry.Focus();
                isValid = false;
            }
            ...
            else if (dtTransactionDate.DateTime.Date > DateTime.Now.Date)
            {
                if (this.ShowConfirmationMessage("The selected voucher date is in the future. Do you want to continue?", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == System.Windows.Forms.DialogResult.No)
                {
                    isValid = false;
                }
                if (string.IsNullOrEmpty(txtNameAddress.Text))
                {
                    if (this.AppSetting.EnablePANandAddressAleretMsg == "1")
                    {
                        DialogResult result = this.ShowConfirmationMessage("Do you want to save without Name and Address?", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
            ...
            else if (string.IsNullOrEmpty(txtNameAddress.Text))
            {
                if (this.AppSetting.EnablePANandAddressAleretMsg == "1")
                {
                    DialogResult result = this.ShowConfirmationMessage("Do you want to save without Name and Address?", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                    ...
                    DtGSTInvoiceMasterLedgerDetails.Rows.Count > 0 && !string.IsNullOrEmpty(GSTVendorInvoiceNo))
            {
                ...
                //                == System.Windows.Forms.DialogResult.No)
                if (this.ShowConfirmationMessage(this.GetMessage(MessageCatalog.Common.GENERAL_SURE_UPDATE_GST_INVOICE_GST_AMOUNT), MessageBoxButtons.YesNo, MessageBoxIcon.Question)
                             == System.Windows.Forms.DialogResult.No)
                {
                    isValid = false;
                }
                    ...
                    foreach (DataRowView drv in dtLedgers.DefaultView)
                    {
                            ...
                            && contravoucher != DefaulContraVoucher.CashDeposit)
                        {
                            ...
                            if (this.AppSetting.AllowMultiCurrency == 1)
                            {
                                ...
                                msg = this.GetMessage(MessageCatalog.Common.GENERAL_DD_REFRENCE_NUMBER_MISSING_BANK_ACCOUNT);
            ...
            try
            {
                if (VoucherId > 0)
                {
                    using (VoucherTransactionSystem voucherSystem = new VoucherTransactionSystem(VoucherId))
                    {
                        ...
                        txtExchangeRate.Text = voucherSystem.ExchangeRate.ToString("0.00##");
                        txtLiveExchangeRate.Text = voucherSystem.LiveExchangeRate.ToString("0.00##");
                        ...
                        // CHANGE (17/04/2026): Same id for reload order; duplicate mode must not carry booking.
                        _bookingGstInvoiceIdPendingFromVoucher = DuplicateVoucher ? 0 : voucherSystem.BookingGSTInvoiceId;
                        ...
                        if (resultArgs.Success)
                        {
                            ...
                            // 29/01/2025: LIVE_EXCHANGE_RATE now comes from voucher_trans when binding; no need to overwrite with EXCHANGE_RATE.
                            //if (dtTrans.Columns.Contains("LIVE_EXCHANGE_RATE") && dtTrans.Columns.Contains("EXCHANGE_RATE"))
                            //{
                            //    foreach (DataRow dr in dtTrans.Rows)
                            //    {
                            //        if (dr["EXCHANGE_RATE"] != null && dr["EXCHANGE_RATE"] != DBNull.Value)
                            //            dr["LIVE_EXCHANGE_RATE"] = dr["EXCHANGE_RATE"];
                            //    }
                            //}
                            ...
                            if (resultArgs.Success)
                            {
                                ...
                                // 29/01/2025: LIVE_EXCHANGE_RATE now comes from voucher_trans when binding; no need to overwrite with EXCHANGE_RATE.
                                //if (dtBank.Columns.Contains("LIVE_EXCHANGE_RATE") && dtBank.Columns.Contains("EXCHANGE_RATE"))
                                //{
        ...
        private void BindDataSource(DataView dvTrans, DataView dvCashTrans)
        {
            ...
            {
                // 29/01/2025: LIVE_EXCHANGE_RATE now comes from voucher_trans when binding; no need to overwrite with EXCHANGE_RATE.
                //if (dtTransTemp.Columns.Contains("LIVE_EXCHANGE_RATE") && dtTransTemp.Columns.Contains("EXCHANGE_RATE"))
                //{
            ...
            {
                // 29/01/2025: LIVE_EXCHANGE_RATE now comes from voucher_trans when binding; no need to overwrite with EXCHANGE_RATE.
                //if (dtCashBankTemp.Columns.Contains("LIVE_EXCHANGE_RATE") && dtCashBankTemp.Columns.Contains("EXCHANGE_RATE"))
                //{
                //    foreach (DataRow dr in dtCashBankTemp.Rows)
                //    {
                //        if (dr["EXCHANGE_RATE"] != null && dr["EXCHANGE_RATE"] != DBNull.Value)
                //            dr["LIVE_EXCHANGE_RATE"] = dr["EXCHANGE_RATE"];
                //    }
        ...
        private void FetchVoucherMethod()
        {
            using (ProjectSystem projectSystem = new ProjectSystem())
            {
                ...
                if (resultArgs.DataSource.Table != null && resultArgs.DataSource.Table.Rows.Count != 0)
                {
                    ...
                    EnableLedgerNarration = VoucherEnabled == 0 ? false : true;
                    //bharath 04/06/2026 Ledger PAN and GST set
                    int PanEnabled = this.UtilityMember.NumberSet.ToInteger(resultArgs.DataSource.Table.Rows[0][projectSystem.AppSchema.Voucher.IS_PAN_ENABLEDColumn.ColumnName].ToString());
                    int GSTEnabled = this.UtilityMember.NumberSet.ToInteger(resultArgs.DataSource.Table.Rows[0][projectSystem.AppSchema.Voucher.IS_GST_ENABLEDColumn.ColumnName].ToString());
                    EnableLedgerPan = PanEnabled == 0 ? false : true;
                    EnableLedgerGST = GSTEnabled == 0 ? false : true;
                    if (TransVoucherMethod == 1)
                    {
        ...
        private void ShowHideDonor(bool dontloadagain = false)
        {
            ...
            colLedgerBal.VisibleIndex = visibleIndex++;
            ...
            if (colGStAmt.Visible)
                //colGStAmt.VisibleIndex = 7; //bharath 04/06/2026 Ledger PAN and GST set
                if (colLedgerGST.Visible)
                {
                    colGStAmt.VisibleIndex = 7;
            ...
            try
            {
                ...
                for (int i = 0; i < gvTransaction.DataRowCount; i++)
                {
                    ...
                    if (lid > 0)
                    {
                        ...
                        // 29/01/2025, in order to subscribe live exchange rate
                        gvTransaction.SetRowCellValue(handle, colLiveExchangeAmount, UtilityMember.NumberSet.ToDecimal(txtLiveExchangeRate.Text));
                        gvTransaction.UpdateCurrentRow();
                    }
                }
                ...
                for (int i = 0; i < gvBank.DataRowCount; i++)
                {
                    ...
                    if (lid > 0)
                    {
                        ...
                        else
                        {
                            ...
                            // 29/01/2025, in order to subscribe live exchange rate
                            gvBank.SetRowCellValue(handle, colCashBankLiveExchangeAmount, UtilityMember.NumberSet.ToDecimal(txtLiveExchangeRate.Text));
                        }
                        gvBank.UpdateCurrentRow();
                    }
                }
        ...
        private void LoadPendingGSTInvoices()
        {
            try
            {
                // CHANGE (17/04/2026): Read EditValue before Never/null; BookingGSTInvoiceId getter is 0 when layout is Never.
                Int32 prevInvoiceId = 0;
                if (lkpGSTInvoices.EditValue != null && lkpGSTInvoices.EditValue != DBNull.Value)
                    prevInvoiceId = this.UtilityMember.NumberSet.ToInteger(lkpGSTInvoices.EditValue.ToString());
                if (_bookingGstInvoiceIdPendingFromVoucher > 0)
                    prevInvoiceId = _bookingGstInvoiceIdPendingFromVoucher;
                ...
                if (this.AppSetting.IncludeGSTVendorInvoiceDetails == "2" && (rgTransactionType.SelectedIndex == 0 || rgTransactionType.SelectedIndex == 1) && IsGeneralInvolice) //|| rgTransactionType.SelectedIndex == 1
                {
                    ...
                    {
                        // On 20/01/2026, Chinna - To pass currency country ID to filter invoices by selected currency
                        ...
                        resultArgs = voucherTransSys.FetchGSTPenindgInvoices(VoucherId, ProjectId, vtype, dtTransactionDate.DateTime, voucherCurrencyCountryId);
                        if (resultArgs.Success)
                        {
                            ...
                            if ((rgTransactionType.SelectedIndex == 0 || rgTransactionType.SelectedIndex == 1) && resultArgs.DataSource.Table.Rows.Count > 1) //"first item is select Invoice"
                            {
                                lgGSTInvoce.Visibility = LayoutVisibility.Always;
                            }
                            lkpGSTInvoices.EditValue = prevInvoiceId;
                            if (_bookingGstInvoiceIdPendingFromVoucher > 0 && prevInvoiceId == _bookingGstInvoiceIdPendingFromVoucher)
                                _bookingGstInvoiceIdPendingFromVoucher = 0;
                            this.SetGridLookPopupWindowSize(lkpGSTInvoices);
                            SyncGSTInvoiceAmountLabels();
        ...
        private void ClearControls()
        {
            if (VoucherId == 0)
            {
                ...
                txtLiveExchangeRate.Text = string.Empty;
                txtNameAddress.Text = string.Empty;
                ...
                DtGSTInvoiceMasterDetails = null;
                DtGSTInvoiceMasterLedgerDetails = null;
                // CHANGE (17/04/2026): Clear pending booking id with other GST clears.
                _bookingGstInvoiceIdPendingFromVoucher = 0;
        ...
        private void ShowBankChequeDDReferenceNumberDetails(bool cashbankgrid)
        {
            ...
            string BeneAccount = string.Empty;
            string BeneBankBranch = string.Empty;
            //bharath 19/03/2026 add cheque in favour
            string ChequeFavour = string.Empty;
            string Crossusing = string.Empty;
            ...
            else if ((voucherType == DefaultVoucherTypes.Payment || voucherType == DefaultVoucherTypes.Contra) && cashbankgrid) // Chinna 21/09/2023 to show ref available transfer mode
            {
                ...
                BeneName = gvBank.GetFocusedRowCellValue(colBeneName) != null ? gvBank.GetFocusedRowCellValue(colBeneName).ToString() : string.Empty;
                BeneAccount = gvBank.GetFocusedRowCellValue(colBeneAccNo) != null ? gvBank.GetFocusedRowCellValue(colBeneAccNo).ToString() : string.Empty;
                BeneBankBranch = gvBank.GetFocusedRowCellValue(colBeneBankBranch) != null ? gvBank.GetFocusedRowCellValue(colBeneBankBranch).ToString() : string.Empty;

                ChequeFavour = gvBank.GetFocusedRowCellValue(colChequeFavour) != null ? gvBank.GetFocusedRowCellValue(colChequeFavour).ToString() : string.Empty;
                //bharath
                //Crossusing = gvBank.GetFocusedRowCellValue(colBeneCrossUsing) != null ? gvBank.GetFocusedRowCellValue(colBeneCrossUsing).ToString() : "0";
                Crossusing = gvBank.GetFocusedRowCellValue(colBeneCrossUsing) != null ? gvBank.GetFocusedRowCellValue(colBeneCrossUsing).ToString() : string.Empty;
                bankname = gvBank.GetFocusedRowCellValue(colChequeRefBankName) != null ? gvBank.GetFocusedRowCellValue(colChequeRefBankName).ToString() : string.Empty;
                branch = gvBank.GetFocusedRowCellValue(colChequeRefBranch) != null ? gvBank.GetFocusedRowCellValue(colChequeRefBranch).ToString() : string.Empty;
            ...
            if (cashbankgrid)
            {
                ...
                gvBank.SetRowCellValue(gvBank.FocusedRowHandle, colBeneName, string.Empty);
                gvBank.SetRowCellValue(gvBank.FocusedRowHandle, colBeneAccNo, string.Empty);
                gvBank.SetRowCellValue(gvBank.FocusedRowHandle, colBeneBankBranch, string.Empty);
                gvBank.SetRowCellValue(gvBank.FocusedRowHandle, colChequeFavour, string.Empty);
                //gvBank.SetRowCellValue(gvBank.FocusedRowHandle, colBeneCrossUsing, "0");
                gvBank.SetRowCellValue(gvBank.FocusedRowHandle, colBeneCrossUsing, null);
            }
            else
            {
                ...
                gvBank.SetRowCellValue(gvTransaction.FocusedRowHandle, colBeneName, string.Empty);
                gvBank.SetRowCellValue(gvTransaction.FocusedRowHandle, colBeneAccNo, string.Empty);
                gvBank.SetRowCellValue(gvTransaction.FocusedRowHandle, colBeneBankBranch, string.Empty);
                gvBank.SetRowCellValue(gvTransaction.FocusedRowHandle, colChequeFavour, string.Empty);
                //gvBank.SetRowCellValue(gvTransaction.FocusedRowHandle, colBeneCrossUsing, "0");
                gvBank.SetRowCellValue(gvTransaction.FocusedRowHandle, colBeneCrossUsing, null);
            ...
            if (!string.IsNullOrEmpty(ChequeDDReferenceNo) && LedgerGroupId == (int)FixedLedgerGroup.BankAccounts)
            {
                ...
                frmVoucherBankReferenceDetails voucherbankreferencedetails =
                        new frmVoucherBankReferenceDetails(ChequeDDReferenceNo, vdate, referencedate, bankname, branch, fundtansfer, BeneName, BeneAccount, BeneBankBranch, rgTransactionType.SelectedIndex, ChequeFavour, Crossusing);
                if (voucherbankreferencedetails.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    ...
                    if (bankreferencedetails.GetUpperBound(0) > 0)
                    {
                        ...
                        if (cashbankgrid)
                        {
                            ...
                            gvBank.SetRowCellValue(gvBank.FocusedRowHandle, colBeneName, bankreferencedetails.GetValue(4).ToString().Trim());
                            gvBank.SetRowCellValue(gvBank.FocusedRowHandle, colBeneAccNo, bankreferencedetails.GetValue(5).ToString().Trim());
                            gvBank.SetRowCellValue(gvBank.FocusedRowHandle, colBeneBankBranch, bankreferencedetails.GetValue(6).ToString().Trim());
                            gvBank.SetRowCellValue(gvBank.FocusedRowHandle, colChequeFavour, bankreferencedetails.GetValue(7).ToString().Trim());
                            //gvBank.SetRowCellValue(gvBank.FocusedRowHandle, colBeneCrossUsing, bankreferencedetails.GetValue(8).ToString().Trim());
                            object crossUsingObj = bankreferencedetails.GetValue(8);
                            string crossUsingVal = crossUsingObj == null ? string.Empty : crossUsingObj.ToString().Trim();
                            // blank must stay empty (not "0"); write via DataRow to avoid editor forcing default
                            DataRow bankRow = gvBank.GetDataRow(gvBank.FocusedRowHandle);
                            if (bankRow != null && bankRow.Table.Columns.Contains(colBeneCrossUsing.FieldName))
                                bankRow[colBeneCrossUsing.FieldName] = string.IsNullOrEmpty(crossUsingVal) ? (object)DBNull.Value : crossUsingVal;
                            else
                                gvBank.SetRowCellValue(gvBank.FocusedRowHandle, colBeneCrossUsing, string.IsNullOrEmpty(crossUsingVal) ? null : crossUsingVal);
                        }
                        else
                        {
                            ...
                            gvTransaction.SetRowCellValue(gvTransaction.FocusedRowHandle, colTransBeneficiaryName, bankreferencedetails.GetValue(4).ToString().Trim());
                            gvTransaction.SetRowCellValue(gvTransaction.FocusedRowHandle, colTransBeneficiaryAccNo, bankreferencedetails.GetValue(5).ToString().Trim());
                            gvTransaction.SetRowCellValue(gvTransaction.FocusedRowHandle, colTransBeneficiaryBankBranch, bankreferencedetails.GetValue(6).ToString().Trim());
                        }
                    }
        ...
        private void PrintVoucher(int vid)
        {
            if (XtraMessageBox.Show(this.GetMessage(MessageCatalog.Transaction.VocherTransaction.SAVED_PRINT_VOUCHER), this.GetMessage(MessageCatalog.Common.COMMON_MESSAGE_TITLE), MessageBoxButtons.YesNo, MessageBoxIcon.Information, MessageBoxDefaultButton.Button1) == DialogResult.Yes)
            {
                ...
                using (VoucherTransactionSystem voucherSystem = new VoucherTransactionSystem())
                {
                    ...
                    if (rid == 0)
                    {
                        if (this.AppSetting.IS_IB_SGYC)
                        {
                            DialogResult drReport = XtraMessageBox.Show("Which report do you want to print?\n\nYes - Cheque Deposit Slip / Pay-in-Slip\nNo - Voucher Receipt Report",
                            "Select Report",
                            MessageBoxButtons.YesNo,
                            MessageBoxIcon.Question,
                            MessageBoxDefaultButton.Button2);
                            if (drReport == DialogResult.Yes)
                            {
                                rptVoucher = UtilityMember.EnumSet.GetDescriptionFromEnumValue(VoucherPrint.VOUCHERBANKRECEIPTS);
                            }
                            else
                            {
                                rptVoucher = UtilityMember.EnumSet.GetDescriptionFromEnumValue(VoucherPrint.CASHBANKRECEIPTS);
                            }
                        }
                        else
                        {
                    ...
                    {
                        rptVoucher = rid == 1 ? UtilityMember.EnumSet.GetDescriptionFromEnumValue(VoucherPrint.CASHBANKPAYMENTS) : UtilityMember.EnumSet.GetDescriptionFromEnumValue(VoucherPrint.CASHBANKCONTRA);
                    }
                ...
                if (dr.RowState != DataRowState.Deleted)
                {
                    ...
                    using (LedgerSystem ledger = new LedgerSystem())
                    {
                        ...
                        else
                        {
                            ...
                            gvTransaction.SetRowCellValue(gvBank.FocusedRowHandle, colTransBeneficiaryName, string.Empty);
                            gvTransaction.SetRowCellValue(gvBank.FocusedRowHandle, colTransBeneficiaryAccNo, string.Empty);
                            gvTransaction.SetRowCellValue(gvBank.FocusedRowHandle, colTransBeneficiaryBankBranch, string.Empty);
                        }
                    }
                }
            }
                ...
                foreach (DataRowView drTrans in dv)
                {
                    ...
                    if (isValid && this.AppSetting.AllowMultiCurrency == 1)
                    {
                        if (ExchangeAmt != UtilityMember.NumberSet.ToDecimal(txtExchangeRate.Text))
                        {
                            ...
                            validateMessage = this.GetMessage(MessageCatalog.Common.GENERAL_RATE_WITH_EXCHANGE_RATE);
                            gvTransaction.FocusedColumn = colAmount;
                            isValid = false;
                            break;
                        }
        ...
        /// <summary>
        /// EnableSingleMode: Receipt allows only one CR; Payment allows only one DR.
        /// e.g. Receipt CR, CR, DR → cannot save
        /// </summary>
        private bool IsValidSingleModeEntry()
        {
            bool isValid = true;
            //if (this.AppSetting.EnableTransMode == "1" && this.AppSetting.EnableSingleMode == "1"
            if (IsEnableSingleModeApplicable
                && (rgTransactionType.SelectedIndex == 0 || rgTransactionType.SelectedIndex == 1))
            {
                ...
                if (dtTrans != null)
                {
                    // Receipt primary = CR (To); Payment primary = DR (By)
                    int primarySource = (rgTransactionType.SelectedIndex == 0) ? (int)Source.To : (int)Source.By;
                    int primaryCount = 0;
                    ...
                    {
                        int source = this.UtilityMember.NumberSet.ToInteger(drv["SOURCE"].ToString());
                    ...
                    if (primaryCount > 1)
                    {
                        this.ShowMessageBox("Single entry mode is enabled. Cannot save.");
                        isValid = false;
                        FocusTransactionGrid();
                    }
                }
        ...
        private void SetBudgetInfo()
        {
            ...
            colReferenceNumber.VisibleIndex = visibleIndex++;
            ...
            else
            {
                ...
                if (colLedgerPan.Visible || colLedgerGST.Visible)
                {
        ...
        private void SetGSTInfo()
        {
            ...
            colReferenceNumber.VisibleIndex = visibleIndex++;
            colLedgerBal.VisibleIndex = visibleIndex++;
            colBudgetAmount.VisibleIndex = visibleIndex++;
            colGStAmt.VisibleIndex = visibleIndex++;
            colAction.VisibleIndex = visibleIndex++;
            colCostCentre.VisibleIndex = visibleIndex++;
        ...
        private void FetchDateDuration()
        {
            try
            {
                ...
                else
                {
                    ...
                    //On 22/07/2026, locking temp - For IS_SDB_PDO (INMPDO), lock vouchers till 31-03-2026 (no portal grace data)
                    else if (this.AppSetting.IS_SDB_PDO)
                    {
                        dtLockDateFrom = this.UtilityMember.DateSet.ToDate(this.AppSetting.BookBeginFrom, false);
                        // Uses <= DATE_TO in IsLockedTransaction, so 31-03-2026 inclusive
                        dtLockDateTo = new DateTime(2026, 3, 31, 0, 0, 0);
                    }
                    else
                    {
        ...
        private bool IsLockedTransaction(DateTime dtVoucherDate)
        {
            ...
            try
            {
                // On 22/07/2026, locking temp - IS_SDB_PDO lock range is filled in FetchDateDuration (same path as SDBINM)
            ...
            if (AppSetting.IS_DIOMYS_DIOCESE && AppSetting.EnableSubLedgerVouchers == "1" && IsSubLedgerExists)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        MessageRender.ShowMessage("'" + ledgername + "'" + this.GetMessage(MessageCatalog.Common.GENERAL_SUB_LEDGERS_SELECTED_LEDGER));
                    }
                }
                    ...
                    using (CountrySystem countrySystem = new CountrySystem())
                    {
                        ...
                        if (result.Success)
                        {
                            ...
                            if (this.AppSetting.AllowMultiCurrency == 1)
                            {
                                ...
                                txtLiveExchangeRate.Text = UtilityMember.NumberSet.ToNumber(countrySystem.ExchangeRate);
                ...
                foreach (DataRow drRefledger in dtRefLedgers.Rows)
                {
                    ...
                    else
                    {
                        if (VoucherId > 0)
                        {
                            ...
                            //Dinesh 04/07/2025
                            //if (this.ShowConfirmationMessage("if you change '" + GetLedgerName(ref_LedgerId) + "'  To  '" + GetLedgerName(LegId) +
                            //    "'  Ledger that Referered Amount will be cleared. Do you want to proceed?", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                            //Implemantaion
                            if (this.ShowConfirmationMessage(this.GetMessage(MessageCatalog.Common.GENERAL_IF_YOU_CHANGE) + GetLedgerName(ref_LedgerId) + "'  To  '" + GetLedgerName(LegId) +
                            (this.GetMessage(MessageCatalog.Common.GENERAL_DO_WANT_TO_PROCEED)), MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                            {
                    ...
                    if (!(dtRefDate <= dtTransactionDate.DateTime) && (!(dtRefDate == DateTime.MinValue)))
                    {
                        ...
                        //Implemantaion
                        if (this.ShowConfirmationMessage((this.GetMessage(MessageCatalog.Common.GENERAL_PAYMENT_IS_MADE_JOURNAL_REFERENCE)), MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                        {
                            DataRow[] ledgerdetails = dtReferenceVoucherNumber.Select("LEDGER_ID" + " = " + ref_LedgerId);
                            if (ledgerdetails.Length > 0)
                            {
                ...
                if (resultarg.Success)
                {
                    ...
                    if (dtRefBalances.Rows.Count > 0)
                    {
                        ...
                        if (refBalanceAmount != ref_Amount)
                        {
                            ...
                            //Implemantaion
                            this.ShowMessageBox(this.GetMessage(MessageCatalog.Common.GENERAL_CHECK_REVIEW_PAYMENT_REFERENCE_DETAIL));
                            gvTransaction.FocusedColumn = colAmount;
                            gvTransaction.ShowEditor();
                            Rtn = false;
                        }
        ...
        private bool ValidateGSTInvoiceDetails()
        {
            ...
            catch (Exception err)
            {
                ...
                MessageRender.ShowMessage(this.GetMessage(MessageCatalog.Common.GENERAL_VALIDATE_GST_INVOICE_DETAILS) + err.Message);
            }
        ...
        /// <summary>
        /// Highlights AST (asset / stock-type) linked invoices in the GST invoice lookup popup.
        /// </summary>
        private void gvGSTInvocies_RowStyle(object sender, RowStyleEventArgs e)
        {
            if (e.RowHandle < 0)
                return;
            GridView view = sender as GridView;
            if (view == null || gccolvouchersubtype == null || string.IsNullOrEmpty(gccolvouchersubtype.FieldName))
                return;
            object val = view.GetRowCellValue(e.RowHandle, gccolvouchersubtype.FieldName);
            if (val == null || val == DBNull.Value)
                return;
            if (string.Equals(val.ToString().Trim(), VoucherSubTypes.AST.ToString(), StringComparison.OrdinalIgnoreCase))
            {
        ...
        /// <summary>
        /// Updates pending-invoice amount labels from the lookup row or bound table (programmatic EditValue may not populate GetSelectedDataRow in time).
        /// </summary>
        private void SyncGSTInvoiceAmountLabels()
        {
            lblGSTInvoiceTotalAmount.Text = "0.00";
            lblGSTInvoiceBalance.Text = "0.00";
            if (lkpGSTInvoices.EditValue == null || lkpGSTInvoices.EditValue == DBNull.Value)
                return;
            int invId = this.UtilityMember.NumberSet.ToInteger(lkpGSTInvoices.EditValue.ToString());
            if (invId <= 0)
                return;

            using (VoucherSystem vsystem = new VoucherSystem())
            {
                string amtCol = vsystem.AppSchema.GSTInvoiceMaster.AMOUNTColumn.ColumnName;
                string balCol = vsystem.AppSchema.GSTInvoiceMaster.BALANCEColumn.ColumnName;
                object amt = null;
                object bal = null;
                ...
                {
                    DataTable dt = lkpGSTInvoices.Properties.DataSource as DataTable;
                    if (dt != null)
                    {
                        string vm = lkpGSTInvoices.Properties.ValueMember;
                        if (!string.IsNullOrEmpty(vm) && dt.Columns.Contains(vm))
                        {
                            DataRow[] rows = dt.Select(vm + " = " + invId);
                            if (rows.Length > 0)
                            {
                                amt = rows[0][amtCol];
                                bal = rows[0][balCol];
                ...
                if (amt != null && bal != null && amt != DBNull.Value && bal != DBNull.Value)
                {
                    lblGSTInvoiceTotalAmount.Text = UtilityMember.NumberSet.ToNumber(UtilityMember.NumberSet.ToDouble(amt.ToString()));
                    lblGSTInvoiceBalance.Text = UtilityMember.NumberSet.ToNumber(UtilityMember.NumberSet.ToDouble(bal.ToString()));
                }
            }
        }
        ...
        private void glkpCurrencyCountry_EditValueChanged(object sender, EventArgs e)
        {
            ...
            // On 20/01/2026, Chinna - To reload pending invoices when currency changes to filter by selected currency
            LoadPendingGSTInvoices();
        }
