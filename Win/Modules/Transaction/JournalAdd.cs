...
namespace ACPP.Modules.Transaction
{
    public partial class JournalAdd : frmFinanceBaseAdd
    {
        ...
        // Chinna 10/04/2026: full journal ledger lookup (cash/bank journal); used to restore DataSource after Option C popup filter
        private DataTable _journalLedgerLookupFull;
        private bool _rglkpLedgerCloseUpHooked;
        #endregion
        ...
        private bool IsCurrencyEnabledVoucher
        {
            get
            {
                ...
                // 29/01/2025, in order to subscribe live exchange rate
                ...
                return (currencycountry > 0 && currencyamt > 0 && exchagnerate > 0 && actalamount > 0);
            }
        }
        ...
        private int LedgerId
        {
            ...
            {
                int ledgerId = 0;
                // Chinna 23/07/2026: cash/bank journal binds colLedger to LEDGER_LOOKUP_KEY; real id stays in LEDGER_ID
                if (IsCashBankJournalLedgerLookupEnabled)
                {
                    DataRow focusedRow = gvTransaction.GetDataRow(gvTransaction.FocusedRowHandle);
                    if (focusedRow != null && focusedRow.Table.Columns.Contains("LEDGER_ID") && focusedRow["LEDGER_ID"] != DBNull.Value
                        && focusedRow["LEDGER_ID"] != null && !string.IsNullOrWhiteSpace(focusedRow["LEDGER_ID"].ToString()))
                        ledgerId = this.UtilityMember.NumberSet.ToInteger(focusedRow["LEDGER_ID"].ToString());
                }
                else
                {
                ...
                if (ledgerId > 0)
                {
                    DataRowView rowSelectedLedger = null;
                    if (IsCashBankJournalLedgerLookupEnabled)
                    {
                        object lookupKey = gvTransaction.GetFocusedRowCellValue(colLedger);
                        rowSelectedLedger = rglkpLedger.GetRowByKeyValue(lookupKey) as DataRowView;
                    }
                    else
                        rowSelectedLedger = rglkpLedger.GetRowByKeyValue(ledgerId) as DataRowView;
        ...
        /// <summary>CONTRA_MULTI_FLAG from master when editing an inter-project pair
        private int journalLoadedContraMultiFlag;
        ...
        private int ConfirmVoucherAuthorization
        {
            get
            {
                ...
                if (VoucherId == 0 && AppSetting.ConfirmAuthorizationVoucherEntry == 1)
                {
                    ...
                    //Inmplemantaion
                    string msg = this.GetMessage(MessageCatalog.Common.GENERAL_IS_THIS_VOUCHER_AUTHORIZED);
        ...
        private void frmTransactionMultiAdd_Load(object sender, EventArgs e)
        {
            ...
            // 29/01/2025, live exchange rate – show textbox when multi-currency
            lciLiveExchangeRate.Visibility = (this.AppSetting.AllowMultiCurrency == 1 ? LayoutVisibility.Always : LayoutVisibility.Never);
            // Fix old defect: ensure Exchange Rate caption shows correctly (not "LabellblDi" etc.)
            lcExchangeRate.Text = "Exchange Rate";
            ...
            // Chinna 10/04/2026: after ledger popup closes, restore full list when Option C applied a filtered DataView
            if (!_rglkpLedgerCloseUpHooked)
            {
                this.rglkpLedger.CloseUp += RglkpLedger_CloseUpRestoreLookupDataSource;
                _rglkpLedgerCloseUpHooked = true;
            }
        }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (IsJournalValid())
            {
                ...
                using (VoucherTransactionSystem voucherTransaction = new VoucherTransactionSystem())
                {
                    ...
                    voucherTransaction.LoadedContraMultiFlag = journalLoadedContraMultiFlag;
                    ...
                    // Inter-project bank transfer: only when the grid passes the simple two-line multi-currency screen do we run detection (DB) 
                    // messages; otherwise unchanged legacy journal.
                    string ipbRejectReason = null;
                    if (this.InterProjectBankJournalFlowAppliesNow)
                    {
                        if (!InterProjectBankJournalTransfer.TryDetectFromJournalGrid(this.AppSetting.AllowMultiCurrency, dtTrans, voucherTransaction, out ipbRejectReason)
                            && !string.IsNullOrEmpty(ipbRejectReason))
                        {
                            MessageRender.ShowMessage(ipbRejectReason);
                            return;
                        }
                    }
                    else
                        voucherTransaction.ClearInterProjectBankTransfer();
                    //Voucher Trans Details
                    ...
                    if (this.AppSetting.AllowMultiCurrency == 1)
                    {
                        if (this.Transaction.TransInfo != null)
                        {
                            ...
                            foreach (DataRowView drv in dv)
                            {
                                ...
                                // 29/01/2025, live exchange rate – save from txtliveExchangeRate, fallback to exchange rate
                                string liveRateText = string.IsNullOrEmpty(txtliveExchangeRate.Text) ? txtExchangeRate.Text : txtliveExchangeRate.Text;
                                if (string.IsNullOrEmpty(liveRateText)) liveRateText = "1";
                                drv[voucherTransaction.AppSchema.VoucherTransaction.LIVE_EXCHANGE_RATEColumn.ColumnName] = liveRateText;
                                drv.EndEdit();
                            }
                        }
                        ...
                        voucherTransaction.LiveExchangeRate = this.UtilityMember.NumberSet.ToDecimal(txtliveExchangeRate.Text);
                    ...
                    string ipPairSaveMsg;
                    if (this.InterProjectBankJournalFlowAppliesNow
                        && !InterProjectBankJournalTransfer.ValidatePairSaveAllowed(voucherTransaction, out ipPairSaveMsg))
                    {
                        MessageRender.ShowMessage(ipPairSaveMsg);
                        return;
                    }
        ...
        private void rglkpLedger_QueryPopUp(object sender, CancelEventArgs e)
        {
            ...
            if (sender != null)
            {
                ...
                // Chinna 10/04/2026: Option C — same bank (group 12) cannot be chosen on opposite Dr/Cr side
                if (this.UIAppSetting.EnableCashBankJournal == 1 && _journalLedgerLookupFull != null
                    && _journalLedgerLookupFull.Columns.Contains("GROUP_ID") && _journalLedgerLookupFull.Columns.Contains("LEDGER_ID"))
                {
                    bool onDebit = gvTransaction.FocusedColumn == colDebit;
                    bool onCredit = gvTransaction.FocusedColumn == colCredit;
                    if (onDebit || onCredit)
                    {
                        HashSet<int> excludeOppositeBankIds = CollectOppositeSideBankLedgerIdsForPopup(onDebit);
                        if (excludeOppositeBankIds.Count > 0)
                        {
                            bool ledgerIdIsString = _journalLedgerLookupFull.Columns["LEDGER_ID"].DataType == typeof(string);
                            string idList = ledgerIdIsString
                                ? string.Join(",", excludeOppositeBankIds.Select(i => "'" + i.ToString(CultureInfo.InvariantCulture) + "'"))
                                : string.Join(",", excludeOppositeBankIds.Select(i => i.ToString(CultureInfo.InvariantCulture)));
                            DataView dvFiltered = new DataView(_journalLedgerLookupFull)
                            {
                                RowFilter = "(GROUP_ID <> 12 OR LEDGER_ID NOT IN (" + idList + "))"
                            };
                            editor.Properties.DataSource = dvFiltered;
                        }
                    }
                }
            }
        ...
        /// <summary>
        /// Chinna 10/04/2026: After popup closes, revert from filtered DataView to full ledger table for next open.
        /// </summary>
        private void RglkpLedger_CloseUpRestoreLookupDataSource(object sender, EventArgs e)
        {
            if (this.UIAppSetting.EnableCashBankJournal != 1 || _journalLedgerLookupFull == null)
                return;
            GridLookUpEdit editor = sender as GridLookUpEdit;
            if (editor != null)
                editor.Properties.DataSource = _journalLedgerLookupFull;
        }

        /// <summary>
        /// Chinna 10/04/2026: Bank (GROUP_ID 12) ledger ids already used on the opposite side (for Option C filter).
        /// </summary>
        private HashSet<int> CollectOppositeSideBankLedgerIdsForPopup(bool editingDebitColumn)
        {
            HashSet<int> result = new HashSet<int>();
            DataTable dtTrans = gcTransaction.DataSource as DataTable;
            if (dtTrans == null || _journalLedgerLookupFull == null)
                return result;
            ...
            foreach (DataRow row in dtTrans.Rows)
            {
                if (row.RowState == DataRowState.Deleted)
                    continue;
                int lid = this.UtilityMember.NumberSet.ToInteger(row["LEDGER_ID"] != DBNull.Value ? row["LEDGER_ID"].ToString() : "0");
                if (lid <= 0)
                    continue;

                if (editingDebitColumn)
                {
                    if (this.UtilityMember.NumberSet.ToDecimal(row["CREDIT"].ToString()) > 0 && IsJournalLookupBankLedgerId(lid))
                        result.Add(lid);
                }
                else
                {
                    if (this.UtilityMember.NumberSet.ToDecimal(row["DEBIT"].ToString()) > 0 && IsJournalLookupBankLedgerId(lid))
                        result.Add(lid);
                }
            }
            return result;
        ...
        private bool IsJournalLookupBankLedgerId(int ledgerId)
        {
            if (_journalLedgerLookupFull == null || !_journalLedgerLookupFull.Columns.Contains("LEDGER_ID") || !_journalLedgerLookupFull.Columns.Contains("GROUP_ID"))
                return false;
            foreach (DataRow r in _journalLedgerLookupFull.Rows)
            {
                if (r.RowState == DataRowState.Deleted)
                    continue;
                int id = this.UtilityMember.NumberSet.ToInteger(r["LEDGER_ID"].ToString());
                if (id == ledgerId)
                    return this.UtilityMember.NumberSet.ToInteger(r["GROUP_ID"].ToString()) == 12;
            }
            return false;
        }
        ...
        private void btnRemoveVendorGSTInvoce_Click(object sender, EventArgs e)
        {
            ...
            //Implemantaion
            if (this.ShowConfirmationMessage(this.GetMessage(MessageCatalog.Common.GENERAL_ARE_YOU_SURE_TO_REMOVE_VENDOR_GST_INVOICE_DETAILS), MessageBoxButtons.OKCancel,
                 MessageBoxIcon.Question) == System.Windows.Forms.DialogResult.OK)
            {
                using (VoucherTransactionSystem vouchersystem = new VoucherTransactionSystem())
                {
                    ...
                    if (result.Success)
                    {
                        ...
                        //Implemantaion
                        this.ShowMessageBox(this.GetMessage(MessageCatalog.Common.GENERAL_INVOICE_DETAILS_IS_REMOVED));
                    }
                }
            }
        }
            ...
            if (colGStAmt.Visible)
            {
                if (deDate.DateTime >= this.AppSetting.GSTStartDate)
                {
                    ...
                    else
                    {
                        ...
                        MessageRender.ShowMessage(this.GetMessage(MessageCatalog.Common.GENERAL_GST_LEDGERS_ARE_NOT_AVIALBLE));
                    }
                }
                else
                {
        ...
        private void SetLoadDefault()
        {
            ...
            ShowReferenceNumber();
            ApplyAmountMaskCulture();
        }
        /// <summary>
        /// GlobalSetting stores custom DecimalSeparator/GroupingSeparator on CurrentCulture.NumberFormat.
        /// DevExpress numeric masks parse and commit cell text using Mask.Culture; when unset they follow
        /// UICulture defaults, so "500,12" can be read as 50012 on PostEditor/UpdateCurrentRow.
        /// </summary>
        private void ApplyAmountMaskCulture()
        {
            CultureInfo amountCulture = Thread.CurrentThread.CurrentCulture;

            rtxtDebit.Mask.Culture = amountCulture;
            rtxtDebit.Mask.UseMaskAsDisplayFormat = true;
            txtCredit.Mask.Culture = amountCulture;
            txtCredit.Mask.UseMaskAsDisplayFormat = true;

            txtCurrencyAmount.Properties.Mask.Culture = amountCulture;
            txtCurrencyAmount.Properties.Mask.UseMaskAsDisplayFormat = true;
            txtExchangeRate.Properties.Mask.Culture = amountCulture;
            txtExchangeRate.Properties.Mask.UseMaskAsDisplayFormat = true;
            txtActualAmount.Properties.Mask.Culture = amountCulture;
            txtActualAmount.Properties.Mask.UseMaskAsDisplayFormat = true;
            txtliveExchangeRate.Properties.Mask.Culture = amountCulture;
            txtliveExchangeRate.Properties.Mask.UseMaskAsDisplayFormat = true;
        }

        private string AmountRegExMask()
        {
            return @"\d*\" + AppSetting.DecimalSeparator + @"?\d*";
        }

        private void ApplyNumericAmountMask(TextEdit textEdit)
        {
            if (textEdit == null)
                return;
            textEdit.Properties.Mask.MaskType = MaskType.Numeric;
            textEdit.Properties.Mask.EditMask = "n";
            textEdit.Properties.Mask.UseMaskAsDisplayFormat = true;
            textEdit.Properties.Mask.Culture = Thread.CurrentThread.CurrentCulture;
        }

        private void txtCurrencyAmount_Enter(object sender, EventArgs e)
        {
            txtCurrencyAmount.Properties.Mask.MaskType = MaskType.RegEx;
            txtCurrencyAmount.Properties.Mask.EditMask = AmountRegExMask();
        }

        private void txtCurrencyAmount_Validating(object sender, CancelEventArgs e)
        {
            double val = this.UtilityMember.NumberSet.ToDouble(txtCurrencyAmount.Text);
            ApplyNumericAmountMask(txtCurrencyAmount);
            txtCurrencyAmount.Text = this.UtilityMember.NumberSet.ToNumber(val);
        }

        private void txtExchangeRate_Enter(object sender, EventArgs e)
        {
            txtExchangeRate.Properties.Mask.MaskType = MaskType.RegEx;
            txtExchangeRate.Properties.Mask.EditMask = AmountRegExMask();
        }

        private void txtExchangeRate_Validating(object sender, CancelEventArgs e)
        {
            double val = this.UtilityMember.NumberSet.ToDouble(txtExchangeRate.Text);
            ApplyNumericAmountMask(txtExchangeRate);
            txtExchangeRate.Text = this.UtilityMember.NumberSet.ToNumber(val);
        }

        private void txtActualAmount_Enter(object sender, EventArgs e)
        {
            txtActualAmount.Properties.Mask.MaskType = MaskType.RegEx;
            txtActualAmount.Properties.Mask.EditMask = AmountRegExMask();
        }

        private void txtActualAmount_Validating(object sender, CancelEventArgs e)
        {
            double val = this.UtilityMember.NumberSet.ToDouble(txtActualAmount.Text);
            ApplyNumericAmountMask(txtActualAmount);
            txtActualAmount.Text = this.UtilityMember.NumberSet.ToNumber(val);
        }

        private void gvTransaction_ShownEditor(object sender, EventArgs e)
        {
            if (gvTransaction.FocusedColumn != colDebit && gvTransaction.FocusedColumn != colCredit)
                return;

            TextEdit editor = gvTransaction.ActiveEditor as TextEdit;
            if (editor == null)
                return;

            CultureInfo amountCulture = Thread.CurrentThread.CurrentCulture;
            editor.Properties.Mask.Culture = amountCulture;
        ...
        private void Construct()
        {
            ...
            EnsureJournalTransactionTableHasRowProjectIdColumn(dt);
            if (IsCashBankJournalLedgerLookupEnabled)
                EnsureJournalTransactionTableHasLedgerLookupKeyColumn(dt);
            gcTransaction.DataSource = dt;
        }

        private const string JournalTransColumnRowProjectId = "ROW_PROJECT_ID";
        // Chinna 23/07/2026: unique lookup value when EnableCashBankJournal splits bank (GROUP 12) by project; ValueMember cannot stay LEDGER_ID alone
        private const string JournalTransColumnLedgerLookupKey = "LEDGER_LOOKUP_KEY";

        private bool IsCashBankJournalLedgerLookupEnabled
        {
            get { return this.UIAppSetting != null && this.UIAppSetting.EnableCashBankJournal == 1; }
        }

        /// <summary>From <c>FetchLedgerByGroup</c> (see LedgerSQL): project id for the joined <c>PROJECT_LEDGER</c> row; 0 if unmapped.</summary>
        private static void EnsureJournalTransactionTableHasRowProjectIdColumn(DataTable dt)
        {
            if (dt == null || dt.Columns.Contains(JournalTransColumnRowProjectId))
                return;
            dt.Columns.Add(JournalTransColumnRowProjectId, typeof(string));
            foreach (DataRow r in dt.Rows)
            {
                if (r.RowState == DataRowState.Deleted)
                    continue;
                r[JournalTransColumnRowProjectId] = "0";
            }
        }

        private static string BuildLedgerLookupKey(object ledgerId, object rowProjectId)
        {
            string lid = (ledgerId == null || ledgerId == DBNull.Value) ? "0" : ledgerId.ToString().Trim();
            string pid = (rowProjectId == null || rowProjectId == DBNull.Value) ? "0" : rowProjectId.ToString().Trim();
            if (string.IsNullOrEmpty(lid)) lid = "0";
            if (string.IsNullOrEmpty(pid)) pid = "0";
            return lid + "_" + pid;
        }

        private static void EnsureJournalTransactionTableHasLedgerLookupKeyColumn(DataTable dt)
        {
            if (dt == null || dt.Columns.Contains(JournalTransColumnLedgerLookupKey))
                return;
            dt.Columns.Add(JournalTransColumnLedgerLookupKey, typeof(string));
        }

        private static void EnsureLedgerLookupKeyOnLookupTable(DataTable dtLookup)
        {
            if (dtLookup == null)
                return;
            EnsureJournalTransactionTableHasLedgerLookupKeyColumn(dtLookup);
            bool hasRowProject = dtLookup.Columns.Contains(JournalTransColumnRowProjectId);
            foreach (DataRow r in dtLookup.Rows)
            {
                if (r.RowState == DataRowState.Deleted)
                    continue;
                object pid = hasRowProject ? r[JournalTransColumnRowProjectId] : "0";
                r[JournalTransColumnLedgerLookupKey] = BuildLedgerLookupKey(r["LEDGER_ID"], pid);
            }
        }

        /// <summary>Cash/bank journal only: bind lookup to LEDGER_LOOKUP_KEY so bank+project rows are selectable; other modes keep LEDGER_ID.</summary>
        private void ApplyCashBankJournalLedgerLookupBinding(DataTable dtLookup, RepositoryItemGridLookUpEdit glkpLedger)
        {
            if (glkpLedger == null)
                return;
            if (!IsCashBankJournalLedgerLookupEnabled)
            {
                ...
                if (colLedger != null)
                    colLedger.FieldName = "LEDGER_ID";
                return;
            }

            EnsureLedgerLookupKeyOnLookupTable(dtLookup);
            glkpLedger.DisplayMember = "LEDGER_NAME";
            glkpLedger.ValueMember = JournalTransColumnLedgerLookupKey;
            if (colLedger != null)
                colLedger.FieldName = JournalTransColumnLedgerLookupKey;

            DataTable dtTrans = gcTransaction != null ? gcTransaction.DataSource as DataTable : null;
            if (dtTrans != null)
            {
                EnsureJournalTransactionTableHasRowProjectIdColumn(dtTrans);
                EnsureJournalTransactionTableHasLedgerLookupKeyColumn(dtTrans);
                SyncTransactionLedgerLookupKeys(dtTrans);
            }
        }

        private void SyncTransactionLedgerLookupKeys(DataTable dtTrans)
        {
            if (dtTrans == null || !IsCashBankJournalLedgerLookupEnabled)
                return;
            EnsureJournalTransactionTableHasLedgerLookupKeyColumn(dtTrans);
            EnsureJournalTransactionTableHasRowProjectIdColumn(dtTrans);
            foreach (DataRow row in dtTrans.Rows)
            {
                if (row.RowState == DataRowState.Deleted)
                    continue;
                object lid = row["LEDGER_ID"];
                if (lid == null || lid == DBNull.Value || string.IsNullOrWhiteSpace(lid.ToString())
                    || this.UtilityMember.NumberSet.ToInteger(lid.ToString()) <= 0)
                {
                    row[JournalTransColumnLedgerLookupKey] = string.Empty;
                    continue;
                }
                object pid = row[JournalTransColumnRowProjectId];
                row[JournalTransColumnLedgerLookupKey] = BuildLedgerLookupKey(lid, pid);
            }
        }

        /// <summary>Cash/bank journal: selected lookup key → LEDGER_ID + ROW_PROJECT_ID on the grid row.</summary>
        private void SyncLedgerIdAndRowProjectFromLookupKey(DataRow row, object lookupKeyValue)
        {
            if (row == null || !IsCashBankJournalLedgerLookupEnabled)
                return;

            if (lookupKeyValue == null || lookupKeyValue == DBNull.Value || string.IsNullOrWhiteSpace(lookupKeyValue.ToString()))
            {
                row["LEDGER_ID"] = "0";
                if (row.Table.Columns.Contains(JournalTransColumnRowProjectId))
                    row[JournalTransColumnRowProjectId] = "0";
                if (row.Table.Columns.Contains(JournalTransColumnLedgerLookupKey))
                    row[JournalTransColumnLedgerLookupKey] = string.Empty;
                return;
            }

            string key = lookupKeyValue.ToString().Trim();
            DataTable src = ResolveJournalLedgerLookupTable();
            if (src != null && src.Columns.Contains(JournalTransColumnLedgerLookupKey))
            {
                foreach (DataRow lr in src.Rows)
                {
                    if (lr.RowState == DataRowState.Deleted)
                        continue;
                    string k = lr[JournalTransColumnLedgerLookupKey] != DBNull.Value ? lr[JournalTransColumnLedgerLookupKey].ToString().Trim() : string.Empty;
                    if (k != key)
                        continue;
                    row["LEDGER_ID"] = lr["LEDGER_ID"] != DBNull.Value ? lr["LEDGER_ID"].ToString() : "0";
                    if (row.Table.Columns.Contains(JournalTransColumnRowProjectId))
                    {
                        object v = lr.Table.Columns.Contains(JournalTransColumnRowProjectId) ? lr[JournalTransColumnRowProjectId] : null;
                        row[JournalTransColumnRowProjectId] = (v != null && v != DBNull.Value) ? v.ToString() : "0";
                    }
                    if (row.Table.Columns.Contains(JournalTransColumnLedgerLookupKey))
                        row[JournalTransColumnLedgerLookupKey] = key;
                    return;
                }
            }
            ...
            if (us > 0)
            {
                row["LEDGER_ID"] = key.Substring(0, us);
                if (row.Table.Columns.Contains(JournalTransColumnRowProjectId))
                    row[JournalTransColumnRowProjectId] = key.Substring(us + 1);
                if (row.Table.Columns.Contains(JournalTransColumnLedgerLookupKey))
                    row[JournalTransColumnLedgerLookupKey] = key;
            }
        }

        private void UpdateJournalRowRowProjectIdFromLedgerLookup(DataRow row, object ledgerIdValue)
        {
            if (row == null || !row.Table.Columns.Contains(JournalTransColumnRowProjectId))
                return;

            // Chinna 23/07/2026: cash/bank journal passes LEDGER_LOOKUP_KEY from colLedger
            if (IsCashBankJournalLedgerLookupEnabled)
            {
                SyncLedgerIdAndRowProjectFromLookupKey(row, ledgerIdValue);
                return;
            }

            if (ledgerIdValue == null || ledgerIdValue == DBNull.Value || string.IsNullOrWhiteSpace(ledgerIdValue.ToString()))
            {
                row[JournalTransColumnRowProjectId] = "0";
                return;
            }
            string key = ledgerIdValue.ToString().Trim();
            if (key == "0")
            {
                row[JournalTransColumnRowProjectId] = "0";
                return;
            ...
            DataTable src = _journalLedgerLookupFull;
            if (src == null || !src.Columns.Contains(JournalTransColumnRowProjectId))
            {
                src = rglkpLedger.DataSource as DataTable;
                if (src == null)
                {
                    DataView dvSrc = rglkpLedger.DataSource as DataView;
                    if (dvSrc != null)
                        src = dvSrc.Table;
                }
            }
            if (src == null || !src.Columns.Contains(JournalTransColumnRowProjectId))
            {
                row[JournalTransColumnRowProjectId] = ProjectId.ToString(CultureInfo.InvariantCulture);
                return;
            }
            ...
            foreach (DataRow lr in src.Rows)
            {
                if (lr.RowState == DataRowState.Deleted)
                    continue;
                string lid = lr["LEDGER_ID"] != DBNull.Value ? lr["LEDGER_ID"].ToString().Trim() : string.Empty;
                if (lid == key)
                {
                    object v = lr[JournalTransColumnRowProjectId];
                    row[JournalTransColumnRowProjectId] = v != null && v != DBNull.Value ? v.ToString() : "0";
                    return;
                }
            }
        ...
        /// <summary>Ledger lookup used to resolve <c>GROUP_ID</c> / <c>ROW_PROJECT_ID</c> (full cash/bank list or <see cref="rglkpLedger"/> source).</summary>
        private DataTable ResolveJournalLedgerLookupTable()
        {
            if (_journalLedgerLookupFull != null)
                return _journalLedgerLookupFull;
            if (rglkpLedger == null)
                return null;
            object ds = rglkpLedger.DataSource;
            if (ds == null)
                return null;
            DataTable t = ds as DataTable;
            if (t != null)
                return t;
            DataView v = ds as DataView;
            return v != null ? v.Table : null;
        }

        /// <summary>After loading a voucher, fill <see cref="JournalTransColumnRowProjectId"/> from the current ledger lookup so edit mode can evaluate inter-project flow.</summary>
        private void RefreshRowProjectIdsForLoadedJournal(DataTable dtTrans)
        {
            if (dtTrans == null || !dtTrans.Columns.Contains(JournalTransColumnRowProjectId))
                return;
            foreach (DataRow row in dtTrans.Rows)
            {
                if (row.RowState == DataRowState.Deleted)
                ...
                if (lid == null || lid == DBNull.Value)
                    continue;
                if (this.UtilityMember.NumberSet.ToInteger(lid.ToString()) <= 0)
                    continue;

                // Chinna 23/07/2026: keep existing ROW_PROJECT_ID when already set (inter-project edit); prefer current ProjectId among bank splits
                int existingPid = this.UtilityMember.NumberSet.ToInteger(
                    row[JournalTransColumnRowProjectId] != DBNull.Value && row[JournalTransColumnRowProjectId] != null
                        ? row[JournalTransColumnRowProjectId].ToString() : "0");
                if (existingPid > 0)
                    continue;
                ...
                if (IsCashBankJournalLedgerLookupEnabled)
                {
                    ...
                    if (src != null && src.Columns.Contains(JournalTransColumnRowProjectId))
                    {
                        string lidKey = lid.ToString().Trim();
                        string preferPid = ProjectId.ToString(CultureInfo.InvariantCulture);
                        string foundPid = null;
                        foreach (DataRow lr in src.Rows)
                        {
                            if (lr.RowState == DataRowState.Deleted)
                                continue;
                            string lrLid = lr["LEDGER_ID"] != DBNull.Value ? lr["LEDGER_ID"].ToString().Trim() : string.Empty;
                            if (lrLid != lidKey)
                                continue;
                            string lrPid = lr[JournalTransColumnRowProjectId] != DBNull.Value ? lr[JournalTransColumnRowProjectId].ToString().Trim() : "0";
                            if (foundPid == null)
                                foundPid = lrPid;
                            if (lrPid == preferPid)
                            {
                        ...
                        if (foundPid != null)
                            row[JournalTransColumnRowProjectId] = foundPid;
                        else
                            row[JournalTransColumnRowProjectId] = preferPid;
                        continue;
                    }
                }

                UpdateJournalRowRowProjectIdFromLedgerLookup(row, lid);
            }
        ...
        /// <summary>True when the grid is a two-line multi-currency bank pair with different <c>ROW_PROJECT_ID</c> (GROUP 12 both sides). Single place to branch inter-project logic.</summary>
        public bool InterProjectBankJournalFlowAppliesNow
        {
            get
            {
                int allowMc = this.AppSetting != null ? this.AppSetting.AllowMultiCurrency : 0;
                DataTable dt = gcTransaction != null ? gcTransaction.DataSource as DataTable : null;
                return InterProjectBankJournalTransfer.ShouldRunInterProjectBankJournalFlow(allowMc, dt, ResolveJournalLedgerLookupTable());
            }
        }
        ...
        private void LoadLedger(string NatureId, RepositoryItemGridLookUpEdit glkpLedger)
        {
            try
            {
                using (LedgerSystem ledgerSystem = new LedgerSystem())
                {
                    // Chinna 10/04/2026: keep default project-scoped query (BankAllProjects 0)
                    ledgerSystem.BankAllProjects = 0;
            ...
            try
            {
                using (LedgerSystem ledgerSystem = new LedgerSystem())
                {
                    ...
                    if (this.UIAppSetting.EnableCashBankJournal == 1)
                    {
                        ...
                        // Chinna 10/04/2026: FetchLedgerByGroup uses ?BANK_ALL=1 to list all bank ledgers with project names; other callers leave BankAllProjects default 0
                        ledgerSystem.BankAllProjects = 1;
                    }
                    else
                    {
                    ...
                    if (resultArgs != null && resultArgs.Success && resultArgs.DataSource.Table != null && resultArgs.DataSource.Table.Rows.Count > 0)
                    {
                        ...
                        if (this.UIAppSetting.EnableCashBankJournal == 1)
                            _journalLedgerLookupFull = dtBaseLedger;

                        glkpLedger.DisplayMember = "LEDGER_NAME";
                        // Chinna 23/07/2026: cash/bank journal uses LEDGER_LOOKUP_KEY so split bank+project rows are selectable
                        ApplyCashBankJournalLedgerLookupBinding(dtBaseLedger, glkpLedger);
                    }
                    else
                    {
        ...
        private bool IsJournalValid()
        {
            ...
            // 29/07/2026, commande Purpose - in cash/bank journal with multi-currency, same-project bank-to-bank should be entered via Contra, not Journal
            else if (!ValidateMultiCurrencyBankPairProjectRule())
            {
                isValid = false;
                gcTransaction.Focus();
            }
            ...
            else if (!isExistInvoiceNo())
            {
                isValid = false;
                //Dinesh 04/07/2025
                //GST Invoice No is already exists
                MessageRender.ShowMessage("'" + GSTVendorInvoiceNo + "'" + this.GetMessage(MessageCatalog.Common.GENERAL_GST_INVOICE_NO_IS_ALREADY_EXISTS));
                gcTransaction.Select();
                gcTransaction.Focus();
            }
            ...
            { //On 04/09/2024, If multi currency enabled, all the currency details must be filled
                ...
                //Dinesh 04/07/2025
                //As Multi Currency option is enabled, All the Currecny details should be filled.
                MessageRender.ShowMessage(this.GetMessage(MessageCatalog.Common.GENERAL_ALL_THE_CURRECNY_DETAILS_SHOULD_BE_FILLED));
                glkpCurrencyCountry.Select();
                glkpCurrencyCountry.Focus();
                isValid = false;
            }
            ...
            {
                //Dinesh 04/07/2025
                //Are you sure to Replicate Voucher Entry ?
                if (this.ShowConfirmationMessage(this.GetMessage(MessageCatalog.Common.GENERAL_ARE_YOU_SURE_TO_REPLICATE_VOUCHER_ENTRY), MessageBoxButtons.YesNo, MessageBoxIcon.Question)
        ...
        // 29/07/2026, commande purposr - save-time validation for cash/bank journal: block same-project bank pair when multi-currency is enabled
        private bool ValidateMultiCurrencyBankPairProjectRule()
        {
            if (this.AppSetting == null || this.AppSetting.EnableCashBankJournal != 1 || this.AppSetting.AllowMultiCurrency != 1)
                return true;

            DataTable dtTrans = gcTransaction.DataSource as DataTable;
            if (dtTrans == null || !dtTrans.Columns.Contains("LEDGER_ID") || !dtTrans.Columns.Contains("DEBIT")
                || !dtTrans.Columns.Contains("CREDIT") || !dtTrans.Columns.Contains(JournalTransColumnRowProjectId))
                return true;

            DataRow creditRow = null;
            DataRow debitRow = null;
            ...
            foreach (DataRow row in dtTrans.Rows)
            {
                ...
                int ledgerId = this.UtilityMember.NumberSet.ToInteger(row["LEDGER_ID"] != DBNull.Value ? row["LEDGER_ID"].ToString() : "0");
                if (ledgerId <= 0)
                    continue;

                decimal dr = this.UtilityMember.NumberSet.ToDecimal(row["DEBIT"] != DBNull.Value ? row["DEBIT"].ToString() : "0");
                decimal cr = this.UtilityMember.NumberSet.ToDecimal(row["CREDIT"] != DBNull.Value ? row["CREDIT"].ToString() : "0");
                if (dr <= 0 && cr <= 0)
                    continue;
            ...
            if (creditRow == null || debitRow == null)
                return true;

            int creditLedgerId = this.UtilityMember.NumberSet.ToInteger(creditRow["LEDGER_ID"].ToString());
            int debitLedgerId = this.UtilityMember.NumberSet.ToInteger(debitRow["LEDGER_ID"].ToString());
            if (creditLedgerId <= 0 || debitLedgerId <= 0)
                return true;

            int creditGroupId = GetLedgerGroupId(creditLedgerId);
            int debitGroupId = GetLedgerGroupId(debitLedgerId);
            if (creditGroupId != (int)FixedLedgerGroup.BankAccounts || debitGroupId != (int)FixedLedgerGroup.BankAccounts)
                return true;

            int creditProjectId = this.UtilityMember.NumberSet.ToInteger(creditRow[JournalTransColumnRowProjectId] != DBNull.Value ? creditRow[JournalTransColumnRowProjectId].ToString() : "0");
            int debitProjectId = this.UtilityMember.NumberSet.ToInteger(debitRow[JournalTransColumnRowProjectId] != DBNull.Value ? debitRow[JournalTransColumnRowProjectId].ToString() : "0");
            if (creditProjectId > 0 && debitProjectId > 0 && creditProjectId == debitProjectId)
            {
                this.ShowMessageBox("Same-project bank-to-bank entry is not allowed in Journal when multi-currency is enabled. Please use Contra voucher.");
                return false;
            }
        ...
        private void CalculateExchangeRate()
        {
            try
            {
                ...
                lblCalculatedAmt.Text = this.UtilityMember.NumberSet.ToNumber(ActualAmt);
                txtActualAmount.Text = this.UtilityMember.NumberSet.ToNumber(ActualAmt);
            }
            catch (Exception ex)
            {
                MessageRender.ShowMessage(ex.ToString(), true);
        ...
        private void ShowCurrencyDetails()
        {
            ...
            try
            {
                if (CountryId != 0)
                {
                    ...
                    // On 20/01/2026, Chinna - To show attach invoice button for any currency when multi-currency enabled and IncludeGSTVendorInvoiceDetails == "2"
                    bool show = false;
                    if (this.AppSetting.IncludeGSTVendorInvoiceDetails == "2")
                    {
                        if (this.AppSetting.AllowMultiCurrency == 1)
                        {
                            // Show for any currency when multi-currency enabled
                            Int32 currencycountry = glkpCurrencyCountry.EditValue == null ? 0 : this.UtilityMember.NumberSet.ToInteger(glkpCurrencyCountry.EditValue.ToString());
                            show = (currencycountry > 0);
                        }
                        else
                        {
                            // Show only for local currency when multi-currency not enabled
        ...
        private bool IsValidCashBankTransactions()
        {
            ...
            {
                validateMessage = this.GetMessage(MessageCatalog.Common.GENERAL_BANK_AND_CASH_CANNOT_BE_IN_THE_SAME_TRANSACTION);
                isValid = false;
            }
            ...
            //    validateMessage = this.GetMessage(MessageCatalog.Common.GENERAL_A_BANK_ENTRY_CANNOT_EXIST_ON_BOTH_DEBIT_AND_CREDIT_SIDES);
            //    isValid = false;
            //}
            else if (cashDebitCount > 0 && cashCreditCount > 0)
            {
                ...
                validateMessage = this.GetMessage(MessageCatalog.Common.GENERAL_A_CASH_ENTRY_CANNOT_EXIST_ON_BOTH_DEBIT_AND_CREDIT_SIDES);
                isValid = false;
            }
            else if ((bankDebitCount > 0 || cashDebitCount > 0) && totalDebitEntries > 1)
            {
                ...
                validateMessage = this.GetMessage(MessageCatalog.Common.GENERAL_IF_A_BANK_OR_CASH_ENTRY_IS_IN_THE_DEBIT_SIDE);
                isValid = false;
            }
            else if ((bankCreditCount > 0 || cashCreditCount > 0) && totalCreditEntries > 1)
            {
                ...
                validateMessage = this.GetMessage(MessageCatalog.Common.GENERAL_NO_OTHER_CREDIT_ENTRIES_ARE_ALLOWED);
                isValid = false;
            }
        ...
        private Int32 GetLedgerGroupId(Int32 ledgerId)
        {
            ...
            catch (Exception Err)
            {
                ...
                MessageRender.ShowMessage(this.GetMessage(MessageCatalog.Common.GENERAL_COULD_NOT_GET_LEDGER_GROUP) + " " + Err.Message);
            }
        ...
        private void ClearControls()
        {
            ...
            txtliveExchangeRate.Text = "1";
            lblCalculatedAmt.Text = this.UtilityMember.NumberSet.ToNumber(0);
            txtVoucher.Text = "";
            txtJNarration.Text = "";
        ...
        private void AssignValues()
        {
            try
            {
                journalLoadedContraMultiFlag = 0;
                ...
                if (VoucherId != 0)
                {
                    using (VoucherTransactionSystem voucherSystem = new VoucherTransactionSystem())
                    {
                        ...
                        resultArgs = voucherSystem.FillVoucherDetails(VoucherId);
                        ...
                        if (this.AppSetting.AllowMultiCurrency == 1)
                        {
                            ...
                            txtCurrencyAmount.Text = UtilityMember.NumberSet.ToNumber(UtilityMember.NumberSet.ToDouble(voucherSystem.ContributionAmount.ToString()));
                            lblCalculatedAmt.Text = UtilityMember.NumberSet.ToNumber(UtilityMember.NumberSet.ToDouble(voucherSystem.CalculatedAmount.ToString()));
                            txtActualAmount.Text = UtilityMember.NumberSet.ToNumber(UtilityMember.NumberSet.ToDouble(voucherSystem.ActualAmount.ToString()));
                            ShowCurrencyDetails();
                            //txtExchangeRate.Text = voucherSystem.ExchangeRate.ToString();
                            //txtliveExchangeRate.Text = voucherSystem.LiveExchangeRate.ToString();
                            txtExchangeRate.Text = UtilityMember.NumberSet.ToNumber(UtilityMember.NumberSet.ToDouble(voucherSystem.ExchangeRate.ToString()));
                            txtliveExchangeRate.Text = UtilityMember.NumberSet.ToNumber(UtilityMember.NumberSet.ToDouble(voucherSystem.LiveExchangeRate.ToString()));
                            if (string.IsNullOrEmpty(txtliveExchangeRate.Text)) txtliveExchangeRate.Text = "1";
                        }
                        ...
                        DataTable dtForCostCentre = dt;
                        if (resultArgs.Success && resultArgs.RowsAffected > 0 && this.AppSetting.EnableCashBankJournal == 1
                            && this.AppSetting.AllowMultiCurrency == 1 && voucherSystem.LoadedContraMultiFlag > 0)
                        {
                            ResultArgs raPair = voucherSystem.FetchInterProjectJournalBankLinesForEdit(voucherSystem.LoadedContraMultiFlag);
                            if (raPair != null && raPair.Success && raPair.DataSource.Table != null && raPair.DataSource.Table.Rows.Count == 2)
                            {
                                dt = raPair.DataSource.Table;
                                resultArgs = raPair;
                            }
                        }
                        if (resultArgs.Success && resultArgs.RowsAffected > 0)
                        {
                            ...
                            DataTable dtTransBound = dtTrans.DefaultView.ToTable();
                            EnsureJournalTransactionTableHasRowProjectIdColumn(dtTransBound);
                            int allowMc = this.AppSetting != null ? this.AppSetting.AllowMultiCurrency : 0;
                            bool runInterProjectDetect = (allowMc == 1 && voucherSystem.LoadedContraMultiFlag > 0)
                                || InterProjectBankJournalTransfer.ShouldRunInterProjectBankJournalFlow(allowMc, dtTransBound, ResolveJournalLedgerLookupTable());
                            if (runInterProjectDetect)
                            {
                                string ignoreReason;
                                InterProjectBankJournalTransfer.TryDetectFromJournalGrid(allowMc, dtTransBound, voucherSystem, out ignoreReason);
                            }
                            gcTransaction.DataSource = dtTransBound;
                            RefreshRowProjectIdsForLoadedJournal(dtTransBound);
                        }
                        dsCostCentre.Clear();
                        for (int i = 0; i < dtForCostCentre.Rows.Count; i++)
                        {
                            int LedId = this.UtilityMember.NumberSet.ToInteger(dtForCostCentre.Rows[i][voucherSystem.AppSchema.VoucherTransaction.LEDGER_IDColumn.ColumnName].ToString());
                        ...
                        journalLoadedContraMultiFlag = voucherSystem.LoadedContraMultiFlag;
                    }
                }
        ...
        {
            if (IsAuditorViewOnlyMode())
            {
                this.ShowMessageBox(this.GetMessage(MessageCatalog.Common.AUDITOR_VIEW_ONLY_MESSAGE));
                return;
            }
        ...
        private void ShowVendorGSTInvoiceDetails()
        {
            ...
            string alertmessage = this.GetMessage(MessageCatalog.Common.GENERAL_GST_AMOUNT_IS_NOT_AVAILABLE);
            if (CanShowVendorGST)
            {
                    ...
                    using (VoucherTransactionSystem sysvoucher = new VoucherTransactionSystem())
                    {
                        ...
                        */
                        ...
                        if (result.Success)
                        {
                            ...
                            // On 20/01/2026, Chinna - To pass currency country ID from voucher to GST invoice master table
                            Int32 voucherCurrencyCountryId = 0;
                            if (this.AppSetting.AllowMultiCurrency == 1)
                            {
                                voucherCurrencyCountryId = glkpCurrencyCountry.EditValue == null ? 0 :
                                    this.UtilityMember.NumberSet.ToInteger(glkpCurrencyCountry.EditValue.ToString());
                            }
                            ...
                            frmVoucherVendorGSTInvoiceDetails frmVendorGST = new frmVoucherVendorGSTInvoiceDetails(DefaultVoucherTypes.Journal, ProjectId, GSTInvoiceId, DtGSTInvoiceMasterDetails, DtGSTInvoiceMasterLedgerDetails, VoucherId,
                                                    transdate, GSTVendorId, GSTVendorInvoiceNo, GSTVendorInvoiceType, GSTVendorInvoiceDate, (cgst + sgst + igst), cgst, sgst, igst, txtJNarration.Text, SummaryCredit, lname, voucherCurrencyCountryId);
        ...
        private void RemoveTDSEntry()
        {
            try
            {
                if (dtRemoveTDSTransSummary != null && dtRemoveTDSTransSummary.Rows.Count > 0)
                {
                    ...
                    DataTable dtTdsFiltered = dvFilterRow.ToTable();
                    EnsureJournalTransactionTableHasRowProjectIdColumn(dtTdsFiltered);
                    gcTransaction.DataSource = dtTdsFiltered;
                    gcTransaction.RefreshDataSource();
        ...
        {
            if (e.Column == colLedger)
            {
                DataTable dtGrid = gcTransaction.DataSource as DataTable;
                if (dtGrid != null && dtGrid.Columns.Contains(JournalTransColumnRowProjectId))
                {
                    DataRow row = gvTransaction.GetDataRow(e.RowHandle);
                    if (row != null)
                        UpdateJournalRowRowProjectIdFromLedgerLookup(row, e.Value);
                }
            }
            if (isDeductTDS)
            {
        ...
        private bool ValidateCCAmoutWithLedgerAmount()
        {
                                    ...
                                    if (dtLedgerCC != null && dtLedgerCC.Rows.Count > 0)
                                    {
                                        ...
                                        if (dtLedgerCC.DefaultView.Count > 0)
                                        {
                                            ...
                                            if (CCamountGSt != 0)
                                            {
                                                ...
                                                MessageRender.ShowMessage("'" + ledgername + "'" + this.GetMessage(MessageCatalog.Common.GENERAL_CHECK_COST_CENTRE_ALLOCATION_DETAILS));
                                                Rtn = false;
                                                break;
                                            }
                                        }
            ...
            catch (Exception err)
            {
                ...
                MessageRender.ShowMessage(this.GetMessage(MessageCatalog.Common.GENERAL_SELECT_PROJECT) + " " + err.Message);
            }
        ...
        private bool ValidateGSTInvoiceDetails()
        {
                            ...
                            if (DtGSTInvoiceMasterDetails != null && DtGSTInvoiceMasterLedgerDetails.Rows.Count > 0 && !string.IsNullOrEmpty(GSTVendorInvoiceNo))
                            {
                                foreach (DataRow dr in dtGSTLegersTrans.Rows)
                                {
                                    ...
                                    if (DtGSTInvoiceMasterLedgerDetails.DefaultView.Count == 0)
                                    {
                                        ...
                                        msg = "'" + ledgername + "'" + this.GetMessage(MessageCatalog.Common.GENERAL_ATTACH_VENDOR_GST_INVOICE_AND_CHECK_DETAILS);
                                        Rtn = false;
                                        break;
                                    }
                                }
                        ...
                        if (lcVendor.Visibility == LayoutVisibility.Always)
                        {
                            ...
                            if (DtGSTInvoiceMasterDetails != null && DtGSTInvoiceMasterDetails.Rows.Count > 0)
                            {
                                ...
                                if (invoicedate > deDate.DateTime)
                                {
                                    ...
                                    MessageRender.ShowMessage(this.GetMessage(MessageCatalog.Common.GENERAL_INVOICE_DATE_MUST_BE_LESS_THAN_OR_EQUAL_TO_VOUCHER_DATE));
                                    deDate.Focus();
                                }
                                else if (IsGeneralInvolice && invoiceduedate != DateTime.MinValue && invoiceduedate < deDate.DateTime)
                                {
                                    ...
                                    MessageRender.ShowMessage(this.GetMessage(MessageCatalog.Common.GENERAL_INVOICE_DUE_DATE_GRATER_THEN_DATE));
                                    deDate.Focus();
                                }
                            }
                            ...
                            if (Rtn && !IsGeneralInvolice)
                            {
                                if (Rtn && DtGSTInvoiceMasterDetails == null && dtGSTLegersTrans.Rows.Count > 0)
                                {
                                    ...
                                    msg = this.GetMessage(MessageCatalog.Common.GENERAL_REMOVE_ALL_INVOICE);
                                    Rtn = false;
                                }
                            ...
                            else if (Rtn && IsGeneralInvolice)
                            {
                                ...
                                using (VoucherTransactionSystem vsystem = new VoucherTransactionSystem())
                                {
                                    ...
                                    else if (SummaryDebit != SumTotalAmount)
                                    {
                                        ...
                                        msg = this.GetMessage(MessageCatalog.Common.GENERAL_REMOVEING_ALL_MISMATHCEING_AMOUNT_ATTACH_VOUCHER);
                                        Rtn = false;
                                    }
                                }
                            }
            ...
            catch (Exception err)
            {
                ...
                MessageRender.ShowMessage(this.GetMessage(MessageCatalog.Common.GENERAL_COULD_VALIDATE_GST_INVOICE_DETAILS) + "" + err.Message);
            }
        ...
        private bool ValidateGST()
        {
            ...
            try
            {
                if (dtSource != null && dtSource.Rows.Count > 0)
                {
                    ...
                    if ((dtDebitSource != null && dtDebitSource.Rows.Count > 0) && (dtCreditSource != null && dtCreditSource.Rows.Count > 0))
                    {
                        ...
                        Msg = this.GetMessage(MessageCatalog.Common.GENERAL_BOTH_DEBIT_SIDE_AND_CREDIT_SIDE_GST_CALCULATIONS);
                    }
                }
                if (!isValid)
                {
            ...
            catch (Exception err)
            {
                ...
                MessageRender.ShowMessage(this.GetMessage(MessageCatalog.Common.GENERAL_COULD_NOT_VALIDATE_GST) + "" + err.Message);
            }
        ...
        private string GetInvoiceLedgerName()
        {
            ...
            if (gcTransaction.DataSource != null)
            {
                ...
                if (IsGeneralInvolice)
                {
                    ...
                    {
                        Int32 ledgerId = 0;
                        const int SundryDebtorsGroupId = 17; // TDSDefaultLedgers.SundryDebtors
                        bool useCreditLedger = false;

                        // Step 1: Check if Sundry Debtors (Group ID 17) exists on Credit side
                        dtLegersTrans.DefaultView.RowFilter = sysvoucher.AppSchema.VoucherTransaction.CREDITColumn.ColumnName + " > 0";
                        if (dtLegersTrans.DefaultView.Count > 0)
                        {
                            // Check each credit ledger to find Sundry Debtors
                            foreach (DataRowView drvCredit in dtLegersTrans.DefaultView)
                            {
                                Int32 tempLedgerId = UtilityMember.NumberSet.ToInteger(drvCredit[sysvoucher.AppSchema.Ledger.LEDGER_IDColumn.ColumnName].ToString());

                                if (tempLedgerId > 0)
                                {
                                    using (LedgerSystem ledgersystem = new LedgerSystem())
                                    {
                                        ResultArgs ledgerResult = ledgersystem.FetchLedgerDetailsById(tempLedgerId);

                                        if (ledgerResult.Success && ledgerResult.DataSource.Table != null && ledgerResult.DataSource.Table.Rows.Count > 0)
                                        {
                                            DataRow drLedgerInfo = ledgerResult.DataSource.Table.Rows[0];

                                            // Get Group ID from the ledger details
                                            if (drLedgerInfo.Table.Columns.Contains("GROUP_ID"))
                                            {
                                                Int32 tempGroupId = UtilityMember.NumberSet.ToInteger(drLedgerInfo["GROUP_ID"].ToString());

                                                // If Sundry Debtors found on Credit side, use it
                                                if (tempGroupId == SundryDebtorsGroupId)
                                                {
                                                    ledgerId = tempLedgerId;
                                                    useCreditLedger = true;
                                                    break; // Found Sundry Debtors on CR, exit loop
                                                }
                                            }
                                        }
                                    }
                        ...
                        // Step 2: If Sundry Debtors not found on CR, use first Debit ledger (default behavior)
                        if (!useCreditLedger)
                        {
                            dtLegersTrans.DefaultView.RowFilter = sysvoucher.AppSchema.VoucherTransaction.DEBITColumn.ColumnName + " > 0";
                            if (dtLegersTrans.DefaultView.Count > 0)
                            {
                                ledgerId = UtilityMember.NumberSet.ToInteger(dtLegersTrans.DefaultView[0][sysvoucher.AppSchema.Ledger.LEDGER_IDColumn.ColumnName].ToString());
                            }
                        }

                        // Step 3: Get ledger name using FetchLedgerDetailsById (more reliable than GetLegerName)
                        if (ledgerId > 0)
                        {
                            using (LedgerSystem lsystem = new LedgerSystem())
                            {
                                ResultArgs ledgerResult = lsystem.FetchLedgerDetailsById(ledgerId);

                                if (ledgerResult.Success && ledgerResult.DataSource.Table != null && ledgerResult.DataSource.Table.Rows.Count > 0)
                                {
                                    DataRow drLedgerInfo = ledgerResult.DataSource.Table.Rows[0];

                                    // Get ledger name from the result
                                    if (drLedgerInfo.Table.Columns.Contains("LEDGER"))
                                    {
                                        rtn = drLedgerInfo["LEDGER"].ToString();
                                    }
                                    else if (drLedgerInfo.Table.Columns.Contains("LEDGER_NAME"))
                                    {
                                        rtn = drLedgerInfo["LEDGER_NAME"].ToString();
                                    }
                                }
                            }
                        }
            ...
            /// 15/0//2026, Chinna to assign the debit Ledger
            //string rtn = string.Empty;
            //if (gcTransaction.DataSource != null)
            //{
            //    DataTable dtTransaction = gcTransaction.DataSource as DataTable;
                    ...
                    foreach (DataRowView drTrans in dv)
                    {
                        ...
                        if (isExists != 0)
                        {
                            if ((dtVoucherDate != DateTime.MinValue && deDate.DateTime > dtVoucherDate))
                            {
                                ...
                                this.ShowMessageBox(this.GetMessage(MessageCatalog.Common.GENERAL_YOU_HAVE_INCREASE_TRANSACTION_DATE));
                                deDate.Focus();
                                isValid = false;
                                break;
                            }
                            ...
                            if (SummaryDebit < TempDebitAmount)
                            {
                                ...
                                if (resultArgs.Success && resultArgs.DataSource.Table.Rows.Count > 0)
                                {
                                    ...
                                    if (SummaryDebit < RefereredAmount)
                                    {
                                        ...
                                        this.ShowMessageBox(this.GetMessage(MessageCatalog.Common.GENERAL_YOU_CANT_REDUSE_LESS_TEAN_REFERERD_AMOUNT));
                        ...
                        if (!string.IsNullOrEmpty(ReferenceNumber))
                        {
                            ...
                            if (Value > 0)
                            {
                                ...
                                this.ShowMessageBox(this.GetMessage(MessageCatalog.Common.GENERAL_THE_REFERENCE_NO_AVAILABLE_LEDGER));
                ...
                using (VoucherTransactionSystem voucherSystem = new VoucherTransactionSystem())
                {
                    ...
                    if (result.Success && result.RowsAffected > 0)
                    {
                        ...
                        if ((dtMinRPdate != DateTime.MinValue && deDate.DateTime > dtMinRPdate))
                        {
                            ...
                            this.ShowMessageBox(this.GetMessage(MessageCatalog.Common.GENERAL_THE_INVOICE_AMOUNT_IS_ALREADY_SETTLED_RECEIPT_JOURNAL));
                            deDate.Focus();
                            isValid = false;
                        }
                        else if (SummaryDebit < dtRPVoucherAmount)
                        {
                            ...
                            this.ShowMessageBox(this.GetMessage(MessageCatalog.Common.GENERAL_AS_INVOICE_IS_REFERERD_RECIPET_VOUCHER));
                        ...
                        if (isValid)
                        {
                            ...
                            if (resultArgs.Success && resultArgs.DataSource.Table != null && resultArgs.DataSource.Table.Rows.Count > 0)
                            {
                                ...
                                foreach (DataRow drInvoiceLedgers in dtInvoiceLedgers.Rows)
                                {
                                    ...
                                    // Get TRANS_MODE from invoice details
                                    string transMode = string.Empty;
                                    DataRow[] invoiceRows = dtInvoiceDetails.Select(voucherSystem.AppSchema.VoucherTransaction.LEDGER_IDColumn.ColumnName + "=" + invoiceLid);
                                    if (invoiceRows.Length > 0 && invoiceRows[0].Table.Columns.Contains(voucherSystem.AppSchema.GSTInvoiceMasterLedgerDetails.TRANS_MODEColumn.ColumnName))
                                    {
                                        transMode = invoiceRows[0][voucherSystem.AppSchema.GSTInvoiceMasterLedgerDetails.TRANS_MODEColumn.ColumnName].ToString();
                                    }

                                    DataTable dtTransTable = dv.ToTable();
                                    double TransLAmount = 0;

                                    // Check appropriate side based on TRANS_MODE, or check both if TRANS_MODE not available
                                    if (!string.IsNullOrEmpty(transMode) && (transMode.ToUpper() == "CR" || transMode.ToUpper() == TransSource.Cr.ToString().ToUpper()))
                                    {
                                        // For Receipt vouchers - check CREDIT side
                                        TransLAmount = UtilityMember.NumberSet.ToDouble(dtTransTable.Compute("SUM(" + voucherSystem.AppSchema.VoucherTransaction.CREDITColumn.ColumnName + ")",
                                                    voucherSystem.AppSchema.VoucherTransaction.LEDGER_IDColumn.ColumnName + "=" + invoiceLid).ToString());
                                    }
                                    else
                                    {
                                        // For Payment vouchers or if TRANS_MODE not available - check DEBIT side
                                        TransLAmount = UtilityMember.NumberSet.ToDouble(dtTransTable.Compute("SUM(" + voucherSystem.AppSchema.VoucherTransaction.DEBITColumn.ColumnName + ")",
                                                    voucherSystem.AppSchema.VoucherTransaction.LEDGER_IDColumn.ColumnName + "=" + invoiceLid).ToString());

                                        // If TRANS_MODE not available and DEBIT is 0, also check CREDIT (fallback)
                                        if (string.IsNullOrEmpty(transMode) && TransLAmount == 0)
                                        {
                                            TransLAmount = UtilityMember.NumberSet.ToDouble(dtTransTable.Compute("SUM(" + voucherSystem.AppSchema.VoucherTransaction.CREDITColumn.ColumnName + ")",
                                                        voucherSystem.AppSchema.VoucherTransaction.LEDGER_IDColumn.ColumnName + "=" + invoiceLid).ToString());
                                        }
                                    ...
                                    if (TransLAmount < invoiceLAmount)
                                    {
                                        //Dinesh 04/07/2025
                                        //this.ShowMessageBox("As Invoice is referered in Receipt Voucher, You can not change Ledger or Amount in Journal Voucher");
                                        this.ShowMessageBox(this.GetMessage(MessageCatalog.Common.GENERAL_YOU_CANT_CHANGE_LEDGER_OR_JOURNAL_VOUCHER));
                                        gvTransaction.FocusedColumn = (!string.IsNullOrEmpty(transMode) && (transMode.ToUpper() == "CR" || transMode.ToUpper() == TransSource.Cr.ToString().ToUpper())) ? colCredit : colDebit;
                                        isValid = false;
                                        break;
                                    }
                                }
                ...
                using (VoucherTransactionSystem voucherTransaction = new VoucherTransactionSystem())
                {
                    if (!IsLockedTransaction(deDate.DateTime) && ucAdditionalInfo.LockDeleteVocuher)
                    {
                        ...
                        if (this.ShowConfirmationMessage(this.GetMessage(MessageCatalog.Common.COMMON_DELETE_CONFIRMATION), MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                        {
                            ...
                            resultArgs = voucherTransaction.DeleteJournalVoucherWithPairSupport();
                            if (resultArgs.Success)
                            {
                                if (UpdateHeld != null)
                                {
        ...
        private void txtliveExchangeRate_EditValueChanged(object sender, EventArgs e)
        {
            if (this.UtilityMember.NumberSet.ToDouble(txtliveExchangeRate.Text) >= 0)
            {
                if (this.AppSetting.AllowMultiCurrency == 1 && this.Transaction != null && this.Transaction.TransInfo != null)
                {
                    DataView dv = this.Transaction.TransInfo;
                    const string colName = "LIVE_EXCHANGE_RATE";
                    if (dv.Table.Columns.Contains(colName))
                    {
                        string safeValue = string.IsNullOrEmpty(txtliveExchangeRate.Text) ? "1" : txtliveExchangeRate.Text;
                        foreach (DataRowView drv in dv)
                        {
                            drv.BeginEdit();
                            drv[colName] = safeValue;
                            drv.EndEdit();
                        }
                    }
                }
        ...
        #region Inter-project pair context table (journal screen)

        /// <summary>
        /// Single-row <see cref="VoucherTransactionSystem.InterProjectPairContext"/> layout. Only used when cash/bank journal + inter-project flow applies (<see cref="InterProjectBankJournalFlowAppliesNow"/>).
        /// Column names must match <c>VoucherTransactionSystem</c> private <c>InterProjectPairColumnNames</c> (same strings).
        /// </summary>
        public static class InterProjectPairContextTable
        {
            public const string TableName = "InterProjectBankJournalPair";
            public const string Active = "INTER_PROJECT_ACTIVE";
            public const string CreditLedgerId = "CREDIT_LEDGER_ID";
            public const string DebitLedgerId = "DEBIT_LEDGER_ID";
            public const string CreditProjectId = "CREDIT_PROJECT_ID";
            public const string DebitProjectId = "DEBIT_PROJECT_ID";
            public const string CreditAdjLedgerId = "CREDIT_ADJ_LEDGER_ID";
            public const string DebitAdjLedgerId = "DEBIT_ADJ_LEDGER_ID";
            public const string TransferAmount = "TRANSFER_AMOUNT";
            // 31/07/2026, Chinna - 1 = credit-project voucher first (legacy); 0 = debit-project voucher first (grid row 1 was DR)
            public const string SaveCreditProjectFirst = "SAVE_CREDIT_PROJECT_FIRST";

            public static DataTable CreateFromDetection(int creditLedgerId, int debitLedgerId, int creditProjectId, int debitProjectId, int creditAdjLedgerId, int debitAdjLedgerId, decimal transferAmount, bool saveCreditProjectFirst)
            {
                DataTable t = new DataTable(TableName);
                t.Columns.Add(Active, typeof(int));
                t.Columns.Add(CreditLedgerId, typeof(int));
                t.Columns.Add(DebitLedgerId, typeof(int));
                t.Columns.Add(CreditProjectId, typeof(int));
                t.Columns.Add(DebitProjectId, typeof(int));
                t.Columns.Add(CreditAdjLedgerId, typeof(int));
                t.Columns.Add(DebitAdjLedgerId, typeof(int));
                t.Columns.Add(TransferAmount, typeof(decimal));
                t.Columns.Add(SaveCreditProjectFirst, typeof(int));
                DataRow r = t.NewRow();
                r[Active] = (int)YesNo.Yes;
                r[CreditLedgerId] = creditLedgerId;
                r[DebitLedgerId] = debitLedgerId;
                r[CreditProjectId] = creditProjectId;
                r[DebitProjectId] = debitProjectId;
                r[CreditAdjLedgerId] = creditAdjLedgerId;
                r[DebitAdjLedgerId] = debitAdjLedgerId;
                r[TransferAmount] = transferAmount;
                r[SaveCreditProjectFirst] = saveCreditProjectFirst ? 1 : 0;
                t.Rows.Add(r);
                return t;
            }
        }
        ...
        #region InterProjectBankJournalTransfer

        /// <summary>
        /// Detects inter-project bank journal rows (multi-currency, two banks, two projects) and assigns <see cref="VoucherTransactionSystem.InterProjectPairContext"/>.
        /// </summary>
        private static class InterProjectBankJournalTransfer
        {
            public const int BankGroupId = (int)FixedLedgerGroup.BankAccounts;

            private static bool TryPickJournalCreditDebitRows(DataTable journalTrans, NumberSetMember ns,
                out DataRow creditRow, out DataRow debitRow, out decimal creditAmt, out decimal debitAmt)
            {
                bool firstMoneyLineIsCredit;
                return TryPickJournalCreditDebitRows(journalTrans, ns, out creditRow, out debitRow, out creditAmt, out debitAmt, out firstMoneyLineIsCredit);
            }

            // 31/07/2026, Chinna - also reports whether grid money-line #1 is the CR side (for save order of VOUCHER_MASTER_TRANS)
            private static bool TryPickJournalCreditDebitRows(DataTable journalTrans, NumberSetMember ns,
                out DataRow creditRow, out DataRow debitRow, out decimal creditAmt, out decimal debitAmt, out bool firstMoneyLineIsCredit)
            {
                creditRow = null;
                debitRow = null;
                creditAmt = debitAmt = 0m;
                firstMoneyLineIsCredit = true;
                if (journalTrans == null)
                    return false;

                var rows = new List<DataRow>();
                foreach (DataRow row in journalTrans.Rows)
                {
                    if (row.RowState == DataRowState.Deleted)
                        continue;
                    int ledgerId = ns.ToInteger(row["LEDGER_ID"] != DBNull.Value && row["LEDGER_ID"] != null ? row["LEDGER_ID"].ToString() : "0");
                    if (ledgerId <= 0)
                        continue;
                    decimal dr = ns.ToDecimal(row["DEBIT"] != DBNull.Value && row["DEBIT"] != null ? row["DEBIT"].ToString() : "0");
                    decimal cr = ns.ToDecimal(row["CREDIT"] != DBNull.Value && row["CREDIT"] != null ? row["CREDIT"].ToString() : "0");
                    if (dr <= 0 && cr <= 0)
                        continue;
                    rows.Add(row);
                }
                ...
                DataRow rowA = rows[0];
                DataRow rowB = rows[1];
                decimal drA = ns.ToDecimal(rowA["DEBIT"] != DBNull.Value ? rowA["DEBIT"].ToString() : "0");
                decimal crA = ns.ToDecimal(rowA["CREDIT"] != DBNull.Value ? rowA["CREDIT"].ToString() : "0");
                decimal drB = ns.ToDecimal(rowB["DEBIT"] != DBNull.Value ? rowB["DEBIT"].ToString() : "0");
                decimal crB = ns.ToDecimal(rowB["CREDIT"] != DBNull.Value ? rowB["CREDIT"].ToString() : "0");

                if (crA > 0 && drA == 0 && drB > 0 && crB == 0)
                {
                    creditRow = rowA;
                    debitRow = rowB;
                    firstMoneyLineIsCredit = true;
                }
                else if (crB > 0 && drB == 0 && drA > 0 && crA == 0)
                {
                    creditRow = rowB;
                    debitRow = rowA;
                    firstMoneyLineIsCredit = false;
                }
                else
                    return false;

                creditAmt = ns.ToDecimal(creditRow["CREDIT"] != DBNull.Value ? creditRow["CREDIT"].ToString() : "0");
                debitAmt = ns.ToDecimal(debitRow["DEBIT"] != DBNull.Value ? debitRow["DEBIT"].ToString() : "0");
                int crId = ns.ToInteger(creditRow["LEDGER_ID"].ToString());
                int drId = ns.ToInteger(debitRow["LEDGER_ID"].ToString());
                if (crId == drId)
                {
                    if (!journalTrans.Columns.Contains(JournalTransColumnRowProjectId))
                        return false;
                    int pCr = ns.ToInteger(creditRow[JournalTransColumnRowProjectId] != DBNull.Value ? creditRow[JournalTransColumnRowProjectId].ToString() : "0");
                    int pDr = ns.ToInteger(debitRow[JournalTransColumnRowProjectId] != DBNull.Value ? debitRow[JournalTransColumnRowProjectId].ToString() : "0");
                    if (pCr <= 0 || pDr <= 0 || pCr == pDr)
                        return false;
                }
                if (Math.Abs(creditAmt - debitAmt) > 0.0001m)
                    return false;
                return true;
            }

            private static int LookupLedgerGroupIdFromTable(DataTable ledgerLookup, int ledgerId, NumberSetMember ns)
            {
                if (ledgerLookup == null || !ledgerLookup.Columns.Contains("GROUP_ID") || !ledgerLookup.Columns.Contains("LEDGER_ID"))
                    return 0;
                string key = ledgerId.ToString(CultureInfo.InvariantCulture).Trim();
                foreach (DataRow lr in ledgerLookup.Rows)
                {
                    if (lr.RowState == DataRowState.Deleted)
                        continue;
                    string lid = lr["LEDGER_ID"] != DBNull.Value ? lr["LEDGER_ID"].ToString().Trim() : string.Empty;
                    if (lid == key)
                        return ns.ToInteger(lr["GROUP_ID"] != DBNull.Value ? lr["GROUP_ID"].ToString() : "0");
                }
                return 0;
            }

            /// <summary>
            /// Multi-currency, two money lines (one CR / one DR), both ledgers bank (GROUP 12 per lookup), different ROW_PROJECT_ID on the grid rows.
            /// </summary>
            public static bool ShouldRunInterProjectBankJournalFlow(int allowMultiCurrency, DataTable journalTrans, DataTable ledgerLookup)
            {
                if (allowMultiCurrency != 1 || journalTrans == null || ledgerLookup == null)
                    return false;
                if (!journalTrans.Columns.Contains(JournalTransColumnRowProjectId))
                    return false;

                var ns = new NumberSetMember();
                DataRow creditRow, debitRow;
                decimal creditAmt, debitAmt;
                if (!TryPickJournalCreditDebitRows(journalTrans, ns, out creditRow, out debitRow, out creditAmt, out debitAmt))
                    return false;

                int crLid = ns.ToInteger(creditRow["LEDGER_ID"].ToString());
                int drLid = ns.ToInteger(debitRow["LEDGER_ID"].ToString());
                if (LookupLedgerGroupIdFromTable(ledgerLookup, crLid, ns) != BankGroupId)
                    return false;
                if (LookupLedgerGroupIdFromTable(ledgerLookup, drLid, ns) != BankGroupId)
                    return false;

                int pCr = ns.ToInteger(creditRow[JournalTransColumnRowProjectId] != DBNull.Value ? creditRow[JournalTransColumnRowProjectId].ToString() : "0");
                int pDr = ns.ToInteger(debitRow[JournalTransColumnRowProjectId] != DBNull.Value ? debitRow[JournalTransColumnRowProjectId].ToString() : "0");
                return pCr > 0 && pDr > 0 && pCr != pDr;
            }

            /// <summary>
            /// Light grid shape check (same row pick as <see cref="ShouldRunInterProjectBankJournalFlow"/> without bank/project ids).
            /// </summary>
            public static bool ScreenJournalGridForPossibleInterProjectBankTransfer(int allowMultiCurrency, DataTable journalTrans)
            {
                if (allowMultiCurrency != 1 || journalTrans == null)
                    return false;
                var ns = new NumberSetMember();
                DataRow creditRow, debitRow;
                decimal creditAmt, debitAmt;
                return TryPickJournalCreditDebitRows(journalTrans, ns, out creditRow, out debitRow, out creditAmt, out debitAmt);
            }

            public static bool TryDetectFromJournalGrid(int allowMultiCurrency, DataTable journalTrans, VoucherTransactionSystem voucherTransaction, out string rejectReason)
            {
                rejectReason = null;
                voucherTransaction.ClearInterProjectBankTransfer();
                ...
                decimal creditAmt, debitAmt;
                bool firstMoneyLineIsCredit;
                if (!TryPickJournalCreditDebitRows(journalTrans, ns, out creditRow, out debitRow, out creditAmt, out debitAmt, out firstMoneyLineIsCredit))
                    return false;

                int creditLedgerId = ns.ToInteger(creditRow["LEDGER_ID"].ToString());
                int debitLedgerId = ns.ToInteger(debitRow["LEDGER_ID"].ToString());

                int creditRowProjectId = ReadRowProjectIdFromGridRow(creditRow, ns);
                int debitRowProjectId = ReadRowProjectIdFromGridRow(debitRow, ns);

                using (var ledger = new LedgerSystem())
                {
                    int creditProjectId;
                    int debitProjectId;
                    //if (!TryGetBankProjectAndGroup(ledger, creditLedgerId, out creditProjectId, out rejectReason))
                    if (!TryGetBankProjectAndGroup(ledger, creditLedgerId, creditRowProjectId, out creditProjectId, out rejectReason))
                        return false;
                    //if (!TryGetBankProjectAndGroup(ledger, debitLedgerId, out debitProjectId, out rejectReason))
                    if (!TryGetBankProjectAndGroup(ledger, debitLedgerId, debitRowProjectId, out debitProjectId, out rejectReason))
                        return false;

                    if (creditProjectId == debitProjectId)
                        return false;

                    int adjCredit = ledger.GetInternalAdjustmentLedgerIdForProject(creditProjectId);
                    int adjDebit = ledger.GetInternalAdjustmentLedgerIdForProject(debitProjectId);
                    if (adjCredit <= 0)
                    {
                        rejectReason = "Internal transfer ledger (name contains '" + LedgerSystem.DefaultInternalAdjustmentLedgerNameMatch + "', ACCESS_FLAG = 2) is not mapped for the credit bank's project.";
                        return false;
                    }
                    if (adjDebit <= 0)
                    {
                        rejectReason = "Internal transfer ledger (name contains '" + LedgerSystem.DefaultInternalAdjustmentLedgerNameMatch + "', ACCESS_FLAG = 2) is not mapped for the debit bank's project.";
                        return false;
                    }

                    // 31/07/2026, Chinna - pass grid order so VOUCHER_MASTER_TRANS create order matches first selected bank line
                    voucherTransaction.AssignInterProjectPairContext(
                        InterProjectPairContextTable.CreateFromDetection(creditLedgerId, debitLedgerId, creditProjectId, debitProjectId, adjCredit, adjDebit, creditAmt, firstMoneyLineIsCredit));
                    return true;
                }
            }
            private static int ReadRowProjectIdFromGridRow(DataRow row, NumberSetMember ns)
            {
                if (row == null || !row.Table.Columns.Contains(JournalTransColumnRowProjectId))
                    return 0;
                object v = row[JournalTransColumnRowProjectId];
                return v != null && v != DBNull.Value ? ns.ToInteger(v.ToString()) : 0;
            }

            //private static bool TryGetBankProjectAndGroup(LedgerSystem ledger, int ledgerId, out int projectId, out string rejectReason)
            private static bool TryGetBankProjectAndGroup(LedgerSystem ledger, int ledgerId, int rowProjectId, out int projectId, out string rejectReason)
            {
                projectId = 0;
                rejectReason = null;
                ResultArgs ra = ledger.FetchBankLedgerProjectContextForInterJournal(ledgerId);
                if (ra == null || !ra.Success || ra.DataSource.Table == null || ra.DataSource.Table.Rows.Count == 0)
                {
                    rejectReason = "Could not resolve project for bank ledger (check PROJECT_LEDGER mapping).";
                    return false;
                }
                DataRow dr = ra.DataSource.Table.Rows[0];
                //int groupId = dr["GROUP_ID"] != DBNull.Value ? new NumberSetMember().ToInteger(dr["GROUP_ID"].ToString()) : 0;
                //projectId = dr["PROJECT_ID"] != DBNull.Value ? new NumberSetMember().ToInteger(dr["PROJECT_ID"].ToString()) : 0;
                var ns = new NumberSetMember();
                int groupId = dr["GROUP_ID"] != DBNull.Value ? ns.ToInteger(dr["GROUP_ID"].ToString()) : 0;
                if (groupId != BankGroupId)
                {
                    rejectReason = "Inter-project transfer is allowed only for Bank Accounts (GROUP_ID = 12).";
                    return false;
                }
                if (rowProjectId > 0)
                {
                ...
                projectId = dr["PROJECT_ID"] != DBNull.Value ? ns.ToInteger(dr["PROJECT_ID"].ToString()) : 0;
                if (projectId <= 0)
                {
                    rejectReason = "Bank ledger must be mapped to a project.";
                    return false;
                }
                return true;
            }
            ...
            /// <summary>
            /// After grid detection and all voucher fields (GST, TDS, currency, etc.) are on <paramref name="voucherTransaction"/>,
            /// confirms the inter-project pair save is still allowed. Actual two-voucher save stays in <see cref="VoucherTransactionSystem.SaveVoucherDetails"/>.</summary>
            public static bool ValidatePairSaveAllowed(VoucherTransactionSystem voucherTransaction, out string rejectReason)
            {
                return voucherTransaction.ValidateInterProjectPairBeforeSave(out rejectReason);
            }
        }
