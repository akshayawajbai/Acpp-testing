...
namespace ACPP
{
    public partial class frmLoginDashboard : frmFinanceBase
    {
        ...
        private void RefreshOverAllBalance()
        {
            ...
            //bharath
            ResourceManager rm = new ResourceManager("ACPP.frmLoginDashboard", typeof(frmLoginDashboard).Assembly);
            string selectedLedgersText = rm.GetString("OVER ALL BALANCE", CultureInfo.CurrentUICulture) ?? "Over All Balance as on ";      
            ucTransviewClosingBalance1.TitleCaption = selectedLedgersText + baldate.ToShortDateString();
        ...
        public void LoadSumaryData()
        {
            using (BalanceSystem balanceSystem = new BalanceSystem())
            {
                ...
                dtsummary.Columns.Add("Group_Id", typeof(int)); //26-02-2026 Purpose of multilanguage Based Bind in Balance Grid ledger Details 

                dtsummary.Rows.Add(this.GetMessage(MessageCatalog.Master.DashBoard.DASHBOARD_CASH_TITLE), CashBalanceProperty.Amount, CashTransMode,(int)FixedLedgerGroup.Cash);
                dtsummary.Rows.Add(this.GetMessage(MessageCatalog.Master.DashBoard.DASHBOARD_BANK_TITLE), bankBalanceProperty.Amount, BankTransMode,(int)FixedLedgerGroup.BankAccounts);
                dtsummary.Rows.Add(this.GetMessage(MessageCatalog.Master.DashBoard.DASHBOARD_FD_TITLE), FDBalanceProperty.Amount, FDTransMode,(int)FixedLedgerGroup.FixedDeposit);
        ...
        private void rpcpeDetailBalance_Click(object sender, EventArgs e)
        {
            ...
            //FetchTransFDClosingBalance((int)fixedLedgerGroup, fixedLedgerGroup); //Commended By Dinesh 26-02-2025
            //Added By Dinesh 26-02-2026 Purpose of Fetch Balance Group_Id Based 
            object groupValue = gvBalance.GetFocusedRowCellValue("Group_Id");
            if (groupValue == null)
                return;
            int groupId = Convert.ToInt32(groupValue);
            FixedLedgerGroup fixedLedgerGroup = (FixedLedgerGroup)groupId;
            FetchTransFDClosingBalance(groupId, fixedLedgerGroup);
        }
