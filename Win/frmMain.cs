    ...
    public partial class frmMain : frmFinanceBase
    {
        ...
        private void frmMain_Load(object sender, EventArgs e)
        {
            ...
            if (this.AppSetting.AllowMultiCurrency == 1)
            {
                ...
                //nbiMapMigration.Visible = 
                    nbiMergeCashBankLedger.Visible = nbiDeleteunusedLedger.Visible = nbiMoveDeleteVouchers.Visible = false;
                nbiGSTPolicy.Visible = false;
                ...
                // Fix multi-currency JN with Cash/Bank lines still marked IS_CASH_BANK_STATUS = 0
                CorrectMultiCurrencyCashBankJournalStatus();
            }
            if (this.AppSetting.IS_SDB_RMG)
            {
                ...
                {
                    purposeSystem.DeleteSDBRMGDefaultPurposes();
                }
            }
            ...
            ApplyAuditorMenuRestrictions();
            //For SABSTP, enable Show Cash/Bank/FD Detail in Budget Proposal
            if (this.AppSetting.IS_SAB && this.UIAppSetting.ShowCashBankFDDetailLedgerInBudgetProposed != 1)
            {
                using (UISettingSystem uisystemsetting = new UISettingSystem())
                {
                    uisystemsetting.SaveUISettingDetails(FinanceSetting.ShowCashBankFDDetailLedgerInBudgetProposed, "1", this.LoginUser.LoginUserId);
                    ApplySettingAgain();
                }
            }
        }
        ...
        private void bbiAsset_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            if (!LoginUser.IsFullRightsReservedUser)
            {
                ...
                //bharath 08/08/2025
                ResourceManager rm = new ResourceManager("ACPP.frmMain", typeof(frmMain).Assembly);
                dpAcme.Text = rm.GetString("FIXED_ASSET_TITLE", CultureInfo.CurrentUICulture) ?? "Fixed Asset";
            }
            else
            {
                CollapseNavBar(nbgInventory.Caption, bbiAsset.Id);
        ...
        private void bbiPayroll_ItemClick(object sender, ItemClickEventArgs e)
        {
            if (!LoginUser.IsFullRightsReservedUser)
            {
                ...
                //bharath 08/08/2025
                ResourceManager rm = new ResourceManager("ACPP.frmMain", typeof(frmMain).Assembly);
                string payrollTitlePrefix = rm.GetString("PAYROLL_TITLE", CultureInfo.CurrentUICulture) ?? "Payroll";
                dpAcme.Text = payrollTitlePrefix;
            }
            else
            {
                ...
                if (Openfile.ShowDialog() == DialogResult.OK)
                {
                    ...
                    if (XtraMessageBox.Show(this.GetMessage(MessageCatalog.Common.COMMON_RESTORE_BACKUP_DATA), (this.GetMessage(MessageCatalog.Common.COMMON_RESTORE_INFO)), MessageBoxButtons.YesNo, MessageBoxIcon.Information) == DialogResult.Yes)
                    {
                        ...
                        if (result.Success)
                        {
                            ...
                            LoadLogoutFunc(skipExportVoucherAlert: true);
                        }
                        else
                        {
        ...
        private void frmMain_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            if (Is_Branch_Active && !AppSetting.IsSplitPreviousYearAcmeerpDB)
            {
                ...
                if (e.CloseReason == CloseReason.UserClosing && (this.AppSetting.ExportVouchersBeforeClose == "1" || this.AppSetting.IS_SDB_ING || this.AppSetting.IS_HAESST))
                {
        ...
        public void LoadLogoutFunc(bool skipExportVoucherAlert = false)
        {
            string[] DB;
            string databaseName = string.Empty;
            SettingProperty.Is_Application_Logout = true;
            ...
            if (Is_Branch_Active && !AppSetting.IsSplitPreviousYearAcmeerpDB)
            {
                ...
                // 15/04/2025, Chinna*
                if (!skipExportVoucherAlert && (this.AppSetting.ExportVouchersBeforeClose == "1" || this.AppSetting.IS_SDB_ING || this.AppSetting.IS_HAESST))
                {
                    DialogResult dialogVoucherExport = AlertToExportVoucher();
        ...
        private void LoadPayroll()
        {
            ...
            if (PayrollSubMenu.PAYROLL_Id > 0)
            {
                ...
                //bharath 08/08/2025
                ResourceManager rm = new ResourceManager("ACPP.frmMain", typeof(frmMain).Assembly);
                string payrollTitlePrefix = rm.GetString("PAYROLL_TITLE_PREFIX", CultureInfo.CurrentUICulture) ?? "Payroll -";
                dpAcme.Text = payrollTitlePrefix + PayrollSubMenu.PAYROLL_MONTH;
            }
        }
        ...
        private void ApplyAuditorMenuRestrictions()
        {
            if (!IsAuditorViewOnlyMode())
            {
                return;
            ...
            nbiMapLedProject.Visible = false;
            nbiMapMigration.Visible = false;
            nbiMergeCashBankLedger.Visible = false;
            nbiDeleteunusedLedger.Visible = false;
            nbiMoveDeleteVouchers.Visible = false;
            nbiAuthorizeVoucher.Visible = false;
            nbiUpdateNarration.Visible = false;
            nbiExportVouchers.Visible = false;
            nbiMapLedgers.Visible = false;
            nbiUploadBranchOfficeDB.Visible = false;
            nbiUserRights.Visible = false;
            nbiImportSplitProject.Visible = false;
            nbiSplitLedger.Visible = false;
            nbiDataMigration.Visible = false;
            nbiExportToTally.Visible = false;
            nbiProjectBulkImportData.Visible = false;
            nbiProjectBulkExportData.Visible = false;
            nbiThirdPartyIntegration.Visible = false;

            BackupNew.Visible = false;
            nbiDBBackup.Visible = false;
        }
                        ...
                        foreach (DataRow drPortallockvoucher in dtPortalLockVouchers.Rows)
                        {
                            ...
                            if (resultargs.Success && AuditTranSystem.LockTypeId > 0 && AuditTranSystem.ProjectId > 0)
                            {
                                if (resultargs.Success)
                                {
                                    ...
                                    resultargs = AuditTranSystem.SaveAuditTrans();
                                    if (resultargs.Success)
                                    {
                                        AuditTranSystem.UpdateUserMain(AuditTranSystem.LockTransId, this.UtilityMember.NumberSet.ToInteger(this.UserId));
                                    }
                                }
                            }
                        }
        ...
        private bool IsLocalDevelopmentIP()
        {
            ...
            bool Rtn = false;
            string[] localIPS = new string[] { "192.168.1.171", "192.168.1.173", "192.168.1.174", "192.168.1.175" , 
        ...
        /// <summary>
        /// Multi-currency: set IS_CASH_BANK_STATUS = 1 for active JN vouchers that involve Cash/Bank
        /// (group 12/13) but were left as 0 (e.g. after column default update).
        /// </summary>
        private void CorrectMultiCurrencyCashBankJournalStatus()
        {
            try
            {
                string sql = @" UPDATE voucher_master_trans vm
                                INNER JOIN
                                (
                                    SELECT DISTINCT vm.voucher_id
                                    ...
                                    WHERE vm.voucher_type = 'JN'
                                        AND vm.IS_MULTI_CURRENCY = 1
                                        AND vm.STATUS = 1
                                        AND ml.group_id in (12,13) 
                                        AND IFNULL(vm.IS_CASH_BANK_STATUS,0) = 0
                                ) x
                                ON vm.voucher_id = x.voucher_id
                                SET vm.IS_CASH_BANK_STATUS = 1;";

                using (DataManager dataManager = new DataManager())
                {
                    dataManager.UpdateData(sql, true);
                }
            }
            catch
            {
            }
            AcMELog.WriteLog("Journal Entries are updated for cash and Bank involved Transaction");
        }
        ...
        /// </summary>
        //--------------------------------------------------------------------------------------------------------------
        public void CorrectSDBAOSProjectNames()
        {
            if (AppSetting.IS_SDB_CONGREGATION && AppSetting.HeadofficeCode.ToUpper() == "SDBAOS")
            {
                string[,] ToCorrectProjects = new string[,]
        {
            { "Administration PDO", "PDO Administration" },
            { "BUREAU DE PROJET", "PROJECT OFFICE" },
            { "DBTI, TATALE", "DBTI, TATALE" },
            { "PARISH, TATALE", "PARISH, TATALE" },
            { "MAISON SDB, TATALE", "SDB HOUSE, TATALE" },
            { "LIGNE D'AIDE À L'ENFANCE (À L'ÉCHELLE NATIONALE), PENDANT LA VILLE", "CHILD HELP LINE (COUNTRYWIDE), DURING TOWN" },
            { "REFUGE POUR FILLES PLUS, DANS LA VILLE", "GIRLS SHELTER PLUS, DURING TOWN" },
            { "Paroisse , Pendant la Ville", "PARISH, DURING TOWN" },
            { "MAISON SDB, Pendant la ville", "SDB HOUSE, DURING TOWN" },
            { "PROGRAMME POUR LES ENFANTS DES RUES ET LES JEUNES EN DIFFICULTÉ, PENDANT LA VILLE", "STREET CHILDREN AND YOUTH AT RISK PROGRAMME, DURING TOWN" },
            { "THERAPEUTIC CENTRE, DURING TOWN", "THERAPEUTIC CENTRE, DURING TOWN" },
            { "CENTRE DE PROTECTION DE L'ENFANCE (CPC), ASHAIMAN", "CHILD-PROTECTION CENTRE (CPC), ASHAIMAN" },
            { "PAROISSE ASHAIMAN", "PARISH, ASHAIMAN" },
            { "MAISON SDB   ASHAIMAN", "SDB HOUSE ASHAIMAN" },
            { "CENTRE DES JEUNES, ABIDJAN", "CENTRE DES JEUNES, ABIDJAN" },
            { "FOYER ESD, ABIDJAN", "FOYER ESD, ABIDJAN" },
            { "PAROISSE, ABIDJAN", "PAROISSE, ABIDJAN" },
            { "MAISON SDB, ABIDJAN", "SDB HOUSE, ABIDJAN" },
            { "MAISON DE FORMATION, LOMÉ", "FORMATION HOUSE, LOMÉ" },
            { "INSTITUT SUPÉRIEUR, LOMÉ", "INSTITUT SUPÉRIEUR, LOMÉ" },
            { "ORATOIRE, LOMÉ", "ORATOIRE, LOMÉ" },
            { "PAROISSE, RUE STREET", "PARISH, FORT STREET" },
            { "ÉCOLE MATERNELLE ET ÉCOLE PRIMAIRE, Rue STREET", "PRE-SCHOOL AND PRIMARY SCHOOL, FORT STREET" },
            { "Maison SDB, RUE FORT", "SDB HOUSE, FORT STREET" },
            { "Centre pour Jeunes, Rue FORT", "YOUTH CENTRE, FORT STREET" },
            { "PAROISSE, MONROVIA", "PARISH, MONROVIA" },
            { "Ecole Primaire et Secondaire, MONROVIA", "PRIMARY & SECONDARY SCHOOL, MONROVIA" },
            { "Maison SDB , MONROVIA", "SDB HOUSE, MONROVIA" },
            { "MINISTÈRE DE LA JEUNESSE, MONROVIA", "YOUTH MINISTRY, MONROVIA" },
            { "CENTRE DE PROTECTION  DE L? ENFANCE, SUNYANI", "CHILD PROTECTION CENTRE, SUNYANI" },
            { "PAROISSE, SUNYANI", "PARISH, SUNYANI" },
            { "MAISON SDB, SUNYANI", "SDB HOUSE, SUNYANI" },
            { "INSTITUT TECHNIQUE PROFESSIONNEL, SUNYANI", "VOCATIONAL TECHNICAL INSTITUTE, SUNYANI" },
            { "ATELIER AUTOMOBILE, ASHAIMAN", "AUTOMOBILE WORKSHOP, ASHAIMAN" },
            { "INSTITUT DE FORMATION DON BOSCO, ASHAIMAN", "DON BOSCO TRAINING INSTITUTE, ASHAIMAN" },
            { "AUTO ECOLE , ASHAIMAN", "DRIVING SCHOOL, ASHAIMAN" },
            { "CENTRE D'ACCUEIL, ASHAIMAN", "HOSPITALITY CENTRE, ASHAIMAN" },
            { "AUBERGE, ASHAIMAN", "HOSTEL, ASHAIMAN" },
            { "MAISON SDB, ASHAIMAN", "SDB HOUSE, ASHAIMAN" },
            { "PROVINCE", "PROVINCE" },
            { "MAISON DE FORMATION, SUNYANI", "FORMATION HOUSE, SUNYANI" },
        };

                for (int i = 0; i <= ToCorrectProjects.GetUpperBound(0); i++)
                {
                    string oldProjectName = ToCorrectProjects[i, 0];
                    string newProjectName = ToCorrectProjects[i, 1];

                    if (!string.IsNullOrEmpty(oldProjectName) && !string.IsNullOrEmpty(newProjectName))
                    {
                        using (ProjectSystem projectSys = new ProjectSystem())
                        {
                            ResultArgs resultarg = projectSys.ChangeProjectName(oldProjectName, newProjectName);
                        }
                    }
                }
            }
        ...
        private DialogResult AlertToExportVoucher()
        {
            ...
            //1. 18/01/2018 Show export voucher form if setting is enbaled ------------------------------------------------------------------------------------
            //If Export voucher is enabled in finance setting, if user does not do it, diable logout
            if (this.AppSetting.ExportVouchersBeforeClose == "1" || this.AppSetting.IS_SDB_ING || this.AppSetting.IS_HAESST)
            {
        ...
        private void nbiUser_ItemChanged(object sender, EventArgs e)
        {
