    ...
    public class frmFinanceBase : Bosco.Utility.Base.frmBase
    {
      ...
      new RectangleF(0, 0, 100, 20), BorderSide.None);
        ...
        public ResultArgs UploadBudgetToAcmeerpPortal(Int32 BudgetId, bool IsMailSend)
        {
            ...
            AcMEService.AcmeERPSSPService AcmeUdegService = new AcMEService.AcmeERPSSPService();
            ...
            else // SDB_RMG congregation
            {
                string ManagementCode = SettingProperty.ManagementCodeIntegration.Trim();
                if (string.IsNullOrEmpty(ManagementCode))
                {
                    this.ShowMessageBox("Management Code is missing or invalid.");
                    AcMELog.WriteLog("Export aborted: Management Code is missing or invalid.");
                }
                else
                {
                    this.ShowWaitDialog("Exporting Budgets...");
                    try
                    {
                        if (this.CheckForInternetConnectionhttp())
                        {
                            using (BudgetSystem budgetsystem = new BudgetSystem())
                            {
                                result = budgetsystem.GetBudgetSourceAPI(BudgetId);
                                if (result != null && result.DataSource.TableSet.Tables.Count > 0)
                                {
                                    string JsonValues = AcmeUdegService.ConvertDatasetToJson(result.DataSource.TableSet);
                                    string status = AcmeUdegService.APIExportBudget(ManagementCode, JsonValues);

                                    this.CloseWaitDialog();
                                    if (!string.IsNullOrEmpty(status) && status.IndexOf("Success", StringComparison.OrdinalIgnoreCase) >= 0)
                                    {
                                        AcMELog.WriteLog(string.Format("Budget {0} exported to Udeg Portal successfully.", BudgetId));
                                        this.ShowMessageBox("Budget has been exported to Udeg Portal.");
                                    }
                                    else
                                    {
                                        string failMsg = string.Format("{0} - Failed in Export Budget", status);
                                        this.ShowMessageBox(failMsg);
                                        AcMELog.WriteLog(string.Format("Failed in Export Budget: {0}", status));
                                    }
                                }
                                else
                                {
                                    this.ShowMessageBox("No budget data found to export.");
                                    AcMELog.WriteLog("No budget data found to export.");
                                }
                            }
                        }
                        else
                        {
                            this.CloseWaitDialog();
                            string netMsg = "Unable to reach Cloud Portal. Please check your Internet connection or FTP Rights.";
                            this.ShowMessageBox(netMsg);
                            result.Message = netMsg;
                            AcMELog.WriteLog(netMsg);
                        }
                    }
                    catch (Exception ex)
                    {
                        this.CloseWaitDialog();
                        result.Message = ex.Message;
                        AcMELog.WriteLog(string.Format("Exception during SDB_RMG export: {0}", ex.Message));
                    }
                    finally
                    {
                        this.Cursor = Cursors.Default;
                                    ...
                                    using (ExportMasters exportbuget = new ExportMasters())
                                    {
                                        ...
                                        if (result.Success)
                                        {
                                            ...
                                            if (dsBOBudgetDetails.Tables.Count >= 3 && dsBOBudgetDetails.Tables.Contains("BudgetMaster") && dsBOBudgetDetails.Tables.Contains("BudgetProject") && dsBOBudgetDetails.Tables.Contains("BudgetLedger"))
                                            {
                                                ...
                                                if (dtMistMatchedProjects != null && dtMistMatchedProjects.Rows.Count == 0)
                                                {
                                                    DataTable dtBudgetLedger = dsBOBudgetDetails.Tables["BudgetLedger"];
                                                    DataTable dtMismatchedLedgers = null;

                                                    // Check if BudgetLedger table exists and has rows before calling GetMismatchedLedgers
                                                    if (dtBudgetLedger != null && dtBudgetLedger.Rows.Count > 0)
                                                    {
                                                        try
                                                        {
                                                            // On 23/01/2025 - Process BudgetLedger in batches to avoid 413 Request Entity Too Large error
                                                            // The server has a limit on request size, so we split large datasets into smaller batches
                                                            const int BATCH_SIZE = 100; // Process 100 ledgers at a time

                                                            if (dtBudgetLedger.Rows.Count > BATCH_SIZE)
                                                            {
                                                                // Split into batches
                                                                int totalBatches = (int)Math.Ceiling((double)dtBudgetLedger.Rows.Count / BATCH_SIZE);
                                                                AcMELog.WriteLog("Processing " + dtBudgetLedger.Rows.Count + " budget ledgers in " + totalBatches + " batches for budget ID: " + budgetid);

                                                                dtMismatchedLedgers = null;

                                                                for (int batchIndex = 0; batchIndex < totalBatches; batchIndex++)
                                                                {
                                                                    int startIndex = batchIndex * BATCH_SIZE;
                                                                    int endIndex = Math.Min(startIndex + BATCH_SIZE, dtBudgetLedger.Rows.Count);

                                                                    // Create batch DataTable
                                                                    DataTable dtBatch = dtBudgetLedger.Clone();
                                                                    for (int i = startIndex; i < endIndex; i++)
                                                                    {
                                                                        DataRow newRow = dtBatch.NewRow();
                                                                        newRow.ItemArray = dtBudgetLedger.Rows[i].ItemArray;
                                                                        dtBatch.Rows.Add(newRow);
                                                                    }

                                                                    // Process this batch
                                                                    DataTable dtBatchMismatched = dataclientService.GetMismatchedLedgers(this.AppSetting.HeadofficeCode, this.AppSetting.BranchOfficeCode, dtBatch);

                                                                    // Merge results
                                                                    if (dtBatchMismatched != null && dtBatchMismatched.Rows.Count > 0)
                                                                    {
                                                                        if (dtMismatchedLedgers == null)
                                                                        {
                                                                            // First batch with results - copy the table (includes schema)
                                                                            dtMismatchedLedgers = dtBatchMismatched.Copy();
                                                                        }
                                                                        else
                                                                        {
                                                                            // Merge into existing table
                                                                            foreach (DataRow row in dtBatchMismatched.Rows)
                                                                            {
                                                                                DataRow newRow = dtMismatchedLedgers.NewRow();
                                                                                newRow.ItemArray = row.ItemArray;
                                                                                dtMismatchedLedgers.Rows.Add(newRow);
                                                                            }
                                                                        }
                                                                    }
                                                                }

                                                                // If no mismatched ledgers found in any batch, create empty DataTable
                                                                if (dtMismatchedLedgers == null)
                                                                {
                                                                    dtMismatchedLedgers = new DataTable();
                                                                }
                                                            }
                                                            else
                                                            {
                                                                // Small enough to process in one call
                                                                dtMismatchedLedgers = dataclientService.GetMismatchedLedgers(this.AppSetting.HeadofficeCode, this.AppSetting.BranchOfficeCode, dtBudgetLedger);
                                                            }

                                                            // Ensure dtMismatchedLedgers is never null
                                                            if (dtMismatchedLedgers == null)
                                                            {
                                                                dtMismatchedLedgers = new DataTable();
                                                            }
                                                        ...
                                                        {
                                                            AcMELog.WriteLog("Error in GetMismatchedLedgers for budget ID " + budgetid + ": " + exLedger.Message + ". StackTrace: " + exLedger.StackTrace);
                                                            CloseWaitDialog();
                                                            result.Message = "Error validating ledgers: " + exLedger.Message;
                                                            this.Cursor = Cursors.Default;
                                                            return result;
                                                        }
                                                    }
                                                    ...
                                                    {
                                                        // BudgetLedger table is empty or null, create empty DataTable for comparison
                                                        dtMismatchedLedgers = new DataTable();
                                                    }
                                                    ...
                                                    else if (dtMismatchedLedgers != null && dtMismatchedLedgers.Rows.Count > 0)
                                                    {
                                                        CloseWaitDialog();
                                                    ...
                                                    {
                                                        // dtMismatchedLedgers is null - log error
                                                        AcMELog.WriteLog("GetMismatchedLedgers returned null for budget ID: " + budgetid);
                                                        CloseWaitDialog();
                                                        result.Message = "Error validating ledgers: Service returned null response. Please check logs.";
                                                        this.Cursor = Cursors.Default;
                                                    }
                                                }
                                                else
            ...
            try
            {
                ...
                using (AuditLockTransSystem AuditSystem = new AuditLockTransSystem())
                {
                    ...
                    else
                    {
                        ...
                        {
                            LockFromDate = this.UtilityMember.DateSet.ToDate(this.AppSetting.BookBeginFrom, false);
                            // Uses <= LockToDate, so 31-03-2026 inclusive
                            LockToDate = new DateTime(2026, 3, 31, 0, 0, 0);
            ...
            try
            {
                ...
                using (AuditLockTransSystem AuditSystem = new AuditLockTransSystem())
                {
                    ...
                    else
                    {
                        ...
                        //On 22/07/2026, locking temp - For IS_SDB_PDO (INMPDO), lock vouchers till 31-03-2026 (no portal grace data)
                        else if (dtAuditLockDetails == null && this.AppSetting.IS_SDB_PDO)
                        {
                            DateTime dtPdoLockFrom = this.UtilityMember.DateSet.ToDate(this.AppSetting.BookBeginFrom, false);
                            // DATE_TO exclusive for View-style filter; for range overlap use 2026-04-01
                            DateTime dtPdoLockTo = new DateTime(2026, 4, 1, 0, 0, 0);

                            dtAuditLockDetails = AuditSystem.AppSchema.AuditLockTransType.DefaultView.ToTable();
                            DataRow dr = dtAuditLockDetails.NewRow();
                            dr[AuditSystem.AppSchema.AuditLockTransType.DATE_FROMColumn.ColumnName] = dtPdoLockFrom;
                            dr[AuditSystem.AppSchema.AuditLockTransType.DATE_TOColumn.ColumnName] = dtPdoLockTo;
                            dtAuditLockDetails.Rows.Add(dr);

                            if ((dVoucherFrmDate < dtPdoLockTo || dVoucherToDate < dtPdoLockTo))
                            {
                                LockProjectName = ProjectName;
                                rtn = true;
                                lockexists = true;
